...
// MyRouter model ID → ChatGPT internal slug. The public ChatGPT Web catalog
// keeps MyRouter's historical dot-form IDs (e.g. "gpt-5.5-pro"), while
...
function configuredProPollTimeoutMs(): number {
  const raw = Number(process.env.MYROUTER_CGPT_WEB_PRO_TIMEOUT_MS);
  if (!Number.isFinite(raw) || raw <= 0) return DEFAULT_PRO_POLL_TIMEOUT_MS;
  return Math.floor(raw);
}

function configuredProPollIntervalMs(): number {
  const raw = Number(process.env.MYROUTER_CGPT_WEB_PRO_POLL_INTERVAL_MS);
 ...
 * dozen "MyRouter" entries clutter the sidebar and can interact with
 ...
 * chat but returned to MyRouter as a bare "completed without image markdown".
 */
 ...
 * often reach MyRouter from a container while the user's browser needs a
...
function derivePublicBaseUrl(
  ...
  const explicitPublicBase = normalizePublicBaseUrl(process.env.MYROUTER_PUBLIC_BASE_URL);
  if (explicitPublicBase) {
    log?.debug?.("CGPT-WEB", `derivePublicBaseUrl: using MYROUTER_PUBLIC_BASE_URL`);
    return explicitPublicBase;
  }

  const headerBase = deriveHeaderBaseUrl(clientHeaders);
  const configuredBase =
    normalizePublicBaseUrl(process.env.MYROUTER_BASE_URL) ||
...
/**
 * Download a chatgpt.com signed image URL and re-serve it from MyRouter's
 ...
 * and return a browser-fetchable MyRouter URL.
 */
...
function configuredAsyncImageTimeoutMs(): number {
  const raw = Number(process.env.MYROUTER_CGPT_WEB_IMAGE_TIMEOUT_MS);
...
function makeImageResolver(ctx: ResolverContext): ImageResolver {
  ...
  return async (assetPointer, conversationId, parentMessageId) => {
    ...
    if (fileId) {
      ...
      // MyRouter's short-lived image cache.
      //
    ...
    if (signedUrl) {
      ...
      // so we materialize the bytes into our own cache and emit an MyRouter
