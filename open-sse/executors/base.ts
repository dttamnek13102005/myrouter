...
/**
 * Strip the MyRouter provider prefix from versioned built-in tool model
 ...
 * leaked MyRouter prefix makes Anthropic reject the request. Mutates in place.
 */
      ...
      try {
        ...
        if (
          ...
          if (Array.isArray(tb.tools)) {
            ...
            // Also strip MyRouter provider prefix from versioned built-in tool
          ...
          //   x-myrouter-effort:   low | medium | high | xhigh | max | off
          //   x-myrouter-thinking: adaptive | off
          // A header value applies only when the corresponding body field is
          // not already set; "off" force-strips the field.
          const headerEffort = (
            clientHeaders?.["x-myrouter-effort"] ?? clientHeaders?.["X-MyRouter-Effort"]
          )
            ?.trim()
            .toLowerCase();
          const headerThinking = (
            clientHeaders?.["x-myrouter-thinking"] ?? clientHeaders?.["X-MyRouter-Thinking"]
          )
            ?.trim()
            .toLowerCase();
          ...
          if (effThinking === "adaptive") {
            ...
            // must opt in with x-myrouter-thinking; force-injecting adaptive thinking
          ...
          // Anthropic correlate accounts behind one MyRouter.
          ...
          // is pinned to the captured CLI wire image, not MyRouter's Node.
