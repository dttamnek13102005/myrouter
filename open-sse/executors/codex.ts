...
function consumeResponsesStoreMarker(body: Record<string, unknown>): unknown {
  const marker = body._myrouterResponsesStore;
  delete body._myrouterResponsesStore;
  return marker;
}

/**
 * Global Codex WebSocket kill-switch (feature flag MYROUTER_CODEX_WS_ENABLED,
...
function isCodexWsGloballyEnabled(): boolean {
  try {
    return isFeatureFlagEnabled("MYROUTER_CODEX_WS_ENABLED");
  } catch {
    return true;
  }
}
...
export function isCodexResponsesWebSocketRequired(_model: string, credentials: unknown): boolean {
  ...
  // MyRouter is an HTTP→SSE gateway — WebSocket transport is unnecessary and
...
function codexDropNonstandardEvents(): boolean {
  const v = process.env.MYROUTER_CODEX_DROP_NONSTANDARD_EVENTS;
...
// streaming Response when the MYROUTER_CODEX_DROP_NONSTANDARD_EVENTS kill-switch is on.
...
export function encodeResponseSseEvent(raw: string): { sse: string; terminal: boolean } {
  ...
  // MYROUTER_CODEX_DROP_NONSTANDARD_EVENTS (the HTTP transport is handled
...
export class CodexExecutor extends BaseExecutor {
  ...
  transformRequest(
    ...
    // These are often injected by MyRouter's fallback logic for store=true,
    ...
    const RESPONSES_API_ALLOWLIST = new Set([
      ...
      "text",
      // Internal markers used by MyRouter pipeline
      "_myrouterResponsesStore",
    ]);
