...
/**
 * Map MyRouter's reasoning-effort inputs onto the binary thinking switch exposed by
...
export function supportsMaxEffortForProvider(provider: string, model: string): boolean {
  ...
  // normalized to xhigh (the MyRouter-internal top tier) and rejected by the
...
// MyRouter carries the requested effort on up to three shapes:
//   1. top-level `reasoning_effort`        — OpenAI / MyRouter-internal
...
export function sanitizeReasoningEffortForProvider(
  ...
  // ONLY as {high, max} (its own top tier is literally "max"). MyRouter's internal
