export const PROVIDER_PLUGIN_MANIFEST_HEADER = "X-MyRouter-Provider-Manifest-Url";
...
export function resolveProviderPluginManifestUrl(origin?: string | null): string {
  const configured = process.env.MYROUTER_PROVIDER_MANIFEST_URL?.trim();
  ...
  const protocol = process.env.MYROUTER_PUBLIC_PROTOCOL || "http";
