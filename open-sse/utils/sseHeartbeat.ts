...
function buildHeartbeatPayload(
  ...
  switch (shape) {
    ...
    case HEARTBEAT_SHAPES.OPENAI_CHUNK: {
      const payload = {
        id: opts.chunkId ?? "myrouter-keepalive",
        object: "chat.completion.chunk",
        created: Math.floor(Date.now() / 1000),
        model: opts.chunkModel ?? "myrouter",
...
/**
 * Whether MyRouter may emit SSE `:` comment lines (e.g. the `: keepalive` heartbeat).
 * Some strict OpenAI-compatible clients parse every SSE line as JSON and crash on `:` comments.
 * Set MYROUTER_SSE_COMMENTS=off to suppress comment-shaped heartbeats (they become a no-op).
...
export function sseCommentsEnabled(): boolean {
  ...
  const v = process.env.MYROUTER_SSE_COMMENTS;
...
export function createSseHeartbeatTransform({
  ...
  // crash on `:` comment heartbeats. MYROUTER_SSE_COMMENTS=off disables comment-shaped
