...
export function getProxyDispatcherConnectionLimit(
  ...
  const raw = env.MYROUTER_PROXY_DISPATCHER_CONNECTIONS;
  if (raw == null || raw.trim() === "") return DEFAULT_PROXY_DISPATCHER_CONNECTIONS;

  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed < 1) {
    console.warn(
      `[ProxyDispatcher] Invalid MYROUTER_PROXY_DISPATCHER_CONNECTIONS="${raw}". Using default ${DEFAULT_PROXY_DISPATCHER_CONNECTIONS}.`
    );
...
export function getDefaultDispatcherConnectionLimit(
  ...
  const raw = env.MYROUTER_DIRECT_DISPATCHER_CONNECTIONS;
  if (raw == null || raw.trim() === "") return DEFAULT_PROXY_DISPATCHER_CONNECTIONS;

  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed < 1) {
    console.warn(
      `[ProxyDispatcher] Invalid MYROUTER_DIRECT_DISPATCHER_CONNECTIONS="${raw}". Using default ${DEFAULT_PROXY_DISPATCHER_CONNECTIONS}.`
    );
    return DEFAULT_PROXY_DISPATCHER_CONNECTIONS;
  }
...
export function createProxyDispatcher(proxyUrl: string): Dispatcher {
  ...
  if (parsed.protocol === "socks5:") {
    ...
    dispatcher = new ProxyAgent({
      ...
      // that only speak CONNECT then reject it (501). MyRouter tunnels ALL proxied
