...
export function resolveAgentGoalPolicy(
  ...
  const policyEnabled = parseBoolean(env.MYROUTER_AGENT_GOAL_POLICY_ENABLED, true);
  if (!policyEnabled) {
    return {
      detected: false,
  ...
  const forcedByHeader = parseBoolean(readHeader(headers, "x-myrouter-agent-goal"), false);
  const detected = forcedByHeader || isAgentGoalRequestBody(body);
  const readinessMaxTimeoutMs = readPositiveMs(
    env,
    "MYROUTER_AGENT_GOAL_READINESS_MAX_TIMEOUT_MS",
    DEFAULT_AGENT_GOAL_READINESS_MAX_TIMEOUT_MS
  );
  const streamRecoveryEnabled =
    detected && parseBoolean(env.MYROUTER_AGENT_GOAL_STREAM_RECOVERY, true);
