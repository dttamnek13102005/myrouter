...
const FALLBACK_KIE_CALLBACK_URL = "https://myrouter.local/api/kie/callback";
...
function getConfiguredKieCallbackUrl(): string {
  const explicit =
    process.env.KIE_CALLBACK_URL?.trim() || process.env.MYROUTER_KIE_CALLBACK_URL?.trim();
  if (explicit) return explicit;

  return (
    callbackUrlFromBaseUrl(process.env.MYROUTER_PUBLIC_URL) ||
