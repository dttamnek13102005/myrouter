 ...
 * `stream`. Without this hint, MyRouter previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // MyRouter accumulates the provider stream and converts it to JSON for the client
  ...
  // MyRouter fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve MyRouter's streaming default unless
