...
const E = [
  { name: "myrouter_get_health", description: "health status uptime memory", scopes: ["read:health"] },
  { name: "myrouter_list_combos", description: "list combos and strategies", scopes: ["read:combos"] },
  { name: "myrouter_check_quota", description: "remaining quota per provider", scopes: ["read:quota"] },
];

describe("searchTools", () => {
  it("ranks name+desc hit on top", () => {
    const r = searchTools(E, "health", 8);
    expect(r[0].name).toBe("myrouter_get_health");
  });
  it("no hit ⇒ empty", () => { expect(searchTools(E, "zzzzz", 8)).toEqual([]); });
  it("respects limit + deterministic tie-break", () => {
    const r = searchTools(E, "myrouter", 2);
