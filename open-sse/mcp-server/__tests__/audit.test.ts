...
describe("MCP audit shutdown", () => {
  ...
  beforeEach(() => {
    vi.resetModules();
    globalThis.__myrouterMcpAuditDb = undefined;
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-mcp-audit-"));
  ...
  it(
    ...
    async () => {
      ...
      await audit.logToolCall("myrouter_get_health", { ok: true }, { ok: true }, 12, true);
  ...
  it("still closes the audit database when checkpoint fails", async () => {
    ...
    await audit.logToolCall("myrouter_get_health", {}, {}, 5, true);
    expect(audit.closeAuditDb()).toBe(true);
    expect(mockDb.close).toHaveBeenCalledTimes(1);
  });
  ...
  it("falls back to node:sqlite when better-sqlite3 binding is missing", async () => {
    ...
    await audit.logToolCall("myrouter_get_health", { ok: true }, { ok: true }, 4, true);
