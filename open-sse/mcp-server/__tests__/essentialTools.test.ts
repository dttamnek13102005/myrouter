 ...
 * The myrouter_web_search tests use InMemoryTransport + Client to exercise
...
describe("MCP Essential Tools", () => {
  ...
  describe("Tool schema validation", () => {
    ...
    it("all tools should have myrouter_ prefix", () => {
      const schemas = MCP_ESSENTIAL_TOOLS;
      for (const schema of schemas) {
        expect(schema.name).toMatch(/^myrouter_/);
      }
    });
  });
...
// ── myrouter_web_search: handler dispatch tests ──────────────────────────────
...
describe("myrouter_web_search handler (via MCP dispatch)", () => {
  let client: Client;
  ...
  it("should appear in tools/list after registration", async () => {
    ...
    const webSearch = tools.find((t) => t.name === "myrouter_web_search");
