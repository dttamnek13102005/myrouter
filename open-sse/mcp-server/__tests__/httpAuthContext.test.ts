...
describe("MCP HTTP auth context", () => {
  ...
  it("forwards request auth through registered core tools", async () => {
    ...
    try {
      ...
      await withMcpHttpAuthContext(request, () =>
        client.callTool({ name: "myrouter_list_combos", arguments: {} })
      );
  ...
  it("forwarded caller auth wins over the MYROUTER_API_KEY env fallback (#5819)", async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ combos: [] }),
    });
    vi.stubGlobal("fetch", fetchMock);
    vi.stubEnv("MYROUTER_API_KEY", "env-fallback-key");
  ...
  it("forwards request auth through advanced tool apiFetch", async () => {
    ...
    try {
      ...
      await withMcpHttpAuthContext(request, () =>
        client.callTool({ name: "myrouter_cache_stats", arguments: {} })
      );
