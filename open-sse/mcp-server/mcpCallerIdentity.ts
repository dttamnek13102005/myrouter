 ...
 * `extra.authInfo` (MyRouter authenticates with API keys, not OAuth client ids —
 ...
 *
 * On stdio transport (`myrouter --mcp`) there is no HTTP context, so we fall
 * back to the MYROUTER_API_KEY / ROUTER_API_KEY environment variable.
 *
 ...
 *
 * Falls back to MYROUTER_API_KEY / ROUTER_API_KEY env vars for stdio transport
...
/**
 * Resolve the principal id from the MYROUTER_API_KEY (or ROUTER_API_KEY) env var.
...
async function resolvePrincipalFromEnv(): Promise<string | undefined> {
  const rawKey = process.env.MYROUTER_API_KEY || process.env.ROUTER_API_KEY;
  if (!rawKey) return undefined;
  try {
