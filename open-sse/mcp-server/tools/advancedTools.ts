/**
 * MyRouter MCP Advanced Tools — 13 intelligence tools that differentiate
 * MyRouter from all other AI gateways.
 *
 * Tools:
 *   1. myrouter_simulate_route     — Dry-run routing simulation
 *   2. myrouter_set_budget_guard   — Session budget with degrade/block/alert
 *   3. myrouter_set_routing_strategy — Runtime strategy switch for combos
 *   4. myrouter_set_resilience_profile — Circuit breaker/retry profiles
 *   5. myrouter_test_combo         — Live test each provider in a combo
 *   6. myrouter_get_provider_metrics — Detailed per-provider metrics
 *   7. myrouter_best_combo_for_task — AI-powered combo recommendation
 *   8. myrouter_explain_route      — Post-hoc routing decision explainer
 *   9. myrouter_get_session_snapshot — Full session state snapshot
 *  10. myrouter_db_health_check   — Diagnose and repair DB state drift
 *  11. myrouter_sync_pricing      — Sync provider pricing from external source
 */
...
const MYROUTER_API_KEY = process.env.MYROUTER_API_KEY || "";
...
async function apiFetch(path: string, options: RequestInit = {}): Promise<unknown> {
  ...
  const headers: Record<string, string> = {
    ...
    ...(MYROUTER_API_KEY ? { Authorization: `Bearer ${MYROUTER_API_KEY}` } : {}),
...
export async function handleSimulateRoute(args: {
  ...
  try {
    ...
    await logToolCall("myrouter_simulate_route", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_simulate_route", args, null, Date.now() - start, false, msg);
...
export async function handleSetBudgetGuard(args: {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_set_budget_guard", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleSetRoutingStrategy(args: {
  ...
  try {
    ...
    await logToolCall("myrouter_set_routing_strategy", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_set_routing_strategy", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleSetResilienceProfile(args: {
  ...
  try {
    ...
    // Apply to MyRouter via API using the plan-aligned resilience structure.
    ...
    await logToolCall("myrouter_set_resilience_profile", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall(
...
export async function handleTestCombo(args: { comboId: string; testPrompt: string }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_test_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleGetProviderMetrics(args: { provider: string }) {
  ...
  try {
    ...
    await logToolCall("myrouter_get_provider_metrics", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_get_provider_metrics", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleBestComboForTask(args: {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_best_combo_for_task", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleExplainRoute(args: { requestId: string }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_explain_route", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleSyncPricing(args: { sources?: string[]; dryRun?: boolean }) {
  ...
  try {
    ...
    await logToolCall("myrouter_sync_pricing", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_sync_pricing", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleGetSessionSnapshot() {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_get_session_snapshot", {}, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleDbHealthCheck(args: { autoRepair?: boolean }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_db_health_check", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleCacheStats() {
  ...
  try {
    ...
    await logToolCall("myrouter_cache_stats", {}, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_cache_stats", {}, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleCacheFlush(args: { signature?: string; model?: string }) {
  ...
  try {
    ...
    await logToolCall("myrouter_cache_flush", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_cache_flush", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleOneproxyFetch(
  ...
  try {
    ...
    await logToolCall("myrouter_oneproxy_fetch", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_oneproxy_fetch", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleOneproxyRotate(
  ...
  try {
    ...
    await logToolCall("myrouter_oneproxy_rotate", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_oneproxy_rotate", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleOneproxyStats(args: Record<string, never> = {}) {
  ...
  try {
    ...
    await logToolCall("myrouter_oneproxy_stats", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("myrouter_oneproxy_stats", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
