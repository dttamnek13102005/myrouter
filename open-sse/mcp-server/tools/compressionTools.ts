/**
 * MyRouter MCP Compression Tools — Manage and monitor prompt compression.
 *
 * Tools:
 *   1. myrouter_compression_status   — Get compression config, analytics, and cache stats
 *   2. myrouter_compression_configure — Update compression settings
...
export async function handleCompressionStatus(
  ...
  try {
    ...
    await logToolCall("myrouter_compression_status", args, result, duration, true);
...
export async function handleCompressionConfigure(
  ...
  try {
    ...
    const duration = Date.now() - start;
    await logToolCall("myrouter_compression_configure", args, result, duration, true);
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    await logToolCall(
      "myrouter_compression_configure",
      args,
      { error: errorMessage },
      duration,
      false,
...
export async function handleCcrStoreTool(
  ...
  await logToolCall("myrouter_ccr_store", auditInput, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrRetrieveTool(
  ...
  if ((!args.mode || args.mode === "full") && metadata.bytes > MAX_CCR_MCP_FULL_BYTES) {
    ...
    await logToolCall("myrouter_ccr_retrieve", args, output, Date.now() - start, true);
    return output;
  }
...
export async function handleCcrInspectTool(
  ...
  await logToolCall("myrouter_ccr_inspect", args, output, Date.now() - start, Boolean(metadata));
  return output;
}
...
export async function handleCcrListTool(
  ...
  await logToolCall("myrouter_ccr_list", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrDeleteTool(
  ...
  await logToolCall("myrouter_ccr_delete", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleCcrStatsTool(
  ...
  await logToolCall("myrouter_ccr_stats", args, output, Date.now() - start, true);
  return output;
}
...
export async function handleRtkDiscover(
  ...
  await logToolCall("myrouter_rtk_discover", args, result, Date.now() - start, true);
  return result;
}
...
export async function handleRtkLearn(
  ...
  await logToolCall("myrouter_rtk_learn", args, result, Date.now() - start, true);
  return result;
}
...
export const compressionTools = {
  ...
  myrouter_set_compression_engine: {
  ...
  myrouter_list_compression_combos: {
  ...
  myrouter_compression_combo_stats: {
  ...
  myrouter_ccr_inspect: {
    name: "myrouter_ccr_inspect",
  ...
  myrouter_rtk_discover: {
    name: "myrouter_rtk_discover",
    description:
