 ...
 * `MYROUTER_EMERGENCY_FALLBACK=false` (or `0`). Default remains enabled.
 */

import { isFeatureFlagEnabled } from "@/shared/utils/featureFlags";

const EMERGENCY_FALLBACK_FLAG_KEY = "MYROUTER_EMERGENCY_FALLBACK";
...
function isEmergencyFallbackRawEnvEnabled(): boolean {
  const raw = process.env.MYROUTER_EMERGENCY_FALLBACK;
...
export function shouldUseFallback(
  ...
  if (!isEmergencyFallbackEnvEnabled()) {
    return {
      shouldFallback: false,
      reason: "emergency fallback disabled via MYROUTER_EMERGENCY_FALLBACK",
    };
  }
