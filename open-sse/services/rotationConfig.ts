 ...
 *
 * MyRouter's account-fallback engine (accountFallback.ts) historically rotated accounts using
 ...
 *   - master enable                            MYROUTER_ROTATION_ENABLED               (default true)
 *   - rate-limit reset/cooldown seconds        MYROUTER_ROTATION_RATE_LIMIT_RESET_SECONDS (0 => engine default)
 *   - per-status fallback enable               MYROUTER_ROTATE_ON_{429,500,502,400}    (429/500/502 default true, 400 default false)
 *   - per-status threshold (errors in window)  MYROUTER_ROTATE_{status}_THRESHOLD      (default 1 => immediate, current behavior)
 *   - per-status window seconds                MYROUTER_ROTATE_{status}_WINDOW_SECONDS (default 120)
 *
...
const GLOBAL_KEY = "__myrouter_rotation_config__";
...
function buildFromEnv(): RotationConfig {
  return {
    enabled: envBool("MYROUTER_ROTATION_ENABLED", true),
    rateLimitResetMs: envInt("MYROUTER_ROTATION_RATE_LIMIT_RESET_SECONDS", 0, 0) * 1000,
    disableTagWithoutReset: envBool("MYROUTER_ROTATION_DISABLE_TAG_WITHOUT_RESET", true),
    rateLimit429: buildClass(
      "MYROUTER_ROTATE_ON_429",
      "MYROUTER_ROTATE_429_THRESHOLD",
      "MYROUTER_ROTATE_429_WINDOW_SECONDS",
      true
    ),
    serverError500: buildClass(
      "MYROUTER_ROTATE_ON_500",
      "MYROUTER_ROTATE_500_THRESHOLD",
      "MYROUTER_ROTATE_500_WINDOW_SECONDS",
      true
    ),
    badGateway502: buildClass(
      "MYROUTER_ROTATE_ON_502",
      "MYROUTER_ROTATE_502_THRESHOLD",
      "MYROUTER_ROTATE_502_WINDOW_SECONDS",
      true
    ),
    badRequest400: buildClass(
      "MYROUTER_ROTATE_ON_400",
      "MYROUTER_ROTATE_400_THRESHOLD",
      "MYROUTER_ROTATE_400_WINDOW_SECONDS",
      false
    ),
  };
}
...
const COUNTER_KEY = "__myrouter_rotation_counters__";
