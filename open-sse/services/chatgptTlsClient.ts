...
const DEFAULT_TIMEOUT_MS =
  Number.parseInt(process.env.MYROUTER_CHATGPT_TLS_TIMEOUT_MS || "", 10) || 60_000;
...
const HARD_TIMEOUT_GRACE_MS =
  Number.parseInt(process.env.MYROUTER_CHATGPT_TLS_GRACE_MS || "", 10) || 10_000;
const STREAM_FIRST_BYTE_TIMEOUT_MS =
  Number.parseInt(process.env.MYROUTER_CHATGPT_STREAM_FIRST_BYTE_TIMEOUT_MS || "", 10) || 30_000;
  ...
  if (!clientPromise) {
    clientPromise = (async () => {
      try {
        ...
        // managed sidecar's localhost HTTP calls that MyRouter's global fetch
...
export interface TlsFetchOptions {
   ...
   *   2. `process.env.MYROUTER_TLS_PROXY_URL` (single-flag opt-in)
