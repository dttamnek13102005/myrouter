...
/**
 * Interprets the `x-myrouter-compression` request header into a plan, or null when the
...
/** Renders the X-MyRouter-Compression response header value. */
export function formatCompressionMeta(plan: DerivedPlan): string {
  return `${plan.mode}; source=${plan.source ?? "off"}`;
}

/**
 * Builds the annotation suffix for X-MyRouter-Compression from compression stats.
 ...
 * ASCII-only: this string is appended to the X-MyRouter-Compression HTTP header,
