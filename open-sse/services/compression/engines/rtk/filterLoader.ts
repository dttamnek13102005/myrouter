...
export interface RtkFilterLoadDiagnostic {
  ...
  format?: "myrouter-json" | "rtk-toml-v1";
  path?: string;
  level: "warning" | "error";
  message: string;
}
...
function getModuleDir(): string {
  ...
  return path.join(os.homedir(), ".myrouter");
}
...
function getDataDir(): string {
  return process.env.DATA_DIR || path.join(os.homedir(), ".myrouter");
}
...
function projectFiltersTrusted(
  ...
  if (process.env.MYROUTER_RTK_TRUST_PROJECT_FILTERS === "1") return true;
...
function collectProjectFilterSources(sources: FilterSource[], options: RtkFilterLoadOptions): void {
  const projectCandidates = [
    ...
    { path: path.join(process.cwd(), ".rtk", "filters.json"), format: "myrouter-json" as const },
  ];
...
function collectGlobalFilterSources(sources: FilterSource[]): void {
  const globalCandidates = [
    ...
    { path: path.join(getDataDir(), "rtk", "filters.json"), format: "myrouter-json" as const },
  ];
...
export function loadRtkFilters(options: RtkFilterLoadOptions = {}): RtkFilterDefinition[] {
  const currentCacheKey = [
    ...
    process.env.MYROUTER_RTK_TRUST_PROJECT_FILTERS === "1" ? "env-trust" : "env-normal",
  ].join("|");
  ...
  const formatRank = { "rtk-toml-v1": 2, "myrouter-json": 1 } as const;
  const sorted = filters.sort(
    (a, b) =>
      sourceRank[b.source ?? "builtin"] - sourceRank[a.source ?? "builtin"] ||
      formatRank[b.sourceFormat ?? "myrouter-json"] -
        formatRank[a.sourceFormat ?? "myrouter-json"] ||
