...
/**
 * Handler for the `myrouter_ccr_retrieve` MCP tool.
 *
 ...
 * ENTIRE prompt for any caller that cannot resolve `myrouter_ccr_retrieve`
...
function processMessages(
  ...
  const result = messages.map((msg) => {
    ...
    // MyRouter is used as a chat-completion PROVIDER, the upstream LLM has no
    // way to call `myrouter_ccr_retrieve` and expand markers — replacing tool
...
export const ccrEngine: CompressionEngine = {
  ...
  description:
    ...
    "retrievable via the `myrouter_ccr_retrieve` MCP tool (H4). " +
  ...
  apply(body: Record<string, unknown>, options?: CompressionEngineApplyOptions): CompressionResult {
    ...
    // #8033: teach MCP-capable callers the marker → myrouter_ccr_retrieve contract
