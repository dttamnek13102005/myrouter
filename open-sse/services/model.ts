...
async function getPreferClaudeCodeForUnprefixedClaudeModels() {
  ...
  return isTruthyEnv(process.env.MYROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS);
}
...
async function resolveModelByProviderInference(modelId: string, extendedContext: boolean) {
  ...
  // Bare model IDs from Codex CLI do not preserve MyRouter's `cx/` prefix.
