 ...
 *
 * Goal: ensure the final request body MyRouter sends to Anthropic's
...
export interface InjectBillingHeaderOp {
   ...
   *   - myrouter-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "myrouter-daystamp";
  /**
 ...
 * proven against the live MyRouter deployment (call log
...
/**
 * Compute the `cc_version` suffix per the MyRouter native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    ...
    (options.versionFormat === "myrouter-daystamp"
