...
export const SKIP_UNIVERSAL_HANDOFF_FLAG = "_myrouterSkipUniversalHandoff";
...
async function generateHandoffAsync(options: {
  ...
  const summaryBody = {
    ...
    temperature: 0.1,
    _myrouterSkipContextRelay: true,
    _myrouterInternalRequest: "context-handoff",
  };
...
async function generateUniversalHandoffAsync(options: {
  ...
  const summaryBody: Record<string, unknown> = {
    ...
    temperature: 0.1,
    _myrouterSkipContextRelay: true,
    _myrouterInternalRequest: "universal-handoff",
  };
