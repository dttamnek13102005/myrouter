 ...
 *
 * Override: set MYROUTER_OPENCODE_QUOTA_URL to a working endpoint. If
...
// tracked in anomalyco/opencode#16017).  Set MYROUTER_OPENCODE_QUOTA_URL to override.
const OPENCODE_QUOTA_URL =
  process.env.MYROUTER_OPENCODE_QUOTA_URL ?? "https://opencode.ai/zen/go/v1/quota";
        ...
        if (!_warned404Urls.has(OPENCODE_QUOTA_URL)) {
          ...
          console.warn(
            `[opencodeQuotaFetcher] ${OPENCODE_QUOTA_URL} returned 404 — opencode-go usage API is not yet public. ` +
              `Set MYROUTER_OPENCODE_QUOTA_URL to a working endpoint, or follow ` +
