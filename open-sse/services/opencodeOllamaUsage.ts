...
// MYROUTER_OPENCODE_GO_QUOTA_URL explicitly. Never hardcode a third-party
// host here (a previous default silently sent the user's API key to an
// unrelated Z.AI endpoint).
const OPENCODE_GO_QUOTA_URL = process.env.MYROUTER_OPENCODE_GO_QUOTA_URL?.trim() || "";
const OPENCODE_GO_DASHBOARD_BASE_URL =
  process.env.MYROUTER_OPENCODE_GO_DASHBOARD_URL ?? "https://opencode.ai/workspace";
...
const OLLAMA_CLOUD_USAGE_URL =
  process.env.MYROUTER_OLLAMA_CLOUD_USAGE_URL ?? "https://ollama.com/settings";
...
function resolveOpenCodeGoDashboardConfig(
  ...
  const workspaceId =
    process.env.MYROUTER_OPENCODE_GO_WORKSPACE_ID?.trim() ||
  ...
  const authCookie =
    process.env.MYROUTER_OPENCODE_GO_AUTH_COOKIE?.trim() ||
  ...
  if (!OPENCODE_GO_QUOTA_URL) {
    return {
      message:
        ...
        "or set MYROUTER_OPENCODE_GO_QUOTA_URL to opt in to an explicit quota endpoint.",
    };
  }
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          message:
            ...
            "MYROUTER_OPENCODE_GO_QUOTA_URL endpoint. " +
            "Set OPENCODE_GO_WORKSPACE_ID and OPENCODE_GO_AUTH_COOKIE to enable dashboard quota scraping.",
        };
      }
      return {
        message:
          ...
          "Set MYROUTER_OPENCODE_GO_QUOTA_URL to a working endpoint, or follow " +
...
function resolveOllamaCloudConfig(providerSpecificData?: JsonRecord): OllamaCloudConfig {
  const cookie =
    process.env.MYROUTER_OLLAMA_USAGE_COOKIE?.trim() ||
