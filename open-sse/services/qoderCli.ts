...
export function getQoderCliWorkspace(): string {
  const explicit = String(
    process.env.QODER_CLI_WORKSPACE || process.env.MYROUTER_QODER_WORKSPACE || ""
  ).trim();
...
/**
 * Isolated `--config-dir` for MyRouter-driven qodercli runs. Keeping it separate
...
export function getQoderCliConfigDir(): string {
  ...
  const base = dataDir || path.join(os.homedir() || os.tmpdir(), ".myrouter");
...
export async function runQoderCli(options: QoderCliRunOptions): Promise<QoderCliRunResult> {
  ...
  const args = [
    ...
    "--model",
    level,
    // Disable all built-in tools — MyRouter only wants a plain LM reply, never
...
/**
 * Resolve an MyRouter model id to the exact value to pass to `qodercli -m`.
...
export function buildQoderPrompt(body: unknown): string {
  ...
  const lines = [
    "You are answering an MyRouter OpenAI-compatible request through the Qoder CLI transport.",
...
export function buildCosyHeadersForValidation(bodyStr: string, token: string) {
  ...
  const uid = "myrouter.user@qoder.sh";
  const userInfo = {
    uid: uid,
...
// MyRouter mis-surfaced as "PAT may not be valid for the chat API". We mirror the
...
export async function validateQoderCliPat({
  ...
  if (run.timedOut) {
    return {
      valid: false,
      error:
        "qodercli timed out while validating the token. Check network/proxy access from the MyRouter host.",
      unsupported: false,
    };
  }
