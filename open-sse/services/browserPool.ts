 ...
 * startup cost. Set MYROUTER_BROWSER_POOL=off to fully disable.
 */
...
// (and the myrouter_browser_pool_status MCP tool), giving the previously
...
function isPoolEnabled(): boolean {
  const flag = process.env.MYROUTER_BROWSER_POOL;
...
export async function acquireBrowserContext(
  ...
  if (!isPoolEnabled()) {
    throw new Error(
      "browserPool: MYROUTER_BROWSER_POOL=off — context requested but pool is disabled"
    );
  }
 ...
 * failures, shutdowns). Surfaced via the myrouter_browser_pool_status MCP tool.
 */
