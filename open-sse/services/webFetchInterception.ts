 ...
 * synthetic `myrouter_web_fetch` function tool; the actual `/v1/web/fetch` call
...
export const MYROUTER_WEB_FETCH_FALLBACK_TOOL_NAME = "myrouter_web_fetch";
...
function buildFallbackTool(targetFormat?: string | null): JsonRecord {
  const name = MYROUTER_WEB_FETCH_FALLBACK_TOOL_NAME;
  const description = [
...
export function prepareWebFetchFallbackBody<T extends WebFetchFallbackBody>(
  ...
  if (!toolNames.has(MYROUTER_WEB_FETCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebFetchToolChoice(body.tool_choice)) {
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: MYROUTER_WEB_FETCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: MYROUTER_WEB_FETCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
  ...
  return {
    ...
    fallback: {
      enabled: true,
      toolName: MYROUTER_WEB_FETCH_FALLBACK_TOOL_NAME,
