 ...
 *
 *   X-MyRouter-Mode:            fast | balanced | quality | <raw mode-pack name>  (#6024/#6025)
 *   X-MyRouter-Budget:          <max USD per request>                             (#6023)
 *   X-MyRouter-Budget-Fallback: cheapest | strict                                 (#3470)
 *
...
/**
 * Resolve the `X-MyRouter-Mode` header value into a mode-pack override.
 *
...
/**
 * Parse the `X-MyRouter-Budget` header into a hard per-request cost ceiling (USD).
...
/**
 * Parse the `X-MyRouter-Budget-Fallback` header into a budget-fallback policy override.
...
export function resolveRequestAutoControls(headers: {
  ...
  const modeHeader = headers.get("x-myrouter-mode")?.trim() || null;
  const budgetHeader = headers.get("x-myrouter-budget")?.trim() || null;
  const budgetFallbackHeader = headers.get("x-myrouter-budget-fallback")?.trim() || null;
