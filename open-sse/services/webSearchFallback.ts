...
export const MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME = "myrouter_web_search";
...
function buildFallbackTool(tool: JsonRecord, targetFormat?: string | null): JsonRecord {
  const name = MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME;
...
// built-in web-search tool has to be converted to the myrouter_web_search function
...
export function supportsNativeWebSearchFallbackBypass({
  ...
  // native tool untouched instead of rewriting it to myrouter_web_search. Mirrors the
...
export function prepareWebSearchFallbackBody<T extends WebSearchFallbackBody>(
  ...
  if (!toolNames.has(MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebSearchToolChoice(body.tool_choice)) {
    ...
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
