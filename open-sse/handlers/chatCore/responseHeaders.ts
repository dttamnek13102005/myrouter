...
} from "@/domain/myrouterResponseMeta";
import { MYROUTER_RESPONSE_HEADERS } from "@/shared/constants/headers";
import { defaultLogger } from "@myrouter/open-sse/utils/logger";
 ...
 * This budget includes each header name, separator, value, and trailing CRLF. MyRouter's own
...
function isMyRouterInternalHeader(headerName: string): boolean {
  return headerName.toLowerCase().startsWith("x-myrouter-");
}
 ...
 *
 * If MyRouter re-emits those headers from an App Router route handler, Next
...
export function buildStreamingResponseHeaders(
  ...
  providerHeaders.forEach((value, key) => {
    ...
    if (
      ...
      isMyRouterInternalHeader(normalized)
    ) {
      return;
    }
    candidates.push({
  ...
  attachMyRouterMetaHeaders(responseHeaders, meta);
