 ...
 * shapes), and attach an `_myrouter` meta blob to a log payload. Side-effect-free; behaviour is
...
export function attachLogMeta(
  ...
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { _myrouter: compactMeta, _payload: payload ?? null };
  }
  const existing =
    payload._myrouter &&
    typeof payload._myrouter === "object" &&
    !Array.isArray(payload._myrouter)
      ? payload._myrouter
      : {};
  return {
    ...payload,
