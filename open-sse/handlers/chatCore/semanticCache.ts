  ...
  if (semanticCacheEnabled && isCacheableForRead(body, clientRawRequest?.headers)) {
    ...
    if (cached) {
      ...
      const headers: Record<string, string> = {
        ...
        [MYROUTER_RESPONSE_HEADERS.cache]: "HIT",
      };
      // A cache HIT serves WITHOUT an upstream call, so the incremental cost billed to
      // the client is 0 (consumers that sum X-MyRouter-Response-Cost must not charge for
      // hits). The original/would-have-been cost is surfaced via X-MyRouter-Cost-Saved.
