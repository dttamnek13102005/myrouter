 ...
 * `x-myrouter-no-memory` header. Mirrors the existing `x-myrouter-no-cache`
...
export function isNoMemoryRequested(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-myrouter-no-memory") || "")
    .trim()
    .toLowerCase();
...
/**
 * Per-request compression override via the `x-myrouter-compression` header. Mirrors the
 * `x-myrouter-no-memory` convention (#4290). Returns the raw trimmed value, or null when
...
export function resolveCompressionHeader(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-myrouter-compression") || "").trim();
  return value || null;
}

/**
 * Per-request opt-in to unconditionally strip `reasoning_content` from the
 * non-streaming JSON response via the `x-myrouter-strip-reasoning` header.
...
export function isStripReasoningRequested(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-myrouter-strip-reasoning") || "")
    .trim()
    .toLowerCase();
  return value === "true" || value === "1" || value === "yes";
}
