...
import {
  checkTokenLimits,
  recordTokenUsage,
} from "@myrouter/open-sse/services/tokenLimitCounter.ts";
import {
  COOLDOWN_MS,
  HTTP_STATUS,
...
import { wasRefreshTokenRotated } from "@myrouter/open-sse/services/refreshSerializer.ts";
...
export async function handleChatCore({
  ...
  const traceEnabled = process.env.MYROUTER_TRACE === "true" || process.env.DEBUG === "true";
  ...
  if (webSearchFallbackPlan.enabled) {
    ...
    log?.info?.(
      "TOOLS",
      `Converted ${webSearchFallbackPlan.convertedToolCount} web_search tool(s) to MyRouter fallback for ${provider}`
    );
  }
  ...
  if (webFetchFallbackPlan.enabled) {
    ...
    log?.info?.(
      "TOOLS",
      `Converted ${webFetchFallbackPlan.convertedToolCount} web_fetch tool(s) to MyRouter fallback for ${provider}`
    );
  }
  ...
  const explicitSessionIdHeader =
    (clientRawRequest?.headers && typeof clientRawRequest.headers.get === "function"
      ? clientRawRequest.headers.get("x-myrouter-session-id")
      : getHeaderValueCaseInsensitive(
          clientRawRequest?.headers ?? null,
          "x-myrouter-session-id"
        )) || null;
  ...
  // (#5312 / #5245): `x-myrouter-thinking-marker: off` suppresses it for
  ...
  // They are accepted for compatibility at the MyRouter API boundary only.
  if (body && typeof body === "object") {
  ...
  const reasoningRouteDecision =
    body && typeof body === "object"
      ? (body as Record<string, unknown>)._myrouterReasoningRouteTrace
      : null;
  if (reasoningRouteDecision) {
    reqLogger.logRouteDecision(reasoningRouteDecision);
    body = { ...(body as Record<string, unknown>) };
    delete (body as Record<string, unknown>)._myrouterReasoningRouteTrace;
  }
  ...
  // `x-myrouter-no-memory: true` to skip memory+skills injection (a null owner
  ...
  if (body && Array.isArray(allMessages) && allMessages.length > 0) {
    ...
    try {
      ...
      if (compressionHeader) {
        log?.debug?.("COMPRESSION", `x-myrouter-compression header: ${compressionHeader}`);
      }
  ...
  // rejects unknown/extra params with a strict "400 Param Incorrect". Map MyRouter's
  ...
  if (!stream) {
    ...
    if (clientResponseFormat === FORMATS.OPENAI_RESPONSES) {
      ...
      // Port of decolua/9router#517: opt-in `x-myrouter-strip-reasoning` header
    ...
    // #6426: align response body `model` with the `X-MyRouter-Model` header
  ...
  if (needsResponsesTranslation) {
    ...
    transformStream = createSSETransformStreamWithLogger(
      ...
      // (e.g. OpenCode by UA; any client via `x-myrouter-thinking-marker: off`);
