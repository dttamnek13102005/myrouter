...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `myrouter-export-${timestamp}`);
    const zipPath = path.join(os.tmpdir(), `myrouter-full-backup-${timestamp}.zip`);
    ...
    try {
      ...
      const metadata = {
        ...
        format: "myrouter-full-backup-v1",
        contents: [
      ...
      return new NextResponse(archiveBuffer, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="myrouter-full-backup-${timestamp}.tar.gz"`,
