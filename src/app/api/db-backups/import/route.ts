...
import { setSystemPromptConfig } from "@myrouter/open-sse/services/systemPrompt.ts";
 ...
 * via `MYROUTER_DB_IMPORT_MAX_MB`. Invalid / out-of-range values fall back to the 100 MB
 * default and are clamped to a 4 GB ceiling.
 */
export function resolveMaxUploadSizeBytes(
  env: NodeJS.ProcessEnv = process.env
): number {
  const raw = env.MYROUTER_DB_IMPORT_MAX_MB;
...
// Required tables that must exist in a valid MyRouter database
...
export async function POST(request: Request) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          error:
            ...
            `Set MYROUTER_DB_IMPORT_MAX_MB to raise it, or VACUUM the database before exporting.`,
        },
        { status: 400 }
      );
    }
    ...
    tmpPath = path.join(os.tmpdir(), `myrouter-import-${Date.now()}.sqlite`);
    ...
    try {
      ...
      if (missingTables.length > 0) {
        return NextResponse.json(
          {
            error: `Invalid MyRouter database. Missing tables: ${missingTables.join(", ")}`,
          },
          { status: 400 }
        );
      }
