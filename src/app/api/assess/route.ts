...
const assessor = new Assessor(
  process.env.MyRouter_API_KEY ?? process.env.API_KEY ?? "",
  process.env.MyRouter_BASE_URL ?? "http://localhost:20128/v1"
);
...
async function getAllModels(): Promise<Array<{ providerId: string; modelId: string }>> {
  try {
    const resp = await fetch("http://localhost:20128/v1/models", {
      headers: {
        Authorization: `Bearer ${process.env.MyRouter_API_KEY ?? process.env.API_KEY ?? ""}`,
      },
    });
