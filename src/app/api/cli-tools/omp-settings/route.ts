...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasMyRouter: !!(ymlProvider || creds.hasMyRouter),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const keyRef = apiKey || "sk_myrouter";
    ...
    // 2. Write auth_credentials — so omp sees myrouter as "logged in"
    saveOmpCredentials(PROVIDER_ID, keyRef, normalizedBaseUrl);

    return NextResponse.json({
      success: true,
      message:
        "Oh My Pi settings applied! Run omp and all MyRouter models appear under myrouter in /model.",
      configPath: getOmpModelsYmlPath(),
    });
  } catch (error) {
    return NextResponse.json(
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "MyRouter removed from Oh My Pi",
    });
  } catch (error) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(error) } },
