...
// [model.myrouter] ... until the next [section] header or EOF
...
const PREV_DEFAULT_RE = /^# myrouter-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
 ...
 * only the flat string fields MyRouter itself writes.
 */
...
const buildModelSection = (model: string, baseUrl: string, apiKey: string): string => {
  const lines = [
    ...
    `name = "MyRouter"`,
    `description = "Routed via MyRouter gateway"`,
...
/** Insert/replace the `[model.myrouter]` section, preserving the rest of the file. */
...
const rememberPrevDefault = (toml: string): string => {
  ...
  const marker = `# myrouter-prev-default = "${current}"\n`;
...
const hasMyRouterConfig = (modelCfg: GrokModelSection | null): boolean =>
...
// GET — check Grok Build CLI and return current [model.myrouter] config
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasMyRouter: hasMyRouterConfig(model),
...
// POST — write the [model.myrouter] section into ~/.grok/config.toml and set it default
...
// DELETE — remove the [model.myrouter] section and restore the previous default
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "MyRouter model slot removed from Grok Build",
    });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
