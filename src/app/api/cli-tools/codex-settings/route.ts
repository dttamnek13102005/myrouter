...
const parseToml = (content: string) => {
  ...
  content.split("\n").forEach((line) => {
    ...
    // Section header like [model_providers.myrouter]
...
// Check if config has MyRouter settings
const hasMyRouterConfig = (config: string | null) => {
  if (!config) return false;
  return (
    config.includes("openai_base_url") ||
    config.includes('model_provider = "myrouter"') ||
    config.includes("[model_providers.myrouter]")
  );
};
...
// POST - Update MyRouter settings (merge with existing config)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Update only MyRouter related fields (api_key goes to auth.json, not config.toml)
    ...
    // Always create a custom provider to reliably pass wire_api and use MYROUTER_API_KEY
    parsed._root.model_provider = "myrouter";
    parsed._sections["model_providers.myrouter"] = {
      name: "MyRouter",
...
// DELETE - Remove MyRouter settings only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove MyRouter related root fields
    delete parsed._root.openai_base_url;

    if (parsed._root.model_provider === "myrouter") {
      delete parsed._root.model;
      delete parsed._root.model_provider;
    }

    // Remove myrouter provider section
    delete parsed._sections["model_providers.myrouter"];
    ...
    return NextResponse.json({
      success: true,
      message: "MyRouter settings removed successfully",
    });
  } catch (error) {
