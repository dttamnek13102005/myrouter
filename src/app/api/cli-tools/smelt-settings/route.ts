...
/**
 * Check if the config file contains MyRouter settings.
 */
...
// POST — write MyRouter settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge MyRouter settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove MyRouter settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove MyRouter-managed fields
    ...
    return NextResponse.json({ success: true, message: "Smelt MyRouter settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
