...
/**
 * Render the MyRouter config block in CodeWhale TOML format.
...
function renderCodewhaleConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# CodeWhale config — managed by MyRouter (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
 ...
 */
const hasMyRouterConfig = (content: string | null): boolean => {
  if (!content) return false;
  return content.includes("managed by MyRouter");
};
...
// POST — write MyRouter settings to CodeWhale's config.toml (primary), and
...
// DELETE — remove MyRouter CodeWhale config (primary + legacy, if present)
