...
// POST — write MyRouter settings to jcode config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge MyRouter settings (jcode uses OpenAI-compatible config)
...
// DELETE — remove MyRouter settings from jcode config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "jcode MyRouter settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
