...
import { resolveMyRouterBaseUrl } from "@/shared/utils/resolveMyRouterBaseUrl";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [healthRes, combosRes] = await Promise.allSettled([
      fetch(`${MYROUTER_BASE_URL}/api/monitoring/health`, { signal: AbortSignal.timeout(5000) }),
      fetch(`${MYROUTER_BASE_URL}/api/combos`, { signal: AbortSignal.timeout(5000) }),
    ]);
