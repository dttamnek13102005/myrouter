...
/**
 * Render the MyRouter config block in DeepSeek TUI TOML format.
...
function renderDeepseekTuiConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# DeepSeek TUI config — managed by MyRouter (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
...
// POST — write MyRouter settings to DeepSeek TUI config.toml
...
// DELETE — remove DeepSeek TUI MyRouter config
