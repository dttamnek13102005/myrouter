...
// Check if MyRouter OpenAI-compatible provider is configured
const hasMyRouterConfig = (auth) => {
  if (!auth) return false;
  const routerEntry = auth["openai-compatible"] || auth["myrouter"];
  if (!routerEntry) return false;
  const baseUrl = routerEntry.baseUrl || routerEntry.baseURL || "";
  return (
    baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("myrouter")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasMyRouter: hasMyRouterConfig(auth),
      authPath: AUTH_PATH,
    });
  } catch (error) {
...
// POST - Configure Kilo Code to use MyRouter as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update MyRouter as openai-compatible provider
...
// DELETE - Remove MyRouter config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove MyRouter provider
    delete auth["openai-compatible"];
    delete auth["myrouter"];
    ...
    return NextResponse.json({
      success: true,
      message: "MyRouter settings removed from Kilo Code",
    });
  } catch (error) {
