...
import { sanitizeErrorMessage } from "@myrouter/open-sse/utils/error.ts";
...
// the `myrouter setup-crush` CLI command agree on one canonical location.
...
/**
 * Crush's config uses a `providers.<id>` map. MyRouter is registered under
 * the `myrouter` provider id as an `openai-compat` provider — same shape
...
const hasMyRouterConfig = (settings: Record<string, unknown> | null): boolean => {
  if (!settings) return false;
  const providers = settings.providers as Record<string, unknown> | undefined;
  const myrouter = providers?.myrouter as Record<string, unknown> | undefined;
  return (
    !!myrouter &&
    myrouter.type === "openai-compat" &&
    typeof myrouter.base_url === "string" &&
    myrouter.base_url.length > 0
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reason: runtime.reason,
      config,
      hasMyRouter: hasMyRouterConfig(config),
...
// POST — write MyRouter settings to crush.json (providers.myrouter)
export async function POST(request: Request) {
  ...
  try {
    ...
    const provider: CrushProvider = {
      ...
      api_key: apiKey,
      models: [{ id: model, name: `MyRouter: ${model}`, context_window: DEFAULT_CONTEXT_WINDOW }],
    };
...
// DELETE — remove MyRouter provider from Crush config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Crush MyRouter settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
