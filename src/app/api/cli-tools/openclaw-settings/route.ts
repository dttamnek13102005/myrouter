...
// Check if settings has MyRouter config
const hasMyRouterConfig = (settings: any) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["myrouter"];
};
...
// POST - Update MyRouter settings (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    settings.agents.defaults.model.primary = `myrouter/${model}`;

    // Update models.providers.myrouter
    settings.models.providers["myrouter"] = {
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove MyRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["myrouter"];
    ...
    // Reset agents.defaults.model.primary if it uses myrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("myrouter/")) {
