  ...
  try {
    switch (toolId) {
      ...
      case "opencode":
        ...
        // OpenCode reads ~/.config/opencode/opencode.json — write the MyRouter settings there.
...
async function saveContinueConfig({ baseUrl, apiKey, model }) {
  ...
  // Build the MyRouter model entry
  ...
  // Check if MyRouter entry already exists and update it, or add new
  const existingIdx = models.findIndex(
    (m) =>
      m &&
      (m.myrouterManaged === true ||
        normalizeApiBase(m.apiBase) === normalizedBaseUrl.toLowerCase() ||
        normalizeApiBase(m.apiBase).includes("myrouter") ||
 ...
 * keys, but make sure the MyRouter provider entry is present and selected.
 */
async function saveHermesConfig({ baseUrl, apiKey, model }) {
  ...
  const nextConfig = {
    ...
    providers: {
      ...
      myrouter: {
        ...((existingConfig.providers && existingConfig.providers.myrouter) || {}),
        base_url: providerBaseUrl,
        api_key:
          apiKey ||
          (existingConfig.providers &&
            existingConfig.providers.myrouter &&
            existingConfig.providers.myrouter.api_key) ||
          "",
      },
    },
  };
