...
/**
 * Render the MyRouter provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by MyRouter (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-MyRouter marker comment.
 */
...
// POST — write MyRouter settings to Forge config.toml
...
// DELETE — remove Forge MyRouter config
