...
const getBackupPath = () =>
  path.join(getLocalBackendDir(), "providers", "auth.json.myrouter-backup");
...
// ── Check if a base_url points to MyRouter ──────────────────────────────
const isMyRouterUrl = (baseUrl) => {
  if (!baseUrl) return false;
  return baseUrl.includes(":20128") || baseUrl.includes(":3000") || baseUrl.includes("myrouter");
};

// ── Check if MyRouter is configured ─────────────────────────────────────
const hasMyRouterConfig = (authFile) => {
  if (!authFile?.providers) return false;
  const provider = authFile.providers[PROVIDER_NAME];
  if (!provider) return false;
  return isMyRouterUrl(provider.base_url);
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Detect if lmstudio is already configured for a non-MyRouter endpoint
    let lmstudioConflict = false;
    if (provider && !isMyRouterUrl(provider.base_url)) {
    ...
    return NextResponse.json({
      installed: true,
      config: authFile,
      hasMyRouter: hasMyRouterConfig(authFile),
      lmstudioConflict,
...
// ── POST - Apply MyRouter as LM Studio provider + switch to local mode ──
/**
 ...
 * up a non-MyRouter provider before it is overwritten. Extracted to keep POST under
 * the complexity gate.
 */
...
> {
  ...
  if (existingProvider && !isMyRouterUrl(existingProvider.base_url) && !overwrite) {
  ...
  if (existingProvider && !isMyRouterUrl(existingProvider.base_url)) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ── 1-2. Read auth.json, guard non-MyRouter conflicts, back up before overwrite ──
    ...
    // Clean up legacy lc-myrouter provider if present
    if (authFile.providers?.["lc-myrouter"]) {
      delete authFile.providers["lc-myrouter"];
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Settings applied. Restart Letta CLI, then use /model to select a MyRouter model.",
      needsRestart: true,
    });
  } catch (error) {
    return NextResponse.json(
...
// ── DELETE - Remove MyRouter configuration ──────────────────────────────
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const message = restored
      ? "MyRouter config removed. Your original LM Studio provider has been restored. Restart Letta CLI to take effect."
      : "MyRouter config removed. Restart Letta CLI to take effect.";
