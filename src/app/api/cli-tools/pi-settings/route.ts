...
// POST — write MyRouter settings to Pi config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge MyRouter settings (pi uses OpenAI-compatible config)
...
// DELETE — remove MyRouter settings from Pi config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Pi MyRouter settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
