...
import { handleAudioTranscription } from "@myrouter/open-sse/handlers/audioTranscription.ts";
import {
...
import { attachMyRouterMetaToResponse } from "@/domain/myrouterResponseMeta";
...
export async function POST(request) {
  ...
  if (response?.ok) {
    ...
    response = attachMyRouterMetaToResponse(response, {
      provider,
      model: resolvedModel,
      costUsd: 0,
