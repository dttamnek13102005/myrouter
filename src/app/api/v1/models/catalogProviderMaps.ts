...
import { parseModel } from "@myrouter/open-sse/services/model";
...
/** True when `${prefix}/__myrouter_probe__` parses back to the given providerId. */
export function prefixRoutesToProvider(prefix: string, providerId: string): boolean {
  const parsed = parseModel(`${prefix}/__myrouter_probe__`);
