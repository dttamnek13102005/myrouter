...
function isIssueAgentEnabled(): boolean {
  return ENABLED_VALUES.has((process.env.MYROUTER_ISSUE_AGENT_ENABLED ?? "").toLowerCase());
}
...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        ...
        enabled: false,
        requiredEnv: "MYROUTER_ISSUE_AGENT_ENABLED=true",
      },
      { status: 403 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      {
        ...run,
        runner: "myrouter-chat-completions",
