...
import { estimateTokens } from "@myrouter/open-sse/services/contextManager.ts";
import { adaptBodyForCompression } from "@myrouter/open-sse/services/compression/bodyAdapter.ts";
import type {
  CompressionConfig,
  CompressionResult,
} from "@myrouter/open-sse/services/compression/types.ts";
import { resolveCompressionSettings } from "@myrouter/open-sse/handlers/chatCore/compressionSettings.ts";
import {
  writeCompressionAnalytics,
  writeCompressionSkip,
} from "@myrouter/open-sse/handlers/chatCore/compressionAnalyticsWrite.ts";
...
export async function applyResponsesWsCompression(
  ...
  try {
    ...
    const { selectCompressionStrategy, applyCompressionAsync } = await import(
      "@myrouter/open-sse/services/compression/strategySelector.ts"
    );
