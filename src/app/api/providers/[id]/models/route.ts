...
import { getRegistryEntry } from "@myrouter/open-sse/config/providerRegistry.ts";
...
import { getStaticQoderModels } from "@myrouter/open-sse/services/qoderCli.ts";
import { deriveConfigFromRegistryModelsUrl } from "./discoveryConfig";
import {
  fetchGitHubCopilotModels,
  fetchGheCopilotModels,
} from "@myrouter/open-sse/services/githubCopilotModels.ts";
import { fetchKiroAvailableModels } from "@myrouter/open-sse/services/kiroModels.ts";
import {
  buildGlmCodingHeaders,
  buildGlmModelsUrl,
} from "@myrouter/open-sse/config/glmProvider.ts";
import { getImageProvider } from "@myrouter/open-sse/config/imageRegistry.ts";
import { getVideoProvider } from "@myrouter/open-sse/config/videoRegistry.ts";
import {
  discoverBedrockNativeModels,
  isBedrockNativeApiError,
} from "@myrouter/open-sse/services/bedrock.ts";
import {
  discoverPromptQlModels,
  PROMPTQL_FALLBACK_MODELS,
} from "@myrouter/open-sse/services/promptqlModels.ts";
import {
  discoverNotionWebModels,
  NOTION_WEB_FALLBACK_MODELS,
} from "@myrouter/open-sse/services/notionWebModels.ts";
import {
  AZURE_AI_DEFAULT_BASE_URL,
  buildAzureAiModelsUrl,
} from "@myrouter/open-sse/config/azureAi.ts";
import {
  DATAROBOT_DEFAULT_BASE_URL,
  buildDataRobotCatalogUrl,
  isDataRobotDeploymentUrl,
} from "@myrouter/open-sse/config/datarobot.ts";
import { OCI_DEFAULT_BASE_URL, buildOciModelsUrl } from "@myrouter/open-sse/config/oci.ts";
import {
  SAP_DEFAULT_BASE_URL,
  buildSapModelsUrl,
  getSapResourceGroup,
} from "@myrouter/open-sse/config/sap.ts";
import {
  WATSONX_DEFAULT_BASE_URL,
  buildWatsonxModelsUrl,
} from "@myrouter/open-sse/config/watsonx.ts";
import { getEmbeddingProvider } from "@myrouter/open-sse/config/embeddingRegistry.ts";
import { getRerankProvider } from "@myrouter/open-sse/config/rerankRegistry.ts";
import {
  getSpeechProvider,
...
export async function GET(
  ...
  try {
    ...
    const buildDiscoveryErrorFallbackResponse = (
      ...
      // the local/cached catalog MyRouter ships instead of surfacing a raw 503.
    ...
    if (provider === "vertex" || provider === "vertex-partner") {
      ...
      try {
        const { parseSAFromApiKey, getAccessToken } =
          await import("@myrouter/open-sse/executors/vertex.ts");
        if (accessToken) {
