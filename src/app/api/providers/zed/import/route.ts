 ...
 *
 * For backwards compatibility, when `MYROUTER_ZED_IMPORT_LEGACY_ONE_STEP=true`
...
const LEGACY_ONE_STEP_ENABLED = process.env.MYROUTER_ZED_IMPORT_LEGACY_ONE_STEP === "true";
...
export async function POST(request: Request): Promise<NextResponse<ImportResponse> | Response> {
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "MyRouter is running inside Docker and cannot access the host keychain. " +
    ...
    if (confirmed !== null) {
      ...
      console.warn(
        "[Zed Import] MYROUTER_ZED_IMPORT_LEGACY_ONE_STEP=true — importing all keychain credentials without per-account confirmation. This mode is deprecated and will be removed in v3.9."
      );
    }
