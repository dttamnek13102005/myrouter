 ...
 *
 * Re-issues the captured request through the local MyRouter instance and
...
import { buildErrorBody, sanitizeErrorMessage } from "@myrouter/open-sse/utils/error.ts";
...
const MYROUTER_BASE = process.env.MYROUTER_BASE_URL ?? "http://127.0.0.1:20128";
...
export async function POST(_request: Request, { params }: Params): Promise<Response> {
  ...
  const url = `${MYROUTER_BASE}${entry.path}`;

  const replayHeaders: Record<string, string> = {
    "content-type": "application/json",
    "x-myrouter-source": "inspector-replay",
  };
