...
// cause is running MyRouter in Docker — `localhost` then points at the container,
...
export function augmentDockerLocalhostHint(
  ...
  if (code === "ECONNREFUSED") {
    return "Connection refused — are you running MyRouter in Docker? localhost points to the container, not your host. Use your host IP (e.g. http://192.168.x.x:11434) or http://host.docker.internal:11434 on Linux/Mac.";
  }
  if (code === "ETIMEDOUT") {
    return "Connection timeout — are you running MyRouter in Docker? Use your host IP (e.g. http://192.168.x.x:11434) or http://host.docker.internal:11434 on Linux/Mac.";
  }
