...
export default function HermesAgentToolCard({
  ...
  const setRoleSelection = (roleId: string, model: string, provider = "MyRouter") => {
    setSelections((prev) => ({ ...prev, [roleId]: { model, provider } }));
  };

  const applyToAll = (model: string) => {
    const newSel: Record<string, RoleSelection> = {};
    HERMES_ROLES.forEach((r) => (newSel[r.id] = { model, provider: "MyRouter" }));
  ...
  const effectiveRoles = React.useMemo(() => {
    // If user has pending changes, treat selected roles as MyRouter
    if (Object.keys(selections).length > 0) {
      const map: Record<string, any> = {};
      HERMES_ROLES.forEach((r) => {
        if (selections[r.id]) {
          map[r.id] = { usingMyRouter: true };
  ...
  // Count of roles that are (or will be) routed via MyRouter
  const configuredRolesCount = HERMES_ROLES.filter((role) => {
    // Pending selection always counts as MyRouter intent
    if (selections[role.id]) return true;

    const info = effectiveRoles[role.id];
    if (!info) return false;

    // Support both shapes: detector shape (usingMyRouter) and settings shape (provider + base_url)
    if (typeof info.usingMyRouter === "boolean") {
      return info.usingMyRouter;
    }
    return (
      info?.provider === "myrouter" ||
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {HERMES_ROLES.map((role) => {
              ...
              // - If user has selected something in this session (pending): show as via MyRouter
              // - Else if current from disk: show real provider name + "(not MyRouter)" or "MyRouter"
              let badge: { label: string; pending: boolean; outsideMyRouter: boolean } | null =
                null;

              if (sel) {
                // pending change made via the Select modal / quick apply → will be routed via MyRouter
                const prov = sel.provider || "MyRouter";
                badge = {
                  label: t("hermesViaMyRouter", { provider: prov }),
                  pending: true,
                  outsideMyRouter: false,
                };
              } else if (current) {
                const isOmni =
                  current?.provider === "myrouter" ||
                  (current?.base_url || "").includes("20128") ||
                  (current?.base_url || "").includes("localhost");

                if (isOmni) {
                  badge = { label: "MyRouter", pending: false, outsideMyRouter: false };
                } else {
                  const realProvider = current.provider || t("other");
                  badge = {
                    label: t("hermesNotMyRouter", { provider: realProvider }),
                    pending: false,
                    outsideMyRouter: true,
                  };
                }
              }
        ...
        onSelect={(model: any) => {
          if (modalRole) {
            ...
            if (modelValue) {
              ...
              const prov =
                (model && (model.provider || model.providerId || model.group)) || "MyRouter";
