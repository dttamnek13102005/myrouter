 ...
 *
 * Feature request: https://github.com/diegosouzapw/MyRouter/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("myrouter-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("myrouter-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "myrouter-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "MyRouter",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.myrouter}`,
      models,
    };

    const responsesConfig = {
      name: "MyRouter-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.myrouter}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("myrouter-cli-key-copilot", value);
  };
