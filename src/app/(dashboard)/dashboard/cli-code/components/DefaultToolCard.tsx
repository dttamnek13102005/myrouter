...
export default function DefaultToolCard({
  ...
  const resolveApiKeyValue = useCallback(
    () => selectedKeyObj?.rawKey || (!cloudEnabled ? "sk_myrouter" : t("yourApiKeyPlaceholder")),
  ...
  useEffect(() => {
    const restoreTimer = window.setTimeout(() => {
      const savedModel = localStorage.getItem(`myrouter-cli-model-${toolId}`);
      if (savedModel) {
        if (isMultiModelTool) {
          try {
      ...
      const savedKey = localStorage.getItem(`myrouter-cli-key-${toolId}`);
  ...
  const handleModelChange = useCallback(
    (value) => {
      setModelValue(value);
      if (value) {
        localStorage.setItem(`myrouter-cli-model-${toolId}`, value);
      } else {
        localStorage.removeItem(`myrouter-cli-model-${toolId}`);
      }
    },
    [toolId]
  );
  ...
  const handleModelValuesChange = useCallback(
    (values) => {
      ...
      if (normalized.length > 0) {
        localStorage.setItem(`myrouter-cli-model-${toolId}`, JSON.stringify(normalized));
      } else {
        localStorage.removeItem(`myrouter-cli-model-${toolId}`);
      }
    },
  ...
  const handleApiKeyChange = useCallback(
    (value) => {
      ...
      if (value) {
        ...
        localStorage.setItem(`myrouter-cli-key-${toolId}`, value);
      }
    },
    [toolId]
  );
      ...
      <div className="mt-2 flex items-center gap-2">
        {apiKeys && apiKeys.length > 0 ? (
          ...
          <span className="text-sm text-text-muted">
            {cloudEnabled ? t("noApiKeysCreateOne") : "sk_myrouter"}
          </span>
        )}
      </div>
    );
