...
export default function DroidToolCard({
  ...
  // (#618) Match any custom:MyRouter-<i> entry (multi-model).
  const isMyRouterEntry = (m) => typeof m?.id === "string" && m.id.startsWith("custom:MyRouter");

  const getConfigStatus = () => {
    if (!cliReady) return null;
    const currentConfig = droidStatus.settings?.customModels?.find(isMyRouterEntry);
    ...
    try {
      ...
      const res = await fetch("/api/cli-tools/droid-settings", {
        ...
        body: JSON.stringify({
          ...
          apiKey: !cloudEnabled ? "sk_myrouter" : null,
          keyId: selectedKeyId,
  ...
  const getManualConfigs = () => {
    ...
    const keyToDisplay =
      selectedKeyObj?.key || (!cloudEnabled ? "sk_myrouter" : "<API_KEY_FROM_DASHBOARD>");
          ...
          {!checkingDroid && cliReady && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL — first MyRouter entry, any index (#618) */}
                {droidStatus?.settings?.customModels?.find(isMyRouterEntry)?.baseUrl && (
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {apiKeys.length > 0 ? (
                    ...
                    <span className="flex-1 text-xs text-text-muted px-2 py-1.5">
                      {cloudEnabled ? t("noApiKeysCreateOne") : t("defaultMyrouterKey")}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!droidStatus?.hasMyRouter}
                  loading={restoring}
                >
