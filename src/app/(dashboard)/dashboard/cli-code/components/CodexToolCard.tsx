...
export default function CodexToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_myrouter for localhost if no key, otherwise use selected key
      const keyToUse =
  ...
  const getManualConfigs = () => {
    const keyToUse = !cloudEnabled ? "sk_myrouter" : "<YOUR_MYROUTER_API_KEY>";

    let configContent = `# MyRouter Configuration for Codex CLI
model = "${selectedModel || CODEX_DEFAULT_MODELS[0]}"`;
...
# Utilize the built-in OpenAI provider pointed to MyRouter
