...
export function SkillPreviewPane({
  ...
  const handleCopyRawUrl = useCallback(async () => {
    if (!skillId) return;
    const rawUrl = `https://raw.githubusercontent.com/diegosouzapw/MyRouter/refs/heads/main/skills/${skillId}/SKILL.md`;
    try {
  ...
  const githubUrl = skillId
    ? `https://github.com/diegosouzapw/MyRouter/blob/main/skills/${skillId}/SKILL.md`
    : null;
