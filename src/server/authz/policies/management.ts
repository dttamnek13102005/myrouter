...
function requestPeerAddress(ctx: PolicyContext): string | null {
  ...
  const stamped = resolveStampedPeer(
    ...
    process.env.MYROUTER_PEER_STAMP_TOKEN
  );
...
const WS_BRIDGE_SECRET_HEADER = "x-myrouter-ws-bridge-secret";
...
// server-ws.mjs (MYROUTER_WS_BRIDGE_SECRET). Without this carve-out the MANAGEMENT
...
function isValidWsBridgeRequest(ctx: PolicyContext): boolean {
  ...
  const expected = process.env.MYROUTER_WS_BRIDGE_SECRET || "";
