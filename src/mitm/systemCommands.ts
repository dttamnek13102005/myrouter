 ...
 *
 * Slim Docker images (e.g. `node:24-trixie-slim` used by MyRouter's runtime
...
/**
 * Truthy-env check for `MYROUTER_NO_SUDO`. Inlined (not imported from
...
export function isNoSudoEnv(): boolean {
  const value = process.env.MYROUTER_NO_SUDO;
 ...
 * the operator opts into root-less mode via `MYROUTER_NO_SUDO` (#6122). No
...
export function execFileWithPassword(
  ...
  // `MYROUTER_NO_SUDO` (root-less / user-namespace deployments — #6122), skip
...
export async function runElevatedPowerShell(script: string): Promise<string> {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-elevate-"));
  const scriptName = `myrouter-elevate-${crypto.randomUUID()}.ps1`;
