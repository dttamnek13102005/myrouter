...
// Inspector. The token is injected by manager.ts (same value the MyRouter
 ...
 * shape. The route-only namespace is reserved for client-facing MyRouter model ids;
...
async function passthrough(req, res, bodyBuffer) {
  ...
  // Defense-in-depth loop guard (Gap 14). The x-myrouter-source header is the
...
async function intercept(req, res, bodyBuffer, override, sourceModel) {
  ...
  // The MyRouter router uses these to distinguish AgentBridge traffic from
  ...
  try {
    ...
    const response = await fetch(forward.url, {
      ...
      headers: {
        ...
        "x-myrouter-source": "agent-bridge",
        "x-myrouter-agent": agentId,
      },
    ...
    if (!response.ok) {
      ...
      throw new Error(`MyRouter ${response.status}: ${errText}`);
    }
...
async function startMitmServer() {
  ...
  const server = https.createServer(sslOptions, async (req, res) => {
    ...
    if (req.headers["x-myrouter-source"] === "myrouter") {
      vlog(1, `[MITM] → PASSTHROUGH (MyRouter source loop)`);
