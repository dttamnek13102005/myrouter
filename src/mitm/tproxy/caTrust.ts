 ...
 * trust store under a DEDICATED slot (`myrouter-tproxy-ca.crt`), separate from
 * the static MITM cert (`myrouter-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "myrouter-tproxy-ca.crt";
...
export async function installTproxyCa(
  ...
  if (process.env.MYROUTER_SKIP_SYSTEM_TRUST === "1" && deps.run === undefined) {
    console.log("[tproxy-ca] MYROUTER_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
