 ...
 *   - forwarding to the MyRouter router (Next.js API)
 *   - SSE piping
 ...
 * Routes through `@myrouter/open-sse/utils/error.sanitizeErrorMessage` (Hard Rule #12)
...
async function safeErrorMessage(err: unknown): Promise<string> {
  try {
    const mod = (await import("@myrouter/open-sse/utils/error")) as {
...
export abstract class MitmHandlerBase {
   ...
   *   3. Call `this.fetchRouter(...)` for the MyRouter router round-trip.
  ...
  /**
   * Forward the prepared body to the MyRouter router (Next.js API).
   * Adds AgentBridge correlation headers (`x-myrouter-source`, `x-myrouter-agent`)
  ...
  protected async fetchRouter(
    ...
    const base = process.env.MYROUTER_BASE_URL ?? "http://127.0.0.1:20128";
