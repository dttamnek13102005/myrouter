 ...
 * `thinkingConfig`, …). The MyRouter router endpoint `/v1/chat/completions`
 ...
 *   - forward to `/v1/chat/completions` on the MyRouter router,
 ...
 * @param model      resolved MyRouter model string
  ...
  async intercept(
    ...
    try {
      ...
      if (!upstream.ok) {
        ...
        throw new Error(`MyRouter ${upstream.status}: ${errText}`);
      }
