...
const LINUX_CERT_NAME = "myrouter-mitm.crt";
  ...
  return new Promise((resolve) => {
    exec(
      script,
      {
        ...
        env: {
          ...process.env,
          CERT_NAME: "MyRouter MITM Root CA",
          CERT_PATH: certPath || "",
          ACTION: action,
        },
      },
...
export async function installCert(sudoPassword: string, certPath: string): Promise<void> {
  ...
  if (process.env.MYROUTER_SKIP_SYSTEM_TRUST === "1") {
    console.log("[cert] MYROUTER_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
 ...
 * cert-path-agnostic and keep writing to the same `myrouter-mitm.crt`
 ...
 * cleanup needed. Distinct from TPROXY's own `myrouter-tproxy-ca.crt` slot
...
// They install / remove the MyRouter MITM root CA from the OS trust store and
