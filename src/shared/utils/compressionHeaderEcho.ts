 ...
 *
 * When a request carries `x-myrouter-compression`, docs promise the response echoes
 * `X-MyRouter-Compression: <mode>; source=<source>`. Internal paths (idempotency
...
const COMPRESSION_REQUEST_HEADER = "x-myrouter-compression";
const COMPRESSION_RESPONSE_HEADER = MYROUTER_RESPONSE_HEADERS.compression;
...
/**
 * Wrap a Response so it carries `X-MyRouter-Compression: <mode>; source=request-header`
 * when the request supplied `x-myrouter-compression` and the inner pipeline did not
