...
// of `./outboundUrlGuard.ts` so the CLI (`myrouter setup-opencode`, loaded via tsx with no
...
export const PRIVATE_PROVIDER_URLS_ENV = "MYROUTER_ALLOW_PRIVATE_PROVIDER_URLS";
...
// cloud-metadata endpoints stay blocked. Defaults ON (MyRouter is local-first); operators
// who only use public providers can disable it to restore strict SSRF blocking.
export const LOCAL_PROVIDER_URLS_ENV = "MYROUTER_ALLOW_LOCAL_PROVIDER_URLS";
 ...
 * (MyRouter is local-first — local OpenAI-compatible providers should validate out of the
 * box). Disable via the `MYROUTER_ALLOW_LOCAL_PROVIDER_URLS` flag (DB toggle or env) to
 ...
 * provider URLs (`MYROUTER_ALLOW_PRIVATE_PROVIDER_URLS`, default OFF). Protocol and
