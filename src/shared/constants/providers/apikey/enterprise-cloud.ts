...
export const APIKEY_PROVIDERS_ENTERPRISE = {
  ...
  "azure-ai": {
    ...
    apiHint:
      "Foundry uses the OpenAI v1 surface with deployment names as models. MyRouter normalizes root resource URLs to the v1 chat and /models endpoints.",
    passthroughModels: true,
  },
  bedrock: {
    id: "bedrock",
    ...
    authHint:
      "Use your Amazon Bedrock API key and configure the AWS region where your models are enabled (for example eu-west-2). MyRouter calls Bedrock's native Converse API directly.",
    apiHint:
  ...
  oci: {
    ...
    apiHint:
      "OCI exposes OpenAI-compatible chat and responses endpoints. Project ID is optional in MyRouter but may be required for Responses and agentic workflows.",
    passthroughModels: true,
  },
  sap: {
    id: "sap",
  ...
  modal: {
    ...
    apiHint:
      "Modal commonly serves user-hosted OpenAI-compatible apps on /v1. MyRouter will probe /v1/models and route chat traffic to /v1/chat/completions.",
    hasFree: true,
  ...
  clarifai: {
    ...
    authHint:
      "Use your Clarifai PAT or app-specific API key. MyRouter targets the OpenAI-compatible endpoint at https://api.clarifai.com/v2/ext/openai/v1 and authenticates with Authorization: Key <token>.",
    apiHint:
