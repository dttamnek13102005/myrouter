...
export function resolveModelSyncInternalBaseUrl(_candidate?: string): string {
  ...
  const nativeTls = process.env.MYROUTER_INTERNAL_SCHEME === "https";
  const origin = nativeTls
    ? `https://localhost:${dashboardPort}`
    : `http://127.0.0.1:${dashboardPort}`;
  return `${origin}${normalizeInternalBasePath(process.env.MYROUTER_BASE_PATH)}`;
}
...
const globalState = globalThis as typeof globalThis & {
  __myrouterModelSyncInternalAuthToken?: string;
};
...
function getInternalAuthToken(): string {
  if (!internalAuthToken) {
    internalAuthToken = globalState.__myrouterModelSyncInternalAuthToken || randomUUID();
    globalState.__myrouterModelSyncInternalAuthToken = internalAuthToken;
  }
...
export function isModelSyncInternalRequest(request: { headers: Headers }): boolean {
  if (!internalAuthToken && globalState.__myrouterModelSyncInternalAuthToken) {
    internalAuthToken = globalState.__myrouterModelSyncInternalAuthToken;
  }
 ...
 * @param apiBaseUrl — internal base URL to call MyRouter's own API
