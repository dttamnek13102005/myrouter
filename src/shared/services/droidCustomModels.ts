/**
 * Build the MyRouter `customModels` entries for Factory Droid's `settings.json`.
 *
 * Ported from upstream PR decolua/9router#618 (author Anurag Saxena) — multi-model
 * support for the Factory Droid CLI tool. Adapted to MyRouter branding
 * (`custom:MyRouter-<i>`) and extracted as a pure helper so the route-handler
...
/** True when a `customModels` entry was written by MyRouter (any index). */
export function isMyRouterCustomModel(entry: { id?: unknown } | null | undefined): boolean {
  return typeof entry?.id === "string" && entry.id.startsWith("custom:MyRouter");
}
