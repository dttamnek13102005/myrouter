...
export const QWEN_CODE_ENV_KEY = "MYROUTER_API_KEY";
...
/**
 * Identifies only entries owned by MyRouter. Generic custom endpoints are not
...
export const isManagedQwenCodeModel = (value: unknown): boolean => {
  ...
  if (value.envKey === QWEN_CODE_ENV_KEY || value.id === "myrouter") return true;

  return (
    typeof value.name === "string" &&
    value.name.endsWith(" (MyRouter)") &&
...
export const buildQwenCodeModel = ({
  ...
  return {
    id: normalizedModel,
    name: `${String(modelName || normalizedModel).trim()} (MyRouter)`,
...
/**
 * Merge one MyRouter model into Qwen Code's current V4 settings contract.
...
export const mergeQwenCodeSettings = (
  ...
  if (next.selectedProvider === "myrouter") delete next.selectedProvider;

  return next;
};

export const findMyRouterQwenCodeModel = (settings: unknown): JsonRecord | undefined => {
...
export const hasMyRouterQwenCodeConfig = (settings: unknown): boolean =>
  findMyRouterQwenCodeModel(settings) !== undefined;

/** Remove only MyRouter-owned Qwen Code entries and selection state. */
...
const OWNED_ENV_LINE = /^\s*(?:export\s+)?MYROUTER_API_KEY\s*=/;
...
export const mergeQwenCodeEnv = (existing: unknown, apiKey: unknown): string => {
  ...
  lines.push(`${QWEN_CODE_ENV_KEY}=${JSON.stringify(String(apiKey || "sk_myrouter"))}`);
  return `${lines.join("\n")}\n`;
};
