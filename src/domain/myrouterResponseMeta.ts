...
export function getMyRouterTokenCounts(usage: UsageLike): { input: number; output: number } {
...
export function formatMyRouterCost(costUsd: unknown): string {
  const normalized = toFiniteNumber(costUsd);
  return normalized > 0 ? normalized.toFixed(10) : "0.0000000000";
}

/**
 * Build the `X-MyRouter-Decision` composite header value: `strategy=<name>;
 ...
 * (same alias normalization the `X-MyRouter-Provider` header already applies)
 ...
 */
export function buildMyRouterDecisionHeaderValue({
  strategy = null,
  provider = null,
  latencyMs = 0,
}: {
...
export function buildMyRouterResponseMetaHeaders({
  cacheHit = false,
  costUsd = 0,
   ...
   * Emitted as `X-MyRouter-Cost-Saved` only when provided (omitted on normal
   ...
   * request) surfaced via `X-MyRouter-Decision`. See #6022.
   */
  strategy?: string | null;
  usage?: UsageLike;
}): Record<string, string> {
  const tokens = getMyRouterTokenCounts(usage);
  const headers: Record<string, string> = {
    [MYROUTER_RESPONSE_HEADERS.cacheHit]: toHeaderValue(String(cacheHit)),
    [MYROUTER_RESPONSE_HEADERS.latencyMs]: toHeaderValue(String(toNonNegativeInteger(latencyMs))),
    [MYROUTER_RESPONSE_HEADERS.responseCost]: toHeaderValue(formatMyRouterCost(costUsd)),
    [MYROUTER_RESPONSE_HEADERS.tokensIn]: toHeaderValue(String(tokens.input)),
    [MYROUTER_RESPONSE_HEADERS.tokensOut]: toHeaderValue(String(tokens.output)),
    [MYROUTER_RESPONSE_HEADERS.version]: toHeaderValue(APP_CONFIG.version),
  };

  if (typeof model === "string" && model.trim().length > 0) {
    headers[MYROUTER_RESPONSE_HEADERS.model] = toHeaderValue(model);
  }

  if (typeof requestId === "string" && requestId.trim().length > 0) {
    headers[MYROUTER_RESPONSE_HEADERS.requestId] = toHeaderValue(requestId);
  }

  if (typeof provider === "string" && provider.trim().length > 0) {
    headers[MYROUTER_RESPONSE_HEADERS.provider] = toHeaderValue(getProviderAlias(provider));
  }
  ...
  if (costSavedUsd != null) {
    headers[MYROUTER_RESPONSE_HEADERS.costSaved] = toHeaderValue(
      formatMyRouterCost(costSavedUsd)
    );
  }

  const attempts = toNonNegativeInteger(fallbackAttempts);
  if (attempts > 0) {
    headers[MYROUTER_RESPONSE_HEADERS.fallbackAttempts] = toHeaderValue(String(attempts));
  }

  const decisionValue = buildMyRouterDecisionHeaderValue({ strategy, provider, latencyMs });
  if (decisionValue !== null) {
    headers[MYROUTER_RESPONSE_HEADERS.decision] = decisionValue;
  }

  return headers;
}

export function buildMyRouterSseMetadataComment(
  options: Parameters<typeof buildMyRouterResponseMetaHeaders>[0]
): string {
  const headers = buildMyRouterResponseMetaHeaders(options);
...
/**
 * Single choke-point for attaching the X-MyRouter-* response meta headers.
 ...
 */
export function attachMyRouterMetaHeaders(
  headers: Headers | Record<string, string>,
  meta: Parameters<typeof buildMyRouterResponseMetaHeaders>[0]
): void {
  const built = buildMyRouterResponseMetaHeaders(meta);
...
/**
 * Attach the X-MyRouter-* meta headers onto an already-built Response, ADDING
 ...
 */
export function attachMyRouterMetaToResponse(
  response: Response,
  meta: Parameters<typeof buildMyRouterResponseMetaHeaders>[0]
): Response {
  ...
  try {
    attachMyRouterMetaHeaders(response.headers, meta);
    return response;
  } catch {
    ...
    attachMyRouterMetaHeaders(cloned.headers, meta);
    return cloned;
  }
}
