...
import { getUsageForProvider } from "@myrouter/open-sse/services/usage.ts";
...
import { getCodexQuotaWindowFilterForModel } from "@myrouter/open-sse/config/codexQuotaScopes.ts";
import { getAntigravityQuotaFamily } from "@myrouter/open-sse/services/antigravityQuotaFamily.ts";
...
declare global {
  var __myrouterQuotaCacheState: QuotaCacheState | undefined;
}

function getState(): QuotaCacheState {
  if (!globalThis.__myrouterQuotaCacheState) {
    globalThis.__myrouterQuotaCacheState = {
      cache: new Map(),
  ...
  return globalThis.__myrouterQuotaCacheState;
}
