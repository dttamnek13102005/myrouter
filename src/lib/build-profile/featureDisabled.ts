/**
 * Helper used by the `MYROUTER_BUILD_PROFILE=minimal` stubs to surface a
...
export class FeatureDisabledError extends Error {
  ...
  constructor(featureName: string) {
    super(
      `Feature "${featureName}" is disabled in this build (MYROUTER_BUILD_PROFILE=minimal). ` +
        `Install the full myrouter artifact instead of myrouter-secure if you need this feature.`
    );
