 ...
 * can be iframed inside the MyRouter dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between MyRouter and the embedded service.
 ...
 * Prevents MyRouter session cookies and Authorization from leaking upstream.
 */
 ...
 *   MyRouter origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the MyRouter iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (MyRouter controls framing via its own CSP).
