...
import { splitClaudeEffortSuffix } from "@myrouter/open-sse/config/providerModels.ts";
...
export function attachReasoningRuleDirective(
  ...
  body._myrouterReasoningRouteTrace = {
...
export function applyReasoningRuleDirective(bodyInput: unknown): unknown {
  const source = asRecord(bodyInput);
  const directive = asRecord(source._myrouterReasoningRule);
  if (!directive.id) return bodyInput;
  const body = { ...source };
  delete body._myrouterReasoningRule;
