...
export interface VncProviderEntry {
  ...
  url: string;
  /** Canonical MyRouter credential contract for this provider. */
...
const profileRoot =
  process.env.MYROUTER_VNC_PROFILE_DIR ||
  `${process.env.HOME || "/tmp"}/.myrouter/browser-login-profiles`;

export const VNC_CONFIG = {
  /**
   * This feature uses Chromium CDP only. Build docker/vnc-browser/chromium and
   * tag it with this name, or override MYROUTER_VNC_IMAGE.
   */
  image: process.env.MYROUTER_VNC_IMAGE || "myrouter-vnc-chromium:local",
  containerVncPort: Number(process.env.MYROUTER_VNC_CONTAINER_VNC_PORT || 3000),
  containerCdpPort: Number(process.env.MYROUTER_VNC_CONTAINER_CDP_PORT || 9223),
  containerProfileDir: process.env.MYROUTER_VNC_CONTAINER_PROFILE_DIR || "/config",
  profileDir: profileRoot,
  persistProfiles: envFlag("MYROUTER_VNC_PERSIST_PROFILES", false),
  idleTimeoutMs: Number(process.env.MYROUTER_VNC_IDLE_MS || 10 * 60 * 1000),
  maxSessionMs: Number(process.env.MYROUTER_VNC_MAX_MS || 30 * 60 * 1000),
  maxSessions: Number(process.env.MYROUTER_VNC_MAX_SESSIONS || 4),
  dockerBin: process.env.MYROUTER_DOCKER_BIN || "docker",
  browserReadyTimeoutMs: Number(process.env.MYROUTER_VNC_READY_MS || 45_000),
  harvestTimeoutMs: Number(process.env.MYROUTER_VNC_HARVEST_MS || 20_000),
  chromiumArgs:
    process.env.MYROUTER_VNC_CHROMIUM_ARGS ||
