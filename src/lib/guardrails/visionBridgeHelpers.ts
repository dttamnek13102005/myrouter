 ...
 *      MyRouter itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      MyRouter's own `/v1` so the vision model can be any provider
 *      registered in MyRouter (`google/gemini-2.0-flash`,
 *      `openrouter/...`, etc.) instead of being limited to OpenAI/Anthropic.
 *   2. `OPENAI_API_URL` env var (legacy)
 *   3. MyRouter self-loop (`http://localhost:20128/v1`) — auto-detected when
 *      the model uses a known MyRouter-internal provider (e.g. `kr/`, `if/`,
 ...
 *                that require MyRouter self-loop routing.
 */
export function resolveVisionBridgeBaseUrl(model?: string): string {
  ...
  // anthropic/), it can only be resolved through MyRouter's own router,
  // not through a direct OpenAI/Anthropic endpoint. Use the operator-configured
  // port via MYROUTER_PORT / PORT env vars, falling back to the default 20128.
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // MyRouter itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the MyRouter self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so MyRouter can resolve the
      ...
      // When routing through MyRouter's own API, omit the vision-bridge
      // guardrail on the sub-request to prevent infinite recursion.
      // Use sk_myrouter as fallback for self-loop if no API key is resolved.
      const selfLoopApiKey = resolvedApiKey || "sk_myrouter";
      const headers: Record<string, string> = {
        "Content-Type": "application/json",
        Authorization: `Bearer ${selfLoopApiKey}`,
      };
      if (useFullModelId) {
        headers["x-myrouter-disabled-guardrails"] = "vision-bridge";
      }
