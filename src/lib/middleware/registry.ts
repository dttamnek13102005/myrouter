...
declare global {
  var __myrouterPreRequestRegistry:
    | {
        initialized: boolean;
...
function getRegistryState() {
  if (!globalThis.__myrouterPreRequestRegistry) {
    globalThis.__myrouterPreRequestRegistry = {
      initialized: false,
      hooks: new Map(),
      middlewares: new Map(),
      logs: [],
  ...
  return globalThis.__myrouterPreRequestRegistry;
}
...
function compileHookCode(code: string, hookName: string): HookMiddleware {
  ...
  try {
    script = new vm.Script(`(async () => { ${code} })();`, {
      filename: `myrouter-hook:${hookName}`,
    });
  } catch (err: unknown) {
