...
const HEADER_NAME = "x-myrouter-cli-token";
...
/**
 * Validates the CLI machine-id token sent by the local myrouter CLI.
 * Only accepted from loopback IPs. Disabled via MYROUTER_DISABLE_CLI_TOKEN=true.
 */
export async function isCliTokenAuthValid(request: Request): Promise<boolean> {
  if (process.env.MYROUTER_DISABLE_CLI_TOKEN === "true") return false;
