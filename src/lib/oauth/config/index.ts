 ...
 * with the running MyRouter server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.MYROUTER_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.MYROUTER_TOKEN || process.env.CLI_TOKEN || "",
    userId: process.env.MYROUTER_USER_ID || process.env.CLI_USER_ID || "cli",
  };
}
