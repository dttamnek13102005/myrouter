 ...
 *      MYROUTER_LOCAL_ENDPOINTS_ENABLED=1 is set. The flag is opt-in so
 *      accidental dev deployments do not expose the API.
 *   3. Trust-list the MyRouter desktop app via a shared bearer token
 *      (MYROUTER_LOCAL_ENDPOINTS_TOKEN). The desktop app injects the header
...
export function isLocalRequestAllowed(): { allowed: true } | { allowed: false; reason: string } {
  ...
  if (headers) {
    ...
    const expected = process.env.MYROUTER_LOCAL_ENDPOINTS_TOKEN;
    if (expected) {
  ...
  // Production opt-in
  if (process.env.NODE_ENV === "production" && process.env.MYROUTER_LOCAL_ENDPOINTS_ENABLED !== "1") {
