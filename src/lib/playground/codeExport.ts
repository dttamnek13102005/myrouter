...
/**
 * Endpoint suportado pelo Playground Studio. Reflete os 13 endpoints da API MyRouter
...
export const API_KEY_PLACEHOLDER = "$MYROUTER_API_KEY";
...
function buildCurlSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    `# Set your API key: export MYROUTER_API_KEY="your-key-here"`,
    `curl -s -X POST \\`,
    `  "${url}" \\`,
...
function buildPythonSnippet(state: PlaygroundState): string {
  ...
  const lines: string[] = [
    ...
    `import requests`,
    ``,
    `api_key = os.environ["MYROUTER_API_KEY"]`,
    ``,
    `url = "${url}"`,
    `headers = {`,
