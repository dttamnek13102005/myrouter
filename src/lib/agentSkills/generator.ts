...
function buildApiBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  if (ops.length === 0) {
    ...
    for (const op of ops) {
      ...
      lines.push('  -H "Authorization: Bearer $MYROUTER_TOKEN"');
...
function buildCliBody(skill: AgentSkill, sources: BuildSources): string {
  ...
  lines.push("```bash");
  lines.push("npm install -g myrouter   # or: npx myrouter");
  lines.push("myrouter --version");
  lines.push("```");
  lines.push("");
