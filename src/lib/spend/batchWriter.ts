...
function getFlushIntervalMs() {
  const parsed = Number.parseInt(process.env.MYROUTER_SPEND_FLUSH_INTERVAL_MS || "", 10);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : DEFAULT_FLUSH_INTERVAL_MS;
}

function getMaxBufferSize() {
  const parsed = Number.parseInt(process.env.MYROUTER_SPEND_MAX_BUFFER_SIZE || "", 10);
