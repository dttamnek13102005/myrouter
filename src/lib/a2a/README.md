...
> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use MyRouter as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes MyRouter as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through MyRouter intelligent pipeline",
...
Routes prompts through MyRouter's intelligent pipeline with full observability.
...
Discovers MyRouter agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use MyRouter A2A as a custom LLM.
"""
...
An orchestrator agent delegates code generation to MyRouter, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via MyRouter A2A
    ...
    # Step 2: Review the code via MyRouter A2A (different model)
...
Multiple agents share quota through MyRouter, using the quota skill to coordinate.
...
```
Authorization: Bearer YOUR_MYROUTER_API_KEY
```

If no API key is configured on the server (`MYROUTER_API_KEY` is empty), authentication is bypassed.
