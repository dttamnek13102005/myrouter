 ...
 * Priority: system daemon socket → MyRouter custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to MyRouter custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `myrouter-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");
