 ...
 *   2. The `exec` permission additionally requires MYROUTER_PLUGINS_ALLOW_EXEC=1
...
function createSandbox(permissions: string[], pluginDir: string): Record<string, unknown> {
  ...
  if (permissions.includes("exec")) {
    if (process.env.MYROUTER_PLUGINS_ALLOW_EXEC !== "1") {
      throw new Error(
        `Plugin '${name}' requested the 'exec' permission, which is disabled. Set MYROUTER_PLUGINS_ALLOW_EXEC=1 to enable (local operator only).`
      );
    }
