...
import { parseModel, resolveCanonicalProviderModel } from "@myrouter/open-sse/services/model.ts";
import {
  MODEL_SPECS,
...
import { getUnsupportedParams } from "@myrouter/open-sse/config/providerRegistry.ts";
import {
  getLearnedThinkingCap,
  GEMINI_FALLBACK_THINKING_CAP,
} from "@myrouter/open-sse/services/learnedThinkingCaps.ts";
...
function reverseModelsDevProviders(provider: string): string[] {
  // models.dev may store capabilities under a different MyRouter provider id
