...
function resolveModel(input: RecordedTriageExecutionInput): string {
  const model = configuredString(input.model, "MYROUTER_ISSUE_AGENT_MODEL") ?? DEFAULT_MODEL;
  const provider = configuredString(input.provider, "MYROUTER_ISSUE_AGENT_PROVIDER");
...
function resolveTimeoutMs(input: RecordedTriageExecutionInput): number {
  const configured = input.timeoutMs ?? Number(process.env.MYROUTER_ISSUE_AGENT_TIMEOUT_MS);
  ...
  return [
    {
      role: "system",
      content:
        "You are the MyRouter Issue Agent. Analyze only the recorded GitHub context and produce a concise, actionable triage response. Do not claim to have accessed external state.",
    },
    {
      role: "user",
      content: [
...
export async function executeRecordedTriageChatCompletion(
  ...
  const routingPolicy = configuredString(
    input.routingPolicy,
    "MYROUTER_ISSUE_AGENT_ROUTING_POLICY"
  );
    ...
    const response = await post(
      new Request("http://localhost/api/v1/chat/completions", {
        ...
        headers: {
          ...
          ...(routingPolicy ? { "X-MyRouter-Mode": routingPolicy } : {}),
        },
        body: JSON.stringify({
