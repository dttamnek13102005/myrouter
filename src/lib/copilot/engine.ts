...
Use \`createCombo\` tool or \`runMyRouterCli\` to create them.`;
  }
...
MyRouter supports 212+ providers across categories:
...
I can use CodeGraph to explore the MyRouter codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## MyRouter Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`myrouter health\`
- "CLI list-combos" → ejecuta \`myrouter list-combos\`
- "CLI set-budget 10" → ejecuta \`myrouter set-budget 10\`

### Conocimiento
- "Cómo funciona MyRouter?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with MyRouter.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runMyRouterCli") {
    ...
    return {
      message: result,
      toolCalls: [{ name: "runMyRouterCli", args: intent.args, result }],
    };
  }
