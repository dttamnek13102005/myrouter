...
function getLHCState() {
  if (!globalThis.__myrouterLocalHC) {
    globalThis.__myrouterLocalHC = {
      initialized: false,
      sweepTimer: null,
      healthCache: new Map(),
  ...
  return globalThis.__myrouterLocalHC;
}
...
function isLocalHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("MYROUTER_DISABLE_LOCAL_HEALTHCHECK") ||
    isBuildProcess() ||
