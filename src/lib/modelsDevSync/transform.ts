 ...
 * types, the provider-id mapping table, and the raw→MyRouter transform
...
// ─── Provider mapping: models.dev provider ID → MyRouter provider IDs/aliases ──
//
// models.dev uses canonical provider IDs (e.g. "openai", "anthropic", "google").
// MyRouter uses both full IDs and short aliases (e.g. "cc" for claude, "cx" for codex).
// We map each models.dev provider to ALL MyRouter identifiers that should receive
...
export const MODELS_DEV_PROVIDER_MAP: Record<string, string[]> = {
  ...
  // that is the historical alias pair. But MyRouter's catalog & combo targets
  ...
  // Additional providers that may overlap with MyRouter
  alibaba: ["ali", "alibaba"],
...
/**
 * Map a models.dev provider ID to MyRouter provider IDs.
...
/**
 * Transform models.dev raw data → MyRouter PricingByProvider format.
 *
 * models.dev costs are already in $/1M tokens (same as MyRouter format).
...
export function transformModelsDevToPricing(raw: ModelsDevData): PricingByProvider {
  ...
  for (const [providerId, providerData] of Object.entries(raw)) {
    const myRouterProviders = mapProviderId(providerId);
    ...
    for (const [modelId, model] of Object.entries(providerData.models || {})) {
      ...
      // Write to ALL mapped MyRouter providers
      for (const myrouterProvider of myRouterProviders) {
        if (!result[myrouterProvider]) result[myrouterProvider] = {};
        result[myrouterProvider][modelId] = entry;
      }
    }
  }
...
export function transformModelsDevToCapabilities(raw: ModelsDevData): CapabilitiesByProvider {
  ...
  for (const [providerId, providerData] of Object.entries(raw)) {
    ...
    for (const [modelId, model] of Object.entries(providerData.models || {})) {
      ...
      for (const myrouterProvider of myRouterProviders) {
        if (!result[myrouterProvider]) result[myrouterProvider] = {};
        result[myrouterProvider][modelId] = cap;
      }
    }
  }
