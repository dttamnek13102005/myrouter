...
import { withRateLimit } from "@myrouter/open-sse/services/rateLimitManager";

const INTERNAL_ORIGIN = "http://myrouter.internal";
...
export function buildInternalChatRequest(
  ...
  return new Request(`${INTERNAL_ORIGIN}/v1/chat/completions`, {
    ...
    headers: {
      ...
      "X-MyRouter-Compression": "off",
      "X-Request-Id": `model-test-${randomUUID()}`,
      ...(connectionId ? { "X-MyRouter-Connection": connectionId } : {}),
    },
...
export async function runSingleModelTest(
  ...
  const testBody = isRerank
    ? {
        model: fullModelStr,
        query: "What is MyRouter?",
        documents: [
          "MyRouter routes AI requests across configured providers.",
