...
import { isObsoleteKiroModelAlias } from "@myrouter/open-sse/services/kiroModels.ts";
...
// expose (as opposed to the flat `supportedThinkingEfforts` field MyRouter's own
...
// Maps common upstream synonyms onto MyRouter's canonical effort vocabulary
...
export function normalizeDiscoveredModels(models: unknown): SyncedAvailableModel[] {
  ...
  for (const item of items) {
    ...
    deduped.set(id, {
      ...
      ...(() => {
        // #7694/#8347: the flat field (MyRouter's own import format) wins verbatim when
