/**
 * Cloudflare Worker source emitter for the MyRouter proxy relay.
 *
 ...
 *
 * The MyRouter variant intentionally diverges from the upstream PR:
...
export function buildCloudflareWorkerUploadRequest(
  ...
  const boundary = `----MyRouterCFWorker${randomUUID().replace(/-/g, "")}`;
  const CRLF = "\r\n";
  const parts: Buffer[] = [
    Buffer.from(
      `--${boundary}${CRLF}` +
...
export function buildCloudflareWorkerScript(relayAuth: string): string {
  ...
  return `// MyRouter Cloudflare Worker proxy relay — generated at deploy time.
