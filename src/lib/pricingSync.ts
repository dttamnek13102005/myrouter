...
// ─── Provider mapping: LiteLLM provider → MyRouter aliases ─────
...
/**
 * Transform LiteLLM raw data → MyRouter PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (MyRouter format).
 ...
 */
export function transformToMyRouter(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  for (const [modelKey, info] of Object.entries(raw)) {
    ...
    // Map to MyRouter providers
    const litellmProvider = info.litellm_provider || "";
    const myRouterProviders = LITELLM_PROVIDER_MAP[litellmProvider];

    if (myRouterProviders) {
      for (const provider of myRouterProviders) {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToMyRouter(raw);
