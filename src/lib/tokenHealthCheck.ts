...
import {
  ...
  refreshCopilotToken,
} from "@myrouter/open-sse/services/tokenRefresh.ts";
...
function isHealthCheckDisabled(): boolean {
  return (
    isEnvFlagEnabled("MYROUTER_DISABLE_TOKEN_HEALTHCHECK") ||
    isBuildProcess() ||
 ...
 * (e.g. "codex,openai"). Targeted alternative to MYROUTER_DISABLE_TOKEN_HEALTHCHECK:
...
function getHealthCheckSkipProviders(): Set<string> {
  const raw = process.env.MYROUTER_HEALTHCHECK_SKIP_PROVIDERS || "";
  return new Set(
    raw
      .split(",")
...
async function shouldHideLogs(): Promise<boolean> {
  if (
    isEnvFlagEnabled("MYROUTER_HIDE_HEALTHCHECK_LOGS") ||
    isBuildProcess() ||
    isAutomatedTestProcess()
  ) {
    return true;
...
declare global {
  var __myrouterTokenHC:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null; sweeping: boolean }
    | undefined;
}
function getHCState() {
  if (!globalThis.__myrouterTokenHC) {
    globalThis.__myrouterTokenHC = { initialized: false, interval: null, sweeping: false };
  }
  return globalThis.__myrouterTokenHC;
}
...
export async function checkConnection(conn) {
  ...
  // Codex). 9router did not have this background sweep; it was introduced in MyRouter
