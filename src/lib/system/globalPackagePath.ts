...
/**
 * Resolve the real install directory of the globally-installed `myrouter` package — the directory
 * that owns `node_modules/better-sqlite3` and so is the correct cwd for `npm rebuild`.
 *
 * Replaces the hardcoded `${globalRoot}/myrouter/app` assumption (Bug 3, security-report v3.8.15):
 * the global package root is `${npm root -g}/myrouter`, not `/myrouter/app`. We probe the real
 ...
 * global install.
 */
export async function resolveGlobalMyrouterPath(
  ...
  const packageRoot = path.join(globalRoot, "myrouter");
