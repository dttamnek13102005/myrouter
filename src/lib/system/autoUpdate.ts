...
export function getAutoUpdateConfig(env: NodeJS.ProcessEnv = process.env): AutoUpdateConfig {
  const dataDir = env.DATA_DIR || "/tmp/myrouter";
  const repoDir = env.AUTO_UPDATE_REPO_DIR || "/workspace/myrouter";
  ...
  return {
    ...
    composeService: env.AUTO_UPDATE_SERVICE || "myrouter-cli",
...
export async function validateAutoUpdateRuntime(
  ...
  if (!(await existsImpl("/var/run/docker.sock"))) {
    return {
      supported: false,
      reason: "Docker socket is not mounted into the MyRouter container.",
      composeCommand: null,
    };
  }
  ...
  try {
    ...
    return {
      supported: false,
      reason: "git is not available inside the MyRouter container.",
      composeCommand: null,
    };
  }
  ...
  if (!composeCommand) {
    return {
      supported: false,
      reason:
        "Neither docker compose nor docker-compose is available inside the MyRouter container.",
      composeCommand: null,
    };
  }
...
export function buildNpmUpdateScript(latest: string): string {
  return [
    ...
    `npm install -g myrouter@${latest} --include=optional --ignore-scripts --legacy-peer-deps`,
    "if command -v pm2 >/dev/null 2>&1; then",
    "  pm2 restart myrouter || true",
    "fi",
...
export function buildSourceUpdateScript(latest: string, gitRemote = "origin"): string {
  ...
  return [
    ...
    "npm run build",
    "if command -v pm2 >/dev/null 2>&1; then",
    "  pm2 restart myrouter --update-env || true",
    "fi",
