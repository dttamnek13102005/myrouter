 ...
 * ONLY from `npm info myrouter version --json` (the `npm` CLI binary). When that binary
...
const NPM_REGISTRY_LATEST_URL = "https://registry.npmjs.org/myrouter/latest";
...
const GITHUB_RELEASES_LATEST_URL =
  "https://api.github.com/repos/diegosouzapw/MyRouter/releases/latest";
  ...
  try {
    const res = await fetchImpl(GITHUB_RELEASES_LATEST_URL, {
      ...
      headers: {
        ...
        "User-Agent": "myrouter-version-check",
