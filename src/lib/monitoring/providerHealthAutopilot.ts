...
import { clearProviderFailure, clearModelLock } from "@myrouter/open-sse/services/accountFallback";
...
export async function buildProviderHealthAutopilotReport(
  ...
  const [{ getAllCircuitBreakerStatuses }, { getAllModelLockouts }, quotaMonitor] =
    await Promise.all([
      ...
      import("@myrouter/open-sse/services/quotaMonitor.ts").catch(() => null),
    ]);
