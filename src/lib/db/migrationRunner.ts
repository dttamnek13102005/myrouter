...
function resolveMigrationsDir(): string {
  const configuredDir = process.env.MYROUTER_MIGRATIONS_DIR;
  ...
  throw new Error(
    "[Migration] Could not resolve migrations directory. Set MYROUTER_MIGRATIONS_DIR."
  );
}
 ...
 *
 * Set the threshold to 0 (via `MYROUTER_MAX_PENDING_MIGRATIONS`) to disable
 * this safety check.
 */
 ...
 * the default via the `MYROUTER_MAX_PENDING_MIGRATIONS` env var (#3416). This
 ...
 * `MYROUTER_MIGRATIONS_DIR` convention used in resolveMigrationsDir(). Falls
 * back to the default on missing or invalid (non-numeric / negative) input.
 */
function resolveMaxPendingMigrations(): number {
  const raw = process.env.MYROUTER_MAX_PENDING_MIGRATIONS;
...
function ensureMigrationsTable(db: SqliteAdapter): void {
  db.exec(`
    CREATE TABLE IF NOT EXISTS _myrouter_migrations (
...
function supportsFts5(db: SqliteAdapter): boolean {
  ...
  try {
    const probeTable = `__myrouter_fts5_probe_${crypto.randomUUID().replace(/-/g, "_")}`;
    db.transaction(() => {
...
function getMigrationFiles(): Array<{ version: string; name: string; path: string }> {
  ...
  // would record version=NNN in _myrouter_migrations; the rest would never run).
  ...
  // Extra directories registered via MYROUTER_EXTRA_MIGRATIONS_DIRS, appended
...
function getAppliedVersions(db: SqliteAdapter): Set<string> {
  const rows = db.prepare("SELECT version FROM _myrouter_migrations").all() as Array<{
    version: string;
  }>;
...
function getAppliedRecords(db: SqliteAdapter): Array<{ version: string; name: string }> {
  return db
    .prepare("SELECT version, name FROM _myrouter_migrations ORDER BY version")
    .all() as Array<{
    version: string;
    name: string;
  }>;
...
function reconcileRenumberedMigrations(
  ...
  for (const compatibility of RENAMED_MIGRATION_COMPATIBILITY) {
    ...
    const legacyRow = db
      .prepare("SELECT version, name FROM _myrouter_migrations WHERE version = ? AND name = ?")
    ...
    const targetRow = db
      .prepare("SELECT version FROM _myrouter_migrations WHERE version = ?")
      .get(compatibility.toVersion) as { version: string } | undefined;

    const applyRepair = db.transaction(() => {
      if (targetRow) {
        db.prepare("DELETE FROM _myrouter_migrations WHERE version = ? AND name = ?").run(
          compatibility.fromVersion,
          compatibility.fromName
        );
      } else {
        db.prepare(
          "UPDATE _myrouter_migrations SET version = ?, name = ? WHERE version = ? AND name = ?"
        ).run(
    ...
    const residualRow = db
      .prepare("SELECT version, name FROM _myrouter_migrations WHERE version = ?")
    ...
    if (residualRow) {
      ...
      db.prepare("DELETE FROM _myrouter_migrations WHERE version = ?").run(
        compatibility.fromVersion
      );
    }
  }
...
function rehomeLegacyVersionSlotMigrations(
  ...
  for (const legacy of LEGACY_VERSION_SLOT_MIGRATIONS) {
    ...
    const applyRepair = db.transaction(() => {
      ...
      db.prepare("UPDATE _myrouter_migrations SET version = ? WHERE version = ? AND name = ?").run(
        legacyVersion,
        legacy.version,
        legacy.name
      );
...
export function runMigrations(db: SqliteAdapter, options?: { isNewDb?: boolean }): number {
  ...
  if (mismatches.length > 0) {
    ...
    console.error(
      `[Migration] The version-only tracking will skip these (version already applied), ` +
        `but please report this to the MyRouter maintainers.`
    );
  }
  ...
  // from the _myrouter_migrations table.
  ...
  // #3416: resolve the threshold at call time so MYROUTER_MAX_PENDING_MIGRATIONS
  ...
  if (
    ...
    if (plausiblePendingCount !== null && actionablePending.length <= plausiblePendingCount) {
      ...
      const bypassHint =
        ...
        `tracking table was wiped), set MYROUTER_MAX_PENDING_MIGRATIONS=0 in your ` +
      ...
      if (memoizedSafetyAbort && memoizedSafetyAbort.message === msg) {
        console.error(
          `[Migration] 🛑 ABORT (repeat — see earlier detail): ` +
            ...
            `Set MYROUTER_MAX_PENDING_MIGRATIONS=0 to bypass.`
        );
  ...
  for (const migration of pending) {
    ...
    const applyMigration = db.transaction(() => {
      ...
      db.prepare("INSERT INTO _myrouter_migrations (version, name) VALUES (?, ?)").run(
        migration.version,
        migration.name
      );
    });
    ...
    try {
      ...
      if (message.includes("duplicate column name")) {
        const applyMarkerOnly = db.transaction(() => {
          db.prepare(
            "INSERT OR IGNORE INTO _myrouter_migrations (version, name) VALUES (?, ?)"
...
export function getMigrationStatus(db: SqliteAdapter): {
  ...
  const appliedRows = db
    .prepare("SELECT version, name, applied_at FROM _myrouter_migrations ORDER BY version")
