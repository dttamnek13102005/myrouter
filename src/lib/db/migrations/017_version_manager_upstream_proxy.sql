...
CREATE TABLE IF NOT EXISTS version_manager (
  ...
  tool              TEXT NOT NULL UNIQUE,           -- 'cliproxyapi' | 'myrouter'
  ...
  auto_start        INTEGER NOT NULL DEFAULT 0,    -- 1 = start with MyRouter
...
-- Determines whether each provider uses MyRouter's native executor,
