  ...
  try {
    ...
    console.warn(
      `[DB] Cannot create data directory '${DATA_DIR}': ${msg}\n` +
        ...
        `[DB]   DATA_DIR=/path/to/writable/dir myrouter`
    );
  }
}
...
declare global {
  var __myrouterDb: SqliteAdapter | undefined;
  ...
  var __myrouterDbProbeRestoreCount: number | undefined;
  ...
  var __myrouterDbOomFailureCount: number | undefined;
}

function getDb(): SqliteDatabase | null {
  return globalThis.__myrouterDb ?? null;
}

function setDb(db: SqliteDatabase | null): void {
  if (db) {
    globalThis.__myrouterDb = db;
  } else {
...
function shouldRunStartupDbHealthCheck(): boolean {
  if (process.env.MYROUTER_FORCE_DB_HEALTHCHECK === "1") return true;
...
function getDbHealthCheckIntervalMs(): number {
  const rawValue = process.env.MYROUTER_DB_HEALTHCHECK_INTERVAL_MS;
...
function startDbHealthCheckScheduler(db: SqliteDatabase) {
  ...
  dbHealthCheckTimer = setInterval(() => {
    try {
      ...
      runDbHealthCheck(db, {
        autoRepair: true,
        skipIntegrityCheck: process.env.MYROUTER_SKIP_DB_HEALTHCHECK === "1",
...
export function getDbInstance(): SqliteDatabase {
  ...
  if (!fs.existsSync(sqliteFile) && probeFailureBackups.length > 0) {
    ...
    if (
      (globalThis.__myrouterDbProbeRestoreCount =
        (globalThis.__myrouterDbProbeRestoreCount || 0) + 1) > 3
    ) {
      throw new Error(
  ...
  if (fs.existsSync(sqliteFile)) {
    try {
      ...
      if (
        ...
        if (
          (globalThis.__myrouterDbOomFailureCount =
            (globalThis.__myrouterDbOomFailureCount || 0) + 1) > 3
        ) {
          throw new Error(
  ...
  db.exec(`
    ...
    INSERT OR IGNORE INTO _myrouter_migrations (version, name)
  ...
  if (shouldRunStartupDbHealthCheck()) {
    const skipIntegrityCheck = process.env.MYROUTER_SKIP_DB_HEALTHCHECK === "1";
    if (skipIntegrityCheck) {
      console.log("[DB] Health check skipped (MYROUTER_SKIP_DB_HEALTHCHECK=1)");
    }
    runDbHealthCheck(db, {
      autoRepair: true,
      expectedSchemaVersion: "1",
 ...
 * can place it in ~/.myrouter/runtime/ without blocking a synchronous caller.
 *
