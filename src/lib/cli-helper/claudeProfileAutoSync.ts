...
function isAutoSyncEnabled() {
  // Opt-in, default OFF. Backed by the MYROUTER_AUTO_SYNC_CLAUDE_PROFILES feature flag
  ...
  return isFeatureFlagEnabled("MYROUTER_AUTO_SYNC_CLAUDE_PROFILES");
}
...
export async function autoSyncClaudeProfilesFromLiveCatalog(
  ...
  // Each generated profile points ANTHROPIC_BASE_URL at the MyRouter this server serves.
  // Strip a trailing /v1 (Claude Code appends the version segment itself).
  const profileBaseUrl = internalBase.replace(/\/v1$/, "");

  // Reuse the CLI generator so automatic sync and `myrouter setup-claude` stay
