 ...
 * source is the user's OWN MyRouter instance, so loopback/private hosts are the
...
/**
 * Fetch the live `/v1/models` catalog from MyRouter. The catalog is the
 ...
 */
export async function fetchMyRouterCatalog(
  baseUrl: string,
  apiKey: string,
  timeoutMs = 5_000
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `MyRouter /v1/models returned ${response.status} ${response.statusText}`
      );
    }
...
/**
 * Generate a full `opencode.json` document for MyRouter. The catalog is the
...
export async function generateOpencodeConfig(
  ...
  const providerId = options.providerId?.trim() || "myrouter";
  ...
  if (fetchCatalog) {
    const result = await fetchMyRouterCatalog(baseURL, options.apiKey, timeoutMs);
  ...
  const provider: Record<string, unknown> = {
    name: existingProvider?.name ?? "MyRouter",
