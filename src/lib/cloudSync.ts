...
const CLOUD_SYNC_SECRET = process.env.MYROUTER_CLOUD_SYNC_SECRET || "";
...
const CLOUD_SYNC_SECRETS_ENABLED = process.env.MYROUTER_CLOUD_SYNC_SECRETS === "true";
...
//      `HMAC-SHA256(MYROUTER_CLOUD_SYNC_SECRET, rawBody)` and returns the hex
...
// If `MYROUTER_CLOUD_SYNC_SECRET` is unset, signature validation is logged but
...
export function verifyCloudSignature(rawBody: string, sigHeader: string | null): boolean {
  if (!CLOUD_SYNC_SECRET) {
    ...
    console.warn(
      "[cloudSync] MYROUTER_CLOUD_SYNC_SECRET is not set and the Cloud response carries no X-Cloud-Sig. " +
 ...
 * `MYROUTER_CLOUD_SYNC_SECRETS=true`. The default mode syncs non-credential
