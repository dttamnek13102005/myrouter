import { handleEmbedding } from "@myrouter/open-sse/handlers/embeddings.ts";
import {
  parseEmbeddingModel,
...
import { handleComboChat } from "@myrouter/open-sse/services/combo.ts";
...
import { attachMyRouterMetaHeaders } from "@/domain/myrouterResponseMeta";
...
export async function createEmbeddingResponse(
  ...
  if (result.success) {
    ...
    attachMyRouterMetaHeaders(responseHeaders, {
      provider,
