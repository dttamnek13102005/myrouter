...
import { createBuiltinAutoCombo } from "@myrouter/open-sse/services/autoCombo/builtinCatalog.ts";
import * as log from "../utils/logger";
import { updateProviderCredentials } from "../services/tokenRefresh";
import {
  detectFormatFromEndpoint,
  getTargetFormat,
} from "@myrouter/open-sse/services/provider.ts";
import {
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@myrouter/open-sse/config/providerModels.ts";
import { handleChatCore } from "@myrouter/open-sse/handlers/chatCore.ts";
import {
  errorResponse,
  modelCooldownResponse,
  providerCircuitOpenResponse,
  unavailableResponse,
} from "@myrouter/open-sse/utils/error.ts";
...
} from "@myrouter/open-sse/utils/proxyFetch.ts";
...
import { getRuntimeProviderProfile } from "@myrouter/open-sse/services/accountFallback.ts";
...
export async function safeLogEvents({
  ...
  try {
    ...
    try {
      ...
      const { proxyConfigToUrl } = await import("@myrouter/open-sse/utils/proxyDispatcher.ts");
...
export function withSessionHeader(response: Response, sessionId: string | null): Response {
  ...
  try {
    response.headers.set("X-MyRouter-Session-Id", sessionId);
    return response;
  } catch {
    ...
    cloned.headers.set("X-MyRouter-Session-Id", sessionId);
    return cloned;
  }
}
...
export function withSelectedConnectionHeader(
  ...
  try {
    response.headers.set("X-MyRouter-Selected-Connection-Id", connectionId);
    return response;
  } catch {
    const cloned = new Response(response.body, {
      status: response.status,
    ...
    cloned.headers.set("X-MyRouter-Selected-Connection-Id", connectionId);
    return cloned;
  }
}
