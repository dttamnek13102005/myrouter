/**
 * MyRouter Electron Desktop App - Main Process
 *
 ...
 * #14 Removed dead myrouter:// protocol (no handler existed)
...
function resolveNodeExecutable(env = process.env) {
  ...
  // (e.g. MyRouter.app/Contents/MacOS/MyRouter). Spawning it with
  ...
  if (process.platform === "darwin" && !isDev) {
    ...
    // ("MyRouter"), whereas app.getName() reads package.json `name` ("myrouter-desktop")
...
function resolveDataDir(overridePath, env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return path.join(appData, "myrouter");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return path.join(path.resolve(xdg), "myrouter");

  return path.join(require("os").homedir(), ".myrouter");
}
    ...
    new Promise((r) =>
      setTimeout(() => {
        try {
          ...
          // bare SIGKILL of the direct child, keeping myrouter.exe locked).
...
function installUpdate() {
  if (nextServer) {
    ...
    // grandchildren) keeps myrouter.exe locked and the updater fails with "file in use".
...
function setupContentSecurityPolicy() {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    ...
    const csp = [
      ...
      `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* wss://localhost:* wss://127.0.0.1:* https://*.myrouter.online https://*.myrouter.dev`,
      scriptSrc,
...
function startNextServer() {
  ...
  if (changed) {
    serverEnv.MYROUTER_BOOTSTRAPPED = "true";
    try {
      fs.mkdirSync(dataDir, { recursive: true });
      const lines = [
        "# Auto-generated by MyRouter bootstrap",
        "",
  ...
  // MYROUTER_MEMORY_MB or a pre-set --max-old-space-size still wins. Mirrors
  ...
  const serverNodeOptions = (() => {
    ...
    const explicit = parseInt(serverEnv.MYROUTER_MEMORY_MB, 10);
    let heapMb;
  ...
  nextServer.stdout?.on("data", (data) => {
    ...
    if (text.includes("Ready") || text.includes("started") || text.includes("listening")) {
      ...
      const isHeadless =
        ...
        process.env.MYROUTER_HEADLESS === "true";
      if (isHeadless && !global.loggedHeadlessReady) {
        global.loggedHeadlessReady = true;
        console.log("\n\x1b[32m✔ MyRouter Headless CLI Server is ready and listening!\x1b[0m");
...
function stopNextServer() {
  if (nextServer) {
    ...
    // (myrouter.exe-as-node) spawns grandchildren that a bare SIGTERM leaves alive,
    // holding a lock on myrouter.exe and blocking updates.
    killProcessTree(nextServer, { signal: "SIGTERM" });
    nextServer = null;
  }
}
  ...
  try {
    ...
    const desktopFileContent =
      [
        ...
        "Type=Application",
        "Name=MyRouter",
        "Comment=MyRouter Desktop Client",
    ...
    fs.writeFileSync(path.join(autostartDir, "myrouter-desktop.desktop"), desktopFileContent, {
      mode: 0o644,
    });
    return true;
  } catch (err) {
...
function isLinuxDesktopAutostartEnabled() {
  try {
    ...
    return fs.existsSync(
      path.join(os.homedir(), ".config", "autostart", "myrouter-desktop.desktop")
    );
  } catch {
    return false;
  }
