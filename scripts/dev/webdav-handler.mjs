/**
 * WebDAV file server handler for MyRouter.
 *
...
// Key derivation: scryptSync(secret, "myrouter-field-encryption-v1", 32)
// ─────────────────────────────────────────────────────────────────────────────

const ENC_PREFIX = "enc:v1:";
const STATIC_SALT = "myrouter-field-encryption-v1";
const KEY_LENGTH = 32;
...
/**
 * Decrypt a value that may be encrypted with the MyRouter enc:v1: format.
...
function getDefaultDataDir() {
  ...
  const legacyDir = path.join(homeDir, ".myrouter"); // getLegacyDotDataDir()

  // 2) Preserve the legacy ~/.myrouter path if it already exists (avoid data loss).
  try {
  ...
  if (typeof xdgConfigHome === "string" && xdgConfigHome.trim().length > 0) {
    return path.join(path.resolve(xdgConfigHome.trim()), "myrouter");
  }

  // 5) Default → legacy ~/.myrouter (even if it does not exist yet).
  return legacyDir;
}
...
function sendUnauthorized(res) {
  sendError(res, 401, "Unauthorized", {
    "WWW-Authenticate": 'Basic realm="MyRouter WebDAV"',
  });
}
...
async function handlePut(req, res, absPath) {
  ...
  const tmpPath = absPath + ".myrouter-webdav-tmp-" + Date.now();
  let writeStream;
  try {
