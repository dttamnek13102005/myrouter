...
function normalizeEndpoint(rawEndpoint) {
  ...
  let parsed;
  try {
    parsed = new URL(endpoint, "http://myrouter.local");
  } catch {
    return null;
  }

  if (parsed.origin !== "http://myrouter.local") {
    return null;
  }
...
class WebSocketSession {
  ...
  async executeRequest(requestId, endpoint, payload, abortController) {
    const headers = {
      ...
      "content-type": "application/json",
      "x-myrouter-ws-session-id": this.sessionId,
      "x-myrouter-ws-request-id": requestId,
    };
...
export function createMyrouterWsBridge({
  baseUrl,
  fetchImpl = fetch,
  ...
  if (!isText(baseUrl)) {
    throw new Error("createMyrouterWsBridge requires a baseUrl");
  }
