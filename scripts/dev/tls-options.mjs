...
//
// Pure, dependency-light helpers for MyRouter's opt-in native HTTPS/TLS serving
 ...
 *
 * Reads `MYROUTER_TLS_CERT` and `MYROUTER_TLS_KEY` (filesystem paths). Returns
...
export function resolveTlsOptions(
  ...
  const certPath = typeof env?.MYROUTER_TLS_CERT === "string" ? env.MYROUTER_TLS_CERT.trim() : "";
  const keyPath = typeof env?.MYROUTER_TLS_KEY === "string" ? env.MYROUTER_TLS_KEY.trim() : "";
  ...
  if (!certPath || !keyPath) {
    warn(
      `[myrouter][tls] HTTPS not enabled: both MYROUTER_TLS_CERT and MYROUTER_TLS_KEY ` +
  ...
  try {
    ...
    warn(
      `[myrouter][tls] HTTPS not enabled: could not read TLS cert/key ` +
