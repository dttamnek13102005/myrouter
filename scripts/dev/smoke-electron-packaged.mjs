...
export const LINUX_EXECUTABLE_NAMES = ["myrouter-desktop", "myrouter", "MyRouter"];
...
function discoverMacExecutable() {
  ...
  const candidates = [
    ...
    join(distDir, "mac", "MyRouter.app", "Contents", "MacOS", "MyRouter"),
    join(distDir, "mac-arm64", "MyRouter.app", "Contents", "MacOS", "MyRouter"),
  ];
...
function discoverWindowsExecutable() {
  const distDir = join(ROOT, "electron", "dist-electron");
  const candidates = [
    join(distDir, "win-unpacked", "MyRouter.exe"),
    join(distDir, "win-x64-unpacked", "MyRouter.exe"),
    join(distDir, "win-arm64-unpacked", "MyRouter.exe"),
  ];

  return (
    candidates.find((candidate) => existsSync(candidate)) ||
    findExecutableByName(distDir, ["MyRouter.exe"]) ||
    candidates[0]
  );
}
...
async function assertPortIsFree(url) {
  try {
    ...
    throw new Error(
      `Smoke URL already responded with HTTP ${response.status}: ${url}. Stop the existing MyRouter process before running this check.`
    );
  } catch (error) {
...
async function ensureSmokeEnvDirs(smokeEnv, dataDir) {
  ...
  if (platform() === "win32" && smokeEnv.APPDATA) {
    for (const subdir of ["myrouter-desktop", "MyRouter", "myrouter"]) {
...
async function main() {
  ...
  if (!existsSync(appExecutable)) {
    throw new Error(
      `Packaged MyRouter executable not found at ${appExecutable}. Build it first with \`npm run build:<target> --prefix electron\` or set ELECTRON_SMOKE_APP_EXECUTABLE.`
    );
  }
  ...
  const dataDir =
    ...
    (await mkdtemp(join(tmpdir(), "myrouter-electron-smoke-")));
  const removeDataDir =
