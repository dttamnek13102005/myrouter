#!/usr/bin/env node
/**
 * system-info.mjs — MyRouter System Information Reporter (#280)
 *
...
lines.push("MyRouter System Information Report");
...
const installedGlobal = run("npm list -g myrouter --depth=0 2>/dev/null | grep myrouter");
lines.push(`Global npm: ${installedGlobal || "not installed globally"}`);

const pm2Status = run("pm2 list 2>/dev/null | grep myrouter | awk '{print $4, $10, $12}'");
...
console.log(
  `📎 Attach this file when reporting issues at: https://github.com/diegosouzapw/MyRouter/issues`
);
