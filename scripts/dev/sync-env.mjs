...
const CRYPTO_SECRETS = {
  ...
  // Generated in bin/myrouter.mjs:ensureStorageEncryptionKey() and persisted to
  // ~/.myrouter/.env to survive across upgrades. This prevents credential loss
  // when upgrading MyRouter (issue #1622).
  MACHINE_ID_SALT: () => `myrouter-${randomBytes(8).toString("hex")}`,
};
 ...
 * It's generated at server startup in bin/myrouter.mjs and persisted to
 * ~/.myrouter/.env to survive across upgrades.
...
function resolveDataDir(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "myrouter");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "myrouter");

  return join(homedir(), ".myrouter");
}
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.myrouter/server.env, ~/.myrouter/.env, ` +
