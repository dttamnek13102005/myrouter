...
import { createMyrouterWsBridge } from "./v1-ws-bridge.mjs";
...
if (fs.existsSync(rootAppDir) && fs.statSync(rootAppDir).isDirectory()) {
  ...
  console.error("Please rename or delete the root 'app/' directory before starting MyRouter.\n");
  process.exit(1);
}
...
process.env.MYROUTER_INTERNAL_SCHEME = "http";

const { dashboardPort } = runtimePorts;
const hostname = process.env.HOST || "0.0.0.0";
// Turbopack by default in dev (matches the Next 16 CLI default and the production
// build default in build-next-isolated.mjs); MYROUTER_USE_TURBOPACK=0 is the
// webpack escape hatch.
const useTurbopack = dev && mergedEnv.MYROUTER_USE_TURBOPACK !== "0";
...
async function start() {
  ...
  const wsBridge = createMyrouterWsBridge({
    baseUrl: `http://127.0.0.1:${dashboardPort}`,
  });
