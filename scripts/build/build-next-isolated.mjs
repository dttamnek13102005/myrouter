...
const backupRoot = path.join(os.tmpdir(), `myrouter-build-isolated-${process.pid}-${Date.now()}`);
...
export function getTransientBuildPaths(rootDir = projectRoot, env = process.env) {
  ...
  if (env.MYROUTER_BUILD_MOVE_TASKS === "1") {
    paths.push({
...
export function resolveNextBuildBundlerFlag(baseEnv = process.env) {
  ...
  return baseEnv.MYROUTER_USE_TURBOPACK === "0" ? "--webpack" : "--turbopack";
}
...
export function getWindowsBuildProfileDir() {
  return path.join(os.tmpdir(), `myrouter-build-winhome-${process.pid}`);
}
...
export function resolveNextBuildEnv(baseEnv = process.env, platform = process.platform) {
  ...
  // sets one — don't clobber/duplicate) and let MYROUTER_BUILD_MEMORY_MB override.
  ...
  // #6283. On memory-constrained machines, set MYROUTER_USE_TURBOPACK=0 (webpack
  ...
  if (!/--max-old-space-size/.test(env.NODE_OPTIONS || "")) {
    ...
    const heapMb = Number(baseEnv.MYROUTER_BUILD_MEMORY_MB) || 8192;
...
export async function main() {
  ...
  try {
    ...
    if (isBackendOnlyBuild()) {
      console.log(
        "[build-next-isolated] MYROUTER_BUILD_BACKEND_ONLY set — building API only (dashboard UI stubbed)"
      );
    ...
    if (result.code === 0 && (await exists(standaloneDir))) {
      ...
      try {
        ...
        if (basePathWrite.status !== 0) {
          console.warn(
            "[build-next-isolated] Non-fatal error writing BUILD_MYROUTER_BASE_PATH sentinel"
          );
        }
      } catch (assembleErr) {
