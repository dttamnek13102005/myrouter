...
/**
 * MyRouter — Prepublish Build Script
 *
...
console.log("🔨 MyRouter — Building for npm publish...\n");
...
const cliSrcFile = join(ROOT, "bin", "myrouter.ts");
const cliDestFile = join(ROOT, "bin", "myrouter.mjs");
  ...
  try {
    execFileSync(
      ...
      [
        ...
        "--packages=external",
        "--format=esm",
        "--outfile=bin/myrouter.mjs",
      ],
      { cwd: ROOT, stdio: "inherit" }
    );
    chmodSync(cliDestFile, 0o755);
    console.log("  ✅ CLI Entrypoint bundled to bin/myrouter.mjs");
  } catch (err: any) {
    console.warn("  ⚠️  CLI bundle error:", err.message);
  }
}

// ── Step 8.8: Build @myrouter/opencode-plugin ──────────────
// The plugin ships bundled inside the myrouter npm package (see root
// package.json "files": ["@myrouter/", ...]). Its built `dist/` MUST be
// present in the publish tarball so `myrouter setup opencode` can copy it
...
const opencodePluginSrc = join(ROOT, "@myrouter", "opencode-plugin");
...
if (existsSync(opencodePluginSrc) && existsSync(join(opencodePluginSrc, "package.json"))) {
  ...
  if (!pluginAlreadyBuilt) {
    console.log("\n  🔨 Building @myrouter/opencode-plugin (tsup)...");
    try {
      ...
      console.log("  ✅ @myrouter/opencode-plugin bundled to @myrouter/opencode-plugin/dist/");
    } catch (err: any) {
      console.error("  ❌ Failed to build @myrouter/opencode-plugin:", err.message);
      console.error("     The published package would be missing the plugin dist.");
      console.error(
        "     Run `cd @myrouter/opencode-plugin && npm install && npm run build` to debug."
      );
      process.exit(1);
    }
  } else {
    console.log("  ✅ @myrouter/opencode-plugin dist/ already present (skipping rebuild)");
  }
  ...
  if (existsSync(pluginNodeModules)) {
    ...
    console.log("  🧹 Removed @myrouter/opencode-plugin/node_modules (hard link guard)");
  }
} else {
  console.log("  ⏭️  @myrouter/opencode-plugin not found in workspace (skipping build)");
}
