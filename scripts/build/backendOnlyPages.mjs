 ...
 *
 * When `MYROUTER_BUILD_BACKEND_ONLY=1` (or `MYROUTER_BUILD_PROFILE=backend`) is set,
 * `build-next-isolated.mjs` calls `stubDashboardPages()` BEFORE `next build` and
 * `restoreDashboardPages()` in a `finally` afterward.
 *
 * WHY: MyRouter embedders that only consume the HTTP API (`/api/*`, `/v1/*`, `/v1beta/*`)
...
export const BACKEND_ONLY_STUB_MARKER =
  "/* myrouter:backend-only-stub (auto-restored after build) */";
...
/**
 * Strip a leading `"use server"` module directive. Some MyRouter API Route Handlers
