#!/usr/bin/env node
/**
 * MyRouter — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.myrouter/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by MyRouter bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/myrouter.mjs`.
  //
  ...
  if (needsPersist) {
    merged.MYROUTER_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "myrouter-field-encryption-v1", 32)
              const legacySalt = "myrouter-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.myrouter/server.env or ~/.myrouter/.env,"
                );
                log(
                  "   or run: myrouter reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
