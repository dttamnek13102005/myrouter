 ...
 * `MYROUTER_MEMORY_MB`, mirroring `myrouter serve`. Clamped to [64, 16384];
 * invalid/unset → fallback (512). The standalone launcher uses this so
 * MYROUTER_MEMORY_MB can override the Docker image's NODE_OPTIONS fallback
 ...
 * when `MYROUTER_MEMORY_MB` is unset. A fixed 512MB default crashed boxes with
 ...
 * explicit MYROUTER_MEMORY_MB override always wins.
 ...
 * (`--max-old-space-size=…`). Used to decide whether `myrouter serve` may
 ...
 * the user already exported. #5238: `myrouter serve` used to UNCONDITIONALLY
