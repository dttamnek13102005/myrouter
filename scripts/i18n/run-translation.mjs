#!/usr/bin/env node
/**
 * MyRouter — Docs translation pipeline (hash-based, incremental).
 *
 ...
 *   MYROUTER_TRANSLATION_API_URL     e.g. https://cloud.myrouter.dev/v1
 *   MYROUTER_TRANSLATION_API_KEY     bearer token (kept out of logs)
 *   MYROUTER_TRANSLATION_MODEL       e.g. cx/gpt-5.6-sol
 *   MYROUTER_TRANSLATION_TIMEOUT_MS  optional, default 60000
 *   MYROUTER_TRANSLATION_CONCURRENCY optional, default 4
 */
...
function backendConfig() {
  const apiUrl = requireEnv("MYROUTER_TRANSLATION_API_URL").replace(/\/$/, "");
  const apiKey = requireEnv("MYROUTER_TRANSLATION_API_KEY");
  const model = requireEnv("MYROUTER_TRANSLATION_MODEL");
  const timeoutMs = Number(process.env.MYROUTER_TRANSLATION_TIMEOUT_MS || 60000);
...
async function main() {
  ...
  if (!opts.dryRun) {
    ...
    else backend.concurrency = Number(process.env.MYROUTER_TRANSLATION_CONCURRENCY || 4);
    logInfo(
      `backend: ${backend.apiUrl} (model=${backend.model}, concurrency=${backend.concurrency}, timeout=${backend.timeoutMs}ms)`
    );
  } else {
    const apiUrl = (process.env.MYROUTER_TRANSLATION_API_URL || "").replace(/\/$/, "");
