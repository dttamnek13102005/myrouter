...
whose source SHA-256 changed), runs the MyRouter Cloud LLM through
MYROUTER_TRANSLATION_* env vars, and is wired into `npm run i18n:run`
...
def translate_block(text, target_language, api_url, api_key, model):
    ...
    prompt = (
        f"You are a professional technical translator working on the MyRouter proxy project documentation.\n"
...
def main():
    parser = argparse.ArgumentParser(description="MyRouter Auto-Translator for i18n Markdown")
    parser.add_argument("--api-url", default="http://localhost:20128/v1", help="Base URL of MyRouter or target provider")
