...
async function runAutomatedChecks() {
  ...
  for (const code of legacyCheckLocales) {
    ...
    try {
      ...
      if (content.includes("**[English](#-myrouter--the-free-ai-gateway)**")) {
        anchorLineRemoved = false;
      }
      if (content.includes("## 🇧🇷 MyRouter")) {
...
async function main() {
  ...
  automatedChecksLines.push(
    ...
    `- Apêndice "## 🇧🇷 MyRouter" removido em ja/ar: **${automated.brAppendixRemoved ? "OK" : "PENDENTE"}**`,
