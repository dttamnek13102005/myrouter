#!/usr/bin/env node
/**
 * MyRouter — UI i18n key sync (next-intl message catalogs).
 *
 ...
 *
 * --translate-markers calls the MyRouter translation backend (same env vars
...
async function processLocale(locale, source, config, opts, backend) {
  ...
  if (opts.translateMarkers && placeholderCountBefore > 0 && backend) {
    ...
    if (!localeEntry) {
      ...
      const concurrency =
        opts.concurrency ?? Number(process.env.MYROUTER_TRANSLATION_CONCURRENCY || 4);
