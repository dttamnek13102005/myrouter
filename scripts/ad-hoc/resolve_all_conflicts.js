...
async function main() {
  ...
  if (fs.existsSync(modelsRoute)) {
    ...
    content = content.replace(
      /<<<<<<< HEAD\r?\n=======\r?\nimport \{ sanitizeErrorMessage \} from "@myrouter\/open-sse\/utils\/error";\r?\nimport \{ getStaticQoderModels \} from "@myrouter\/open-sse\/services\/qoderCli\.ts";\r?\n>>>>>>> release\/v3\.8\.4/g,
...
# arbitrary code at install time. MyRouter's own postinstall (
