...
set -euo pipefail
echo "[verify-published] npm i -g myrouter@\${VERIFY_VERSION} (public registry)"
npm install -g "myrouter@\${VERIFY_VERSION}"
export DATA_DIR=/tmp/myrouter-data JWT_SECRET=verify-published-secret-with-sufficient-length API_KEY_SECRET=verify-published-api-key-secret DISABLE_SQLITE_AUTO_BACKUP=true MYROUTER_SKIP_SYSTEM_TRUST=1
mkdir -p "\$DATA_DIR"
myrouter serve --port "\$VERIFY_PORT" &
node -e '
...
function main() {
  ...
  console.log(`[verify-published] clean-container verify of myrouter@${version}…`);
  ...
  console.error(`[verify-published] ❌ FAILED (exit ${r.status}) — consider: npm deprecate myrouter@${version} "<reason>"`);
  process.exit(1);
}
