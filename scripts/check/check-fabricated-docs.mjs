...
//   3. CLI commands `myrouter ...`     → must exist in bin/cli/commands/ or bin/
...
const ENV_VAR_ALLOWLIST = new Set([
  ...
  "VERBOSE",
  "LOG_LEVEL",
  "PORT", // generic, not MyRouter-specific
  "DATA_DIR",
  "REQUIRE_API_KEY",
  "MYROUTER_BUILD_PROFILE", // build-time only
  "MYROUTER_BUILD_SHA",
  "MYROUTER_URL", // used by ad-hoc tooling, validated elsewhere
  "MYROUTER_KEY", // ditto
  ...
  // subprocess — never read via `process.env.X` in MyRouter's own source, so the
  ...
  "OPENAI_BASE_URL", // env var MyRouter passes to downstream CLIs (AGENT_PROTOCOLS_GUIDE.md)
...
const ENDPOINT_ALLOWLIST = new Set([
  ...
  // paths on the UPSTREAM service (Claude.ai web, Blackbox), not MyRouter routes.
...
// Env var helper wrappers used across MyRouter — envInt(NAME, 5), envBool(NAME),
...
export function buildCodebaseIndex(root = ROOT) {
  ...
  // documented MyRouter env var (including external-CLI / docker / electron vars
  ...
  // Set of `myrouter <subcommand>` strings that exist in bin/
...
const COARSE_PATTERNS = {
  ...
  // myrouter <verb> <sub> ... — only on the same line, captures first 2 tokens
  cliCmd: /\bmyrouter\s+([a-z][a-z0-9-]+)(?:\s+([a-z][a-z0-9-]+))?/g,
...
export function scanDocFile(absPath, index, root = ROOT) {
  ...
  // 3) CLI commands: `myrouter foo bar` — only flag when the line is in
  //    a code-like context (inside backticks or a shell block). Bare prose
  //    like "we use myrouter and..." is not a command claim.
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.cliCmd)) {
    ...
    const isShellLike = /^[ \t]*\$\s|^```sh|^```bash|^```shell|`myrouter/.test(lineText);
    ...
    findings.push({
      ...
      value: `myrouter ${sub}`,
      line: ln,
      msg: `myrouter subcommand '${sub}' not registered in bin/`,
    });
  }
  ...
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.fileRef)) {
    ...
    // (`@myrouter/opencode-provider/src/index.ts`). Neither resolves from repo root.
...
export function formatHumanReport(result) {
  ...
  const KIND_LABELS = {
    ...
    "cli-cmd": "myrouter subcommands not registered",
