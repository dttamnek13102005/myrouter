 ...
 *
 * Enforces that every env var referenced in MyRouter source code appears in
...
const IGNORE_FROM_CODE = new Set([
  ...
  "LOCALAPPDATA",
  "XDG_CONFIG_HOME",
  // XDG Base Directory cache root — read (never defined by MyRouter) so the
  // Android/Termux serve path can honor an operator-set cache location (#8519).
  "XDG_CACHE_HOME",
  "USERPROFILE",
  "PREFIX",
  // X11 display server — set by the OS/session manager, not MyRouter config.
  "DISPLAY",
  ...
  // bundler is active — framework-internal. The MyRouter-facing knob is
  // MYROUTER_USE_TURBOPACK (scripts/dev/run-next.mjs), which IS documented.
  "TURBOPACK",
  "NODE_TEST_CONTEXT",
  "VITEST",
  // Instruction snippet shown to users (Traffic Inspector HttpProxySnippetCard) — not MyRouter config.
  ...
  // points at MyRouter). A downstream client-tool var, not an MyRouter server
  ...
  // (scripts/check/check-openapi-breaking.mjs) — a build/check signal, not MyRouter runtime config.
  "BASE_REF",
  // PR body injected by GitHub Actions into the pr-evidence gate (github.event.pull_request.body);
  // a CI-only signal, never an MyRouter runtime config (Phase 7.10).
  "PR_BODY",
  // CLI machine-id token opt-out (server-side flag; not user-configurable via .env).
  "MYROUTER_DISABLE_CLI_TOKEN",
  ...
  // ("http://192.168.0.15:20128" / null), never MyRouter runtime config (#5151).
  "COMBO_LIVE_BASE_URL",
  "COMBO_LIVE_API_KEY",
  ...
  // These are external signals set by the host OS or cloud provider — not MyRouter config.
  "CODESPACES",
  ...
  // internals, never MyRouter runtime config.
  "VERIFY_DEADLINE_S",
  "VERIFY_PORT",
  "VERIFY_VERSION",
  // Doctor diagnostic flags (no runtime behavior yet — placeholders).
  "MYROUTER_DOCTOR_HOST",
  "MYROUTER_DOCTOR_LIVENESS_URL",
  "MYROUTER_PROVIDER_CATALOG_PATH",
  "MYROUTER_PROVIDER_TEST_MODEL",
  // Test-only opt-out: instructs bin/myrouter.mjs to skip auto-loading the
  // repository .env so isolation tests get a deterministic environment.
  "MYROUTER_CLI_SKIP_REPO_ENV",
  ...
  // A dev/ops measurement tool, never MyRouter runtime config.
  "MYROUTER_EVAL_CREDENTIALS",
  ...
  "MYROUTER_INTERNAL_SCHEME",
  // Source typo / placeholder.
  "OMNIROUT",
  // Static config alias path (the canonical var is MYROUTER_PAYLOAD_RULES_PATH).
  "PAYLOAD_RULES_PATH",
  // Node.js module resolution path — OS/Node internal, not an MyRouter config var.
  ...
  "NVIDIA_BASE_URL",
  "NVIDIA_MODEL",
  // XDG standard data directory — set by OS/desktop session, not MyRouter config.
