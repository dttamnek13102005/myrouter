#!/usr/bin/env python3
"""
tcp-close-analyzer.py — dependency-free pcap analyzer for MyRouter<->Caddy
...
(the myrouter-dev container)?

Why this exists: the dashboard's HTTP-level status (499 "Request aborted")
only tells us MyRouter's own executor detected a dropped connection — it
doesn't tell us whether the underlying TCP socket was actually closed by the
far end, or whether MyRouter itself is the one tearing it down (e.g. an
idle/read timeout on MyRouter's side that then gets misreported as a client
...
container's OWN namespace via its PID instead:
    ...
    PID=$(podman inspect myrouter-dev --format '{{.State.Pid}}')
    sudo nsenter -t "$PID" -n tcpdump -i any -w /tmp/myrouter-capture.pcap \\
        'host <myrouter-container-ip> and port 20128'

Find the container's IP first with:
    podman inspect myrouter-dev --format '{{.NetworkSettings.Networks}}'

That captures all traffic between Caddy and the myrouter-dev container on
...
again), then Ctrl+C the tcpdump. Make the file readable:
    ...
    sudo chmod 644 /tmp/myrouter-capture.pcap

Then run this script against it:

    python3 scripts/sre/tcp-close-analyzer.py /tmp/myrouter-capture.pcap
...
<pcap>.streams.jsonl next to the input file), each with:
  ...
  - correlationId / requestId, if an `x-correlation-id:` / `x-request-id:`
    ...
    MyRouter doesn't echo these on every hop, so this is a bonus, not the
...
That captures all traffic between Caddy and the myrouter-dev container —
...
Then run:
    ...
    python3 scripts/sre/tcp-close-analyzer.py /tmp/myrouter-capture.pcap --find "<marker>"
"""
    )
