#!/usr/bin/env bash
# =============================================================================
# codex-ws — roda a OpenAI Codex CLI contra um MyRouter LOCAL usando o
#            transporte Responses-over-WebSocket (em vez do HTTP da Cloud).
#
# POR QUE ESTE WRAPPER EXISTE
# ---------------------------
# O MyRouter expõe um proxy WebSocket para a API de Responses do Codex em
...
#     account"). É preciso mandar o id "puro" -> "gpt-5.5". (O MyRouter, no
...
#   MYROUTER_WS_BASE   base URL do MyRouter local   (default abaixo)
#   MYROUTER_WS_MODEL  modelo codex (id puro)        (default "gpt-5.5")
#   MYROUTER_LOCAL_KEY API key (qualquer valor se REQUIRE_API_KEY=false)
...
MYROUTER_WS_BASE="${MYROUTER_WS_BASE:-http://127.0.0.1:20128/v1}"  # base do MyRouter local
MYROUTER_WS_MODEL="${MYROUTER_WS_MODEL:-gpt-5.5}"                  # id PURO (sem "codex/")
...
export MYROUTER_LOCAL_KEY="${MYROUTER_LOCAL_KEY:-local}"
...
# Gerado por codex-ws.sh — config isolada para o WS local do MyRouter.
model = "$MYROUTER_WS_MODEL"
model_provider = "myrouter-local"

[model_providers.myrouter-local]
name = "MyRouter Local (WS)"
base_url = "$MYROUTER_WS_BASE"   # a URL WebSocket é derivada desta base pela CLI
...
env_key = "MYROUTER_LOCAL_KEY"   # a CLI lê a key Bearer desta env var
...
overrides=(-c model="$MYROUTER_WS_MODEL" -c model_provider="myrouter-local")
