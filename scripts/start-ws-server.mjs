 ...
 *   MYROUTER_ENABLE_LIVE_WS — Set to "0" or "false" to disable
 */
...
const BOOTSTRAPPED_ENV = "MYROUTER_LIVE_WS_BOOTSTRAPPED";
...
async function main() {
  ...
  // child is re-spawned with MYROUTER_ENABLE_LIVE_WS="0" purely to stop liveServer.ts
  ...
  if (
    ...
    (process.env.MYROUTER_ENABLE_LIVE_WS === "0" ||
      process.env.MYROUTER_ENABLE_LIVE_WS?.toLowerCase() === "false")
  ) {
    console.log("[LiveWS] Disabled via MYROUTER_ENABLE_LIVE_WS");
    process.exit(0);
  }
