#!/usr/bin/env node
/**
 * Post-install warm-up for MyRouter native runtimes.
 *
 * Runs after the main `scripts/build/postinstall.mjs` binary-copy step.
 * Tries to pre-resolve better-sqlite3 into ~/.myrouter/runtime/ so that
...
if (
  process.env.MYROUTER_SKIP_POSTINSTALL === "1" ||
  process.env.CI === "true" ||
  process.env.CI === "1"
) {
  process.exit(0);
...
(async () => {
  try {
    ...
    console.warn(`[myrouter] postinstall warm-up skipped: ${err?.message ?? err}`);
  }
})();
