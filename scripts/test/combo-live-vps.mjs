 ...
 *
 * Phase-3 VPS HTTP scenario driver for MyRouter combo routing.
...
// Known approximate input cost ($/M tokens) from MyRouter's default-pricing constants.
...
//
// MyRouter's sortModelsByCost uses getPricingForModel which merges
...
async function scenarioCostOptimized(healthy) {
  ...
  try {
    ...
    // Both direct match and provider-substring match are accepted since MyRouter
...
//   404, MyRouter calls recordProviderCooldown(providerA, undefined) — a provider-
...
//   1. SSH to read STORAGE_ENCRYPTION_KEY from /root/.myrouter/.env
//   2. Encrypt a wrong API key using scryptSync(key,"myrouter-field-encryption-v1",32)+AES-256-GCM
...
// ---------------------------------------------------------------------------
async function main() {
  console.log("=== MyRouter combo-live-vps scenario driver ===");
