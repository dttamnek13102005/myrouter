...
# If MYROUTER_MEMORY_MB is set, build NODE_OPTIONS dynamically so the
# user can tune heap size via environment without editing the Dockerfile.
if [ -n "$MYROUTER_MEMORY_MB" ]; then
  export NODE_OPTIONS="${NODE_OPTIONS:-} --max-old-space-size=${MYROUTER_MEMORY_MB}"
fi

# Hard Rule #13: never interpolate MYROUTER_BASE_PATH (or any runtime path)
...
if [ -d "$DATA_PATH" ] && [ ! -w "$DATA_PATH" ]; then
  ...
  if [ "${CONTAINER_HOST:-}" = "podman" ]; then
    ...
    echo "  https://github.com/diegosouzapw/MyRouter/blob/main/contrib/podman/README.md#data-directory-permissions-by-topology"
  else
