 ...
 *
 * Purely informational invocations must NOT create `~/.myrouter/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `myrouter --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `myrouter` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `myrouter` runs the default `serve` command → must provision.
