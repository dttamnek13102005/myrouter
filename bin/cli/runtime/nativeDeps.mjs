...
export function ensureRuntimeDir() {
  ...
  if (!existsSync(pkgPath)) {
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: "myrouter-runtime",
          version: "1.0.0",
          private: true,
          description: "User-writable runtime deps for MyRouter (native binaries)",
        },
        null,
        2
      )
...
export function npmInstallRuntime(pkgs, opts = {}) {
  ...
  if (!opts.silent) {
    process.stdout.write(`[myrouter][runtime] npm ${npmArgs.join(" ")}\n`);
  }
...
export function ensureBetterSqliteRuntime({ silent = false, force = false } = {}) {
  ...
  if (valid && !force) {
    if (!silent) process.stdout.write("[myrouter][runtime] better-sqlite3 OK\n");
    return { betterSqlite: true };
  }
  const ok = npmInstallRuntime([`better-sqlite3@${BETTER_SQLITE3_VERSION}`], { silent });
  if (!ok && !silent) {
    process.stderr.write("[myrouter][runtime] better-sqlite3 install failed\n");
  }
