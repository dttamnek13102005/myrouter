...
export async function loadSystray(): Promise<(new (...args: unknown[]) => unknown) | null> {
  ...
  if (!isInstalled()) {
    try {
      ...
      console.warn(`[myrouter] tray runtime install failed: ${(err as Error).message}`);
      return null;
    }
  }
  ...
  try {
    ...
    console.warn(`[myrouter] tray runtime import failed: ${(err as Error).message}`);
    return null;
  }
}
...
function ensureRuntimeDir(): void {
  ...
  if (!existsSync(pkg)) {
    writeFileSync(pkg, JSON.stringify({ name: "myrouter-runtime", private: true }), "utf-8");
  }
}
