...
const APP_LABEL = "com.myrouter.autostart";
const WIN_REG_VALUE = "MyRouter";
const WIN_STARTUP_FILE = "MyRouter.vbs";
const LINUX_SERVICE_NAME = "myrouter.service";
const LINUX_DESKTOP_NAME = "myrouter.desktop";

function resolveCliPath() {
  const candidates = [];
  if (process.argv[1]) candidates.push(process.argv[1]);
  try {
    const which = execSync("command -v myrouter 2>/dev/null", { encoding: "utf8" }).trim();
    if (which) candidates.push(which);
  } catch {
    // command -v unavailable
  }
  candidates.push(join(dirname(fileURLToPath(import.meta.url)), "..", "..", "myrouter.mjs"));

  for (const candidate of candidates) {
    if (!candidate) continue;
    try {
      const resolved = realpathSync(candidate);
      if (resolved.endsWith("myrouter.mjs") && existsSync(resolved)) return resolved;
    } catch {
      // try next candidate
    }
  }
...
function writeLinuxSystemdUnit(cliPath) {
  ...
  const envFile = join(userHomeDir(), ".myrouter", ".env");
  const lines = [
    "[Unit]",
    "Description=MyRouter AI proxy router",
 ...
 * running process. When the running MyRouter cli was itself spawned by the
...
/**
 * Builds the VBScript source that launches MyRouter with WSH's Run method
