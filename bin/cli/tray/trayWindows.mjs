...
const MYROUTER_IPC_PORT_BASE = 29128;

export function initWinTray({ port, onQuit, onOpenDashboard, onShowLogs }) {
  if (process.platform !== "win32") return null;

  const ipcPort = MYROUTER_IPC_PORT_BASE + (port % 1000);
  const scriptPath = join(tmpdir(), `myrouter-tray-${process.pid}.ps1`);
...
$mQuit = $menu.Items.Add("Quit MyRouter")
