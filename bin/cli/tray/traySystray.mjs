...
// ~/.myrouter/runtime by trayRuntime.ts (loadSystray). The previous inline
...
export async function initSystrayUnix(
  ...
  const items = [
    { title: `MyRouter  •  port ${port}`, tooltip: "Server running", enabled: false },
    ...
    { title: "Quit MyRouter", enabled: true },
  ];
  ...
  tray.ready().catch((err) => {
    process.stderr.write(`[myrouter][tray] systray2 failed: ${err?.message ?? String(err)}\n`);
  });
