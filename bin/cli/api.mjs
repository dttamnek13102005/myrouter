...
export function getBaseUrl(opts = {}) {
  ...
  const envUrl = process.env.MYROUTER_BASE_URL;
  if (envUrl) return stripTrailingSlash(envUrl);

  // Resolve from the active context (canonical store + legacy profile fallback).
  // This is what makes "remote mode" work: `myrouter contexts use <remote>`
...
export async function buildHeaders(opts) {
  ...
  //
  // The active context's scoped token MUST win over the ambient MYROUTER_API_KEY:
  // `myrouter connect <remote>` saves the context's token, but users keep
  // MYROUTER_API_KEY in their shell. The global `--api-key` option is bound to
  // that env var (.env("MYROUTER_API_KEY")), so commands that spread
  ...
  const ambientKey = process.env.MYROUTER_API_KEY || null;
  ...
  const cliToken = opts.cliToken ?? process.env.MYROUTER_CLI_TOKEN ?? (await getCliToken());
...
export async function apiFetch(path, opts = {}) {
  ...
  const timeout =
    opts.timeout ?? (Number.parseInt(process.env.MYROUTER_HTTP_TIMEOUT_MS || "", 10) || 30000);
  const maxAttempts = opts.retry === false ? 1 : (opts.retryMax ?? RETRY_DEFAULTS.maxAttempts);
  const verbose = opts.verbose ?? process.env.MYROUTER_VERBOSE === "1";
