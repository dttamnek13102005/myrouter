...
// prebuilt addon, etc.), every `myrouter doctor` DB check reported a false FAIL
  ...
  if (message.includes("NODE_MODULE_VERSION") || message.includes("ERR_DLOPEN_FAILED")) {
    return new Error(
      "better-sqlite3 native binding is incompatible with this Node.js runtime. " +
        "Run `npm rebuild better-sqlite3` in the MyRouter project and try again. " +
        "Or run: myrouter runtime repair  " +
...
export async function openMyRouterDb() {
...
export async function readDatabaseHealth(dbPath) {
  return withReadonlySqlite(dbPath, (db) => {
    ...
    const appliedMigrationVersions = hasMigrationTable
      ? db
          .prepare("SELECT version FROM _myrouter_migrations")
          .all()
          .map((row) => row.version)
      : [];
