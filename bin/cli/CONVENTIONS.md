# MyRouter CLI — Internal Conventions
...
```
myrouter keys add openai sk-xxx
myrouter combo switch fastest
myrouter memory search "react hooks"
```

**Not allowed**:

```
myrouter --add-key openai sk-xxx     # ❌ flag-as-verb
myrouter add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `MYROUTER_BASE_URL` env or `~/.myrouter/config.json`
  (active profile).
- Injects `Authorization: Bearer ${MYROUTER_API_KEY}` when available.
- Injects `x-myrouter-cli-token` when applicable (see task 8.12).
...
- Detection order: `--lang` flag → `MYROUTER_LANG` env → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
- Locale persisted via `config lang set <code>` — saves `MYROUTER_LANG` to `~/.myrouter/.env`.
