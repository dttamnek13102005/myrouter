# bin/cli — MyRouter CLI internals

This directory contains the CLI runtime, helpers, and commands for the `myrouter` binary.
...
├── sqlite.mjs              ← openMyRouterDb() — DB bootstrap
...
All HTTP calls to the MyRouter server must go through this wrapper.
...
- `baseUrl` — override base URL (default: `MYROUTER_BASE_URL` env or `localhost:20128`)
- `apiKey` — override API key (default: `MYROUTER_API_KEY`)
...
Locale detection order: `MYROUTER_LANG` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
...
2. `MYROUTER_LANG` environment variable
...
```bash
myrouter config lang set pt-BR       # saves to ~/.myrouter/.env
myrouter config lang list            # show all 42 available locales
myrouter config lang get             # show currently active locale
```

**One-time override:**

```bash
myrouter --lang de providers list    # run in German, not persisted
MYROUTER_LANG=ja myrouter status    # same effect via env
```
