...
function readCache() {
  try {
    ...
    if (process.env.MYROUTER_DEBUG_COMPLETION) {
      console.error("[myrouter completion] readCache failed:", err?.message ?? err);
    }
  }
  return null;
}
...
async function refreshCache(opts = {}) {
  ...
  try {
    ...
    if (process.env.MYROUTER_DEBUG_COMPLETION) {
      console.error("[myrouter completion] refreshCache failed:", err?.message ?? err);
    }
  }
  ...
  try {
    ...
    if (process.env.MYROUTER_DEBUG_COMPLETION) {
      console.error("[myrouter completion] writeCache failed:", err?.message ?? err);
    }
  }
  return data;
}
...
function installPath(shell) {
  const home = homedir();
  if (shell === "zsh") return join(home, ".zsh", "completions", "_myrouter");
  if (shell === "fish") return join(home, ".config", "fish", "completions", "myrouter.fish");
  return join(home, ".bash_completion.d", "myrouter");
}

function generateZshScript() {
  return `#compdef myrouter

# MyRouter zsh completion (dynamic)
_myrouter_get_cache() {
  local key="$1"
  local cache="$HOME/.myrouter/completion-cache.json"
  ...
  if [[ $((now - mtime)) -gt 3600 ]]; then
    myrouter completion refresh --quiet >/dev/null 2>&1
  fi
...
_myrouter() {
  local -a commands
  commands=(
    'serve:Start the MyRouter server'
    'stop:Stop the server'
    'restart:Restart the server'
    'setup:Configure MyRouter'
  ...
  case $state in
    ...
    arg)
      case $words[1] in
        combo)
          case $words[2] in
            switch|delete|show)
              local -a combos
              combos=($(_myrouter_get_cache combos))
        ...
        providers|keys)
          case $words[2] in
            add|remove|test)
              local -a providers
              providers=($(_myrouter_get_cache providers))
      ...
      case $state in
        models)
          local -a models
          models=($(_myrouter_get_cache models))
...
compdef _myrouter myrouter
`;
}

function generateBashScript() {
  return `#!/bin/bash
# MyRouter CLI bash completion (dynamic)
...
complete -F _myrouter myrouter
`;
}

function generateFishScript() {
  return `# MyRouter CLI fish completion (dynamic)
complete -c myrouter -f
...
for cmd in $commands
  complete -c myrouter -n '__fish_is_nth_token 1' -a $cmd
end

# Subcommands
complete -c myrouter -n '__fish_seen_subcommand_from combo' -a 'list switch create delete show suggest'
complete -c myrouter -n '__fish_seen_subcommand_from keys' -a 'add list remove regenerate revoke reveal usage'
complete -c myrouter -n '__fish_seen_subcommand_from providers' -a 'available list test test-all'
complete -c myrouter -n '__fish_seen_subcommand_from config' -a 'list get set validate contexts'
complete -c myrouter -n '__fish_seen_subcommand_from completion' -a 'zsh bash fish install refresh'
complete -c myrouter -n '__fish_seen_subcommand_from open' -a 'combos providers api-manager cli-tools agents settings logs memory skills evals audit cost resilience'

# Dynamic completions from cache (requires python3)
function __myrouter_cache_get
  set -l key $argv[1]
  set -l cache "$HOME/.myrouter/completion-cache.json"
...
complete -c myrouter -n '__fish_seen_subcommand_from combo; and __fish_seen_subcommand_from switch delete' -a '(__myrouter_cache_get combos)'
complete -c myrouter -l model -a '(__myrouter_cache_get models)'
complete -c myrouter -l combo -a '(__myrouter_cache_get combos)'
`;
}
...
export function registerCompletion(program) {
  ...
  // Backward-compat: `myrouter completion <shell>` (positional arg form)
  comp
    .command("<shell>")
