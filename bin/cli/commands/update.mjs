...
export async function getLatestVersion(execFn = execFileAsync) {
  try {
    const { stdout } = await execFn("npm", ["view", "myrouter", "version", "--prefer-online"], {
      timeout: 15000,
    });
    return stdout.trim();
  } catch {
...
export async function createBackup() {
  ...
  const backupDir = path.join(homedir(), ".myrouter", "backups", `myrouter-${Date.now()}`);
  ...
  try {
    ...
    const files = ["myrouter.mjs", "cli", "nodeRuntimeSupport.mjs", "mcp-server.mjs"];
...
export async function runUpdateCommand(opts = {}) {
  ...
  if (showChangelog) {
    try {
      const { stdout } = await execFileAsync("npm", ["view", "myrouter", "changelog"], {
        timeout: 10000,
      });
      if (stdout.trim()) {
        console.log(stdout.trim());
      } else {
        console.log(`Changelog: https://github.com/your-org/myrouter/releases/tag/v${latest}`);
      }
    } catch {
      console.log(`Changelog: https://github.com/your-org/myrouter/releases/tag/v${latest}`);
    }
  ...
  if (checkOnly) {
    console.log("\n  Run `myrouter update --apply` to install automatically.");
    return 1; // exit 1 = outdated (useful for scripts)
  }

  if (dryRun) {
    console.log("\n  [DRY RUN] Would run: npm install -g myrouter@latest --include=optional");
    if (!skipBackup) console.log("  [DRY RUN] Would create backup in ~/.myrouter/backups/");
    return 0;
  }
  ...
  try {
    ...
    execSync("npm install -g myrouter@latest --include=optional", { stdio: "inherit" });
    printSuccess(`Updated to version ${latest}`);
    printInfo("Run `myrouter --version` to verify.");
    return 0;
  } catch (err) {
    printError(`Update failed: ${err.message}`);
    printInfo("Restore from backup:");
    const backupDir = path.join(homedir(), ".myrouter", "backups");
