/**
 * myrouter setup-opencode — Remote-aware OpenCode provider generator
 * (openai-compatible). Distinct from `myrouter setup opencode` (which wires the
 * @myrouter/opencode-plugin). This writes the `myrouter` provider into
 * ~/.config/opencode/opencode.json with every catalog model, so you can run
 * `opencode -m myrouter/<model>`.
 *
...
const ENV_KEY_REF = "{env:MYROUTER_API_KEY}";
...
export function resolveOpencodeTarget(opts = {}) {
  ...
  if (opts.remote) {
    ...
    try {
      const c = resolveActiveContext(opts.context ?? process.env.MYROUTER_CONTEXT);
      baseUrl = c?.baseUrl;
    } catch {
      /* no context */
    }
  ...
  if (!apiKey) apiKey = process.env.MYROUTER_API_KEY || "";
...
export function postProcessOpencodeConfig(rawJson, opts = {}) {
  ...
  const prov = config.provider?.myrouter;
...
export async function runSetupOpencodeCommand(opts = {}) {
  ...
  printHeading("MyRouter → OpenCode provider (openai-compatible)");
  printInfo(`Connecting to ${baseUrl} …`);

  // Deferred import: opencode.ts is TypeScript; tsx is registered by
  // bin/myrouter.mjs before any command runs, so importing here is safe.
  let raw;
  try {
    const { generateOpencodeConfig } = await import(
      "../../../src/lib/cli-helper/config-generator/opencode.ts"
    );
    raw = await generateOpencodeConfig({ baseUrl, apiKey, model: opts.model, providerId: "myrouter" });
  } catch (err) {
    printError(`Failed to generate opencode.json: ${err?.message || err}`);
    printInfo("Make sure MyRouter is running and --remote/--api-key are correct.");
    return 1;
  }
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${modelCount} model(s) under provider 'myrouter' → ${configPath}`);
    return 0;
  }

  if (!existsSync(configDir)) mkdirSync(configDir, { recursive: true });
  writeFileSync(configPath, json, "utf8");
  printSuccess(`opencode.json updated at ${configPath} (${modelCount} models under 'myrouter')`);
  printInfo('Use it:  opencode -m myrouter/<model> "..."   (export MYROUTER_API_KEY first)');
  return 0;
}
...
export function registerSetupOpencode(program) {
  program
    ...
    .description(
      "Generate the MyRouter openai-compatible provider in ~/.config/opencode/opencode.json " +
        "from the live model catalog (local or remote VPS)"
    )
    .option("--port <port>", "Local MyRouter port (ignored when --remote is set)", "20128")
    .option("--remote <url>", "Remote MyRouter URL, e.g. http://192.168.0.15:20128")
    .option("--api-key <key>", "MyRouter API key (defaults to MYROUTER_API_KEY env var)")
    .option("--model <id>", "Set the default top-level model (myrouter/<id>)")
