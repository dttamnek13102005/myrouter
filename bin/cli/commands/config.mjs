...
function ensureBackup(configPath) {
  ...
  const backupDir = path.join(path.dirname(configPath), ".myrouter.bak");
...
async function runConfigGetCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: myrouter config get <tool>");
    return 1;
  }
...
async function runConfigSetCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: myrouter config set <tool> [options]");
    return 1;
  }
  ...
  if (!apiKey) {
    printError("API key required. Use --api-key or set MYROUTER_API_KEY.");
    return 1;
  }
...
async function runConfigValidateCommand(toolId, opts = {}) {
  if (!toolId) {
    printError("Tool ID required. Usage: myrouter config validate <tool>");
    return 1;
  }
...
export async function runConfigLangSetCommand(code, opts = {}) {
  ...
  upsertEnvLine(envPath, "MYROUTER_LANG", code);
  setLocale(code);
  ...
  config
    ...
    .action(async (tool, opts, cmd) => {
      ...
      const exitCode = await runConfigSetCommand(tool, {
        ...opts,
        apiKey: opts.apiKey || globalOpts.apiKey || process.env.MYROUTER_API_KEY,
        baseUrl: opts.baseUrl || globalOpts.baseUrl || process.env.MYROUTER_BASE_URL,
