/**
 * myrouter setup-cline — configure the Cline AI coding agent to use MyRouter.
 *
 ...
 * writes the CLI-mode files (matching the MyRouter dashboard) AND prints the
...
export function resolveClineTarget(opts = {}) {
  ...
  if (opts.remote) baseUrl = stripToRoot(opts.remote);
    ...
    try {
      baseUrl = stripToRoot(resolveActiveContext(opts.context ?? process.env.MYROUTER_CONTEXT)?.baseUrl);
    } catch {
      /* none */
    }
...
/** Merge MyRouter openai-compatible settings into Cline's globalState (Plan + Act). */
...
export function buildClineSecrets(existing, { apiKey }) {
  return { ...(existing || {}), openAiApiKey: apiKey || "sk_myrouter" };
}
...
export async function runSetupClineCommand(opts = {}) {
  ...
  printHeading("MyRouter → Cline (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(`\n── [dry-run] ${secPath} ── (openAiApiKey: ${apiKey ? "set" : "sk_myrouter"})`);
  } else {
...
export function registerSetupCline(program) {
  program
    .command("setup-cline")
    .description(
      "Configure Cline for MyRouter: write ~/.cline/data (CLI mode) + print VS Code extension settings"
    )
