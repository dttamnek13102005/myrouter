/**
 * myrouter setup opencode — Wire the bundled @myrouter/opencode-plugin
 * into a local OpenCode install.
 *
 * Closes the gap where `npm install -g myrouter` ships the plugin
 * inside the myrouter package (`@myrouter/opencode-plugin/dist/`) but
 ...
 *   3. Copies the built plugin into `<opencode>/plugins/myrouter/`.
 *   4. Creates or updates `opencode.json` with a single `plugin` entry
 *      pointing at the local copy (so OC ≥1.15 picks it up).
 *   5. Optionally runs `opencode auth login --provider myrouter`
...
// We walk up from this file to find the myrouter package root. The script
// lives at `<myrouter>/bin/cli/commands/setup-open-code.mjs`, so the
...
// The bundled plugin ships at PACKAGE_ROOT/@myrouter/opencode-plugin/
// (see root package.json `files`: ["@myrouter/", ...]). The env override
// exists so tests can point at a fixture without building the real plugin.
const BUNDLED_PLUGIN_DIR =
  process.env.MYROUTER_OPENCODE_PLUGIN_DIR || join(PACKAGE_ROOT, "@myrouter", "opencode-plugin");
...
/**
 * Locate the bundled @myrouter/opencode-plugin dist. The plugin may be
 * present in two states:
 *
 *   - Built (`dist/index.cjs` + `dist/index.js` exist) — preferred,
 *     ships from a published myrouter tarball after Step 8.8 of
...
function resolveBundledPlugin() {
  if (!existsSync(BUNDLED_PLUGIN_DIR)) {
    throw new Error(
      `Bundled @myrouter/opencode-plugin not found at ${BUNDLED_PLUGIN_DIR}.\n` +
        `This usually means myrouter was installed from a source tree that does not ` +
        `include the workspace package. Try reinstalling myrouter (npm install -g myrouter) ` +
        `or run \`cd @myrouter/opencode-plugin && npm install && npm run build\` from the source repo.`
    );
  }
  ...
  if (!existsSync(esmEntry)) {
    throw new Error(
      `@myrouter/opencode-plugin dist/ not built (looked for ${esmEntry}).\n` +
...
/**
 * Copy the plugin package into `<opencodeConfig>/plugins/myrouter/`. We
 ...
 * resolve the peer deps from the myrouter package's tree, which is
 * unreliable.
 */
function installPluginToOpenCode(pluginInfo, opencodeConfigDir) {
  const targetDir = join(opencodeConfigDir, "plugins", "myrouter");
...
function registerPluginInOpenCodeConfig({
    ...
    try {
      ...
      throw new Error(
        `Failed to parse existing ${configPath}: ${err.message}\n` +
          `Fix or remove the file manually, then re-run \`myrouter setup opencode\`.`
      );
    }
  }
  ...
  const entry = [
    `./plugins/myrouter/dist/index.js`,
    {
      providerId,
      baseURL,
  ...
  // drop a legacy `opencode-myrouter-auth` entry if present — that
  // package is the obsolete predecessor of @myrouter/opencode-plugin
  // and was the root cause of issue #3711.
  const filtered = plugins.filter((p) => {
    if (typeof p === "string") {
      return !p.includes("opencode-myrouter-auth");
    }
    if (Array.isArray(p) && p[1] && typeof p[1] === "object") {
      ...
      if (typeof p[0] === "string" && p[0].includes("opencode-myrouter-auth")) {
        return false;
      }
    }
    return true;
 ...
 *
 * @param {object} opts
 * @param {string} [opts.providerId="myrouter"]
...
export async function runSetupOpenCodeCommand(opts = {}) {
  const providerId = opts.providerId || "myrouter";
  // Remote-aware: explicit --remote/--base-url → active context → localhost.
  let baseURL = opts.remote || opts.baseURL || opts.baseUrl;
  if (!baseURL) {
    try {
      const ctx = resolveActiveContext(opts.context ?? process.env.MYROUTER_CONTEXT);
      baseURL = ctx?.baseUrl;
    } catch {
      /* no context */
    }
  ...
  printHeading("MyRouter → OpenCode Plugin Setup");
...
/**
 * Register the `myrouter setup opencode` subcommand on the parent
 * `setup` command. Commander builds the doc/help from the chain, so
 * `myrouter setup --help` automatically shows the new subcommand.
 *
...
export function registerSetupOpenCode(setupCommand) {
  setupCommand
    ...
    .description(
      t("setup.opencode") ||
        "Install and register the bundled @myrouter/opencode-plugin with a local OpenCode install"
    )
    .option(
      "--provider-id <id>",
      "OpenCode provider id to register (default: myrouter)",
      "myrouter"
    )
    .option(
      "--base-url <url>",
      "MyRouter base URL the plugin should talk to (default: active context or http://localhost:20128)"
    )
    .option(
      "--remote <url>",
      "Remote MyRouter URL, e.g. http://192.168.0.15:20128 (overrides --base-url and the context)"
    )
