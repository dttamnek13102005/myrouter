 ...
 *  in the shell can't shadow the myrouter provider (defense-in-depth). Mirrors
...
const NO_AUTH_SENTINEL = "myrouter-no-auth";
 ...
 * (`model_providers.myrouter.name="MyRouter"` arrives unquoted and no longer
...
/**
 * Resolve the MyRouter root base URL + auth for codex, honouring (in order):
...
export function resolveCodexTarget(opts = {}) {
  ...
  if (explicit) {
    ...
    let fromCtx;
    try {
      fromCtx = resolveActiveContext(opts.context ?? process.env.MYROUTER_CONTEXT)?.baseUrl;
    } catch {
      /* no context */
    }
    baseUrl = fromCtx
  ...
  if (!authToken) authToken = process.env.MYROUTER_API_KEY;
  return { baseUrl, authToken };
}

/** Health-check an MyRouter root URL before launching Codex. */
 ...
 * MYROUTER_API_KEY (the provider env_key) to the resolved token or a sentinel.
...
export function buildCodexEnv(baseEnv, authToken) {
  ...
  env.MYROUTER_API_KEY = (authToken && String(authToken).trim()) || NO_AUTH_SENTINEL;
  return env;
}

/**
 * Codex `-c` flags that define the `myrouter` provider inline, so launch works
 * WITHOUT a pre-existing ~/.codex/config.toml. Mirrors free-claude-code.
 * @param {string} baseUrl  MyRouter root URL (no /v1)
 * @returns {string[]}
 */
export function buildCodexProviderArgs(baseUrl) {
  return [
    "-c",
    tomlAssign("model_provider", "myrouter"),
    "-c",
    tomlAssign("model_providers.myrouter.name", "MyRouter"),
    "-c",
    tomlAssign("model_providers.myrouter.base_url", `${baseUrl}/v1`),
    "-c",
    tomlAssign("model_providers.myrouter.env_key", "MYROUTER_API_KEY"),
    "-c",
    tomlAssign("model_providers.myrouter.wire_api", "responses"),
    "-c",
    tomlAssign("model_providers.myrouter.requires_openai_auth", false),
  ];
}
...
export async function runLaunchCodexCommand(opts = {}, codexArgs = []) {
  ...
  if (!(await healthCheck(baseUrl))) {
    console.error(
      (
        t("launch.notRunning") ||
        "MyRouter is not reachable at {port}. Start it with 'myrouter serve'."
...
export function registerLaunchCodex(program) {
  program
    ...
    .description(
      t("launchCodex.description") || "Launch Codex CLI pointed at MyRouter (local or remote VPS)"
    )
    .option("--port <port>", "Local MyRouter port (ignored when --remote is set)", "20128")
    .option(
      "--remote <url>",
      "Remote MyRouter base URL, e.g. http://192.168.0.15:20128 (overrides --port + context)"
    )
    .option("--profile <name>", "Codex profile to activate (passed as --profile <name>)")
    .option("-p, --p <name>", "Alias for --profile")
    .option(
      "--api-key <key>",
      "MyRouter API key (overrides MYROUTER_API_KEY env var for this invocation)"
    )
