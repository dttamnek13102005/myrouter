...
export async function runCompressionStatus(opts, cmd) {
  const data = await mcpCall("myrouter_compression_status", {}, restCompressionStatus);
...
export async function runCompressionConfigure(opts, cmd) {
  ...
  const data = await mcpCall("myrouter_compression_configure", config, () =>
...
export async function runCompressionEngineSet(name, opts, cmd) {
  ...
  await mcpCall("myrouter_set_compression_engine", { engine: normalized }, () =>
...
export function registerCompression(program) {
  ...
  combos.command("list").action(async (opts, cmd) => {
    const data = await mcpCall("myrouter_list_compression_combos", {}, async () => ({
