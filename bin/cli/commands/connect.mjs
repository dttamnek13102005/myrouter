...
/**
 * `myrouter connect <host>` — remote mode.
 *
 * Logs into a remote MyRouter server and saves the result as the active context
...
export async function runConnectCommand(host, opts = {}) {
  ...
  if (!baseUrl) {
    printError("A host is required, e.g. myrouter connect 192.168.0.15");
    return 2;
  }
  ...
  cfg.contexts[name] = {
    ...
    accessToken,
    scope,
    description: `Remote MyRouter (${host})`,
  };
  ...
  printInfo("Switch back to local with: myrouter contexts use default");
  return 0;
}
...
export function registerConnect(program) {
  program
    ...
    .description(
      t("connect.description") || "Connect to a remote MyRouter server and enter remote mode"
    )
