/**
 * myrouter setup-roo — configure Roo Code (RooVeterinaryInc.roo-cline) for MyRouter.
 *
 ...
 * must support native OpenAI tool-calling (MyRouter does).
 */
...
export function buildRooImport({ baseUrl, apiKey, model }) {
  return {
    providerProfiles: {
      currentApiConfigName: "MyRouter",
      apiConfigs: {
        MyRouter: {
          apiProvider: "openai",
          openAiBaseUrl: baseUrl,
          openAiApiKey: apiKey || "sk_myrouter",
          openAiModelId: model,
...
export async function runSetupRooCommand(opts = {}) {
  ...
  const importPath = opts.importPath ?? opts["import-path"] ?? join(os.homedir(), ".myrouter", "roo-settings.json");
  const vscodePath =
    opts.vscodeSettings ?? opts["vscode-settings"] ?? join(os.homedir(), ".config", "Code", "User", "settings.json");

  printHeading("MyRouter → Roo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    console.log(`\n── [dry-run] ${importPath} ──`);
    console.log(JSON.stringify({ ...importDoc, providerProfiles: { ...importDoc.providerProfiles, apiConfigs: { MyRouter: { ...importDoc.providerProfiles.apiConfigs.MyRouter, openAiApiKey: apiKey ? "set" : "sk_myrouter" } } } }, null, 2));
  ...
  printInfo(`  API Key:   <your MYROUTER_API_KEY>`);
...
export function registerSetupRoo(program) {
  program
    .command("setup-roo")
    .description(
      "Configure Roo Code for MyRouter: write a Roo import JSON + autoImport pointer + print UI steps"
    )
    ...
    .option("--import-path <path>", "Roo import JSON path (default: ~/.myrouter/roo-settings.json)")
