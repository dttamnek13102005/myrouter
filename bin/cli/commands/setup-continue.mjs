/**
 * myrouter setup-continue — configure Continue (continue.dev) for MyRouter.
 *
 ...
 * `apiKey: ${{ secrets.MYROUTER_API_KEY }}` reference (secret never written to
...
const SECRET_REF = "${{ secrets.MYROUTER_API_KEY }}";
...
/**
 * Merge MyRouter models into an existing Continue config object: drop any prior
...
export function mergeContinueConfig(existing, newModels, apiBase) {
  ...
  if (!cfg.name) cfg.name = "MyRouter Config";
...
export async function runSetupContinueCommand(opts = {}) {
  ...
  printHeading("MyRouter → Continue (config.yaml)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${models.length} MyRouter model(s) → ${configPath}`);
    return 0;
  }

  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${models.length} MyRouter models)`);
  printInfo("\nProvide the key (config.yaml references it, not stores it):");
  printInfo("  cn CLI:  export MYROUTER_API_KEY=...   (read from your shell)");
  printInfo("  IDE:     echo 'MYROUTER_API_KEY=...' >> ~/.continue/.env");
...
export function registerSetupContinue(program) {
  program
    ...
    .description(
      "Generate ~/.continue/config.yaml (Continue / cn CLI) from the MyRouter model catalog"
    )
