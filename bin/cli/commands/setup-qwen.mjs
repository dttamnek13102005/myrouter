/** Configure Qwen Code's OpenAI-compatible provider for MyRouter. */
...
export function resolveQwenTarget(opts = {}) {
  ...
  if (!root || !(opts.apiKey ?? opts["api-key"])) {
    try {
      context = resolveActiveContext(opts.context ?? process.env.MYROUTER_CONTEXT);
    } catch {
...
export async function runSetupQwenCommand(opts = {}) {
  ...
  printHeading("MyRouter → Qwen Code (OpenAI-compatible)");
  ...
  try {
    ...
    if (dryRun) {
      ...
      printInfo(`[dry-run] credential → ${envPath} (MYROUTER_API_KEY)`);
      return 0;
    }
    ...
    printSuccess(`Updated ${envPath} (MYROUTER_API_KEY only)`);
...
export function registerSetupQwen(program) {
  program
    .command("setup-qwen")
    .description("Configure Qwen Code's upstream V4 modelProviders format for MyRouter")
    .option("--port <port>", "Local MyRouter port (ignored when --remote is set)", "20128")
    .option("--remote <url>", "Remote MyRouter URL")
    .option("--api-key <key>", "MyRouter API key")
