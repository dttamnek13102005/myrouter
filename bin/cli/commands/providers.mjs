...
export async function runAvailableCommand(opts = {}) {
  ...
  if (opts.json) {
    ...
    printHeading("MyRouter Available Providers");
...
export async function runTestAllCommand(opts = {}) {
  ...
  try {
    ...
    if (opts.json) {
      ...
      printHeading("MyRouter Provider Tests");
...
export async function runValidateCommand(opts = {}) {
  ...
  try {
    ...
    if (opts.json) {
      ...
      printHeading("MyRouter Provider Validation");
      if (results.length === 0) {
        console.log("No providers configured.");
      }
      for (const result of results) {
...
export async function runProvidersRotateCommand(selector, opts = {}) {
  ...
  if (serverUp) {
    try {
      ...
      const { db: db2 } = await openMyRouterDb();
      try {
  ...
  // --- Post-rotation test ---
  if (!opts.skipTest) {
    const { db: db3 } = await openMyRouterDb();
    try {
...
export function registerProviders(program) {
  ...
  providers
    .command("available")
    .description("Show available providers in the MyRouter catalog")
