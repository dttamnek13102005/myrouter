...
function resolveMigrationsDir(rootDir) {
  const configured = process.env.MYROUTER_MIGRATIONS_DIR;
  const candidates = [
    configured,
...
function checkMemory() {
  const configured = process.env.MYROUTER_MEMORY_MB || "512";
  const memoryMb = Number.parseInt(configured, 10);
  if (!Number.isFinite(memoryMb) || memoryMb < 64 || memoryMb > 16384) {
    return fail("Memory", `Invalid MYROUTER_MEMORY_MB: ${configured}`, { configured });
  }
...
function resolveLivenessUrl(options = {}) {
  const explicitUrl = options.livenessUrl || process.env.MYROUTER_DOCTOR_LIVENESS_URL;
  if (explicitUrl) return explicitUrl;

  const port = parsePort(process.env.PORT || "20128", 20128);
  const dashboardPort = parsePort(process.env.DASHBOARD_PORT || String(port), port);
  const host = String(options.livenessHost || process.env.MYROUTER_DOCTOR_HOST || "127.0.0.1")
    .trim()
    .replace(/^https?:\/\//, "")
    .replace(/\/.*$/, "");
