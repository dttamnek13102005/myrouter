/**
 * myrouter setup-cursor — guide Cursor to use MyRouter.
 *
...
export function buildCursorInstructions({ apiBase, models }) {
  const lines = [
    ...
    "  3. Set the OpenAI API Key to your MyRouter key (MYROUTER_API_KEY)",
...
export function registerSetupCursor(program) {
  program
    ...
    .description("Print the steps to point Cursor at MyRouter (chat panel; Cursor config is not file-writable)")
