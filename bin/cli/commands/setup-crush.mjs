/**
 * myrouter setup-crush — configure Crush (charmbracelet/crush) for MyRouter.
 *
 ...
 * include /v1; the api_key may reference an env var (`$MYROUTER_API_KEY`) so the
...
const API_KEY_REF = "$MYROUTER_API_KEY";
...
export function resolveCrushTarget(opts = {}) {
  ...
  if (opts.remote) root = String(opts.remote).replace(/\/+$/, "");
    ...
    try {
      root = resolveActiveContext(opts.context ?? process.env.MYROUTER_CONTEXT)?.baseUrl;
    } catch {
      /* none */
    }
...
export function buildCrushProvider(modelIds, baseUrl) {
  ...
  for (const id of modelIds) {
    ...
    if (!cfg) continue;
    models.push({ id, name: `MyRouter: ${id}`, context_window: cfg.ctx });
  }
  return {
    type: "openai-compat",
    base_url: baseUrl,
...
/** Merge the MyRouter provider into an existing crush.json (preserve the rest). */
export function mergeCrushConfig(existing, provider) {
  const cfg = existing && typeof existing === "object" ? { ...existing } : {};
  cfg.providers = { ...(cfg.providers || {}), myrouter: provider };
  return cfg;
}
...
export async function runSetupCrushCommand(opts = {}) {
  ...
  printHeading("MyRouter → Crush (openai-compat)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${provider.models.length} model(s) under providers.myrouter → ${configPath}`);
    return 0;
  }
  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${provider.models.length} models under providers.myrouter)`);
  printInfo("Provide the key (config references $MYROUTER_API_KEY):  export MYROUTER_API_KEY=...");
...
export function registerSetupCrush(program) {
  program
    .command("setup-crush")
    .description("Generate the MyRouter openai-compat provider in ~/.config/crush/crush.json")
