/**
 * myrouter setup-claude — Remote-aware Claude Code profile generator.
 *
 ...
 * fetches the live /v1/models catalog from a (possibly remote) MyRouter and
 ...
 *
 * Launch a profile with:  myrouter launch --profile <name>
...
export function buildProfileSettings(modelId, baseUrl, cfg) {
  ...
  // NOTE: ANTHROPIC_AUTH_TOKEN is intentionally NOT written here — `myrouter
...
export async function runSetupClaudeCommand(opts = {}) {
  ...
  const baseUrl = (opts.remote ?? `http://localhost:${port}`)
    .replace(/\/+$/, "")
    .replace(/\/v1$/, "");
  const apiKey = opts.apiKey ?? opts["api-key"] ?? process.env.MYROUTER_API_KEY ?? "";
  const claudeHome = opts.claudeHome ?? opts["claude-home"] ?? join(os.homedir(), ".claude");
  const profilesRoot = join(claudeHome, "profiles");
  const dryRun = Boolean(opts.dryRun ?? opts["dry-run"]);

  printHeading("MyRouter → Claude Code profile generator");
  ...
  try {
    ...
    printInfo(
      "Make sure MyRouter is running and the --remote URL is correct.\n" +
        "You may also need --api-key if MyRouter requires authentication."
    );
    return 1;
  }
  ...
  if (!dryRun) {
    ...
    console.log("\nTo use a profile:");
    console.log("  myrouter launch --profile <name>     # e.g. myrouter launch --profile glm52");
    console.log(
...
export function registerSetupClaude(program) {
  program
    ...
    .description(
      "Fetch the live model catalog from MyRouter (local or remote VPS) and generate " +
