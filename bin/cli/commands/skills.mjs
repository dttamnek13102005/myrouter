...
export async function runSkillsEnable(id, opts, cmd) {
  const res = await apiFetch("/api/mcp/tools/call", {
    method: "POST",
    body: { name: "myrouter_skills_enable", arguments: { skillId: id, enabled: true } },
  });
  if (!res.ok) {
    process.stderr.write(`Error: ${res.status}\n`);
    process.exit(1);
...
export async function runSkillsDisable(id, opts, cmd) {
  ...
  const res = await apiFetch("/api/mcp/tools/call", {
    method: "POST",
    body: { name: "myrouter_skills_enable", arguments: { skillId: id, enabled: false } },
  });
  if (!res.ok) {
    process.stderr.write(`Error: ${res.status}\n`);
    process.exit(1);
...
export async function runSkillsExecute(id, opts, cmd) {
  ...
  const res = await apiFetch("/api/mcp/tools/call", {
    method: "POST",
    body: { name: "myrouter_skills_execute", arguments: { skillId: id, input } },
