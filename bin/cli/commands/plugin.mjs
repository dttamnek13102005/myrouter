...
export function registerPlugin(program) {
  const plugin = program
    .command("plugin")
    .description(t("plugin.description") || "Manage CLI plugins (myrouter-cmd-*)");
  ...
  plugin
    ...
    .action(async (opts, cmd) => {
      ...
      if (plugins.length === 0) {
        ...
        process.stdout.write(`Install: myrouter plugin install <name>\n`);
      }
    });
  ...
  plugin
    ...
    .action(async (name, opts) => {
      ...
      const pkgName = isLocal ? name : `myrouter-cmd-${name}`;

      if (!opts.yes) {
        process.stderr.write(
          `⚠ WARNING: Plugins run with the same privileges as myrouter CLI.\n` +
  ...
  plugin
    ...
    .action(async (name, opts) => {
      const pkgName = name.startsWith("myrouter-cmd-") ? name : `myrouter-cmd-${name}`;
      if (!opts.yes) {
  ...
  plugin
    ...
    .action(async (name, opts, cmd) => {
      const plugins = await discoverPlugins();
      const p = plugins.find((x) => x.name === name || x.name === `myrouter-cmd-${name}`);
      if (!p) {
  ...
  plugin
    ...
    .action(async (query) => {
      const q = query ? `myrouter-cmd-${query}` : "myrouter-cmd";
      ...
      try {
        ...
        if (rows.length === 0) {
          process.stdout.write(`No plugins found for '${query || "myrouter-cmd"}'.\n`);
        } else {
          rows.forEach((r) =>
  ...
  plugin
    ...
    .action(async (name) => {
      ...
      const dir = join(process.cwd(), `myrouter-cmd-${safeName}`);
      if (existsSync(dir)) {
      ...
      writeFileSync(
        ...
        JSON.stringify(
          {
            name: `myrouter-cmd-${safeName}`,
            version: "0.1.0",
            type: "module",
            main: "index.mjs",
            description: `MyRouter CLI plugin: ${safeName}`,
      ...
      writeFileSync(
        join(dir, "README.md"),
        `# myrouter-cmd-${safeName}\n\nAn MyRouter CLI plugin.\n\n## Install\n\n\`\`\`bash\nmyrouter plugin install ${safeName}\n\`\`\`\n`
      );
      process.stdout.write(`✓ Scaffolded: ${dir}\n`);
      process.stdout.write(`  Run: cd ${dir} && myrouter plugin install .\n`);
    });
}
