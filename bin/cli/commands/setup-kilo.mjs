/**
 * myrouter setup-kilo — configure Kilo Code to use MyRouter.
 *
 ...
 * This writes BOTH (matching the MyRouter dashboard) and prints the UI settings.
 *
...
/** Merge the MyRouter openai-compatible provider into Kilo's CLI auth.json. */
export function buildKiloAuth(existing, { apiKey, baseUrl, model }) {
  const auth = { ...(existing || {}) };
  auth["openai-compatible"] = {
    ...(auth["openai-compatible"] || {}),
    apiKey: apiKey || "sk_myrouter",
    baseUrl,
    model,
  };
  return auth;
...
export function buildKiloVscodeSettings(existing, { apiKey, baseUrl, model }) {
  const s = { ...(existing || {}) };
  s["kilocode.customProvider"] = { name: "MyRouter", baseURL: baseUrl, apiKey: apiKey || "sk_myrouter" };
...
export async function runSetupKiloCommand(opts = {}) {
  ...
  printHeading("MyRouter → Kilo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(
      JSON.stringify(
        { "openai-compatible": { ...auth["openai-compatible"], apiKey: apiKey ? "set" : "sk_myrouter" } },
        null,
        2
      )
    );
...
export function registerSetupKilo(program) {
  program
    .command("setup-kilo")
    .description(
      "Configure Kilo Code for MyRouter: write ~/.local/share/kilo/auth.json (CLI) + VS Code kilocode.* settings"
    )
