...
// URL scheme for the "MyRouter is running" banner — flipped to https when
...
export function registerServe(program) {
  program
    ...
    .option(
      "--tls-cert <path>",
      t("serve.tls_cert") ||
        "Path to a TLS certificate (PEM) to serve HTTPS (also MYROUTER_TLS_CERT)"
    )
    .option(
      "--tls-key <path>",
      t("serve.tls_key") ||
        "Path to the TLS private key (PEM) to serve HTTPS (also MYROUTER_TLS_KEY)"
    )
    .action(async (opts) => {
      await runServe(opts);
    });
...
export async function runServe(opts = {}) {
  ...
  // Same prep as bin/myrouter.mjs — keep it here so a direct `runServe()` call
...
\x1b[0m`);
  ...
  if (!nodeSupport.nodeCompatible) {
    ...
    console.warn(`\x1b[33m  ⚠  Warning: You are running Node.js ${process.versions.node}.
     ...
     Or run:      myrouter runtime repair  (rebuilds into a user-writable runtime; works without a C++ toolchain)\x1b[0m
`);
  }
  ...
  if (!existsSync(serverJs)) {
    ...
    if (isMise) {
      console.error(
        "  \x1b[33m⚠ mise detected:\x1b[0m If you installed via `npm install -g myrouter`,"
      );
      console.error("    try: \x1b[36mnpx myrouter@latest\x1b[0m  (downloads a fresh copy)");
      console.error("    or:  \x1b[36mmise exec -- npx myrouter\x1b[0m");
    } else if (isNvm) {
      console.error(
        "  \x1b[33m⚠ nvm detected:\x1b[0m Try reinstalling after loading the correct Node version:"
      );
      console.error("    \x1b[36mnvm use --lts && npm install -g myrouter\x1b[0m");
    } else {
      console.error("  Try: \x1b[36mnpm install -g myrouter\x1b[0m  (reinstall)");
      console.error("  Or:  \x1b[36mnpx myrouter@latest\x1b[0m");
    }
    process.exit(1);
  }
  ...
  if (
    ...
    console.error(
      "  Or run: \x1b[36mmyrouter runtime repair\x1b[0m" +
  ...
  // of RAM under load. An explicit MYROUTER_MEMORY_MB still wins.
  const memoryLimit = resolveMaxOldSpaceMb(
    process.env.MYROUTER_MEMORY_MB,
  ...
  const tlsCert = opts.tlsCert ?? process.env.MYROUTER_TLS_CERT;
  const tlsKey = opts.tlsKey ?? process.env.MYROUTER_TLS_KEY;

  const env = {
    ...process.env,
    MYROUTER_PORT: String(port),
    ...
    // matches the OS-reported hostname (the auto-set signature). MYROUTER_SERVER_HOST
    ...
    HOSTNAME:
      process.env.MYROUTER_SERVER_HOST ||
    ...
    ...(tlsCert ? { MYROUTER_TLS_CERT: tlsCert } : {}),
    ...(tlsKey ? { MYROUTER_TLS_KEY: tlsKey } : {}),
  };
...
function runDaemon(serverJs, env, memoryLimit, dashboardPort, apiPort) {
  ...
  server.unref();
  console.log(`\x1b[32m✔ MyRouter started in background (PID: ${server.pid})\x1b[0m`);
...
function runWithoutRecovery(serverJs, env, memoryLimit, dashboardPort, apiPort, noOpen, startedAt) {
  ...
  const shutdown = () => {
    console.log("\n\x1b[33m⏹ Shutting down MyRouter...\x1b[0m");
...
async function runWithSupervisor(
  ...
  if (showLog) process.env.MYROUTER_SHOW_LOG = "1";
  ...
  try {
    ...
    const tray = await initTray({
      ...
      onShowLogs: () => {
        ...
        process.stdout.write(`[myrouter][tray] Logs at: ${dashboardUrl}/logs\n`);
      },
    });
    if (tray) {
    ...
    process.stderr.write(`[myrouter][tray] failed to start: ${err?.message ?? String(err)}\n`);
  }
}
...
async function onReady(dashboardPort, apiPort, noOpen, startedAt) {
  ...
  console.log(`
  \x1b[32m✔ MyRouter is running!\x1b[0m \x1b[2m(started in ${elapsed}s)\x1b[0m
