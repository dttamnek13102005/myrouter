...
export function registerTray(program) {
  ...
  cmd
    ...
    .action(() => {
      process.stderr.write(
        "The tray is managed by `myrouter serve --tray`. Start the server with --tray to enable it.\n"
      );
    });
  ...
  cmd
    .command("quit")
    .description(t("tray.quit") || "Quit MyRouter via tray")
    .action(async () => {
      const { default: pidUtils } = await import("../utils/pid.mjs").catch(() => ({
        default: null,
      }));
      process.stderr.write("Use `myrouter stop` to stop the server.\n");
      process.exit(0);
    });
}
