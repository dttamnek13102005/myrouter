/**
 * myrouter setup-goose — configure Goose (block/goose) for MyRouter.
 *
...
export async function runSetupGooseCommand(opts = {}) {
  ...
  printHeading("MyRouter → Goose (openai-compatible)");
...
export function registerSetupGoose(program) {
  program
    .command("setup-goose")
    .description("Configure Goose for MyRouter: write ~/.config/goose/config.yaml + print the env recipe")
