...
const DEFAULT_NAME = "myrouter-redis";
const DEFAULT_PORT = "6379";
const DEFAULT_VOLUME = "myrouter-redis-data";
...
export function registerRedis(program) {
  const redis = program
    ...
    .description(
      t("redis.description") ||
        "Launch a 1-click local Redis container (Podman or Docker) for MyRouter caching and quota tracking"
    );
...
export async function runRedisUpCommand(opts = {}) {
  ...
  try {
    ...
    info(`Set MYROUTER_REDIS_URL=redis://127.0.0.1:${port} in your .env to wire MyRouter to it.`);
    return 0;
  } catch (err) {
...
export async function runRedisStatusCommand(opts = {}) {
  ...
  if (!running) {
    info(`Run 'myrouter redis up' to launch it.`);
  }
  return 0;
}