...
/**
 * `myrouter configure <cli>` — interactive provider+model picker that writes a
 * local CLI config pointed at the ACTIVE MyRouter context (local or remote).
 *
 * The model catalog comes from the active context's GET /v1/models, so when you
 * are in remote mode (`myrouter connect ...`) you pick from the remote server's
 ...
 * is referenced by env var (MYROUTER_API_KEY) — never written to disk.
 */
...
function buildCodexProfile(modelId, ctx) {
  const lines = [
    ...
    `# ${modelId} — generated by 'myrouter configure codex'`,
    `model                          = "${modelId}"`,
...
async function configureCodex(modelId, ctxWindow, opts) {
  ...
  printInfo("Prereq: ~/.codex/config.toml must define the [model_providers.myrouter] block");
