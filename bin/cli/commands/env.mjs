...
export async function runEnvShowCommand(opts = {}) {
  const current = {};
  for (const key of MYROUTER_ENV_VARS) {
...
export async function runEnvGetCommand(key) {
  if (!key) {
    console.error("Key is required. Usage: myrouter env get <key>");
    return 1;
  }
...
export async function runEnvSetCommand(key, value) {
  if (!key || value === undefined) {
    console.error("Usage: myrouter env set <key> <value>");
    return 1;
  }
