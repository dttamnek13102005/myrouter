/**
 * myrouter setup-aider — configure Aider (aider.chat) for MyRouter.
 *
...
export function resolveAiderTarget(opts = {}) {
  ...
  if (opts.remote) root = stripToRoot(opts.remote);
    ...
    try {
      root = stripToRoot(resolveActiveContext(opts.context ?? process.env.MYROUTER_CONTEXT)?.baseUrl);
    } catch {
      /* none */
    }
...
export function buildAiderRecipe({ apiBase, model }) {
  return [
    ...
    "export OPENAI_API_KEY=$MYROUTER_API_KEY",
...
export async function runSetupAiderCommand(opts = {}) {
  ...
  printHeading("MyRouter → Aider (openai-compatible via LiteLLM)");
...
export function registerSetupAider(program) {
  program
    .command("setup-aider")
    .description("Configure Aider for MyRouter: write ~/.aider.conf.yml + print the env recipe")
