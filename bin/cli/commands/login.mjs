...
/**
 * `myrouter login antigravity` — local OAuth helper for remote installs.
 *
 ...
 *
 * It talks ONLY to Google (no MyRouter server needed locally), so it works even
...
function defaultStartServer(preferredPort) {
  return new Promise((resolve, reject) => {
    ...
    const server = createServer((req, res) => {
      ...
      res.end(
        "<!doctype html><meta charset=utf-8><title>MyRouter</title>" +
...
export async function runAntigravityLogin(opts = {}, deps = {}) {
  ...
  print(
    "\n" +
      ...
      "MyRouter dashboard: Providers → Antigravity → Connect → \"Paste credentials\".\n" +
...
export function registerLogin(program) {
  const login = program
    .command("login")
    .description("Local OAuth helpers for remote MyRouter installs (run on your own machine)");
