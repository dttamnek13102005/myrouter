/**
 * myrouter setup-codex — Remote-aware Codex CLI profile generator.
 *
 * Connects to a running MyRouter instance (local or remote VPS), fetches the
 ...
 *   myrouter setup-codex --remote http://100.67.86.91:20128 --api-key sk-xxx
 *
...
export async function runSetupCodexCommand(opts = {}) {
  ...
  printHeading(`MyRouter → Codex CLI profile generator`);
...
export function registerSetupCodex(program) {
  program
    ...
    .option(
      "--remote <url>",
      "Remote MyRouter URL, e.g. http://100.67.86.91:20128 — fetches models from there"
    )
    .option(
      "--api-key <key>",
      "MyRouter API key for the remote instance (defaults to MYROUTER_API_KEY env var)"
    )
