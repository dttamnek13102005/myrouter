#!/usr/bin/env bash
# bin/rollback.sh — roll MyRouter back to a previous release to mitigate a bad
# deploy. Part of the deploy-rollback incident-recovery flow.
#
# Methods (auto-detected; override with --method):
#   • npm    — `npm install -g myrouter@<version>` and, if PM2 manages it,
#              `pm2 restart myrouter`. This is how the VPS deploy runs.
#   • docker — re-tag the local image myrouter:<version> to myrouter:prod and
...
Rolls MyRouter back to <version> (e.g. 3.8.35 or v3.8.35). With no version,
...
if [ -z "$VERSION" ]; then
  ...
  VERSION="$(npm view myrouter versions --json 2>/dev/null | node -e '
    let s = "";
...
ops_log "target: myrouter@$VERSION via $METHOD"
ops_confirm "Roll MyRouter back to $VERSION via $METHOD?" || ops_die "aborted"

case "$METHOD" in
  npm)
    ops_require_cmd npm
    npm install -g "myrouter@$VERSION"
    if command -v pm2 >/dev/null 2>&1 && pm2 jlist 2>/dev/null | grep -q '"name":"myrouter"'; then
      pm2 restart myrouter --update-env
      ops_log "pm2 restarted myrouter"
    else
      ops_log "installed myrouter@$VERSION — restart the service to apply (no PM2 'myrouter' process found)"
    fi
    ;;
  docker)
    ops_require_cmd docker
    if ! docker image inspect "myrouter:$VERSION" >/dev/null 2>&1; then
      ops_die "local image myrouter:$VERSION not found — build it from the $VERSION checkout first (this compose builds the 'prod' tag, it does not pull a registry tag)"
    fi
    docker tag "myrouter:$VERSION" myrouter:prod
    docker compose -f "$REPO_ROOT/docker-compose.prod.yml" up -d --no-build
    ops_log "recreated prod service from myrouter:$VERSION"
    ;;
