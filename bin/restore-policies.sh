...
[ -f "$MYROUTER_SQLITE" ] || ops_die "no live DB at $MYROUTER_SQLITE (use restore-data.sh for a full restore)"
...
ops_confirm "Replace ${#tables[@]} policy table(s) in $MYROUTER_SQLITE from $snap?" || ops_die "aborted"

# Safety snapshot of the live DB before mutating it.
safety="$MYROUTER_BACKUPS_DIR/pre-policy-restore_$(date -u +%Y%m%dT%H%M%SZ)"
mkdir -p "$safety"
...
BEGIN;"
for t in "${tables[@]}"; do
  if [ -n "$(sqlite3 "$MYROUTER_SQLITE" "SELECT 1 FROM sqlite_master WHERE type='table' AND name='$t' LIMIT 1;")" ]; then
    sql="$sql
DELETE FROM main.\"$t\";
...
printf '%s\n' "$sql" | sqlite3 "$MYROUTER_SQLITE"
ops_log "policies restored from $snap — restart MyRouter to apply"
