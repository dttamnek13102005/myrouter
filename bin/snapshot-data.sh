#!/usr/bin/env bash
# bin/snapshot-data.sh — consistent point-in-time snapshot of the MyRouter data
...
Creates a consistent snapshot of the MyRouter SQLite data under the backups dir
and prints the snapshot id (UTC timestamp) on stdout.

Env: DATA_DIR (default ~/.myrouter), DB_BACKUPS_DIR (default $DATA_DIR/db_backups).
EOF
}
...
[ -f "$MYROUTER_SQLITE" ] || ops_die "no storage.sqlite at $MYROUTER_SQLITE (set DATA_DIR?)"

ts="$(date -u +%Y%m%dT%H%M%SZ)"
id="${ts}${LABEL:+_$LABEL}"
dest="$MYROUTER_BACKUPS_DIR/snapshot_$id"
mkdir -p "$dest"

if command -v sqlite3 >/dev/null 2>&1; then
  # VACUUM INTO yields a transactionally-consistent copy under WAL.
  sqlite3 "$MYROUTER_SQLITE" "VACUUM INTO '$dest/storage.sqlite'"
else
  ops_log "sqlite3 not found — copying files (stop writers first for a clean copy)"
  cp -a "$MYROUTER_SQLITE" "$dest/storage.sqlite"
  for ext in -wal -shm; do
    [ -f "${MYROUTER_SQLITE}${ext}" ] && cp -a "${MYROUTER_SQLITE}${ext}" "$dest/" || true
  done
fi

# Capture sibling SQLite DBs (analytics, etc.) if present.
for f in "$MYROUTER_DATA_DIR"/*.sqlite; do
  [ -e "$f" ] || continue
