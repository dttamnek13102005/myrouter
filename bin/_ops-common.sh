# bin/_ops-common.sh — shared helpers for the MyRouter ops runbook scripts.
#
...
ops_set_data_dir() {
  MYROUTER_DATA_DIR="$1"
  MYROUTER_SQLITE="${MYROUTER_DATA_DIR}/storage.sqlite"
  MYROUTER_BACKUPS_DIR="${DB_BACKUPS_DIR:-${MYROUTER_DATA_DIR}/db_backups}"
}
ops_set_data_dir "${DATA_DIR:-$HOME/.myrouter}"
...
ops_find_snapshot() {
  ...
  for cand in \
    "$id" \
    "$id/" \
    "$MYROUTER_BACKUPS_DIR/$id" \
    "$MYROUTER_BACKUPS_DIR/snapshot_$id"; do
  ...
  if [ -d "$MYROUTER_BACKUPS_DIR" ]; then
    for cand in "$MYROUTER_BACKUPS_DIR"/snapshot_*"$id"*; do
      [ -f "$cand/storage.sqlite" ] && { printf '%s\n' "$cand"; return 0; }
    done
  fi
  ops_die "no snapshot matching '$id' under $MYROUTER_BACKUPS_DIR (run bin/snapshot-data.sh first)"
}
