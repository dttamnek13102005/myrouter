#!/usr/bin/env bash
# bin/restore-data.sh — restore the MyRouter SQLite data volume from a snapshot
...
Stop MyRouter before running, and restart it afterwards.
EOF
}
...
ops_log "restore source: $snap → $MYROUTER_DATA_DIR"
ops_confirm "Overwrite storage.sqlite at $MYROUTER_DATA_DIR from $snap?" || ops_die "aborted"

# Pre-restore safety copy of the live data.
if [ -f "$MYROUTER_SQLITE" ]; then
  safety="$MYROUTER_BACKUPS_DIR/pre-restore_$(date -u +%Y%m%dT%H%M%SZ)"
  mkdir -p "$safety"
  if command -v sqlite3 >/dev/null 2>&1; then
    sqlite3 "$MYROUTER_SQLITE" "VACUUM INTO '$safety/storage.sqlite'" \
      || cp -a "$MYROUTER_SQLITE" "$safety/storage.sqlite"
  else
    cp -a "$MYROUTER_SQLITE" "$safety/storage.sqlite"
  fi
...
mkdir -p "$MYROUTER_DATA_DIR"
# Drop stale WAL/SHM so the restored DB is authoritative, then copy in.
rm -f "$MYROUTER_SQLITE" "${MYROUTER_SQLITE}-wal" "${MYROUTER_SQLITE}-shm"
cp -a "$snap/storage.sqlite" "$MYROUTER_SQLITE"

# Restore sibling DBs captured in the snapshot.
for f in "$snap"/*.sqlite; do
  [ -e "$f" ] || continue
  [ "$(basename "$f")" = "storage.sqlite" ] && continue
  cp -a "$f" "$MYROUTER_DATA_DIR/"
done

ops_log "restore complete — restart MyRouter to pick up the restored data"
