 ...
 *
 * Problem (#7791): when MyRouter is installed via `npm i -g myrouter`, the
 * package files live under `node_modules/myrouter/`. tsx's tsconfig-path
 * resolution does not apply there, so specifiers like `@/shared/utils/featureFlags`
 * (declared in tsconfig.json `paths` as `@/* → ./src/*`) or
 * `@myrouter/open-sse/services/usage` fail with `ERR_MODULE_NOT_FOUND`.
 ...
 *
 * Each entry:
 *   prefix  — specifier prefix to match (e.g. `"@/"`, `"@myrouter/open-sse/"`)
 ...
 *             prefix *without* a trailing slash (e.g. `@myrouter/open-sse` →
...
export const ALIAS_MAP = [
  ...
  { prefix: "@myrouter/open-sse/", target: "open-sse", exact: false },
  { prefix: "@myrouter/open-sse", target: "open-sse", exact: true },
];
 ...
 *   "@/..."                     → <root>/src/...
 *   "@myrouter/open-sse/..."   → <root>/open-sse/...
 *   "@myrouter/open-sse"       → <root>/open-sse/index.*
 *
...
export function resolveAlias(specifier, root) {
  ...
  // Exact match (e.g. `@myrouter/open-sse` with no trailing path) →
