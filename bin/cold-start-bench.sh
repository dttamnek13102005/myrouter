#!/usr/bin/env bash
# bin/cold-start-bench.sh — measure MyRouter cold-start against the target
...
Boots MyRouter, times cold-start to the first /api/health/ping 200, measures
...
if [ -n "$BASE_URL" ]; then
  ...
  PORT="$PORT" $START_CMD >/tmp/myrouter-coldstart.log 2>&1 &
  SERVER_PID="$!"
  deadline=$(($(now_ms) + 30000))
  until ping_ok "$BASE_URL"; do
    kill -0 "$SERVER_PID" 2>/dev/null || ops_die "server process exited during boot (see /tmp/myrouter-coldstart.log)"
