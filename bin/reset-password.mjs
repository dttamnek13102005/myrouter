 ...
 *   myrouter reset-password
 *
 * Non-interactive / scripted usage (piped stdin, e.g. CI or Docker):
 *   printf 'NewPass123\nNewPass123\n' | myrouter reset-password
 *   printf 'NewPass123' | myrouter reset-password --password-stdin
 *
 * Resets the admin password for MyRouter.
...
async function main() {
  ...
  if (!passwordState.exists) {
    ...
    console.error(`   Make sure MyRouter has been started at least once.`);
  ...
  console.log("   Restart MyRouter for changes to take effect.\n");
}

main()
  .then(() => {
    // Explicit exit(0) so a caller that imports this module (bin/myrouter.mjs
    // routes `myrouter reset-password` here) terminates cleanly instead of
