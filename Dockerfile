...
# escape hatch: `--build-arg`/-e MYROUTER_USE_TURBOPACK=0.
# See docs/ops/QUALITY_GATE_PLAYBOOK.md Parte 6.
ENV MYROUTER_USE_TURBOPACK=1

# Next.js basePath is fixed at build time; pass MYROUTER_BASE_PATH here when the
# image should serve under a reverse-proxy subpath without a runtime patch.
ARG MYROUTER_BASE_PATH=""
ENV MYROUTER_BASE_PATH=$MYROUTER_BASE_PATH
...
# (MYROUTER_MEMORY_MB). Override: `--build-arg MYROUTER_BUILD_MEMORY_MB=6144`.
ARG MYROUTER_BUILD_MEMORY_MB=4096
ENV NODE_OPTIONS="--max-old-space-size=${MYROUTER_BUILD_MEMORY_MB}"
...
LABEL org.opencontainers.image.title="myrouter" \
  org.opencontainers.image.description="Unified AI proxy — route any LLM through one endpoint" \
  org.opencontainers.image.url="https://myrouter.online" \
  org.opencontainers.image.source="https://github.com/diegosouzapw/MyRouter" \
...
# `-e MYROUTER_MEMORY_MB=2048` (or higher) if you raise fusionTuning.maxPanel
# above the default cap.
ENV MYROUTER_MEMORY_MB=1024
ENV NODE_OPTIONS="--max-old-space-size=${MYROUTER_MEMORY_MB}"
...
ENV MYROUTER_MIGRATIONS_DIR=/app/migrations
...
#
#  Two image flavors:
#    runner-base  →  myrouter:VERSION        Lean base (~500 MB). No browsers.
#    runner-web   →  myrouter:VERSION-web    +Chromium/Playwright (~800 MB).
#
...
#
#  Build:
#    docker build --target runner-web -t myrouter:web .
#  Compose:
#    build:
#      context: .
#      target: runner-web
