...
MyRouter compression is built around engine contracts. A mode can run one engine directly
...
actually reach `myrouter_ccr_retrieve`** (`callerSupportsCcrRetrieve()` in
...
with a `22-87%` range, and a `~46%` input-compression tool. MyRouter uses the Caveman input-side
...
MyRouter install; when the optional stack is absent, LLMLingua still fail-opens. Only the three
...
- **Global npm (`npm i -g myrouter`) / standalone** — run the install command above inside
...
| Tool                                | Scope               | Purpose                          |
| ----------------------------------- | ------------------- | -------------------------------- |
| `myrouter_compression_status`      | `read:compression`  | Settings, analytics, cache stats |
| `myrouter_compression_configure`   | `write:compression` | Update global settings           |
| `myrouter_set_compression_engine`  | `write:compression` | Set mode and optional pipeline   |
| `myrouter_list_compression_combos` | `read:compression`  | List compression combos          |
| `myrouter_compression_combo_stats` | `read:compression`  | Read combo/engine analytics      |
