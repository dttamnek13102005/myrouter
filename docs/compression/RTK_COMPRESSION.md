...
RTK compression is MyRouter's command-aware compression engine for terminal and tool output. It is
...
`~118,000` standard tokens to `~23,900` RTK tokens, which is `79.7%` saved (`~80%`). MyRouter uses
...
Within the same scope, RTK TOML schema v1 filters take precedence over MyRouter JSON filters. TOML
...
- `MYROUTER_RTK_TRUST_PROJECT_FILTERS=1` is set.
...
MyRouter can parse, validate, test, and install declarative filter files using RTK TOML schema v1.
...
MyRouter receives tool output after the client has already captured it, so `filter_stderr = true`
...
`rtkEngine` (`@myrouter/open-sse/services/compression/engines/rtk`) is a
...
- User directory: `~/.myrouter/rtk/filters/`
