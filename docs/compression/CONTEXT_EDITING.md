...
Delegated **Context Editing** is a Claude-only context-management feature. Unlike MyRouter's local
...
tool-use / tool-result blocks from its own running context window. MyRouter only attaches a body
parameter (`context_management.edits[]`); Claude does the actual clearing against its own tokenizer.

This is a delegated capability by nature: other providers reject the parameter, so MyRouter scopes
...
MyRouter injects a single edit into the outbound Anthropic Messages body:
...
MyRouter already emits on Claude requests.
...
left as-is. If a `clear_thinking_20251015` edit is also present, MyRouter stable-sorts the
...
After a Claude response, MyRouter records how much context the provider actually cleared. This is
...
| ----------------- | --------------------------------------------------- | ------------------------------------------- |
| Where it runs     | In MyRouter, before the request leaves the proxy   | In the provider (Claude), server-side       |
...
The two are complementary: local engines compress the bytes MyRouter sends; Context Editing lets
