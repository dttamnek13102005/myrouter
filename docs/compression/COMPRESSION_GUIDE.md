---
title: "🗜️ Prompt Compression Guide — MyRouter"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🗜️ Prompt Compression Guide — MyRouter
...
MyRouter implements a modular prompt compression pipeline that runs **proactively** before requests hit upstream providers. This means your token savings happen transparently — no changes needed to your workflow.
...
MyRouter documents compression savings from two sources: upstream project benchmarks and
MyRouter's own engine composition.
...
For overlapping tool/context payloads, the default MyRouter combo stacks the engines:
...
Send the `x-myrouter-compression` request header to override the compression plan for a single
...
The applied plan is echoed back in the `X-MyRouter-Compression: <mode>; source=<source>` response
...
Beyond the 7 standard modes, MyRouter includes several advanced compression
