...
> **TL;DR**: MyRouter ships with built-in health monitoring, provider autopilot, quota tracking, and observability hooks. This guide covers the dashboard, alerts, and troubleshooting.
...
MyRouter has **3 layers of monitoring**:
...
│  └─ MCP tools: myrouter_get_health, myrouter_get_session_snapshot │
└──────────────────────────────────────────────────────────────┘
```
...
MyRouter supports **3 alert channels**:
...
Create a `~/.myrouter/dashboard.json`:
...
- MyRouter's quota tracking may be **stale** — the provider's truth is upstream
...
- Run `sqlite3 ~/.myrouter/storage.sqlite "PRAGMA integrity_check;"`
- If "ok" — false alarm, the health check is being too strict
- If anything else — **stop MyRouter** and follow the [disaster recovery guide](./DATABASE_GUIDE.md#disaster-recovery)
