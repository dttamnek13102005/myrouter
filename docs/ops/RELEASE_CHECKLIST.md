...
1. `npm stage list myrouter` — find the stage id (also printed in the workflow summary).
...
`myrouter` in stage-only mode so a leaked long-lived token cannot `npm publish`
directly from anywhere — CI can only stage; only the owner's 2FA releases.

**Broken-artifact playbook (unchanged):** `npm deprecate myrouter@<bad> "<reason> — use <fixed>"`
...
- [ ] If source English docs changed, run `npm run i18n:run` (requires `MYROUTER_TRANSLATION_API_KEY` in `.env`) before tagging
...
- [ ] Test on fresh install: delete `~/.myrouter/myrouter.db` and run `npm run dev`
...
> **Operator note:** the remote VPS image directory remains `/usr/lib/node_modules/myrouter/app/`.
...
- [ ] `myrouter --tray` boots on macOS (systray2 installed into `~/.myrouter/runtime/`)
- [ ] `myrouter --tray` boots on Linux (requires DISPLAY; graceful error if not set)
- [ ] `myrouter --tray` boots on Windows (PowerShell NotifyIcon, no extra binaries)
- [ ] `myrouter config tray enable` creates autostart entry; disable removes it
- [ ] `npm install -g myrouter@<this-version>` runs postinstall without fatal exit
- [ ] Update path keeps optional deps: `myrouter update --apply` and the auto-updater
...
- [ ] `myrouter status` works with no `.env` (CLI token path, loopback only)
...
- [ ] All 10 `skills/myrouter*/SKILL.md` files are publicly fetchable via raw GitHub URL
