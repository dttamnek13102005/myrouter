...
> **TL;DR**: MyRouter uses **SQLite with WAL journaling** as its primary store, with **AES-256-GCM** encryption at rest for sensitive fields. This guide covers the schema, migrations, backup/recovery, and operational runbooks.
...
MyRouter chose SQLite over PostgreSQL/MySQL for several reasons:
...
For **single-user, single-instance** deployments (the primary MyRouter use case), SQLite is simpler and faster.
...
MyRouter's database has **94 domain modules** in `src/lib/db/`. Each module:
...
MyRouter has **94 module files** in `src/lib/db/`. Below is a sampling of core modules; see the directory listing for the complete list:
...
MyRouter uses **versioned, idempotent migrations** in `src/lib/db/migrations/`. Each migration is a single SQL file named `NNN_description.sql`.
...
1. Creates `_myrouter_migrations` table if not exists
...
> **Backwards-incompatible changes** (e.g., dropping columns) are tricky. MyRouter does NOT support downgrade — once a migration is applied, the schema change is permanent. Plan accordingly.
...
MyRouter uses **`migrateLegacyEncryptedString()`** to handle two encryption schemes transparently:
...
| ------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------- |
| `MYROUTER_BACKUP_SCHEDULE_JOB_INTERVAL_MS` | `30000` | Tick interval in ms (min `5000`). Must be shorter than 60 s to reliably land inside the matching cron minute. |
...
This uses SQLite's online backup API — safe to run while MyRouter is running.
...
Long-running MyRouter instances benefit from occasional `VACUUM`:
...
- If persistent, restart MyRouter
...
1. **Stop MyRouter** (prevent further attempts)
2. **Check the DB state** with `sqlite3`
3. **Manually fix** the partial migration
4. **Re-run** MyRouter (the migration will be retried)
