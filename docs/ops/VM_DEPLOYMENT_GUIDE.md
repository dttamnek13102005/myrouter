---
title: "MyRouter — Deployment Guide on VM with Cloudflare"
version: 3.8.40
lastUpdated: 2026-06-28
---

# MyRouter — Deployment Guide on VM with Cloudflare
...
Complete guide to install and configure MyRouter on a VM (VPS) with domain managed via Cloudflare.
...
Keep reverse-proxy stream timeouts aligned with your MyRouter timeout env vars. If you raise
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, raise `proxy_read_timeout` / `proxy_send_timeout`
above the same threshold.

MyRouter uses `NEXT_PUBLIC_BASE_URL` as the canonical browser-facing origin for OAuth
...
`MYROUTER_TRUST_PROXY` if MyRouter is not directly reachable by clients and your proxy
...
See also [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) for the in-repo Cloudflare Tunnel walkthrough. The standalone `myrouterCloud/` worker lives in a separate companion repo.
...
| 20128 | MyRouter   | Localhost only (via nginx) |
