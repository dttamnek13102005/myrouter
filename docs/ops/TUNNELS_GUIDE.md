...
MyRouter can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — MyRouter starts/stops the underlying
...
- **Auto-install.** On first use, MyRouter downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  MyRouter. URLs are ephemeral and will change every restart.
...
| Variable                                             | Purpose                                                                               |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Override the binary path. If set and valid, MyRouter uses it instead of downloading. |
...
must be configured in the ngrok dashboard — MyRouter itself only forwards the
...
1. Install Tailscale (or let MyRouter do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via MyRouter's `login` endpoint).
...
MyRouter's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose MyRouter through a tunnel, the dashboard and OAuth flows must
...
and restart MyRouter before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; MyRouter will track each
independently.
...
MyRouter attempts to auto-install on first use. If the install is blocked
