---
title: "MyRouter Fly.io Deployment Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# MyRouter Fly.io Deployment Guide

This document describes the actual deployment process for MyRouter on Fly.io, covering two scenarios:
...
This guide is based on a verified working configuration for the current project. The application name is `myrouter`.
...
- Access URL: `https://myrouter.fly.dev/`
...
- `app = 'myrouter'` determines which Fly application the deployment targets
...
```toml
app = 'myrouter-yourname'
```

Note:

- Make sure the application you see in the console matches the `app` value in `fly.toml`
- If you previously used a different name, such as `oroute`, do not confuse it with `myrouter`
...
```powershell
flyctl apps create myrouter
```

If you changed the application name, replace `myrouter` with your chosen name.
...
These parameters have been used in actual deployments on the current `myrouter` application:
...
- `MYROUTER_WS_BRIDGE_SECRET` (required in production — used for WebSocket bridge authentication)
...
| `MYROUTER_WS_BRIDGE_SECRET`  | Required in production | WebSocket bridge authentication secret                |
...
| ---------------------- | --------------------------- |
| `DATA_DIR`             | `/data`                     |
| `NEXT_PUBLIC_BASE_URL` | `https://myrouter.fly.dev` |
...
1. **Set `NEXT_PUBLIC_BASE_URL` to your public HTTPS domain**
   ...
   ```powershell
   flyctl secrets set NEXT_PUBLIC_BASE_URL=https://myrouter.fly.dev -a myrouter
   ```

   If you are using a custom domain, replace it with the corresponding domain (e.g. `https://myrouter.yourdomain.com`).
...
2. **Configure the callback URL on the provider console**
   ...
   - `https://myrouter.fly.dev/callback`
...
- Intended for the current project `myrouter`
...
flyctl secrets set `
  ...
  MYROUTER_WS_BRIDGE_SECRET=$wsBridgeSecret `
  DATA_DIR=/data `
  NEXT_PUBLIC_BASE_URL=https://myrouter.fly.dev `
  -a myrouter
```
...
```bash
flyctl secrets set MYROUTER_WS_BRIDGE_SECRET=$(openssl rand -hex 32) -a myrouter
```

Notes:

- `MYROUTER_WS_BRIDGE_SECRET` is required in production; missing it will break the WebSocket bridge handshake

If you also want to set an initial password:

```powershell
flyctl secrets set INITIAL_PASSWORD=your-strong-password -a myrouter
```
...
```powershell
flyctl secrets set KEY=value -a myrouter
```
...
If the current repository is a fork and you want to sync updates from the upstream `https://github.com/diegosouzapw/MyRouter`, follow the workflow below.
...
```powershell
git remote add upstream https://github.com/diegosouzapw/MyRouter.git
```
...
5. `flyctl deploy`
6. `flyctl status -a myrouter`
7. `flyctl logs --no-tail -a myrouter`
...
```powershell
flyctl status -a myrouter
```

### 10.2 View Startup Logs

```powershell
flyctl logs --no-tail -a myrouter
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://myrouter.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- You are viewing a different application (e.g. `oroute` instead of `myrouter`)
...
1. `flyctl auth login`
2. `flyctl apps create myrouter`
3. `flyctl secrets set ... -a myrouter`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a myrouter`
