---
title: "🌐 MyRouter Proxy Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🌐 MyRouter Proxy Guide

> **Bypass geographic blocks, protect your identity, and route AI traffic through any proxy — with zero configuration complexity.**

MyRouter includes a full-featured proxy management system that lets you route upstream AI provider traffic through HTTP, HTTPS, or SOCKS5 proxies. Whether you're in a blocked region, need IP rotation, or want stealth fingerprinting — this guide covers everything.
...
MyRouter supports proxy configuration at **four independent scopes**, resolved in priority order:
...
When MyRouter sends a request to an upstream provider, it calls `resolveProxyForConnectionFromRegistry()` which checks each level in order:
...
Proxies are included in the **Backup/Restore** system. When you export your MyRouter configuration:
...
If you configured proxies in an older version (pre-registry), MyRouter automatically migrates them:
...
> 🆕 **Contributed by [@oyi77](https://github.com/oyi77)** — PR [#1847](https://github.com/diegosouzapw/MyRouter/pull/1847) (Issue [#1788](https://github.com/diegosouzapw/MyRouter/issues/1788))

MyRouter integrates with the **[1proxy](https://1proxy-api.aitradepulse.com)** community platform to provide access to **hundreds of free, validated proxies** from around the world. This is perfect for users who don't have their own proxy infrastructure.
...
1. **Sync** — MyRouter fetches validated proxies from the 1proxy API
...
MyRouter doesn't just route traffic through a proxy — it makes the traffic look legitimate:
...
For providers that use the CLIProxyAPI pattern, MyRouter supports three upstream proxy modes:

| Mode          | Description                                        |
| ------------- | -------------------------------------------------- |
| `native`      | MyRouter handles proxy routing directly (default) |
...
This is normal with cheap proxies that drop idle connections. MyRouter already handles this by:
...
Make sure the proxy is configured **before** starting the OAuth flow. MyRouter routes OAuth token exchange through the configured proxy. Set a global or provider-level proxy first, then connect.
...
MyRouter's **proxy fast-fail** mechanism (`src/lib/proxyHealth.ts`) detects dead proxies in <2s via a quick TCP connection check, then **caches the result** to avoid per-request overhead.
...
MyRouter tracks per-proxy usage to help operators diagnose routing patterns, latency spikes, and recurring failures.

### What's Tracked

For every request through a configured proxy, MyRouter records:
...
When multiple proxies are assigned to a scope, MyRouter uses a **rotation strategy** to pick which one to use for each request. The strategy is configured at the scope level (global, per-provider, per-account, per-combo).
