...
Complete reference for all MyRouter API endpoints.
...
| ------------------------ | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `X-MyRouter-No-Cache`   | Request   | Set to `true` to bypass cache                                                                                                                                                     |
| `x-myrouter-no-memory`  | Request   | Set to `true` to skip memory + skills injection for this request (mirrors no-cache; avoids the per-call token/cost overhead)                                                      |
| `X-MyRouter-Progress`   | Request   | Set to `true` for progress events                                                                                                                                                 |
| `X-Session-Id`           | Request   | Sticky session key for external session affinity                                                                                                                                  |
| `x_session_id`           | Request   | Underscore variant also accepted (direct HTTP)                                                                                                                                    |
| `X-MyRouter-Session-Id` | Request   | Caller-supplied session/conversation tag (also feeds memory). When present, persisted verbatim to `call_logs.session_tag` for per-session cost attribution (#8249) — never synthesized when absent |
| `Idempotency-Key`        | Request   | Dedup key (5s window)                                                                                                                                                             |
| `X-Request-Id`           | Request   | Alternative dedup key                                                                                                                                                             |
| `X-MyRouter-Cache`      | Response  | `HIT` or `MISS` (non-streaming)                                                                                                                                                   |
| `X-MyRouter-Idempotent` | Response  | `true` if deduplicated                                                                                                                                                            |
| `X-MyRouter-Progress`   | Response  | `enabled` if progress tracking on                                                                                                                                                 |
| `X-MyRouter-Session-Id` | Response  | Effective session ID used by MyRouter                                                                                                                                            |
| `X-MyRouter-Request-Id` | Response  | Request correlation id (when known)                                                                                                                                               |
| `X-MyRouter-Version`    | Response  | MyRouter build version (always present)                                                                                                                                          |
| `X-MyRouter-Cost-Saved` | Response  | USD the cache avoided on a HIT (cache hits only)                                                                                                                                  |
| `X-MyRouter-Decision`   | Response  | Routing trace: `strategy=<name>; provider=<alias>; latency_ms=<n>` (`<name>` is the combo strategy, or `single` for a non-combo request) — always present on completion responses |

> Nginx note: if you rely on underscore headers (for example `x_session_id`), enable `underscores_in_headers on;`.

> **Cost telemetry headers:** non-streaming success responses also carry the `X-MyRouter-*` cost-telemetry set — `X-MyRouter-Response-Cost` (USD, fixed 10 decimals; `0.0000000000` for free/unpriced), `X-MyRouter-Tokens-In` / `X-MyRouter-Tokens-Out`, `X-MyRouter-Model`, `X-MyRouter-Provider`, `X-MyRouter-Latency-Ms`, `X-MyRouter-Cache-Hit`, and `X-MyRouter-Fallback-Attempts` (only when > 0), plus `X-MyRouter-Request-Id` and `X-MyRouter-Version`. These are emitted by chat completions, `/v1/responses`, `/v1/messages`, **and the media endpoints** — `/v1/embeddings`, `/v1/images/generations`, `/v1/audio/speech`, `/v1/audio/transcriptions`, `/v1/rerank`, `/v1/videos/generations`, `/v1/music/generations`, and `/v1/moderations` (always cost `0`). Media cost is computed per modality (per-image, per-second, per-character, per search-unit) when pricing is available, otherwise `0` (fail-open).

> **Cache-hit cost semantics:** on a semantic-cache HIT (`X-MyRouter-Cache-Hit: true`) no upstream call is made, so `X-MyRouter-Response-Cost` is `0.0000000000` (the **incremental** cost of serving the hit). The original/would-have-been cost is reported separately in `X-MyRouter-Cost-Saved`. Billing consumers should sum `X-MyRouter-Response-Cost` (hits cost nothing); cache analytics can aggregate `X-MyRouter-Cost-Saved`.

### `x-myrouter-compression`
...
```
X-MyRouter-Compression: <mode>; source=<source>
```
...
- Remote media URLs must be public HTTPS. MyRouter fetches them server-side with redirect
...
For thinking-capable Claude models, `/v1/models` also advertises a **no-thinking** variant whose id is prefixed with `claude-3-myrouter-no-thinking/`:

```
claude-3-myrouter-no-thinking/<provider>/<model>
```
...
For clients that cannot attach `Authorization: Bearer ...`, MyRouter also accepts API keys in the URL via either query-string compatibility (`?token=...`, `?apiKey=...`, `?api_key=...`, `?key=...`) or the dedicated `/api/v1/vscode/{token}/...` endpoints documented below.
...
wscat -c "ws://localhost:20128/v1/responses?api_key=<MYROUTER_API_KEY>"
# (or: -H "Authorization: Bearer <MYROUTER_API_KEY>")
...
a ChatGPT account`). Send the **bare** id (e.g. `gpt-5.5`). MyRouter's bridge is
...
Point the Codex CLI at MyRouter by adding a custom provider with WebSocket
...
env_key = "MYROUTER_API_KEY"             # holds the MyRouter API key (Bearer)
```

```bash
export MYROUTER_API_KEY=sk-...           # an MyRouter API key (any key if REQUIRE_API_KEY=false)
codex exec "Responda apenas: PONG"
```

The CLI upgrades `base_url + /responses` to a WebSocket and MyRouter tunnels it
...
- URL-based tokens may appear in reverse-proxy logs, browser history, and telemetry outside MyRouter. Treat them as a compatibility option, not the default authentication mode.
...
Outbound webhook subscriptions for MyRouter events (request completion, quota exhaustion, key rotation, etc.).
...
Cloud agent tasks (Claude Code, Codex Cloud, OpenHands, etc.) executed remotely on behalf of MyRouter users.
...
MyRouter exposes three independent temporary-failure mechanisms; the management endpoints below let operators read and override them:
...
Skill framework for extending MyRouter with custom executable handlers, plus marketplace integrations.
...
MyRouter ships an embedded Model Context Protocol server with 3 transports (stdio, SSE, streamable-http) and scoped tools. The dashboard endpoints below read status/audit data and proxy the HTTP transports.
...
MyRouter exposes an A2A (Agent-to-Agent) JSON-RPC 2.0 endpoint plus a REST wrapper for inspection/dashboard use.

### JSON-RPC

```bash
POST /a2a
Authorization: Bearer your-api-key   # optional unless MYROUTER_API_KEY is set
Content-Type: application/json
...
**Auth:** the REST helpers run without management auth (dashboard-readable); the JSON-RPC `/a2a` route uses Bearer `MYROUTER_API_KEY` if configured.
...
Manage CLI tools that integrate with MyRouter (antigravity, chipotle, commandCode,
...
Manage MyRouter plugins (third-party extensions).
...
Inspect the runtime guardrails (PII detection, prompt injection detection, vision bridging). Guardrails run on every request; per-call opt-out is via the `x-myrouter-disabled-guardrails` request header — there is no persisted enable/disable surface.
