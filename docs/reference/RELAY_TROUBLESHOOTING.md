...
serverless backend so MyRouter can egress from a stable region while keeping
the provider API key server-side. This document covers the two failure modes
that operators hit in production and the recovery paths MyRouter ships for
each.
...
Check the relay awareness headers returned by MyRouter's auto-test probe
