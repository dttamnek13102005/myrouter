...
MyRouter advertises that URL to Bifrost and CLIProxyAPI via the
`X-MyRouter-Provider-Manifest-Url` request header. Set
`MYROUTER_PROVIDER_MANIFEST_URL` when the sidecar needs a public or container
...
eventually be owned by a lower-latency sidecar while MyRouter keeps the
...
2. Teach Bifrost or CLIProxyAPI to import the manifest for API-key/static
   providers.
3. Route eligible providers through the sidecar behind `MYROUTER_RELAY_BACKEND`
