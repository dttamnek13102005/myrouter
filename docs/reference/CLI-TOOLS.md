...
MyRouter integrates with three categories of CLI tools spread across three dedicated dashboard pages:

| Page           | Route                   | Concept                                                                   | Count        |
| -------------- | ----------------------- | ------------------------------------------------------------------------- | ------------ |
| **CLI Code's** | `/dashboard/cli-code`   | Coding tools you point at MyRouter (Client → CLI → MyRouter → Provider) | 21           |
| **CLI Agents** | `/dashboard/cli-agents` | Autonomous agents you point at MyRouter (same flow, broader scope)       | 6            |
| **ACP Agents** | `/dashboard/acp-agents` | CLIs that MyRouter spawns as backend via stdio/ACP (reverse flow)        | see registry |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
           │
           ▼  (all point to MyRouter)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (MyRouter routes to the right provider)
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP Agents (reverse spawn flow):
    Client request → MyRouter → spawns CLI via stdio/ACP → response
```
...
You do not have to write each tool's config by hand. MyRouter ships a `setup-*`
command per supported CLI that reads the **live** model catalog from a running
MyRouter (local or remote) and writes the tool's own config on your machine:
...
remote MyRouter), `--dry-run` (preview without writing), and `--port`. Tools
...
`myrouter launch` (Claude Code) and `myrouter launch-codex` (Codex) spawn the CLI
...
This page (renamed from `/dashboard/agents`) shows CLIs that MyRouter can **spawn** as backend execution engines via stdio/ACP protocol. The catalog is maintained separately in `src/lib/acp/registry.ts` and is **not** the same as `CLI_TOOLS`.
...
### Step 1 — Get an MyRouter API Key
...
> Use `opencode run "your prompt" --model myrouter/claude-sonnet-4-5-thinking --variant high`
...
Or use the MyRouter dashboard → **CLI Tools → Cline → Apply Config**.
...
Or use the MyRouter dashboard → **CLI Tools → KiloCode → Apply Config**.
...
Use this when VS Code Insiders is configured for custom endpoint models and you want MyRouter to work without a custom header field.
...
**Example using the tokenized MyRouter alias:**
...
- Replace `sk-your-myrouter-key` with an API key created in MyRouter.
...
# The CLI uses its own auth — MyRouter is not needed as backend for Kiro CLI itself.
# Use kiro-cli alongside MyRouter for other tools.
kiro-cli status
```

For the **Kiro IDE** desktop app, use the MITM endpoint exposed by MyRouter
under `/dashboard/cli-tools → Kiro`.

---

## 10. Internal MyRouter CLI

The `myrouter` binary provides commands for server lifecycle, setup, diagnostics, and provider management. Entry point: `bin/myrouter.mjs`.

```bash
myrouter                              # Start server (default port 20128)
myrouter setup                        # Interactive setup wizard
myrouter doctor                       # Check config, DB, ports, runtime
myrouter providers list               # Configured provider connections
myrouter providers test-all           # Test every active connection
myrouter reset-password               # Reset the admin password
myrouter logs                         # Stream request logs
myrouter health                       # Detailed health (breakers, cache, memory)
myrouter --version                    # Print version
myrouter --help                       # Show all commands
```
...
```bash
myrouter setup                        # Interactive setup wizard
myrouter setup --non-interactive      # CI/automation mode (reads env vars + flags)
myrouter setup --password '<value>'   # Set admin password directly
...
| Var                 | Purpose                                                        |
| ------------------- | -------------------------------------------------------------- |
| `MYROUTER_API_KEY` | Provider API key (bound to `--api-key` via Commander `.env()`) |
| `DATA_DIR`          | Override the MyRouter data directory                          |

All other non-interactive inputs are passed as flags, not environment variables:
`--password`, `--provider`, `--provider-name`, `--provider-base-url`, `--default-model`
(see the `myrouter setup` options above).

### Diagnostics

```bash
myrouter doctor                       # Check config, DB, ports, runtime, memory, liveness
myrouter doctor --json                # Machine-readable JSON
myrouter doctor --no-liveness         # Skip the HTTP health probe
myrouter doctor --host 0.0.0.0        # Override liveness host
myrouter doctor --liveness-url <url>  # Full health endpoint URL override
```
...
```bash
myrouter providers available                       # MyRouter provider catalog
myrouter providers available --search openai       # Filter catalog by id/name/alias/category
myrouter providers available --category api-key    # Filter by category (api-key, oauth, free, ...)
myrouter providers available --json                # Machine-readable JSON

myrouter providers list                            # Configured provider connections
myrouter providers list --json

myrouter providers test <id|name>                  # Test one configured connection
myrouter providers test-all                        # Test every active connection
myrouter providers validate                        # Local-only structural validation
```

> `providers available` reads the MyRouter catalog; `providers list/test/test-all/validate`
> read the local SQLite database directly and do not require the server to be running.

### Recovery & Reset

```bash
myrouter reset-password                # Reset the admin password (also: myrouter-reset-password)
myrouter reset-encrypted-columns       # Show warning + dry-run for encrypted credential reset
myrouter reset-encrypted-columns --force  # Actually null out encrypted credentials in SQLite
```

### Credential Export (⚠ handle with care)

```bash
myrouter auth export                                 # Show warning + confirmation gate — no DB access
myrouter auth export --force                          # Export ALL connections' DECRYPTED credentials to stdout as JSON
myrouter auth export --force --id <id>                 # Export only the matching connection
myrouter auth export --force --format env               # Emit MYROUTER_<PROVIDER>_<FIELD>=<value> lines
myrouter auth export --force --out creds.json           # Write to a file (created with 0600 permissions)
```
...
These assume a running MyRouter server, unless noted otherwise:

```bash
myrouter status                       # Comprehensive runtime status
myrouter logs                         # Stream request logs (--json, --search, --follow)
myrouter config show                  # Display current configuration

myrouter provider list                # List available providers (alias of providers list)
myrouter provider add                 # Register MyRouter as a provider on a tool
myrouter keys add | list | remove     # Manage API keys
myrouter models [provider]            # List models (--json, --search)
myrouter combo list | switch | create | delete

myrouter backup                       # Snapshot config + DB
myrouter restore                      # Restore from a previous snapshot

myrouter health                       # Detailed health (breakers, cache, memory)
myrouter quota                        # Provider quota usage
myrouter cache                        # Cache status
myrouter cache clear                  # Clear semantic + signature caches

myrouter mcp status | restart         # MCP server status / restart
myrouter a2a status | card            # A2A server status / agent card

myrouter tunnel list | create | stop  # Manage tunnels (cloudflare/tailscale/ngrok)
myrouter env show | get <k> | set <k> <v>  # Inspect / set env vars (temporary)

myrouter test                         # Provider connectivity smoke test
myrouter update                       # Check for updates
myrouter completion                   # Generate shell completion
```
...
Ready-to-paste examples with a tokenized MyRouter URL:
...
| Error                                        | Cause                   | Fix                                              |
| -------------------------------------------- | ----------------------- | ------------------------------------------------ |
| `Connection refused`                         | MyRouter not running   | `myrouter serve`                                |
