---
title: "MyRouter Codebase Documentation"
version: 3.8.40
lastUpdated: 2026-06-28
---

# MyRouter Codebase Documentation

> **Version:** v3.8.0
> **Last updated:** 2026-06-28
> **Audience:** Engineers contributing to MyRouter or building integrations on top of it.
>
...
- `@/*` → `src/*`
- `@myrouter/open-sse` → `open-sse/index.ts`
- `@myrouter/open-sse/*` → `open-sse/*`

Default HTTP port: **`20128`** (API and dashboard share the same process). Data
directory is `DATA_DIR` env var, defaulting to `~/.myrouter/`.
...
├── open-sse/             Streaming engine workspace (@myrouter/open-sse)
├── electron/             Desktop wrapper (Electron 41 main + preload)
├── bin/                  CLI entry points (myrouter, reset-password)
...
| `responses.ts`, `myrouterResponseMeta.ts` | Response shape helpers                            |
...
Separate npm workspace published as `@myrouter/open-sse`. Owns request
...
```
bin/
├── myrouter.mjs           Main CLI entry (Node ESM)
...
- `myrouter` → `bin/myrouter.mjs`
- `myrouter-reset-password` → `bin/reset-password.mjs`
