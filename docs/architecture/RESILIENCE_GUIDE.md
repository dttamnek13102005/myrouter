...
MyRouter has three distinct but related resilience mechanisms. Each has a different scope and purpose. Keep them separate when debugging routing behavior.
...
**Scope:** one client session (`X-Session-Id` / `x-codex-session-id` / `x-myrouter-session` header) pinned to one connection, for **any** provider.
...
- **Emergency fallback** — controlled by `MYROUTER_EMERGENCY_FALLBACK`; operators can override it from the Feature Flags page without a restart.
