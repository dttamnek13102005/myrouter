...
Validated by `isValidApiKey()` / `extractApiKey()` in `src/sse/services/auth.ts` and re-exported through `src/shared/utils/apiAuth.ts`. The validator also accepts the `MYROUTER_API_KEY` / `ROUTER_API_KEY` env vars as persistent passthrough keys (issue #1350).
...
Trusted internal headers (defined in `src/server/authz/headers.ts`) are **stripped from incoming requests** before classification — clients cannot pre-populate `x-myrouter-auth-*` to impersonate a subject.
...
request metadata, or `MYROUTER_MCP_SCOPES`.
