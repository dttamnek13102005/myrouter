---
title: "MyRouter Architecture"
version: 3.8.40
lastUpdated: 2026-06-28
---

# MyRouter Architecture
...
MyRouter is a local AI routing gateway and dashboard built on Next.js.
...
MyRouter can install, supervise, and route to locally-running AI tool processes
...
- MyRouter response metadata builder: `src/domain/myrouterResponseMeta.ts`
...
- file: `${DATA_DIR}/storage.sqlite` (or `$XDG_CONFIG_HOME/myrouter/storage.sqlite` when set, else `~/.myrouter/storage.sqlite`)
...
> MyRouter v3.8.0. For the canonical and continuously-updated list, refer to
...
- final client response returned by MyRouter; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/myrouter` -> `~/.myrouter`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@myrouter/open-sse` **npm workspace package**. Source code imports it via `@myrouter/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t myrouter .`
