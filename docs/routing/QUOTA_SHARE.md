...
**Problem it solves:** MyRouter proxies many API keys against the same upstream
...
- All persistence is in the existing MyRouter SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("myrouter:quota-share:pools")` is non-empty.
