# 🚀 MyRouter — Gateway AI Gratis (Bahasa Indonesia)
...
![NPM Downloads](https://img.shields.io/npm/dw/myrouter?label=npm%20down%20week&color=red)
![NPM Downloads](https://img.shields.io/npm/dm/myrouter?label=npm%20down%20month&color=red)

![NPM Downloads](https://img.shields.io/npm/d18m/myrouter?label=npm%20down%20year&color=red)
![Docker Pulls](https://img.shields.io/docker/pulls/diegosouzapw/myrouter)
![GitHub Downloads (all assets, all releases)](https://img.shields.io/github/downloads/diegosouzapw/myrouter/total?style=flat&label=eletron%20donwloads&color=blue)

[![stars](https://custom-icon-badges.demolab.com/github/stars/diegosouzapw/MyRouter?logo=star&style=flat)](https://github.com/diegosouzapw/MyRouter/stargazers)
[![open issues](https://custom-icon-badges.demolab.com/github/issues-raw/diegosouzapw/MyRouter?logo=issue)](https://github.com/diegosouzapw/MyRouter/issues)
[![license](https://custom-icon-badges.demolab.com/github/license/diegosouzapw/MyRouter?logo=law)](https://github.com/diegosouzapw/MyRouter/blob/main/LICENSE)
[![last commit](https://custom-icon-badges.demolab.com/github/last-commit/diegosouzapw/MyRouter?logo=history&logoColor=white)](https://github.com/diegosouzapw/MyRouter/commits/main)
...
[![code size](https://custom-icon-badges.demolab.com/github/languages/code-size/diegosouzapw/MyRouter?logo=file-code&logoColor=white)](https://github.com/diegosouzapw/MyRouter)
[![pr closed](https://custom-icon-badges.demolab.com/github/issues-pr-closed/diegosouzapw/MyRouter?color=purple&logo=git-pull-request&logoColor=white)](https://github.com/diegosouzapw/MyRouter/pulls?q=is%3Apr+is%3Aclosed)
[![tag](https://custom-icon-badges.demolab.com/github/v/tag/diegosouzapw/MyRouter?logo=tag&logoColor=white)](https://github.com/diegosouzapw/MyRouter/tags)
...
[![fork](https://custom-icon-badges.demolab.com/github/forks/diegosouzapw/MyRouter?logo=fork)](https://github.com/diegosouzapw/MyRouter/network/members)
[![watch](https://custom-icon-badges.demolab.com/github/watchers/diegosouzapw/MyRouter?logo=eye)](https://github.com/diegosouzapw/MyRouter/watchers)

[![License](https://img.shields.io/github/license/diegosouzapw/MyRouter)](https://github.com/diegosouzapw/MyRouter/blob/main/LICENSE)
...
[🌐 Website](https://myrouter.online) • [🚀 Mulai Cepat](#-quick-start) • [💡 Fitur](#-key-features) • [📖 Dokumentasi](#-documentation) • [💰 Harga](#-pricing-at-a-glance) • [💬 WhatsApp](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t)
...
<div align="center">
  <img src="./docs/screenshots/MainMyRouter.png" alt="MyRouter Dashboard" width="800"/>
</div>
...
_Hubungkan IDE atau alat CLI berbasis AI apa pun melalui MyRouter — gateway API gratis untuk coding tanpa batas._
...
<sub>📡 Semua agen terhubung melalui <code>http://localhost:20128/v1</code> atau <code>http://cloud.myrouter.online/v1</code> — satu konfigurasi, model dan kuota tak terbatas</sub>
...
- **Issues**: [github.com/diegosouzapw/MyRouter/issues](https://github.com/diegosouzapw/MyRouter/issues)
...
## 🎯 Apa yang Diselesaikan MyRouter — 30 Masalah Nyata & Kasus Penggunaan

> **Setiap developer yang menggunakan alat AI menghadapi masalah ini setiap hari.** MyRouter dibangun untuk menyelesaikannya semua — dari pembengkakan biaya hingga pemblokiran regional, dari alur OAuth yang rusak hingga operasi protokol dan observabilitas enterprise.
...
**Cara MyRouter menyelesaikannya:**
...
**Bagaimana MyRouter menyelesaikannya:**
...
- **Laporan Info Sistem** — `npm run system-info` menghasilkan `system-info.txt` dengan lingkungan lengkap Anda (versi Node, versi MyRouter, OS, alat CLI, status Docker/PM2). Lampirkan saat melaporkan masalah untuk triase instan.
...
- **instal global npm** — `npm install -g myrouter && myrouter` — selesai
...
- Mulai cepat bawaan untuk `myrouter --mcp` dan orientasi klien
...
```txt
1) Start MCP transport (`myrouter --mcp`) for tool-driven operations
...
>
> ```bash
> pnpm add -g myrouter@latest --allow-build=better-sqlite3 --allow-build=@swc/core
> myrouter
> ```
...
Pengguna Arch Linux dapat menginstal [AUR package](https://aur.archlinux.org/packages/myrouter-bin), yang menginstal MyRouter dan menyediakan layanan pengguna systemd:

```bash
yay -S myrouter-bin
systemctl --user enable --now myrouter.service
```

| Command                 | Description                                                     |
| ----------------------- | --------------------------------------------------------------- |
| `myrouter`             | Mulai server (`PORT=20128`, API dan dasbor pada port yang sama) |
| `myrouter --port 3000` | Set canonical/API port to 3000                                  |
| `myrouter --mcp`       | Mulai server MCP (stdio transport)                              |
| `myrouter --no-open`   | Don't auto-open browser                                         |
| `myrouter --help`      | Show help                                                       |
...
Saat Anda tidak lagi memerlukan MyRouter, kami menyediakan dua skrip cepat untuk penghapusan bersih:

| Command                  | Action                                                                                       |
| ------------------------ | -------------------------------------------------------------------------------------------- |
| `npm run uninstall`      | Menghapus aplikasi sistem tetapi **menyimpan DB dan konfigurasi Anda** di `~/.myrouter`.    |
| `npm run uninstall:full` | Menghapus aplikasi DAN secara permanen **menghapus semua konfigurasi, kunci, dan database**. |

> Catatan: Untuk menjalankan perintah ini, navigasikan ke folder proyek MyRouter (jika Anda mengkloningnya) dan jalankan. Alternatifnya, jika diinstal secara global, Anda cukup menjalankan `npm uninstall -g myrouter`.
...
| `STREAM_IDLE_TIMEOUT_MS` | inherits `REQUEST_TIMEOUT_MS` | Kesenjangan maksimum antara potongan streaming sebelum MyRouter membatalkan aliran SSE                                                                                      |
...
Untuk upstream yang kompatibel dengan Kode Claude (`anthropic-compatible-cc-*`), MyRouter juga memperoleh header `X-Stainless-Timeout` keluar dari batas waktu pengambilan yang diselesaikan sehingga batas waktu baca sisi penyedia tetap selaras dengan konfigurasi env Anda.

Untuk reverse proxy pihak ketiga yang kompatibel dengan Claude Code, MyRouter tetap menggunakan default
...
`cache_control`, MyRouter tidak memasukkan penanda milik jembatan.
...
Untuk permintaan streaming, `FETCH_TIMEOUT_MS` hanya mencakup pengaturan koneksi/menunggu respons upstream pertama. Setelah aliran aktif, MyRouter hanya akan dibatalkan pada keadaan terhenti sebenarnya (`STREAM_IDLE_TIMEOUT_MS`) atau tubuh Undici tidak aktif (`FETCH_BODY_TIMEOUT_MS`).

Jika Anda menjalankan MyRouter di belakang Nginx, Caddy, Cloudflare, atau proksi terbalik lainnya, pastikan proksi tersebut
waktu tunggu juga lebih tinggi daripada waktu tunggu aliran/pengambilan MyRouter Anda.
...
### 3) Arahkan alat pengkodean Anda ke MyRouter
...
Untuk pengguna Void Linux, Anda dapat membuat paket asli menggunakan `xbps-src`. Simpan blok ini sebagai `srcpkgs/myrouter/template`:
...
MyRouter tersedia sebagai image Docker publik di [Docker Hub](https://hub.docker.com/r/diegosouzapw/myrouter).
...
- Terowongan Cepat tidak dipulihkan secara otomatis setelah MyRouter atau kontainer dimulai ulang. Aktifkan kembali dari dasbor bila diperlukan.
...
- SQLite berjalan dalam mode WAL. `docker stop` harus dibiarkan selesai sehingga MyRouter dapat memeriksa kembali perubahan terbaru ke `storage.sqlite`.
...
- Setel `CLOUDFLARED_BIN=/absolute/path/to/cloudflared` jika Anda ingin MyRouter menggunakan biner yang sudah ada alih-alih mengunduhnya.

**Menggunakan Docker Compose dengan Caddy (HTTPS Auto-TLS):**

MyRouter dapat diekspos dengan aman menggunakan penyediaan SSL otomatis Caddy. Pastikan data DNS A domain Anda mengarah ke IP server Anda.

```yaml
services:
  myrouter:
    image: diegosouzapw/myrouter:latest
    container_name: myrouter
    restart: unless-stopped
    volumes:
      - myrouter-data:/app/data
    environment:
      - PORT=20128
  ...
  caddy:
    ...
    ports:
      - "80:80"
      - "443:443"
    command: caddy reverse-proxy --from https://your-domain.com --to http://myrouter:20128
...
| ------------------------ | -------- | ------ | --------------------- |
| `diegosouzapw/myrouter` | `latest` | ~250MB | Latest stable release |
| `diegosouzapw/myrouter` | `3.6.2`  | ~250MB | Current version       |

---

## 🖥️ Aplikasi Desktop — Offline & Selalu Aktif

> 🆕 **BARU!** MyRouter kini tersedia sebagai **aplikasi desktop asli** untuk Windows, macOS, dan Linux.

Jalankan MyRouter sebagai aplikasi desktop mandiri — tanpa terminal, tanpa browser, tanpa internet untuk model lokal. Aplikasi berbasis Electron meliputi:

- 🖥️ **Jendela Asli** — Jendela aplikasi khusus dengan integrasi baki sistem
- 🔄 **Mulai Otomatis** — Luncurkan MyRouter saat login sistem
...
Saat diminimalkan, MyRouter ada di baki sistem Anda dengan tindakan cepat:
...
**Tanpa biaya. Jangan pernah berhenti melakukan pengkodean.** Konfigurasikan ini sebagai satu kombo MyRouter dan semua fallback terjadi secara otomatis — tidak pernah ada peralihan manual.
...
> Semua model di bawah **100% gratis tanpa memerlukan kartu kredit**. MyRouter melakukan rute otomatis di antara keduanya ketika satu kuota habis — gabungkan semuanya untuk kombo $0 yang tidak dapat dipecahkan.
...
MyRouter v3.6 dibangun sebagai platform operasional, bukan hanya proxy relai.
...
Ketika kuota, tarif, atau kesehatan gagal, MyRouter secara otomatis berpindah ke kandidat berikutnya tanpa peralihan manual.
...
MyRouter menyertakan kerangka evaluasi bawaan untuk menguji kualitas respons LLM terhadap rangkaian emas. Akses melalui **Analytics → Evals** di dasbor.

### Golden Set Bawaan

"MyRouter Golden Set" yang dimuat sebelumnya berisi kasus uji untuk:
...
2. Jalankan `myrouter_get_health`.
3. Jalankan `myrouter_list_combos`.
...
**Kiat Pro:** Gunakan Opus untuk tugas kompleks, Soneta untuk kecepatan. MyRouter melacak kuota per model!
...
- Perilaku rotasi: MyRouter merutekan ke akun Codex berikutnya yang memenuhi syarat secara otomatis.
...
**Kiat Pro:** API yang kompatibel dengan OpenAI — bekerja secara lancar dengan terjemahan format MyRouter!
...
> **Catatan:** OpenClaw hanya berfungsi dengan MyRouter lokal. Gunakan `127.0.0.1` alih-alih `localhost` untuk menghindari masalah resolusi IPv6.
...
Settings → API Configuration:
  ...
  Base URL: http://localhost:20128/v1
  API Key: [from MyRouter dashboard]
  Model: if/kimi-k2-thinking
```

### OpenCode

**Langkah 1:** Tambahkan MyRouter sebagai penyedia khusus:

```bash
opencode
/connect
# Select "Other" → Enter ID: "myrouter" → Enter your MyRouter API key
```
...
```bash
/models
# Select any MyRouter model from the list
```

> **Tips:** Tambahkan model apa pun yang tersedia di titik akhir MyRouter `/v1/models` Anda ke bagian `models`. Gunakan format `provider/model-id` dari dasbor MyRouter Anda.
...
- Disegarkan secara otomatis oleh MyRouter
...
- MyRouter v1.0.6+ menyertakan validasi fallback melalui penyelesaian obrolan
...
> **⚠️ Penting bagi pengguna yang menjalankan MyRouter di VPS, Docker, atau server jarak jauh mana pun**

Kredensial OAuth yang disertakan dalam MyRouter didaftarkan **hanya untuk `localhost`**. Saat Anda mengakses MyRouter di server jarak jauh (misalnya `https://myrouter.myserver.com`), Google menolak autentikasi dengan:
...
- Nama: apa pun yang Anda suka (mis. `MyRouter Remote`)
...
1. MyRouter membuka URL otorisasi Google
...
4. Tempelkan URL tersebut ke bidang yang ditampilkan di modal koneksi MyRouter
5. Klik **"Hubungkan"**
...
- **Situs Web**: [myrouter.online](https://myrouter.online)
- **Paket**: [npmjs.com/package/myrouter](https://www.npmjs.com/package/myrouter)
- **Pekerja Pelabuhan**: [hub.docker.com/r/diegosouzapw/myrouter](https://hub.docker.com/r/diegosouzapw/myrouter)
...
MyRouter memiliki **218+ fitur yang direncanakan** di berbagai fase pengembangan. Berikut adalah bidang-bidang utamanya:
...
<div align="center">
  ...
  <br/>
  <sub><a href="https://myrouter.online">myrouter.online</a></sub>
</div>
