# Arsitektur MyRouter (Bahasa Indonesia)
...
MyRouter adalah gateway routing AI lokal dan dasbor yang dibangun di atas Next.js.
...
- File: `${DATA_DIR}/storage.sqlite` (atau `$XDG_CONFIG_HOME/myrouter/storage.sqlite` jika diatur, jika tidak `~/.myrouter/storage.sqlite`)
...
- respons klien akhir yang dikembalikan oleh MyRouter; tanggapan yang dialirkan disimpan dalam bentuk ringkasan ringkas yang sama
...
1. `usageDb` dan `localDb` berbagi kebijakan direktori dasar yang sama (`DATA_DIR` -> `XDG_CONFIG_HOME/myrouter` -> `~/.myrouter`) dengan migrasi file lama.
...
5. Direktori `open-sse/` diterbitkan sebagai `@myrouter/open-sse` **paket ruang kerja npm**. Kode sumber mengimpornya melalui `@myrouter/open-sse/...` (diselesaikan oleh Next.js `transpilePackages`). Jalur file dalam dokumen ini masih menggunakan nama direktori `open-sse/` untuk konsistensi.
...
- Bangun gambar Docker: `docker build -t myrouter .`
