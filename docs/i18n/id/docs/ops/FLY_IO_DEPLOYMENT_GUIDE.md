# Panduan Deployment MyRouter di Fly.io
...
Dokumen ini menjelaskan metode deployment MyRouter di Fly.io yang telah terbukti berhasil, mencakup dua skenario utama:
...
Dokumen ini disusun berdasarkan konfigurasi yang telah diverifikasi pada proyek saat ini, dengan nama aplikasi `myrouter`.
...
- Alamat akses: `https://myrouter.fly.dev/`
...
- `app = 'myrouter'` menentukan aplikasi Fly mana yang menjadi target deployment
...
- Jika sebelumnya Anda menggunakan nama lain seperti `oroute`, jangan sampai tertukar dengan `myrouter`
...
Jika Anda sudah mengganti nama aplikasi, ganti `myrouter` dengan nama Anda.
...
Parameter-parameter berikut telah digunakan secara nyata pada aplikasi `myrouter` saat ini:
...
- Berlaku untuk proyek saat ini yaitu `myrouter`
...
```powershell
flyctl secrets set INITIAL_PASSWORD=kata-sandi-kuat-anda -a myrouter
```
...
- Apakah aplikasi yang sedang dilihat adalah `myrouter`
...
Jika repositori saat ini adalah fork dan Anda ingin menyinkronkan pembaruan dari upstream `https://github.com/diegosouzapw/MyRouter`, ikuti alur berikut.
...
- Anda membuka aplikasi yang salah, misalnya `oroute`, bukan `myrouter`
