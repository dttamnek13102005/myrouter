# Dokumentasi Server MCP MyRouter (Bahasa Indonesia)
...
MyRouter MCP sudah tersedia secara bawaan. Jalankan dengan:
...
| Alat                            | Deskripsi                                                         |
| :------------------------------ | :---------------------------------------------------------------- |
| `myrouter_get_health`          | Kesehatan gateway, pemutus sirkuit, uptime                        |
| `myrouter_list_combos`         | Semua combo yang dikonfigurasi beserta modelnya                   |
| `myrouter_get_combo_metrics`   | Metrik performa untuk combo tertentu                              |
| `myrouter_switch_combo`        | Ganti combo aktif berdasarkan ID/nama                             |
| `myrouter_check_quota`         | Status kuota per penyedia atau semua penyedia                     |
| `myrouter_route_request`       | Kirim penyelesaian chat melalui MyRouter                         |
| `myrouter_cost_report`         | Analitik biaya untuk periode waktu tertentu                       |
| `myrouter_list_models_catalog` | Katalog model lengkap beserta kemampuannya                        |

## Alat Lanjutan (8)

| Alat                               | Deskripsi                                                              |
| :--------------------------------- | :--------------------------------------------------------------------- |
| `myrouter_simulate_route`         | Simulasi routing percobaan dengan pohon fallback                       |
| `myrouter_set_budget_guard`       | Anggaran sesi dengan tindakan degrade/block/alert                      |
| `myrouter_set_resilience_profile` | Terapkan preset conservative/balanced/aggressive                       |
| `myrouter_test_combo`             | Uji langsung semua model dalam combo melalui permintaan upstream nyata |
| `myrouter_get_provider_metrics`   | Metrik terperinci untuk satu penyedia                                  |
| `myrouter_best_combo_for_task`    | Rekomendasi kesesuaian tugas beserta alternatifnya                     |
| `myrouter_explain_route`          | Jelaskan keputusan routing yang lalu                                   |
| `myrouter_get_session_snapshot`   | Status sesi lengkap: biaya, token, kesalahan                           |
