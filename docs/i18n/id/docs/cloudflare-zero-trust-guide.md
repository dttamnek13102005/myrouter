...
Panduan ini mendokumentasikan standar infrastruktur jaringan terbaik untuk mengamankan **MyRouter** dan mengekspos aplikasi Anda ke internet secara aman, **tanpa membuka satu pun port (Zero Inbound)**.

## Apa yang Telah Dilakukan pada VM Anda?

Kami mengaktifkan MyRouter dalam mode **Split-Port** melalui PM2:
...
3. Klik **Add a Tunnel**, pilih **Cloudflared**, dan beri nama `MyRouter-VM`.
...
2. Pilih **Self-hosted**.
3. Di **Application name**, masukkan `Panel MyRouter`.
4. Di **Application domain**, masukkan `myrouter.domainanda.com` (sama dengan yang Anda buat di "Rute 2").
5. Klik **Next**.
...
3. Klik **Create rule**.
4. **Name:** `Anti-Penyalahgunaan MyRouter API`
...
2. MyRouter hanya berkomunikasi melalui HTTPS keluar (`cloudflared`) dan tidak menerima koneksi TCP langsung dari internet.
