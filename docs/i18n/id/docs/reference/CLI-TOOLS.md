# Panduan Pengaturan Alat CLI — MyRouter (Bahasa Indonesia)
...
untuk menggunakan **MyRouter** sebagai backend terpadu, memberikan manajemen kunci terpusat,
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Kiro / Cursor / Copilot
           │
           ▼  (semua mengarah ke MyRouter)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (MyRouter meneruskan ke penyedia yang tepat)
...
| **Antigravity**    | `antigravity` | internal   | mitm            | MyRouter        |
...
## Langkah 1 — Dapatkan API Key MyRouter

1. Buka dashboard MyRouter → **API Manager** (`/dashboard/api-manager`)
...
export ANTHROPIC_API_KEY="sk-your-myrouter-key"
...
Atau gunakan dashboard MyRouter → **CLI Tools → Cline → Apply Config**.
...
Atau gunakan dashboard MyRouter → **CLI Tools → KiloCode → Apply Config**.
...
# CLI ini menggunakan autentikasinya sendiri — MyRouter tidak diperlukan sebagai backend untuk Kiro CLI itu sendiri.
# Gunakan kiro-cli bersama MyRouter untuk alat lainnya.
kiro-cli status
```
...
> **Catatan:** Cursor merutekan permintaan melalui cloudnya sendiri. Untuk integrasi MyRouter,
> aktifkan **Cloud Endpoint** di Pengaturan MyRouter dan gunakan URL domain publik Anda.

Melalui GUI: **Settings → Models → OpenAI API Key**

- Base URL: `https://your-domain.com/v1`
- API Key: kunci MyRouter Anda

---

## Konfigurasi Otomatis Dashboard

Dashboard MyRouter mengotomatiskan konfigurasi untuk sebagian besar alat:
...
**Droid** dan **OpenClaw** adalah agen AI yang dibangun langsung ke dalam MyRouter — tidak perlu instalasi.
Keduanya berjalan sebagai rute internal dan menggunakan perutean model MyRouter secara otomatis.
...
| ------------------------- | --------------------------------- | ------------------------------------------ |
| `Connection refused`      | MyRouter tidak berjalan          | `pm2 start myrouter`                      |
...
```bash
# Instal semua CLI dan konfigurasi untuk MyRouter (ganti dengan kunci dan URL server Anda)
MYROUTER_URL="http://localhost:20128/v1"
MYROUTER_KEY="sk-your-myrouter-key"
...
cat > ~/.claude/settings.json   <<< "{\"apiBaseUrl\":\"$MYROUTER_URL\",\"apiKey\":\"$MYROUTER_KEY\"}"
cat > ~/.codex/config.yaml      <<< "model: auto\napiKey: $MYROUTER_KEY\napiBaseUrl: $MYROUTER_URL"
cat >> ~/.bashrc << EOF
export OPENAI_BASE_URL="$MYROUTER_URL"
export OPENAI_API_KEY="$MYROUTER_KEY"
export ANTHROPIC_BASE_URL="$MYROUTER_URL"
export ANTHROPIC_API_KEY="$MYROUTER_KEY"
EOF

source ~/.bashrc
echo "✅ All CLIs installed and configured for MyRouter"
```
