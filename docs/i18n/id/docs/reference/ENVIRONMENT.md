...
> Referensi lengkap untuk setiap variabel lingkungan yang dikenali oleh MyRouter.
...
MyRouter menggunakan **SQLite** (melalui `better-sqlite3`) untuk semua persistensi data. Variabel-variabel ini mengontrol lokasi data, enkripsi, dan siklus hidup data.

| Variable                         | Default              | Source File                                     | Deskripsi                                                                                                              |
| -------------------------------- | -------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `DATA_DIR`                       | `~/.myrouter/`      | `src/lib/db/core.ts`                            | Direktori utama untuk DB SQLite, cadangan, dan file data. Override untuk volume Docker atau path khusus.               |
...
| `MYROUTER_CRYPT_KEY`            | _(unset)_            | `src/lib/db/encryption.ts`                      | **Alias legacy** untuk `STORAGE_ENCRYPTION_KEY`. Diterima sebagai fallback ketika variabel utama tidak ada.            |
| `MYROUTER_API_KEY_BASE64`       | _(unset)_            | `src/lib/db/encryption.ts`                      | **Alias legacy** (bentuk yang dikodekan Base64) diterima sebagai fallback. Didekode secara otomatis sebelum digunakan. |

### Skenario

| Skenario                    | Konfigurasi                                                                                  |
| --------------------------- | -------------------------------------------------------------------------------------------- |
| **Pengembangan lokal**      | Biarkan semua nilai default. DB berada di `~/.myrouter/myrouter.db`.                       |
...
| **CI/Testing**              | `DATA_DIR=/tmp/myrouter-test` — bersifat sementara, tidak perlu enkripsi.                   |
...
| `MYROUTER_PORT`      | _(unset)_    | `src/lib/runtime/ports.ts` | Mengambil prioritas di atas `PORT` saat berjalan di dalam Electron atau wrapper lainnya. |
...
MyRouter menyediakan pertahanan dua lapis: pemindaian injeksi di sisi permintaan dan penghapusan PII di sisi respons.
...
> [!IMPORTANT]
> Saat melakukan deployment di balik reverse proxy (nginx, Caddy), `NEXT_PUBLIC_BASE_URL` **harus** diatur ke URL publik Anda (misalnya, `https://myrouter.example.com`). Tanpa ini, callback OAuth akan gagal karena redirect_uri tidak akan cocok.
...
Mengontrol bagaimana MyRouter menemukan dan menjalankan sidecar CLI (Claude Code, Codex, dll.).
...
| `CLI_ALLOW_CONFIG_WRITES` | `false`    | `src/shared/services/cliRuntime.ts` | Izinkan MyRouter menulis file konfigurasi CLI (penyegaran token, data sesi).    |
...
```bash
# Mount biner host ke kontainer dan beri tahu MyRouter lokasinya:
...
| Variable                                | Default          | Source File                                 | Deskripsi                                                                                                                         |
| --------------------------------------- | ---------------- | ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `MYROUTER_BASE_URL`                    | deteksi otomatis | `open-sse/mcp-server/server.ts`             | URL eksplisit agar alat MCP/A2A dapat menjangkau MyRouter. Menimpa deteksi otomatis localhost.                                   |
| `MYROUTER_API_KEY`                     | _(unset)_        | MCP/A2A modules                             | Kunci API untuk panggilan alat MCP internal dan skill A2A.                                                                        |
| `MYROUTER_API_KEY_ID`                  | _(unset)_        | `open-sse/mcp-server/audit.ts`              | ID kunci untuk atribusi log audit MCP.                                                                                            |
| `ROUTER_API_KEY`                        | _(unset)_        | Legacy                                      | Alias legacy untuk `MYROUTER_API_KEY`.                                                                                           |
| `MYROUTER_MCP_ENFORCE_SCOPES`          | `false`          | `open-sse/mcp-server/server.ts`             | Terapkan kontrol akses berbasis scope pada panggilan alat MCP.                                                                    |
| `MYROUTER_MCP_SCOPES`                  | _(all)_          | `open-sse/mcp-server/server.ts`             | Scope yang dipisahkan koma: `admin`, `combos`, `health`, `models`, `routing`, `budget`, `metrics`, `pricing`, `memory`, `skills`. |
...
| `MYROUTER_DISABLE_BACKGROUND_SERVICES` | `false`          | `src/instrumentation-node.ts`               | Nonaktifkan semua layanan latar belakang (sinkronisasi, harga, pembaruan model). Berguna untuk CI/pengujian.                      |
| `MYROUTER_BOOTSTRAPPED`                | `false`          | `src/app/(dashboard)/dashboard/page.tsx`    | Diatur ke `true` oleh skrip bootstrap setelah pengaturan awal. Mengontrol visibilitas wizard pengaturan.                          |
| `MYROUTER_ALLOW_BODY_PROJECT_OVERRIDE` | `0`              | `open-sse/executors/antigravity.ts`         | Escape hatch: izinkan body permintaan untuk menimpa field proyek Antigravity.                                                     |

### Jembatan CLI OAuth (Internal)

| Variable            | Default          | Source File                     | Deskripsi                                            |
| ------------------- | ---------------- | ------------------------------- | ---------------------------------------------------- |
| `MYROUTER_SERVER`  | deteksi otomatis | `src/lib/oauth/config/index.ts` | URL server untuk jembatan autentikasi CLI↔MyRouter. |
| `MYROUTER_TOKEN`   | _(unset)_        | `src/lib/oauth/config/index.ts` | Token autentikasi untuk jembatan CLI.                |
| `MYROUTER_USER_ID` | `cli`            | `src/lib/oauth/config/index.ts` | ID pengguna untuk sesi jembatan CLI.                 |
| `SERVER_URL`        | _(unset)_        | `src/lib/oauth/config/index.ts` | Alias legacy untuk `MYROUTER_SERVER`.               |
| `CLI_TOKEN`         | _(unset)_        | `src/lib/oauth/config/index.ts` | Alias legacy untuk `MYROUTER_TOKEN`.                |
| `CLI_USER_ID`       | _(unset)_        | `src/lib/oauth/config/index.ts` | Alias legacy untuk `MYROUTER_USER_ID`.              |
...
| `MYROUTER_QODER_WORKSPACE`       | Qoder                   | Alias untuk `QODER_CLI_WORKSPACE`.                                             |
...
Saat diaktifkan, MyRouter mengatur ulang urutan header HTTP dan field body JSON agar cocok dengan tanda tangan persis dari alat CLI resmi. Hal ini mengurangi risiko pemblokiran akun sambil mempertahankan IP proxy Anda.
...
| Variable                   | Default              | Deskripsi                                                                                                                 |
| -------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `MYROUTER_MEMORY_MB`      | `512`                | Batas heap V8 saat runtime. Docker standalone dan `myrouter serve` menggunakannya untuk mengatur `--max-old-space-size`. |
...
| `NEXT_PUBLIC_MYROUTER_E2E_MODE` | _(unset)_ | E2E test harness                          | Atur ke `true` untuk mengaktifkan mode pengujian E2E (autentikasi santai, test hook). |
...
| `INSTANCE_NAME=myrouter`                             | Ada di template docs/env lama tetapi tidak digunakan saat runtime. Mungkin kembali dalam fitur multi-instansi di masa depan. |
