...
MyRouter mendukung **30 bahasa** dengan terjemahan UI dashboard penuh, dokumentasi yang diterjemahkan, dan dukungan RTL untuk bahasa Arab dan Ibrani.
...
- **Pemrosesan bertahap**: Menggabungkan beberapa string dengan pemisah `__MYROUTER_I18N_SEPARATOR__` untuk meminimalkan panggilan API (maks 1800 karakter per permintaan)
...
**Penerjemah sekunder** — menggunakan API LLM yang kompatibel dengan OpenAI (termasuk MyRouter sendiri) untuk menerjemahkan file markdown `docs/i18n/` yang sudah ada. Paling baik digunakan untuk menyempurnakan atau menerjemahkan ulang dokumen dengan kualitas yang lebih baik dari Google Translate.
