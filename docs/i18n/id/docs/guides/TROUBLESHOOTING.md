...
Masalah umum dan solusinya untuk MyRouter.
...
| EACCES: permission denied                           | Atur `DATA_DIR=/path/to/writable/dir` untuk mengganti `~/.myrouter`                                                                                            |
...
| `dlopen` / `slice is not valid mach-o file` (macOS) | Jalankan `cd $(npm root -g)/myrouter/app && npm rebuild better-sqlite3 && myrouter` — lihat [Pembangunan ulang modul native macOS](#macos-native-module-rebuild) di bawah |
...
**Penyebab:** Anda menjalankan versi Node.js di luar batas runtime aman yang disetujui MyRouter. Kasus paling umum adalah menjalankan Node 20, 22, atau 24 versi patch lama yang berada di bawah batas keamanan yang diperlukan MyRouter.
...
3. Instal ulang MyRouter: `npm install -g myrouter`
4. Mulai ulang: `myrouter`
...
**Solusi (v3.5.5+):** MyRouter sekarang menggunakan fungsi `fetch()` milik undici sendiri ketika dispatcher proxy aktif, memastikan perilaku yang konsisten. Perbarui ke v3.5.5+.
...
MyRouter memperbarui token secara otomatis. Jika masalah berlanjut:
...
2. Pastikan `CLOUD_URL` mengarah ke endpoint cloud Anda (misalnya, `https://myrouter.dev`)
...
Ketika banyak permintaan bersamaan mengenai penyedia yang dibatasi lajunya, MyRouter menggunakan mutex + auto rate-limiting untuk membuat serialisasi permintaan dan mencegah kegagalan berantai. Ini berjalan otomatis untuk penyedia dengan API key.

---

## Taksonomi Kegagalan RAG / LLM Opsional (16 masalah)

Beberapa pengguna MyRouter menempatkan gateway di depan tumpukan RAG atau agen. Dalam pengaturan tersebut, umum terjadi pola yang aneh: MyRouter terlihat sehat (penyedia aktif, profil routing baik, tidak ada peringatan batas laju) tetapi jawaban akhir masih salah.
...
1. Saat Anda menyelidiki respons yang buruk, kumpulkan:
   ...
   - route atau combo penyedia di MyRouter
...
3. Simpan nomornya di dashboard, runbook, atau pelacak insiden Anda sendiri di samping log MyRouter.
...
Anda dapat mengabaikan bagian ini jika Anda tidak menjalankan pipeline RAG atau agen di belakang MyRouter.
