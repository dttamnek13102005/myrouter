# MyRouter — Panduan Mencopot Pemasangan (Bahasa Indonesia)
...
Panduan ini menjelaskan cara mencopot pemasangan MyRouter dari sistem Anda secara bersih.

---

## Mencopot Pemasangan dengan Cepat (v3.6.2+)

MyRouter menyediakan dua skrip bawaan untuk penghapusan yang bersih:
...
Perintah ini menghapus aplikasi MyRouter tetapi **mempertahankan** basis data, konfigurasi, kunci API, dan pengaturan penyedia Anda di `~/.myrouter/`. Gunakan ini jika Anda berencana memasang ulang nanti dan ingin menyimpan pengaturan yang ada.
...
- Berkas cadangan
- Berkas log
- Semua berkas di direktori `~/.myrouter/`
...
docker volume rm myrouter-data

# (Optional) Remove the image
docker rmi diegosouzapw/myrouter:latest
```
...
- Buka `Settings → Apps → MyRouter → Uninstall`
- Atau jalankan uninstaller NSIS dari direktori instalasi

**macOS:**

- Seret `MyRouter.app` dari `/Applications` ke Trash
- Hapus data: `rm -rf ~/Library/Application Support/myrouter`

**Linux:**

- Hapus berkas AppImage
- Hapus data: `rm -rf ~/.myrouter`

### Instalasi dari Sumber (git clone)

```bash
# Remove the cloned directory
rm -rf /path/to/myrouter
...
MyRouter menyimpan data di lokasi-lokasi berikut secara default:

| Platform      | Jalur Default                 | Pengganti                 |
| ------------- | ----------------------------- | ------------------------- |
| Linux         | `~/.myrouter/`               | `DATA_DIR` env var        |
| macOS         | `~/.myrouter/`               | `DATA_DIR` env var        |
| Windows       | `%APPDATA%/myrouter/`        | `DATA_DIR` env var        |
| Docker        | `/app/data/` (mounted volume) | `DATA_DIR` env var        |
| XDG-compliant | `$XDG_CONFIG_HOME/myrouter/` | `XDG_CONFIG_HOME` env var |
...
npm list -g myrouter 2>/dev/null

# Check for data directory
ls -la ~/.myrouter/ 2>/dev/null
