...
Panduan lengkap untuk mengonfigurasi penyedia, membuat combo, mengintegrasikan alat CLI, dan menerapkan MyRouter.
...
**Tips Pro:** Gunakan Opus untuk tugas kompleks, Sonnet untuk kecepatan. MyRouter melacak kuota per model!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [dari dasbor myrouter]
...
```json
{
  ...
  "anthropic_api_key": "your-myrouter-api-key"
}
```
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "myrouter/if/glm-4.7" }
    }
  },
  "models": {
    "providers": {
...
CLI secara otomatis memuat `.env` dari `~/.myrouter/.env` atau `./.env`.
...
| Perintah                 | Tindakan                                                                                           |
| ------------------------ | -------------------------------------------------------------------------------------------------- |
| `npm run uninstall`      | Menghapus aplikasi dari sistem tetapi **menyimpan DB dan konfigurasi** di `~/.myrouter`.          |
| `npm run uninstall:full` | Menghapus aplikasi DAN secara permanen **menghapus semua konfigurasi, kunci, dan basis data**. |

> Catatan: Untuk menjalankan perintah ini, navigasikan ke folder proyek MyRouter (jika Anda telah meng-clone-nya) dan jalankan. Atau, jika diinstal secara global, Anda cukup menjalankan `npm uninstall -g myrouter`.
...
Pengguna Void Linux dapat mengemas dan menginstal MyRouter secara native menggunakan framework kompilasi silang `xbps-src`. Ini mengotomasi build standalone Node.js beserta binding native `better-sqlite3` yang diperlukan.
...
| --------------------------------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `myrouter-default-secret-change-me` | Rahasia penandatanganan JWT (**ubah di produksi**)                                                                         |
| `INITIAL_PASSWORD`                      | `123456`                             | Kata sandi login pertama                                                                                                   |
| `DATA_DIR`                              | `~/.myrouter`                       | Direktori data (db, penggunaan, log)                                                                                       |
...
| `CLOUD_URL`                             | `https://myrouter.dev`              | Cloud sinkronisasi titik akhir berbasis URL                                                                                      |
...
| `MYROUTER_MEMORY_MB`                   | `512`                                | Batas heap Node.js dalam MB                                                                                                |
...
- Quick Tunnel tidak dipulihkan otomatis setelah MyRouter atau container di-restart; aktifkan kembali dari dasbor bila diperlukan
...
- **Cache Semantik** — Otomatis menyimpan respons non-streaming, temperature=0 (lewati dengan `X-MyRouter-No-Cache: true`)
- **Idempotensitas Permintaan** — Mendeduplikasi permintaan dalam 5 detik melalui header `Idempotency-Key` atau `X-Request-Id`
- **Pelacakan Progres** — Event SSE `event: progress` yang bisa diaktifkan melalui header `X-MyRouter-Progress: true`

---

### Translator Playground

Akses melalui **Dashboard → Translator**. Debug dan visualisasikan bagaimana MyRouter menerjemahkan permintaan API antar penyedia.
...
MyRouter juga menerima `x_session_id` dan mengembalikan kunci sesi efektif di `X-MyRouter-Session-Id`.
...
MyRouter mengimplementasikan ketahanan tingkat penyedia dengan lima komponen:
...
4. **Tunggu Cooldown** — Jika setiap kandidat koneksi sudah cooldown, MyRouter dapat menunggu cooldown paling awal dan mencoba kembali permintaan klien yang sama secara otomatis.
...
MyRouter mendukung transkripsi audio melalui titik akhir yang kompatibel dengan OpenAI:
...
MyRouter tersedia sebagai aplikasi desktop asli untuk Windows, macOS, dan Linux.
