...
**MyRouter** — єдиний AI проксі/маршрутизатор. Один кінцевий пункт, 160+ постачальників LLM, автоматичне резервування.
...
MyRouter має три пов'язані, але різні механізми тимчасових збоїв. Тримайте їх
...
- **Імпорти**: зовнішні → внутрішні (`@/`, `@myrouter/open-sse`) → відносні
...
- **Псевдоніми шляхів**: `@/*` → `src/`, `@myrouter/open-sse` → `open-sse/`, `@myrouter/open-sse/*` → `open-sse/*`
- **Порт за замовчуванням**: 20128 (API + панель управління на одному порту)
- **Директорія даних**: `DATA_DIR` змінна середовища, за замовчуванням `~/.myrouter/`
