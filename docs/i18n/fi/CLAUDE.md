...
**MyRouter** — yhtenäinen AI-proxy/reititin. Yksi päätepiste, yli 160 LLM-toimittajaa, automaattinen varajärjestelmä.
...
MyRouter:lla on kolme liittyvää mutta erilaista tilapäisen epäonnistumisen mekanismia. Pidä niiden
...
- **Tuonnit**: ulkoiset → sisäiset (`@/`, `@myrouter/open-sse`) → suhteelliset
...
- **Polkualias**: `@/*` → `src/`, `@myrouter/open-sse` → `open-sse/`, `@myrouter/open-sse/*` → `open-sse/*`
- **Oletusportti**: 20128 (API + dashboard samalla portilla)
- **Tietohakemisto**: `DATA_DIR` ympäristömuuttuja, oletuksena `~/.myrouter/`
