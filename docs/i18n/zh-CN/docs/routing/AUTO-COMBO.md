...
1. MyRouter 在 `src/sse/handlers/chat.ts` 中检测 `auto/` 前缀
2. 从数据库查询所有**活跃的服务商连接**
...
MyRouter 的 Combo 引擎支持 **17 种路由策略**（声明在 `src/shared/constants/routingStrategies.ts` → `ROUTING_STRATEGY_VALUES`）。Auto Combo 引擎本身以 `auto` 策略对外暴露；其余策略供持久化 Combo 使用。
...
| 命令                                    | 功能                                                                             |
| :-------------------------------------- | :------------------------------------------------------------------------------- |
| `npm run test:combo:live`               | 进程内真实路由，`RUN_COMBO_LIVE=1`；对活跃的 MyRouter 数据库做快照               |
| `npm run test:combo:live:vps`           | 针对活跃的 MyRouter 服务器进行 HTTP 调用（设置 `COMBO_LIVE_BASE_URL`）            |
