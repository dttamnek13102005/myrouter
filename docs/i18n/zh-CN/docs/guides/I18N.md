...
MyRouter 支持 **42 种语言**，提供完整的控制台 UI 翻译、文档翻译，以及阿拉伯语和希伯来语的 RTL 支持。
...
MyRouter 使用基于哈希的增量文档翻译器，后端为 OpenAI 兼容的 LLM 端点（通常通过 MyRouter Cloud 使用 `cx/gpt-5.4-mini`）：
...
| 变量                                | 用途                                         |
| ----------------------------------- | -------------------------------------------- |
| `MYROUTER_TRANSLATION_API_URL`     | OpenAI 兼容的 base URL，例如 `…/v1`          |
| `MYROUTER_TRANSLATION_API_KEY`     | bearer Token（日志中脱敏）                   |
| `MYROUTER_TRANSLATION_MODEL`       | 模型 ID，例如 `cx/gpt-5.4-mini`              |
| `MYROUTER_TRANSLATION_TIMEOUT_MS`  | 可选，默认 `60000`                           |
| `MYROUTER_TRANSLATION_CONCURRENCY` | 可选，默认 `4`                               |
...
- **分块批处理**：使用 `__MYROUTER_I18N_SEPARATOR__` 分隔符合并多个字符串，最小化 API 调用（每次请求最多 1800 字符）
...
**辅助翻译器** — 使用任意 OpenAI 兼容的 LLM API（包括 MyRouter 自身）翻译已有的 `docs/i18n/` Markdown 文件。适用于优化或重新翻译文档，质量优于 Google Translate。
...
`myrouter` CLI 拥有独立的 i18n 层，与 Next.js 控制台分离。
...
| 优先级 | 来源                     | 示例                                   |
| ------ | ------------------------ | -------------------------------------- |
| 1      | `--lang` 标志            | `myrouter --lang de status`           |
| 2      | `MYROUTER_LANG` 环境变量 | `MYROUTER_LANG=ja myrouter providers` |
...
保存的偏好以原子方式写入 `~/.myrouter/.env`，并在 CLI 引导阶段加载，先于任何命令执行。
