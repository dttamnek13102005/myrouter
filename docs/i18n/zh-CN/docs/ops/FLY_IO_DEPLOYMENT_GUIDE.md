---
title: "MyRouter Fly.io 部署指南"
version: 3.8.40
lastUpdated: 2026-06-28
---

# MyRouter Fly.io 部署指南

本文档记录 MyRouter 在 Fly.io 上的实际部署方法，适用于两类场景：

- 首次把当前项目部署到 Fly.io
- 后续代码更新后继续发布
- 新项目参考同样流程部署

本文基于当前项目已经验证通过的配置整理，应用名为 `myrouter`。
...
- 以前如果用过别的名字，例如 `oroute`，不要和 `myrouter` 混淆
...
如果你已经改成别的应用名，把 `myrouter` 替换成你的名字。
...
- `MYROUTER_WS_BRIDGE_SECRET` (生产环境必需 / required in production / obrigatório em produção — 用于 WebSocket 桥接鉴权 / used for WebSocket bridge authentication)
...
| `MYROUTER_WS_BRIDGE_SECRET` | 生产必需 (required / obrigatório) | WebSocket 桥接鉴权密钥 (WebSocket bridge auth / chave de autenticação da ponte WebSocket) |
...
1. **设置 `NEXT_PUBLIC_BASE_URL` 指向你公开的 HTTPS 域名 (set `NEXT_PUBLIC_BASE_URL` to the public HTTPS domain / defina `NEXT_PUBLIC_BASE_URL` para o domínio HTTPS público)**
   ...
   如果你使用了自定义域名 (if using a custom domain / se usar um domínio personalizado)，请替换为对应域名 (e.g. `https://myrouter.yourdomain.com`)。
...
- `MYROUTER_WS_BRIDGE_SECRET` 在生产环境必需，缺失会导致 WebSocket 桥接握手失败 (required in production; missing it breaks WebSocket bridge handshake / obrigatório em produção; sem ele o handshake da ponte WebSocket falha)

如果你还要加初始密码：

```powershell
flyctl secrets set INITIAL_PASSWORD=你的强密码 -a myrouter
```
...
如果当前仓库是 fork，并且你要同步上游 `https://github.com/diegosouzapw/MyRouter` 的更新，推荐按下面流程执行。
...
- 你打开的是另一个应用，例如 `oroute`，不是 `myrouter`
