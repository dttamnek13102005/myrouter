...
**MyRouter** — proxy/router ya AI iliyounganishwa. Kipengele kimoja, watoa huduma 160+, auto-fallback.
...
MyRouter ina mitambo mitatu inayohusiana lakini tofauti ya kushindwa kwa muda. Hifadhi upeo wao tofauti unapofanya ufuatiliaji wa tabia ya mwelekeo. Tazama
...
- **Maaliko**: nje → ndani (`@/`, `@myrouter/open-sse`) → ya uhusiano
...
- **Majina ya njia**: `@/*` → `src/`, `@myrouter/open-sse` → `open-sse/`, `@myrouter/open-sse/*` → `open-sse/*`
- **Bandari ya kawaida**: 20128 (API + dashibodi kwenye bandari moja)
- **Direktori ya data**: `DATA_DIR` env var, inarudiwa kwa `~/.myrouter/`
