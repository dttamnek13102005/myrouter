...
**MyRouter** — birləşdirilmiş AI proxy/router. Bir uç nöqtə, 160+ LLM təminatçısı, avtomatik geri dönmə.
...
MyRouter-un üç əlaqəli, lakin fərqli müvəqqəti uğursuzluq mexanizmi var. Marşrut davranışını düzəldərkən onların
...
- **İdxallar**: xarici → daxili (`@/`, `@myrouter/open-sse`) → nisbət
...
- **Yol aliasları**: `@/*` → `src/`, `@myrouter/open-sse` → `open-sse/`, `@myrouter/open-sse/*` → `open-sse/*`
- **Default port**: 20128 (API + dashboard eyni portda)
- **Məlumat qovluğu**: `DATA_DIR` env var, varsayılan `~/.myrouter/`
