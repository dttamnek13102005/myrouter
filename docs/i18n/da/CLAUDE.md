...
**MyRouter** — samlet AI proxy/router. Én endpoint, 160+ LLM udbydere, auto-fallback.
...
MyRouter har tre relaterede, men distinkte mekanismer til midlertidige fejl. Hold deres
...
- **Imports**: ekstern → intern (`@/`, `@myrouter/open-sse`) → relativ
...
- **Sti aliaser**: `@/*` → `src/`, `@myrouter/open-sse` → `open-sse/`, `@myrouter/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + dashboard på samme port)
- **Data katalog**: `DATA_DIR` miljøvariabel, standard til `~/.myrouter/`
