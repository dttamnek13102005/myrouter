...
**MyRouter** — jednotný AI proxy/router. Jeden koncový bod, 160+ poskytovatelů LLM, automatické zálohování.
...
MyRouter má tři související, ale odlišné mechanismy dočasného selhání. Udržujte jejich rozsah oddělený při ladění chování routování. Viz
...
- **Importy**: externí → interní (`@/`, `@myrouter/open-sse`) → relativní
...
- **Cestovní aliasy**: `@/*` → `src/`, `@myrouter/open-sse` → `open-sse/`, `@myrouter/open-sse/*` → `open-sse/*`
- **Výchozí port**: 20128 (API + dashboard na stejném portu)
- **Adresář dat**: `DATA_DIR` env var, výchozí hodnota `~/.myrouter/`
