...
Este guia documenta o padrão ouro de infraestrutura de rede para proteger o **MyRouter** e expor sua aplicação de forma segura para a internet, **sem abrir nenhuma porta (Zero Inbound)**.

## O que foi feito na sua VM?

Nós ativamos o MyRouter em modo **Split-Port** através do PM2:
...
3. Clique em **Add a Tunnel**, escolha **Cloudflared** e dê o nome \`MyRouter-VM\`.
...
3. Em **Application name**, coloque \`Painel MyRouter\`.
4. Em **Application domain**, coloque \`myrouter.seuglobal.com.br\` (O mesmo que você fez na "Rota 2").
5. Clique em **Next**.
...
4. **Name:** \`Anti-Abuso MyRouter API\`
...
2. O MyRouter só conversa HTTPS saindo (\`cloudflared\`) e não recebendo TCP direto do mundo.
