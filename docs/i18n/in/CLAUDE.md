...
**MyRouter** — proxy/router AI terpadu. Satu endpoint, 160+ penyedia LLM, auto-fallback.
...
MyRouter memiliki tiga mekanisme kegagalan sementara yang terkait tetapi berbeda. Jaga agar ruang lingkup mereka terpisah saat melakukan debugging perilaku pengalihan. Lihat diagram
...
- **Impor**: eksternal → internal (`@/`, `@myrouter/open-sse`) → relatif
...
- **Alias jalur**: `@/*` → `src/`, `@myrouter/open-sse` → `open-sse/`, `@myrouter/open-sse/*` → `open-sse/*`
- **Port default**: 20128 (API + dashboard di port yang sama)
- **Direktori data**: variabel lingkungan `DATA_DIR`, default ke `~/.myrouter/`
