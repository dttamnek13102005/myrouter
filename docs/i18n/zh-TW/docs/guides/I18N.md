...
MyRouter 支援 **43 種語言**，包含完整的儀表板 UI 翻譯、翻譯文件，以及阿拉伯語和希伯來語的 RTL（從右至左）支援。
...
MyRouter 使用基於雜湊的增量翻譯器來處理說明文件，後端採用與 OpenAI 相容的 LLM 端點（通常是透過 MyRouter Cloud 的 `cx/gpt-5.4-mini`）：
...
| 變數                               | 用途                                  |
| ---------------------------------- | ------------------------------------- |
| `MYROUTER_TRANSLATION_API_URL`    | 與 OpenAI 相容的基礎 URL，例如 `…/v1` |
| `MYROUTER_TRANSLATION_API_KEY`    | Bearer 令牌（不會記錄在日誌中）        |
| `MYROUTER_TRANSLATION_MODEL`      | 模型 ID，例如 `cx/gpt-5.4-mini`       |
| `MYROUTER_TRANSLATION_TIMEOUT_MS` | 選用，預設 `60000`                    |
| `MYROUTER_TRANSLATION_CONCURRENCY`| 選用，預設 `4`                        |
...
- **分塊批次處理**：使用 `__MYROUTER_I18N_SEPARATOR__` 分隔符號將多個字串連接起來，以減少 API 呼叫次數（每個請求最多 1800 字元）
...
**次要翻譯器** — 使用任何與 OpenAI 相容的 LLM API（包括 MyRouter 本身）來翻譯現有的 `docs/i18n/` Markdown 檔案。最適合用於潤飾或重新翻譯文件，品質優於 Google 翻譯。
...
`myrouter` CLI 有獨立於 Next.js 儀表板之外的 i18n 層。
...
| 優先順序 | 來源                     | 範例                                       |
| -------- | ------------------------ | ------------------------------------------ |
| 1        | `--lang` 旗標            | `myrouter --lang de status`               |
| 2        | `MYROUTER_LANG` 環境變數| `MYROUTER_LANG=ja myrouter providers`    |
...
```bash
# 設定語言並儲存至 ~/.myrouter/.env（跨工作階段持續有效）
...
儲存的偏好設定會以原子方式寫入 `~/.myrouter/.env`，並在 CLI 啟動時、任何指令執行前載入。
