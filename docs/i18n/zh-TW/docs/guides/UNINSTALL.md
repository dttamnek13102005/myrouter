...
本指南說明如何從系統中徹底移除 MyRouter。

---

## 快速解除安裝（v3.6.2+）

MyRouter 提供兩個內建指令碼來進行乾淨的移除：
...
此指令會移除 MyRouter 應用程式，但**保留**您的資料庫、設定檔、API 金鑰及提供者設定於 `~/.myrouter/`。若您日後打算重新安裝並保留既有設定，請使用此方式。
...
- 開啟 `設定 → 應用程式 → MyRouter → 解除安裝`
- 或從安裝目錄執行 NSIS 解除安裝程式

**macOS：**

- 將 `/Applications` 中的 `MyRouter.app` 拖入垃圾桶
- 移除資料：`rm -rf ~/Library/Application Support/myrouter`
...
MyRouter 預設將資料存放於以下位置：

| 平台         | 預設路徑                      | 覆蓋方式                   |
| ------------ | ----------------------------- | -------------------------- |
| Linux        | `~/.myrouter/`               | `DATA_DIR` 環境變數        |
| macOS        | `~/.myrouter/`               | `DATA_DIR` 環境變數        |
| Windows      | `%APPDATA%/myrouter/`        | `DATA_DIR` 環境變數        |
| Docker       | `/app/data/`（掛載資料卷）    | `DATA_DIR` 環境變數        |
| XDG 相容模式 | `$XDG_CONFIG_HOME/myrouter/` | `XDG_CONFIG_HOME` 環境變數 |
