# MyRouter MCP Server Documentation (Português (Portugal))
...
| Tool                            | Description                              |
| :------------------------------ | :--------------------------------------- |
| `myrouter_get_health`          | Gateway health, circuit breakers, uptime |
| `myrouter_list_combos`         | All configured combos with models        |
| `myrouter_get_combo_metrics`   | Performance metrics for a specific combo |
| `myrouter_switch_combo`        | Switch active combo by ID/name           |
| `myrouter_check_quota`         | Quota status per provider or all         |
| `myrouter_route_request`       | Send a chat completion through MyRouter |
| `myrouter_cost_report`         | Cost analytics for a time period         |
| `myrouter_list_models_catalog` | Full model catalog with capabilities     |

## Advanced Tools (8)

| Tool                               | Description                                                 |
| :--------------------------------- | :---------------------------------------------------------- |
| `myrouter_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `myrouter_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `myrouter_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `myrouter_test_combo`             | Live-test all models in a combo via a real upstream request |
| `myrouter_get_provider_metrics`   | Detailed metrics for one provider                           |
| `myrouter_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `myrouter_explain_route`          | Explain a past routing decision                             |
| `myrouter_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
