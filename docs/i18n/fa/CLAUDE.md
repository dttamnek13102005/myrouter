...
**MyRouter** — پروکسی/روتر AI یکپارچه. یک نقطه انتهایی، بیش از 160 ارائه‌دهنده LLM، بازگشت خودکار.
...
MyRouter سه مکانیزم موقت شکست مرتبط اما متمایز دارد. دامنه آن‌ها را هنگام اشکال‌زدایی رفتار مسیریابی جدا نگه دارید. برای یک نمای کلی، به
...
- **واردات**: خارجی → داخلی (`@/`, `@myrouter/open-sse`) → نسبی
...
- **آلیاس‌های مسیر**: `@/*` → `src/`، `@myrouter/open-sse` → `open-sse/`، `@myrouter/open-sse/*` → `open-sse/*`
- **پورت پیش‌فرض**: 20128 (API + داشبورد در همان پورت)
- **دایرکتوری داده**: متغیر محیطی `DATA_DIR`، به طور پیش‌فرض به `~/.myrouter/`
