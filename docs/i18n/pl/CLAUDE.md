...
**MyRouter** — unified AI proxy/router. One endpoint, 290 LLM providers, auto-fallback.
...
MyRouter has three related but distinct temporary-failure mechanisms. Keep their
...
- **Imports**: external → internal (`@/`, `@myrouter/open-sse`) → relative
...
repo** instead of polluting the main MyRouter tree.
...
- **Data directory**: `DATA_DIR` env var, defaults to `~/.myrouter/`
...
MyRouter has **~48 quality-gate scripts** (`scripts/check/` + `scripts/quality/`) wired
