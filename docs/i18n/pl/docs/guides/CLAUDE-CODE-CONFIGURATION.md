---
title: "Claude Code CLI — konfiguracja z MyRouter"
version: 3.8.40
lastUpdated: 2026-07-24
---

# Claude Code CLI — konfiguracja z MyRouter

Skieruj CLI **Claude Code** (`claude`) na MyRouter — lokalnie lub na zdalny VPS —
...
| `ANTHROPIC_AUTH_TOKEN`                       | Wysyłany jako `Authorization: Bearer …` — użyj tokenu dostępu / klucza API MyRouter       |
...
`myrouter launch` ustawia to wszystko za Ciebie: rozwiązuje bazowy URL + token
z aktywnego kontekstu (więc `myrouter connect <vps>`, a potem `myrouter launch`
...
picker `/model` zwykle pokazuje **tylko** modele Claude/Anthropic z MyRouter — `kimi/kimi-k2.6`
lub `glm/glm-5.2` się nie pojawią, mimo że routują poprawnie.

MyRouter może odzwierciedlić dowolny włączony model (i combo) pod identyfikatorem `claude/…`,
...
Gdy wybierzesz jeden z nich w Claude Code, MyRouter zdejmuje opakowanie `claude/`
...
wygrywa), więc zwykły MyRouter nie podwaja katalogu dla klientów, które nie używają
Claude Code:
...
`myrouter setup-claude` pobiera żywy katalog `/v1/models` i zapisuje jeden
...
> `myrouter launch --profile <name>` (wstrzykuje `ANTHROPIC_AUTH_TOKEN` z
> aktywnego kontekstu) albo sam wyeksportuj `ANTHROPIC_AUTH_TOKEN` i uruchom
> `CLAUDE_CONFIG_DIR=~/.claude/profiles/<name> claude`.

**Auto-sync po discovery modeli (opt-in).** MyRouter może regenerować te same
...
auto-sync” → Claude Code) albo ustaw `MYROUTER_AUTO_SYNC_CLAUDE_PROFILES=true` (honoruje też
...
Claude Code routuje do poziomów możliwości. Zmapuj każdy na model MyRouter przez env /
...
Po uruchomieniu `myrouter connect <host>` (zobacz
[Tryb zdalny](./REMOTE-MODE.md)), `myrouter launch` i `myrouter setup-claude`
...
`/v1`**, i zrestartuj `claude` (env jest odczytywane raz przy starcie). `myrouter launch`
robi to za Ciebie.
...
MyRouter odmawia zgadywania. Napraw na jeden z dwóch sposobów: przypnij prefiksowane id przez
...
`MYROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS=true` (domyślnie wyłączone;
...
**Błędy auth** — profil nie przechowuje tokenu. Użyj `myrouter launch --profile`
