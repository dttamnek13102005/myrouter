---
title: "🐳 Przewodnik Docker — MyRouter"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 Przewodnik Docker — MyRouter
...
MyRouter dostarcza cztery profile Compose. Wybierz ten, który pasuje do Twojego środowiska.

| Profil            | Usługa           | Kiedy używać                                                                                                                              | Polecenie                                    |
| ----------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `base` (domyślny) | `myrouter-base` | Serwer headless / minimalne runtime, bez dołączonych CLI providerów                                                                       | `docker compose --profile base up -d`        |
| `cli`             | `myrouter-cli`  | Przepływy agentowe wywołujące `myrouter providers/setup/doctor` oraz dołączone CLI (Codex, Claude Code, Droid, OpenClaw)                 | `docker compose --profile cli up -d`         |
| `host`            | `myrouter-host` | Hosty Linux z dostępem do CLI hosta w stylu `network_mode` przez montowanie `~/.local/bin`, `~/.codex`, `~/.claude` itd. tylko do odczytu | `docker compose --profile host up -d`        |
...
MyRouter korzysta z Redis jako zaplecza rozproszonego rate limitera i współdzielonej pamięci podręcznej. Usługa `redis` jest **zawsze zdefiniowana** w `docker-compose.yml` (bez bramki profilu) i startuje razem z każdym innym profilem.

| Szczegół             | Wartość                           |
| -------------------- | --------------------------------- |
| Image                | `redis:7-alpine`                  |
| Container name       | `myrouter-redis`                 |
| Internal port        | `6379`                            |
| Host port (override) | `REDIS_PORT` (defaults to `6379`) |
| Volume               | `myrouter-redis-data` → `/data`  |
...
| Image                  | `myrouter:prod` (built from `runner-cli` target)                                  |
| Redis container        | `myrouter-redis-prod` (`redis:8.6.2`, dedicated `redis-prod-data` volume)         |
| Data volume            | `myrouter-prod-data` (named, persisted across rebuilds)                           |
...
```bash
docker build --target runner-base -t myrouter:base .
docker build --target runner-cli  -t myrouter:cli  .
```

Domyślne wartości eksportowane przez `runner-base`: `PORT=20128`, `HOSTNAME=0.0.0.0`, `NODE_OPTIONS=--max-old-space-size=512`, `DATA_DIR=/app/data`, `MYROUTER_MIGRATIONS_DIR=/app/migrations`.

Zachowanie pamięci w Dockerze:

- `NODE_OPTIONS=--max-old-space-size=512` jest wbudowane w obraz jako fallback.
- Właściwy proces serwera uruchamia launcher standalone, który czyta `MYROUTER_MEMORY_MB` i dopisuje `--max-old-space-size=<MYROUTER_MEMORY_MB>`.
- Node używa ostatniej powtórzonej wartości `--max-old-space-size`, więc ustawienie `MYROUTER_MEMORY_MB` kontroluje efektywny limit heapa w Dockerze.
- Gdy `MYROUTER_MEMORY_MB` nie jest ustawione, launcher używa `512`.
...
| ----------------------------- | -------------------------------------------------------------------------------------------- | ------------------------ |
| `MYROUTER_WS_BRIDGE_SECRET`  | Współdzielony sekret mostu WebSocket. **Wymagany w produkcji** — ustaw silny, losowy ciąg.   | unset (must be provided) |
...
| `AUTO_UPDATE_HOST_REPO_DIR`   | Ścieżka hosta montowana w profilu `cli` pod `/workspace/myrouter` na potrzeby self-update   | `.` (current directory)  |
| `MYROUTER_MEMORY_MB`         | Sufit heapa Node w runtime dla serwera standalone Dockera; nadpisuje fallback obrazu powyżej | `512`                    |
| `DASHBOARD_PORT` / `API_PORT` | Nadpisanie eksponowanych portów dashboardu (20128) i API (20129)                             | `20128` / `20129`        |
| `MYROUTER_BASE_PATH`         | Podścieżka URL, gdy aplikacja jest publikowana za reverse proxy (np. `/myrouter`)           | _(empty = root)_         |
| `NEXT_PUBLIC_BASE_URL`        | Publiczny origin przeglądarki wraz z podścieżką (np. `https://host/myrouter`)               | unset                    |
...
Next.js `basePath` jest kompilowany do bundla standalone. MyRouter zapisuje wbudowaną
...
`MYROUTER_BASE_PATH` przy starcie kontenera. Gdy się różnią, a obraz był
...
NEXT_PUBLIC_BASE_URL=https://myhostname.example.com/myrouter
```
...
`docker-compose.yml` przekazuje `MYROUTER_BASE_PATH` jako Docker build-arg oraz jako
zmienną środowiskową runtime.

### Wstępnie zbudowany obraz root + podścieżka w runtime

Opublikowane obrazy `diegosouzapw/myrouter:*` są budowane pod root domeny. Nadal możesz
ustawić `MYROUTER_BASE_PATH` w runtime; kontener jednorazowo patchuje bundel przy starcie.
...
prefiksu). Traefik powinien routować `PathPrefix(`/myrouter`)` do kontenera bez
`StripPrefix`, żeby Next.js otrzymywał `/myrouter/...` i serwował assety z
`/myrouter/_next/...`.
...
MyRouter można bezpiecznie udostępnić dzięki automatycznemu provisionowaniu SSL w Caddy. Upewnij się, że rekord DNS A domeny wskazuje na IP Twojego serwera.
...
Caddy ustawia standardowe nagłówki forwardingu dla kontenera upstream. MyRouter używa
...
związanej z sesją. Włączaj `MYROUTER_TRUST_PROXY` tylko w zaawansowanych wdrożeniach, gdzie świadomie
chcesz, by MyRouter wyprowadzał publiczny origin z zaufanych nagłówków forwarded zamiast z jawnej
konfiguracji.
...
- Quick Tunnels nie są automatycznie przywracane po restarcie MyRouter ani kontenera. Włącz je ponownie z dashboardu, gdy potrzeba.
...
- Ustaw `CLOUDFLARED_BIN=/absolute/path/to/cloudflared`, jeśli chcesz, by MyRouter używał istniejącego binarium zamiast pobierać własne.

## Tagi obrazów

| Obraz                    | Tag      | Rozmiar | Opis                       |
| ------------------------ | -------- | ------- | -------------------------- |
| `diegosouzapw/myrouter` | `latest` | ~250MB  | Najnowsze stabilne wydanie |
| `diegosouzapw/myrouter` | `3.8.0`  | ~250MB  | Bieżąca wersja             |
...
- **Tryb SQLite WAL:** `docker stop` powinien móc się dokończyć, żeby MyRouter mógł zrobić checkpoint najnowszych zmian z powrotem do `storage.sqlite`. Dołączone pliki Compose ustawiają już 40s grace period stopu. Przy bezpośrednim uruchomieniu obrazu zachowaj `--stop-timeout 40`.
