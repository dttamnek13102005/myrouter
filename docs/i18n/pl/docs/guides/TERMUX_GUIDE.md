...
MyRouter może działać jako serwer headless na Androidzie przez Termux. Aplikacja desktopowa Electron nie jest obsługiwana w Termux, ale webowy dashboard oraz API zgodne z OpenAI działają z lokalnej przeglądarki lub z innych urządzeń w tej samej sieci.
...
> **Wersja Node.js:** MyRouter wymaga Node `>=22.22.2 <23 || >=24.0.0 <27` (zgodnie z `engines` w `package.json` / `SUPPORTED_NODE_RANGE`). Pakiet `nodejs-lts` w Termux zwykle dostarcza Node 20 LTS, który **nie jest już wspierany** — zamiast tego zainstaluj `pkg install nodejs` (current) i sprawdź, czy `node --version` zgłasza linię 22.x/24.x+.

Jeśli kompilacja natywnego pakietu się nie powiedzie, ponów powyższe polecenie `pkg install`, a następnie spróbuj ponownie zainstalować MyRouter.
...
Uruchom MyRouter w trybie serwera headless:
...
```bash
nohup myrouter > myrouter.log 2>&1 &
```
...
```bash
mkdir -p ~/.termux/boot
cat > ~/.termux/boot/myrouter.sh <<'EOF'
#!/data/data/com.termux/files/usr/bin/sh
cd "$HOME"
nohup myrouter > "$HOME/myrouter.log" 2>&1 &
EOF
chmod +x ~/.termux/boot/myrouter.sh
```
...
Trzymaj telefon i klienta w tej samej zaufanej sieci. Jeśli udostępniasz MyRouter poza telefonem, włącz klucze API oraz uwierzytelnianie dashboardu.

## Katalog danych

Domyślnie MyRouter przechowuje dane w katalogu domowym Termux, zgodnie z tą samą ścieżką danych po stronie serwera co na Linuxie. Aby umieścić bazę w konkretnej lokalizacji:

```bash
export DATA_DIR="$HOME/.myrouter"
myrouter
```
...
**Objaw:** `myrouter` / `myrouter serve` wypisuje `✔ MyRouter is running!`, ale każde żądanie do dashboardu lub API zwraca goły `500 Internal Server Error`. Plik `~/.myrouter/logs/application/app.log` pozostaje pusty, `APP_LOG_LEVEL=debug` nic sensownego nie wypisuje, a ciało odpowiedzi to zwykły tekst (`Internal Server Error`) bez szczegółów JSON.
...
Ponieważ hook się nie ładuje, logowanie nigdy się nie uruchamia — błąd 500 wygląda na całkowicie niemożliwy do zdiagnozowania. MyRouter tworzy `~/.cache` (i ustawia `XDG_CACHE_HOME`, gdy nie jest ustawione) w punkcie wejścia CLI przed startem Next.js, aby ta sonda zakończyła się powodzeniem na Androidzie/Termux.
...
W aktualnych buildach MyRouter CLI robi to automatycznie na Androidzie/Termux — świeża instalacja `npx -y myrouter@latest` / globalna nie powinna wymagać kroku ręcznego. Jeśli po aktualizacji nadal widzisz błąd, utwórz raz `~/.cache` jak wyżej i zrestartuj.
