...
| EACCES: permission denied                                  | Ustaw `DATA_DIR=/path/to/writable/dir`, by nadpisać `~/.myrouter`                                                                                                         |
...
| `dlopen` / `slice is not valid mach-o file` (macOS)        | Uruchom `cd $(npm root -g)/myrouter/app && npm rebuild better-sqlite3 && myrouter` — zobacz [przebudowa natywnego modułu na macOS](#macos-native-module-rebuild) poniżej |
...
linków wyglądających jak żądania HTTP. `README.md` MyRouter jest w paczce npm (jest
wymieniony w `package.json` → `files`), więc ląduje w `node_modules/myrouter/README.md` przy
...
`%LOCALAPPDATA%\Programs\MyRouter` (w tym katalogi paczek z sufiksem hasha z
...
1. **Najpierw zweryfikuj pobranie** (wyklucza spreparowany plik). Każde wydanie publikuje
   `latest.yml`, którego pole `sha512` (base64) obejmuje instalator `MyRouter.Setup.<version>.exe`.
   ...
   pobierz ponownie wyłącznie ze [strony wydań GitHub](https://github.com/diegosouzapw/MyRouter/releases).
2. **Przywróć + wyklucz** — przywróć cofnięte elementy z kwarantanny i dodaj wykluczenie
   dla `%LOCALAPPDATA%\Programs\MyRouter` (Kaspersky → Settings → Threats and Exclusions),
...
**Przyczyna:** Uruchamiasz wersję Node.js poza zatwierdzonym bezpiecznym progiem runtime MyRouter. Najczęstszy przypadek to starszy patch Node 22 lub 24 poniżej wymaganego przez MyRouter zabezpieczonego progu.
...
**Przyczyna:** Po globalnym `npm install -g myrouter` natywne binarium `better-sqlite3` w paczce mogło zostać skompilowane pod inną architekturę lub ABI Node.js niż lokalnie uruchomione. To częste na macOS (Apple Silicon i Intel), gdy prebuilt nie pasuje do środowiska.
...
**Naprawa (v3.5.5+):** MyRouter używa teraz własnego `fetch()` z undici, gdy aktywny jest dispatcher proxy, co daje spójne zachowanie. Zaktualizuj do v3.5.5+.

### Proxy MITM pod WSL: aplikacje desktop na hoście Windows nie są przechwytywane

**Przyczyna:** Proxy MITM i jego certyfikat CA instalują się w środowisku, w którym działa MyRouter. Pod WSL to gość Linux, a aplikacje AI desktop (Kiro, Trae, Copilot, Zed, …) działają na hoście Windows. Aplikacje hosta nie ufają magazynowi certyfikatów gościa i nie idą przez systemowe proxy gościa, więc przechwytywanie desktop tam nie działa.

**Rekomendacja:** Uruchamiaj MyRouter natywnie na tym samym OS co aplikacje desktop, które chcesz przechwytywać (Windows dla aplikacji Windows; analogicznie macOS/Linux). Trzymanie MyRouter w WSL przy celowaniu w aplikacje hosta wymaga ręcznego zaufania wygenerowanemu certyfikatowi CA na hoście Windows i wskazania ustawień sieci/proxy każdej aplikacji hosta na endpoint proxy WSL — to nieobsługiwana, krucha konfiguracja.
...
Gdy wiele równoległych żądań trafia w rate-limitowanego providera, MyRouter używa mutex + auto rate-limiting, by serializować żądania i zapobiec kaskadowym awariom. To automatyczne dla providerów z kluczem API.

---

## Opcjonalna taksonomia awarii RAG / LLM (16 problemów)

Część użytkowników MyRouter stawia gateway przed stackami RAG lub agentów. W takich setupach często widać dziwny wzorzec: MyRouter wygląda na zdrowy (providerzy w górze, profile routingu OK, brak alertów rate limit), a końcowa odpowiedź i tak jest zła.
...
1. Przy badaniu złej odpowiedzi zbierz:
   ...
   - route lub combo providera w MyRouter
...
Możesz pominąć tę sekcję, jeśli nie uruchamiasz pipeline’ów RAG ani agentów za MyRouter.
...
2. Zrestartuj MyRouter, by nowe wartości env weszły w życie
...
3. Zrestartuj MyRouter i przetestuj ponownie z **Dashboard → CLI Tools**
...
- Uruchom `myrouter providers` z CLI, by ponownie odpalić flow OAuth, albo
...
**Przyczyna:** Brak zmiennej env `MYROUTER_WS_BRIDGE_SECRET` w środowisku produkcyjnym.

**Naprawa:**

1. Wygeneruj losowy sekret: `openssl rand -hex 32`
2. Ustaw `MYROUTER_WS_BRIDGE_SECRET=<random-secret>` w env serwera produkcyjnego (i każdego klienta łączącego się z mostem)
