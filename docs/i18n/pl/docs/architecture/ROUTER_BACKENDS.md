...
> **Status:** Accepted · **Context:** [#5670](https://github.com/diegosouzapw/MyRouter/issues/5670),
> [#5603](https://github.com/diegosouzapw/MyRouter/issues/5603) · **Contract:** `domain/routing/routerBackends.ts`
> (typed registry — kod ląduje wraz z [#5868](https://github.com/diegosouzapw/MyRouter/pull/5868))
...
1. **Lifecycle** (`RouterBackendLifecycle`) — _jak silnik działa_:
   - `in-process` — działa wewnątrz procesu Node MyRouter (natywny pipeline TS).
   - `supervised` — lokalny proces potomny, który MyRouter instaluje/uruchamia/zatrzymuje/sprawdza
     health przez `ServiceSupervisor`, a następnie konsumuje jako połączenie providera.
   - `external` — endpoint HTTP, do którego MyRouter dysponuje żądania, ale którego **nie**
...
[#5868](https://github.com/diegosouzapw/MyRouter/pull/5868)) deklaruje każdy silnik raz, z jego
...
[#5817](https://github.com/diegosouzapw/MyRouter/pull/5817); do czasu merge
...
- **Selection** (`resolveRelayRoutingBackend`): jeden globalny przełącznik env —
  `MYROUTER_RELAY_BACKEND` / `RELAY_ROUTING_BACKEND` ∈ {`ts`, `bifrost`, `auto`}.
...
([#5869](https://github.com/diegosouzapw/MyRouter/pull/5869) manifest +
[#5870](https://github.com/diegosouzapw/MyRouter/pull/5870) `shouldTryBifrostForRequest`),
