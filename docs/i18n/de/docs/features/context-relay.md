...
- before the active account is exhausted, MyRouter generates a compact structured summary
- after authentication selects a different account for the same session, MyRouter injects
...
If the active provider is enabled in `handoffProviders`, MyRouter generates a structured
...
MyRouter prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, MyRouter converts that payload into a `<context_handoff>` system
