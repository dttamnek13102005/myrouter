...
**MyRouter** — פרוקסי/נתב AI מאוחד. נקודת קצה אחת, 160+ ספקי LLM, חזרה אוטומטית.
...
MyRouter יש שלושה מנגנוני כישלון זמניים הקשורים אך שונים. שמור על התחום שלהם נפרד כאשר אתה מדבג התנהגות נתיב. ראה את
...
- **ייבוא**: חיצוני → פנימי (`@/`, `@myrouter/open-sse`) → יחסי
...
- **Alias נתיב**: `@/*` → `src/`, `@myrouter/open-sse` → `open-sse/`, `@myrouter/open-sse/*` → `open-sse/*`
- **פורט ברירת מחדל**: 20128 (API + לוח מחוונים באותו פורט)
- **ספריית נתונים**: משתנה סביבה `DATA_DIR`, ברירת מחדל ל-`~/.myrouter/`
