---
title: "🐳 Docker Guide — MyRouter"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 Docker Guide — MyRouter
...
MyRouter ships four Compose profiles. Pick the one that matches your environment.

| Profile          | Service          | When to use                                                                                                                       | Command                                      |
| ---------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `base` (default) | `myrouter-base` | Headless server / minimal runtime, no provider CLIs bundled                                                                       | `docker compose --profile base up -d`        |
| `cli`            | `myrouter-cli`  | Agentic workflows that call `myrouter providers/setup/doctor` and bundled CLIs (Codex, Claude Code, Droid, OpenClaw)             | `docker compose --profile cli up -d`         |
| `host`           | `myrouter-host` | Linux hosts that want `network_mode`-like access to host CLIs by mounting `~/.local/bin`, `~/.codex`, `~/.claude`, etc. read-only | `docker compose --profile host up -d`        |
...
MyRouter relies on Redis to back the distributed rate limiter and shared cache. The `redis` service is **always defined** in `docker-compose.yml` (it has no profile gate) and starts alongside any other profile.
...
Defaults exported by `runner-base`: `PORT=20128`, `HOSTNAME=0.0.0.0`, `NODE_OPTIONS=--max-old-space-size=512`, `DATA_DIR=/app/data`, `MYROUTER_MIGRATIONS_DIR=/app/migrations`.

Memory behavior in Docker:

- `NODE_OPTIONS=--max-old-space-size=512` is baked into the image as a fallback.
- The actual server process is started by the standalone launcher, which reads `MYROUTER_MEMORY_MB` and appends `--max-old-space-size=<MYROUTER_MEMORY_MB>`.
- Node uses the last repeated `--max-old-space-size` value, so setting `MYROUTER_MEMORY_MB` controls the effective Docker heap limit.
- If `MYROUTER_MEMORY_MB` is unset, the launcher uses `512`.
...
| ----------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------ |
| `MYROUTER_WS_BRIDGE_SECRET`  | Shared secret for the WebSocket bridge. **Required in production** — set to a strong random string. | unset (must be provided) |
...
| `AUTO_UPDATE_HOST_REPO_DIR`   | Host path mounted into `cli` profile at `/workspace/myrouter` for self-update workflows            | `.` (current directory)  |
| `MYROUTER_MEMORY_MB`         | Runtime Node heap ceiling for the Docker standalone server; overrides the image fallback above      | `512`                    |
| `DASHBOARD_PORT` / `API_PORT` | Override exposed ports for dashboard (20128) and API (20129)                                        | `20128` / `20129`        |
| `MYROUTER_BASE_PATH`         | URL subpath when the app is published behind a reverse proxy (e.g. `/myrouter`)                    | _(empty = root)_         |
| `NEXT_PUBLIC_BASE_URL`        | Public browser origin including the subpath (e.g. `https://host/myrouter`)                         | unset                    |
...
Next.js `basePath` is compiled into the standalone bundle. MyRouter records the baked
...
`MYROUTER_BASE_PATH` when the container starts. When they differ and the image was
...
`docker-compose.yml` forwards `MYROUTER_BASE_PATH` as a Docker build-arg and as a
runtime environment variable.

### Pre-built root image + runtime subpath

Published `diegosouzapw/myrouter:*` images are built for the domain root. You can still
set `MYROUTER_BASE_PATH` at runtime; the container patches the bundle once on startup.
...
prefix). Traefik should route `PathPrefix(`/myrouter`)` to the container without
`StripPrefix`, so Next.js receives `/myrouter/...` and serves assets from
`/myrouter/_next/...`.
...
Caddy sets the standard forwarding headers for the upstream container. MyRouter uses
...
protection. Only enable `MYROUTER_TRUST_PROXY` for advanced deployments where you intentionally
want MyRouter to derive the public origin from trusted forwarded headers instead of explicit
configuration.
...
| `diegosouzapw/myrouter` | `3.8.0`  | ~250MB | Current version       |
...
- **SQLite WAL Mode:** `docker stop` should be allowed to finish so MyRouter can checkpoint the latest changes back into `storage.sqlite`. The bundled Compose files already set a 40s stop grace period. If you run the image directly, keep `--stop-timeout 40`.
