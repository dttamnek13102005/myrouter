---
title: "📖 Setup Guide — MyRouter"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 📖 Setup Guide — MyRouter

> Complete setup reference for MyRouter. For the quick version, see the [Quick Start in README](../README.md#-quick-start).
...
The [AUR package](https://aur.archlinux.org/packages/myrouter-bin) installs MyRouter and provides a systemd user service.
...
MyRouter ships a desktop wrapper built on Electron 41 + electron-builder 26.10. Available scripts (workspace root):
...
Combined with env vars (`INITIAL_PASSWORD`, `MYROUTER_WS_BRIDGE_SECRET`, etc.), this lets you spin up an MyRouter instance fully scriptable.
...
| `myrouter setup`       | Guided CLI onboarding for password and first provider          |
| `myrouter doctor`      | Run local health checks without starting the server            |
| `myrouter providers`   | Discover, list, validate, and test providers from CLI          |
| `myrouter config`      | CLI tool configuration — list, get, set, validate configs      |
| `myrouter status`      | Offline status dashboard — version, DB, tools, config          |
| `myrouter logs`        | Stream usage logs from the API (supports `--follow`)           |
| `myrouter update`      | Check for or apply MyRouter updates                           |
| `myrouter provider`    | Manage provider connections — add, list, remove, test, default |
...
Instead of pasting the base URL and key by hand, let MyRouter write each tool's
...
**remote** MyRouter, plus `--dry-run` to preview. The launchers
...
For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), MyRouter derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, MyRouter keeps the default `anthropic-beta` set conservative and, when `Client Cache Control` is left on `Auto`, only forwards client-provided `cache_control` markers. Enable the per-connection "Enable redact-thinking beta" toggle only when the upstream specifically requires redacted Claude thinking streams.
...
> **Note:** For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, MyRouter will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

### Reverse Proxy Compatibility

If you run MyRouter behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy timeouts are also higher than your MyRouter stream/fetch timeouts.
