...
Common problems and solutions for MyRouter.

---

## Quick Reference

**New to MyRouter?** Start here — these solve 90% of problems:

| I see this              | What it means                       | What to do                                                                                        |
| ----------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------- |
| "Can't connect"         | MyRouter isn't running             | Run `myrouter` or `docker restart myrouter`                                                     |
...
| EACCES: permission denied                           | Set `DATA_DIR=/path/to/writable/dir` to override `~/.myrouter`                                                                                          |
...
| `dlopen` / `slice is not valid mach-o file` (macOS) | Run `cd $(npm root -g)/myrouter/app && npm rebuild better-sqlite3 && myrouter` — see [macOS native module rebuild](#macos-native-module-rebuild) below |
...
HTTP-request-looking links. MyRouter's `README.md` ships inside the npm package (it is
listed in `package.json` → `files`), so it lands at `node_modules/myrouter/README.md` on
...
`%LOCALAPPDATA%\Programs\MyRouter` (including hash-suffixed package directories from the
...
1. **Verify your download first** (rules out a tampered file). Every release publishes
   `latest.yml`, whose `sha512` field (base64) covers the `MyRouter.Setup.<version>.exe`
   installer. In PowerShell, from the folder containing the installer:
   ```powershell
   $b = [System.Security.Cryptography.SHA512]::Create().ComputeHash(
     [System.IO.File]::ReadAllBytes("$PWD\MyRouter.Setup.<version>.exe"))
   [Convert]::ToBase64String($b)
   ```
   The output must match `latest.yml` → `sha512`. If it does not, delete the file and
   re-download only from the [GitHub releases page](https://github.com/diegosouzapw/MyRouter/releases).
2. **Restore + exclude** — restore the rolled-back items from quarantine and add an exclusion
   for `%LOCALAPPDATA%\Programs\MyRouter` (Kaspersky → Settings → Threats and Exclusions),
   then reinstall.
...
**Cause:** You are running a Node.js version outside MyRouter's approved secure runtime floor. The most common case is running an older Node 22 or 24 patch level that falls below the patched security floor MyRouter requires.
...
3. Reinstall MyRouter: `npm install -g myrouter`
4. Restart: `myrouter`
...
**Fix (v3.5.5+):** MyRouter now uses undici's own `fetch()` function when a proxy dispatcher is active, ensuring consistent behavior. Update to v3.5.5+.

### MITM proxy under WSL: desktop apps on the Windows host are not intercepted

**Cause:** The MITM proxy and its CA certificate install into the environment where MyRouter runs. Under WSL that environment is the Linux guest, while the AI desktop apps (Kiro, Trae, Copilot, Zed, …) run on the Windows host. The host apps do not trust the guest's certificate store and do not route through the guest's system proxy, so desktop interception does not engage there.

**Recommendation:** Run MyRouter natively on the same OS as the desktop apps you want to intercept (Windows for Windows apps; macOS/Linux likewise). Keeping MyRouter inside WSL while targeting host apps requires manually trusting the generated CA certificate on the Windows host and pointing each host app's network/proxy settings at the WSL proxy endpoint — an unsupported, fragile setup.
...
MyRouter auto-refreshes tokens. If issues persist:
...
2. Verify `CLOUD_URL` points to your cloud endpoint (e.g., `https://myrouter.dev`)
...
When many concurrent requests hit a rate-limited provider, MyRouter uses mutex + auto rate-limiting to serialize requests and prevent cascading failures. This is automatic for API key providers.

---

## Optional RAG / LLM failure taxonomy (16 problems)

Some MyRouter users place the gateway in front of RAG or agent stacks. In those setups it is common to see a strange pattern: MyRouter looks healthy (providers up, routing profiles ok, no rate limit alerts) but the final answer is still wrong.
...
1. When you investigate a bad response, capture:
   ...
   - route or provider combo in MyRouter
   - any RAG context used downstream (retrieved documents, tool calls, etc)
2. Map the incident to one or two WFGY ProblemMap numbers (`No.1` … `No.16`).
3. Store the number in your own dashboard, runbook, or incident tracker next to the MyRouter logs.
...
You can ignore this section if you do not run RAG or agent pipelines behind MyRouter.
...
2. Restart MyRouter so the new env values are picked up
...
3. Restart MyRouter and re-test from **Dashboard → CLI Tools**
...
- Run `myrouter providers` from the CLI to re-trigger the OAuth flow, or
...
### MYROUTER_WS_BRIDGE_SECRET missing in production
...
**Cause:** The `MYROUTER_WS_BRIDGE_SECRET` env var is missing from the production environment.

**Fix:**

1. Generate a random secret: `openssl rand -hex 32`
2. Set `MYROUTER_WS_BRIDGE_SECRET=<random-secret>` in the production server env (and any client that talks to the bridge)
3. Restart MyRouter
