...
MyRouter supports **43 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
MyRouter uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through MyRouter
Cloud):
...
| Variable                            | Purpose                                 |
| ----------------------------------- | --------------------------------------- |
| `MYROUTER_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `MYROUTER_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `MYROUTER_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
...
- **Chunked batching**: Joins multiple strings with `__MYROUTER_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including MyRouter itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
...
The `myrouter` CLI has its own i18n layer separate from the Next.js dashboard.
...
| -------- | ------------------------ | --------------------------------------- |
| 1        | `--lang` flag            | `myrouter --lang de status`            |
| 2        | `MYROUTER_LANG` env var | `MYROUTER_LANG=ja myrouter providers` |
...
The saved preference is written atomically to `~/.myrouter/.env` and is loaded by the
...
(`MyRouter`, `OAuth`, `MCP`, `A2A`, `DATA_DIR`, …) that must appear verbatim
