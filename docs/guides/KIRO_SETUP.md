...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to MyRouter,
...
## How MyRouter Solves This (v3.8.0+)

Starting with v3.8.0, MyRouter calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each MyRouter connection its own dedicated OIDC
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; MyRouter reads credentials from the
...
4. MyRouter validates the key and saves the connection.
...
(`clientSecretExpiresAt`). MyRouter stores this timestamp in `providerSpecificData`
...
- Ensure MyRouter can reach `https://oidc.us-east-1.amazonaws.com` (or the configured
...
- The key must be able to call `ListAvailableProfiles`; otherwise MyRouter cannot
