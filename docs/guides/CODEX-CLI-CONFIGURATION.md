---
title: "Codex CLI — Configuration with MyRouter"
version: 3.8.49
lastUpdated: 2026-07-26
---

# Codex CLI — Configuration with MyRouter

Complete guide for using the Codex CLI pointed at MyRouter as an OpenAI-compatible backend.
...
**MyRouter solves this transparently:**

```
Codex CLI
  → wire_api = "responses"
  → POST /v1/responses (MyRouter)
    → MyRouter Responses ↔ Chat Completions transformer
    → POST /chat/completions (DeepSeek / Mistral / GLM / Kimi / any provider)
```

You never need a separate translation proxy when using MyRouter. **All models use `wire_api = "responses"`** — MyRouter handles the rest.
...
| Model                                | MyRouter ID                         | Context window         | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ---------------------- | -------------- | ------------------- |
...
All Codex models in MyRouter use the `cx/` prefix:

| Codex CLI name          | MyRouter model    |
| ----------------------- | ------------------ |
...
Other providers use their own prefix (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — the prefix matches the MyRouter provider alias.
...
## Generating profiles automatically with `myrouter setup-codex`

If you run MyRouter on a VPS, you can auto-generate profile files from the live model catalog:

```bash
# From a VPS (uses local MyRouter on port 20128)
myrouter setup-codex

# From any machine — point at your VPS
myrouter setup-codex --remote http://100.x.x.x:20128 --api-key sk-xxx

# Preview without writing files
myrouter setup-codex --remote http://100.x.x.x:20128 --dry-run

# Only generate GLM and Kimi profiles
myrouter setup-codex --only glm,kimi

# Write to a custom directory
myrouter setup-codex --codex-home /path/to/.codex
```
...
MyRouter can also **auto-sync** these same profile files after a successful provider model discovery/import changes the live catalog. This is **opt-in and off by default**: toggle it from the **CLI Code dashboard** ("CLI profile auto-sync" → Codex), or set `MYROUTER_AUTO_SYNC_CODEX_PROFILES=true` (it also honors `CLI_ALLOW_CONFIG_WRITES`, on by default). When enabled it only writes separate `~/.codex/*.config.toml` profile files; it never changes the active/default `~/.codex/config.toml`, Codex-lb settings, auth, or provider selection.

---

## Launching Codex with `myrouter launch-codex`

Health-checks your MyRouter instance before launching Codex:

```bash
# Launch against local MyRouter (default port 20128)
myrouter launch-codex

# Launch with a specific profile
myrouter launch-codex --profile kimi-k27

# Launch against a remote VPS
myrouter launch-codex --remote http://100.x.x.x:20128/v1 --api-key sk-xxx

# Pass extra args to codex
myrouter launch-codex --profile glm52 -- --yolo "fix this bug"
```
...
[model_providers.myrouter.http_headers]
"X-Custom-Header" = "value"

# Headers read from env vars
[model_providers.myrouter.env_http_headers]
"X-Trace-Id" = "TRACE_ID"

# Extra URL query params (useful for Azure api-version)
[model_providers.myrouter.query_params]
...
```toml
[model_providers.myrouter-main]
base_url = "http://192.168.0.1:20128/v1"
env_key  = "MYROUTER_API_KEY"

[model_providers.myrouter-tailscale]
base_url = "http://100.x.x.x:20128/v1"
env_key  = "MYROUTER_API_KEY"
```
...
Two MyRouter defaults can silently sabotage multi-hour Codex CLI sessions. Neither is a Codex CLI setting — both live on the MyRouter side. Users migrating a config from upstream proxies that pin accounts and disable idle cutoffs often hit both and conclude MyRouter “cannot sustain a long session.”
...
Related discussions: [#7126](https://github.com/diegosouzapw/MyRouter/discussions/7126) (long task drops), [#5718](https://github.com/diegosouzapw/MyRouter/discussions/5718) (why affinity defaults off). Tracking: [#7287](https://github.com/diegosouzapw/MyRouter/issues/7287).
...
Critical detail: MyRouter’s synthetic SSE **heartbeat does not reset** the idle clock. Only a real upstream body chunk updates `lastChunkTime`. A quiet model that is still “thinking” looks identical to a stalled upstream from the watchdog’s point of view.
...
In the MyRouter process environment (`.env` / compose / systemd):
...
Restart MyRouter after changing these env vars.
...
2. **Raise / disable idle cutoffs** in MyRouter’s environment:
...
3. Keep the usual Codex `config.toml` (`wire_api = "responses"`, correct `base_url`, `MYROUTER_API_KEY`) — no Codex-side affinity/idle knobs exist for these two behaviors.
4. Restart MyRouter, then start the long Codex task.
...
When the idle watchdog fires, MyRouter logs a line shaped like:

```text
[STREAM] Idle timeout: no data from codex for 600000ms (model: cx/gpt-5.5)
```

Grep for `Idle timeout: no data from` (or the code `stream_idle_timeout` / error name `StreamIdleTimeoutError`). The provider segment is whatever MyRouter used for that request (`codex`, another provider id, or `provider` if unknown) — it is not always the literal string `codex`.
...
Verify the model exists in MyRouter with the correct prefix. Use `myrouter models list` or open `/dashboard/providers/<provider>`.

**`Authentication error`**
Confirm `MYROUTER_API_KEY` is exported: `echo $MYROUTER_API_KEY`.

**`Connection refused`**
Verify MyRouter is running and the `base_url` host/port is correct for your network (local vs Tailscale vs VPS).
...
See [Long-running tasks](#long-running-tasks). Enable session affinity (TTL above task length) and raise or disable `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Grep MyRouter logs for `Idle timeout: no data from`.
