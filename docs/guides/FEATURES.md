---
title: "MyRouter — Dashboard Features Gallery"
version: 3.8.40
lastUpdated: 2026-06-28
---

# MyRouter — Dashboard Features Gallery
...
Visual guide to every section of the MyRouter dashboard.
...
- 💻 **CLI enhancement suite** — 20+ commands including `myrouter providers`, `myrouter combos`, `myrouter doctor`, `myrouter setup`
...
OpenRouter connections can store a per-connection `preset` in Advanced Settings. When set, MyRouter sends it as the OpenRouter top-level request field, for example `"preset": "email-copywriter"`, unless the client request already supplied its own `preset`.
...
A combo strategy that preserves session continuity when account rotation happens mid-conversation. Before the active account is exhausted, MyRouter generates a structured handoff summary in the background. After the next request resolves to a different account, the summary is injected as a system message so the new account continues with full context.
...
Native Electron desktop app for Windows, macOS, and Linux. Run MyRouter as a standalone application with system tray integration, offline support, auto-update, and one-click install.
...
MyRouter now supports **OpenAI-compatible WebSocket clients** via the `/v1/ws` upgrade endpoint. The custom `scripts/dev/v1-ws-bridge.mjs` server wraps Next.js and upgrades WS connections to full bidirectional streaming sessions. Authentication uses the same API key or session cookie as HTTP requests.
...
**Hybrid token counting** also lands in v3.6.6: when a Claude-compatible provider exposes `/messages/count_tokens`, MyRouter calls it before large requests with graceful estimation fallback.
