...
Complete guide for configuring providers, creating combos, integrating CLI tools, and deploying MyRouter.
...
models. Haiku models do not accept the `max` effort tier, so MyRouter downgrades that
...
1. Subscribe: [Moonshot AI](https://platform.kimi.ai?aff=myrouter)
2. Get API key → Dashboard → Add API Key
...
The CLI automatically loads `.env` from `~/.myrouter/.env` or `./.env`.
...
Void Linux users can package and install MyRouter natively using the `xbps-src` cross-compilation framework. This automates the Node.js standalone build along with the required `better-sqlite3` native bindings.
...
| --------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `myrouter-default-secret-change-me` | JWT signing secret (**change in production**)                                                             |
| `INITIAL_PASSWORD`                      | `CHANGEME`                           | First login password                                                                                      |
| `DATA_DIR`                              | `~/.myrouter`                       | Data directory (db, usage, logs)                                                                          |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://myrouter.dev`              | Cloud sync endpoint base URL (replaces legacy `CLOUD_URL`)                                                |
...
| `MYROUTER_MEMORY_MB`                   | `512`                                | Node.js heap limit in MB                                                                                  |
...
### Chaining MyRouter Peers

Another MyRouter gateway can be added as a **Custom OpenAI-compatible** provider. Use the
...
`X-MyRouter-Peer-Trace` header. A gateway rejects a repeated instance ID or exhausted hop
...
into multiple running MyRouter instances.
...
- **Semantic Cache** — Auto-caches non-streaming, temperature=0 responses (bypass with `X-MyRouter-No-Cache: true`)
- **Request Idempotency** — Deduplicates requests within 5s via `Idempotency-Key` or `X-Request-Id` header
- **Progress Tracking** — Opt-in SSE `event: progress` events via `X-MyRouter-Progress: true` header

---

### Translator Playground

Access via **Dashboard → Translator**. Debug and visualize how MyRouter translates API requests between providers.
...
MyRouter also accepts `x_session_id` and returns the effective session key in `X-MyRouter-Session-Id`.
...
MyRouter implements provider-level resilience with five components:
...
4. **Wait For Cooldown** — If every candidate connection is already cooling down, MyRouter can wait for the earliest cooldown and retry the same client request automatically.
...
- Migrate MyRouter between machines
...
MyRouter supports audio transcription via the OpenAI-compatible endpoint:
...
MyRouter clamps it down to that cap before sending the upstream request.
...
MyRouter is both an **MCP server** (Model Context Protocol) and an **A2A server** (Agent-to-Agent JSON-RPC 2.0). Any MCP-compatible IDE or agent host can call MyRouter tools directly — no extra wrapper required.

### MCP transports

- **SSE**: `http://localhost:20128/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:20128/api/mcp/stream`
- **stdio**: `myrouter --mcp` (for IDE plugins that prefer stdio)
...
MyRouter exposes an extensible **skill framework** (`src/lib/skills/`) so agents and the A2A endpoint can run domain-specific routines (e.g. `code-review`, `summarize`, `extract-facts`, `web-research`).
...
MyRouter persists **long-term conversational memory** with hybrid retrieval:
...
Subscribe to MyRouter events for real-time monitoring and automation.
...
- Every payload includes `X-MyRouter-Signature` (HMAC-SHA256) for verification
...
MyRouter integrates with cloud coding agents (**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**) so you can dispatch long-running tasks from the same dashboard that handles your local routing.

- Create tasks in **Dashboard → Cloud Agents** or via `POST /api/v1/agents/tasks`
- Track status, logs, and artifacts per task
- Bring-your-own API key per provider — credentials never leave the MyRouter instance
...
You can manage every MyRouter resource (providers, combos, keys, settings) over HTTP using a **Bearer key with the `manage` scope**.
...
MyRouter ships an internal CLI (`myrouter …`) for setup, diagnostics, and runtime control. This is **separate from the "CLI Tools" page in the dashboard**, which configures third-party CLIs (Claude Code, Cursor, Codex, Cline, …) so they can talk to MyRouter.
...
Tip: pair `myrouter doctor --json` with your monitoring tool to alert on unhealthy provider connections.

---

## 🖥️ Desktop Application (Electron)

MyRouter is available as a native desktop application for Windows, macOS, and Linux.
...
| Variable              | Default | Description                      |
| --------------------- | ------- | -------------------------------- |
| `MYROUTER_PORT`      | `20128` | Server port                      |
| `MYROUTER_MEMORY_MB` | `512`   | Node.js heap limit (64–16384 MB) |
