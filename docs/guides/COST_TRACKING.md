...
How MyRouter estimates, records, and reports the cost of every request — and why the
...
MyRouter attributes a per-request USD cost to every completion by multiplying token
counts by a model's pricing rates. These numbers power the **Costs** dashboard, the
`myrouter cost` / `myrouter usage` CLI, CSV/JSON exports, and per-API-key budgets.

> **The dashboard "cost" is a savings tracker, not a bill.** MyRouter never charges you
...
- It depends on the pricing table MyRouter has for each model. A model with no pricing
...
3. **Hardcoded defaults** — shipped with MyRouter.
...
- API-key spend is buffered and flushed in batches by the
  ...
  | Env var                             | Default | Purpose                            |
  | ----------------------------------- | ------- | ---------------------------------- |
  | `MYROUTER_SPEND_FLUSH_INTERVAL_MS` | `60000` | Flush interval in milliseconds.    |
  | `MYROUTER_SPEND_MAX_BUFFER_SIZE`   | `1000`  | Max buffered entries before flush. |
...
MyRouter's CLI exposes cost, usage, and pricing commands (registered in
...
- **All costs show $0 / "Legacy / Free".** The models in use have no pricing entry.
  Enable external sync (`PRICING_SYNC_ENABLED=true`) and run `myrouter pricing sync`, or
...
- **Spend lags behind real time.** Per-key spend is batched; lower
  `MYROUTER_SPEND_FLUSH_INTERVAL_MS` if you need fresher numbers.
