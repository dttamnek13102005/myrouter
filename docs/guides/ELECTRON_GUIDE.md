...
MyRouter ships a cross-platform desktop app (Windows / macOS / Linux) built on
...
| App version        | `3.8.0`                    |
| App id             | `online.myrouter.desktop` |
| Product name       | `MyRouter`                |
...
- Linux: `$XDG_CONFIG_HOME/myrouter` or `~/.myrouter`
- macOS: `~/.myrouter`
...
- Close button minimizes to tray; the tray menu has **Open MyRouter**, **Open Dashboard** (external browser), **Server Port** submenu, **Check for Updates**, **Quit**.
...
Uses `electron-updater` with the GitHub provider (`diegosouzapw/MyRouter`).
...
Releases are published to GitHub Releases (`diegosouzapw/MyRouter`), which is also where `electron-updater` checks for new versions.
