...
MyRouter ships as a fully installable Progressive Web App. When you access the dashboard from any mobile browser — Android (Chrome) or iOS (Safari) — you can "Add to Home Screen" and get a native app-like experience with no app store required.

## What Is a PWA?

A Progressive Web App turns the MyRouter web dashboard into something that looks and feels like a native mobile app. Once installed, it:
...
1. Open the MyRouter dashboard in Chrome: `http://YOUR_IP:20128`
2. Chrome will show an **"Add MyRouter to Home screen"** banner automatically, or:
   - Tap the **⋮** menu (three dots) → **"Add to Home screen"** or **"Install app"**
3. Confirm the prompt
4. MyRouter appears on your home screen as a standalone app

### iOS (Safari)

1. Open the MyRouter dashboard in Safari: `http://YOUR_IP:20128`
2. Tap the **Share** button (box with arrow)
3. Scroll down and tap **"Add to Home Screen"**
4. Name it (defaults to "MyRouter") and tap **Add**
5. MyRouter appears on your home screen with the app icon

### Desktop (Chrome / Edge)

1. Open the MyRouter dashboard
2. Click the **install icon** in the address bar (or ⋮ → "Install MyRouter...")
3. Confirm the prompt
4. MyRouter opens as a standalone window — no tabs, no address bar
...
MyRouter includes a service worker (`sw.js`) that provides intelligent caching:
...
MyRouter provides icons optimized for each platform:
...
- **Cache versioning**: `myrouter-pwa-v2` — bump this to force a fresh cache on update
...
When running MyRouter on Android via Termux, the PWA works seamlessly:

1. Start MyRouter in Termux: `npx myrouter`
...
Install the PWA on any device that has browser access to your MyRouter server:
