...
> **TL;DR**: MyRouter ranks its **free** providers by model quality using **Arena AI
...
MyRouter aggregates 160+ providers, many of which expose a **free tier** (no-auth,
...
Leaderboard categories map to MyRouter task categories:

| Arena leaderboard | MyRouter task categories                         |
| ----------------- | ------------------------------------------------- |
...
- It is **non-blocking and never fatal** — if the upstream fetch fails, MyRouter keeps
...
stripped and a small alias map expands canonical names into the variants MyRouter uses
