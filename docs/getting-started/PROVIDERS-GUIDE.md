# Providers Guide: Connect AI Models to MyRouter

> **TL;DR**: A provider is a connection to an AI service (like OpenAI, Anthropic, Google). You need at least one provider to use MyRouter.

---

## What Is a Provider?

Think of a provider like a **phone carrier**. Just as you need a phone carrier to make calls, you need an AI provider to use AI models. MyRouter is like a phone that works with **all carriers** — you can switch between them automatically.
...
MyRouter works best with **multiple providers**. This gives you:

- **Automatic fallback** — If one provider fails, MyRouter tries the next
- **Cost optimization** — MyRouter picks the cheapest provider for each request
- **Speed optimization** — MyRouter picks the fastest provider for each request
- **Quality optimization** — MyRouter picks the best provider for each task
...
Then use `model: "auto"` and MyRouter will automatically pick the best one for each request.
...
2. In MyRouter: Providers → Add Provider → OpenAI
3. Paste API key → Connect

### Anthropic

1. Get API key: https://console.anthropic.com/
2. In MyRouter: Providers → Add Provider → Anthropic
3. Paste API key → Connect

### Google (Gemini)

1. Get API key: https://aistudio.google.com/apikey
2. In MyRouter: Providers → Add Provider → Gemini
3. Paste API key → Connect

### DeepSeek

1. Get API key: https://platform.deepseek.com/
2. In MyRouter: Providers → Add Provider → DeepSeek
3. Paste API key → Connect

### Groq

1. Get API key: https://console.groq.com/
2. In MyRouter: Providers → Add Provider → Groq
3. Paste API key → Connect
...
### "Do I need to pay to use MyRouter?"

**No!** MyRouter is free and open-source. You can use free providers (Kiro, OpenCode Free, Pollinations) without paying anything. You only pay if you choose to use paid providers.
...
**Yes!** That's the whole point of MyRouter. Connect multiple providers and use `model: "auto"` to let MyRouter pick the best one for each request.

### "What if a provider goes down?"

MyRouter automatically skips failed providers and tries the next one. You don't need to do anything.
...
**Yes!** If you already have API keys for OpenAI, Anthropic, Google, etc., you can use them in MyRouter. Just paste them when connecting the provider.
