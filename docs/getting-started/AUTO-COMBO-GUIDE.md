# Auto-Combo: Let MyRouter Pick the Best AI for You

> **TL;DR**: Set your model to `auto` and MyRouter automatically picks the best AI provider for each request. No configuration needed.

---

## What It Does

Instead of choosing a specific AI model (like GPT-4o or Claude), you can let MyRouter **automatically pick the best one** for each request. It considers:
...
MyRouter scores all your connected providers and picks the best one. If it fails, it automatically tries the next one.
...
**Step 2**: That's it! MyRouter handles the rest.
...
When you send a request with `model: "auto"`, MyRouter:
...
4. **Auto-recovers** — If it fails, MyRouter tries the next one automatically
...
MyRouter has **three layers of protection**:

### 1. Auto-Fallback
If the best provider fails, MyRouter automatically tries the next one. You don't need to do anything.
...
If all providers fail, MyRouter routes to stable free providers (like Kiro or Qoder) as a last resort.

---

## Multi-Account Support

If you have multiple accounts for the same provider (e.g., two OpenAI keys), MyRouter treats each as a **separate candidate**. This means:
...
MyRouter occasionally **explores** new providers to discover better options:
...
This helps MyRouter learn which providers work best for your usage patterns.
...
MyRouter automatically skips it and tries the next one. If a provider keeps failing, it's excluded temporarily (5-30 minutes). You don't need to do anything.

### "Can I see which provider was used?"

Check the response headers — MyRouter includes the provider and model used in each response.
