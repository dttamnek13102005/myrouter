# Incident Response Runbook — MyRouter (2026-06-18)
...
| **SEV-3** | Latent bug or near-miss; no current user impact but error budget at risk. | Memory leak trending up; circuit breaker tripping on one provider. | Slack `#myrouter-ops` (next standup) | 7 d |
...
| Grafana SLO dashboards | SLO burn-rate panels | Slack `#myrouter-ops` |
...
| User report (support@) | Manual triage | Slack `#myrouter-triage` |
...
1. Check the authz-inventory endpoint:
   `curl https://api.myrouter.dev/api/settings/authz-inventory | jq`.
...
written handoff to the incoming in `#myrouter-ops-handoff` covering:
