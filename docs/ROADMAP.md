...
MyRouter is heading from a monolithic router to a **modular AI platform**: a lightweight
...
| 3.8.53 | `chatCore.ts` decomposition · headless mode (`MYROUTER_HEADLESS=1`) · local candidate build/promote loop |
...
- **`stable/v3`** — the LTS line (3.9.x). Receives fixes, security patches, and provider
  updates. `npm install myrouter` (aka `latest`) stays on v3 during the whole v4 cycle.
...
- **`@myrouter/core`** (npm name stays `myrouter`) — just the engine: `/v1/*`, routing,
  combo/fallback, providers.
- **`@myrouter/sdk`** — one typed contract: hooks, extension points, two-phase lifecycle,
...
- **Modules** (`@myrouter/mod-*`) — cloud agents, traffic inspection (MITM), evals,
