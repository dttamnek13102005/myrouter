...
MyRouter ships a WordPress-style plugin system. Plugins are self-contained
...
Plugins live under the MyRouter data directory:
...
`<home>/.myrouter/plugins`, where `<home>` is taken from the `HOME` /
