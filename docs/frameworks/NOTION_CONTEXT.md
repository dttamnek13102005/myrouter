...
MyRouter can connect to a **Notion** workspace as a **context source** — a read/write
...
blocks — all proxied through MyRouter (with retry, timeout, and error classification)
...
> MyRouter to access with that integration (Notion's permission model is share-based,
> not workspace-wide).
...
MyRouter saves the token and immediately runs a 1-result test search; if Notion
...
`open-sse/mcp-server/server.ts` only when `MYROUTER_MCP_ENFORCE_SCOPES=true`; the
caller's allowed scopes come from `MYROUTER_MCP_SCOPES` (comma-separated) or the
