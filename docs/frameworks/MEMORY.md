...
MyRouter provides persistent conversational memory keyed by API key (and
...
> A client can opt a single request out with the `x-myrouter-no-memory`
...
- `searchSemanticMemory(query, topK, scope)` — embed the query, search the
  collection filtered by `kind = "myrouter_memory"` and optionally by
...
`qdrantApiKey`, `qdrantCollection` default `"myrouter_memory"`,
...
- `myrouter_memory_clear` — `{apiKeyId, type?, olderThan?}` → lists matching
...
- For hard deletion, use `DELETE /api/memory/[id]` or `myrouter_memory_clear`.
...
MyRouter's memory engine supports **four embedding sources** (`src/lib/memory/embedding/`). Each has different trade-offs in **latency, cost, model quality, and setup complexity**.
