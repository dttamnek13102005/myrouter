---
title: "MyRouter A2A Server Documentation"
version: 3.8.40
lastUpdated: 2026-06-28
---

# MyRouter A2A Server Documentation

> Agent-to-Agent Protocol v0.3 — MyRouter as an intelligent routing agent
...
Returns the Agent Card describing MyRouter's capabilities, skills, and authentication requirements.
...
MyRouter exposes 6 A2A skills wired in `src/lib/a2a/taskExecution.ts::A2A_SKILL_HANDLERS`. Each skill module lives in `src/lib/a2a/skills/`.

| Skill              | ID                   | Description                                                                                                     | Tags                       | Examples                               |
| :----------------- | :------------------- | :-------------------------------------------------------------------------------------------------------------- | :------------------------- | :------------------------------------- |
| Smart Routing      | `smart-routing`      | Routes a prompt through the optimal provider/combo using MyRouter's combo engine + scoring                     | routing, providers         | "Route this prompt via the best model" |
...
| List Capabilities  | `list-capabilities`  | Returns the full 42-entry Agent Skills catalog as a markdown table with raw SKILL.md URLs for context injection | catalog, discovery, skills | "List all MyRouter capabilities"      |
...
The `list-capabilities` skill is particularly useful for external agents that need to discover what MyRouter exposes before sending API calls. It returns a structured markdown table artifact:
