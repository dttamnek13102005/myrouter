...
AgentBridge is MyRouter's MITM (Man-in-the-Middle) proxy that intercepts HTTPS traffic from IDE AI agents and reroutes it through MyRouter's unified routing engine. It supports **9 IDE agents** — Antigravity, Kiro, GitHub Copilot, OpenAI Codex, Cursor, Zed, Claude Code, Open Code, and Trae (investigating) — making MyRouter the broadest-coverage MITM proxy for AI coding assistants on the market.
...
- **Reroute any agent to any provider**: Copilot talking to OpenAI? Redirect it to Anthropic Claude, Gemini, or any of MyRouter's 226+ providers.
...
- **Apply MyRouter resilience**: combo routing, circuit breakers, fallbacks, and cost tracking work for IDE agent traffic too.
...
> into the same `myrouter-mitm.crt` trust-store slot the old leaf used
...
AgentBridge intercepts credentials (OAuth tokens, API keys) that the IDE uses to authenticate with upstream providers. These are **masked before logging** (see §2.7) but are visible to MyRouter's MITM layer. First activation of each agent shows a dismissible risk notice modal.
...
The bypass list ensures that financial institutions, OAuth/SSO providers, and other sensitive hosts are **never decrypted**. Their TLS traffic passes through as a transparent TCP tunnel — MyRouter never sees the plaintext.
...
1. Verify the cert was installed: `security find-certificate -c "MyRouter AgentBridge"` (macOS) or `certutil -L -d sql:$HOME/.pki/nssdb` (Linux/NSS)
...
2. Check MyRouter server logs: `APP_LOG_LEVEL=debug` in `.env`
3. Verify `MYROUTER_BASE_URL` points to the correct router endpoint (default: `http://127.0.0.1:20128`)
