...
MyRouter connects to an **Obsidian** vault as a **context source** — a local Markdown
...
vault sync (driven by `src/lib/obsidianSync.ts`). Enabling it points MyRouter at a
...
per call (per-API-key first, then global). Tools that hit the MyRouter **sync server**
(the four `obsidian_sync_*` tools) additionally require the sync auth token configured
in MyRouter settings.
...
| `obsidian_sync_status`       | MyRouter sync server status: running, vault name, port, uptime, last sync.          |
...
`open-sse/mcp-server/server.ts`, gated on `MYROUTER_MCP_ENFORCE_SCOPES=true`, with
allowed scopes sourced from `MYROUTER_MCP_SCOPES` or the API key's scope context. See
