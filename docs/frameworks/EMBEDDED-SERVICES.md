...
Embedded services are locally-installed process sidecar tools that MyRouter installs, supervises, and
exposes as first-class routing targets. Unlike external providers (which are reached over the internet
via API keys), embedded services run on the same machine as MyRouter and communicate over loopback.
...
| --------------- | ----------------------------------------------- | :----------: | ------------------------------------------------------------------------------------------------------------------ |
| **9Router**     | `9router`                                      |    20130     | AI router that MyRouter can use as a sub-provider. Models exposed as `9router/{sub}/{model}`                     |
...
- MyRouter installs them under `DATA_DIR/services/{name}/` (isolated from MyRouter's own `package.json`)
- MyRouter spawns and monitors them as child processes
- MyRouter injects an ephemeral API key into the child's environment and rotates it without downtime (where applicable)
...
| API key management                    | MyRouter generates, encrypts at-rest (AES-256-GCM), and injects via env |
...
`package.json` and `node_modules/`. Does not conflict with MyRouter's own deps.
...
the next time MyRouter boots (if the service is installed).
...
- Strips incoming `cookie` and `authorization` headers (no leakage of MyRouter session)
...
2. Verify `npm` is in PATH: `which npm` from the same user account that runs MyRouter.
...
- `DATA_DIR` or its parent is not writable by the MyRouter process.
- Running inside Docker rootless without write access to the mapped volume.

**Fix:**

1. Check `DATA_DIR` (default: `~/.myrouter/`): `ls -la ~/.myrouter/`
2. Ensure the MyRouter process user owns the directory: `chown -R $USER ~/.myrouter/`
...
3. Try the install manually: `npm install {package}@latest --prefix ~/.myrouter/services/{name}/`.
...
Yes. Both services follow the same local-first model as MyRouter itself. They run
on the same machine and communicate over loopback. "Production" here means the VPS
or local server where MyRouter is deployed, not a remote cloud provider.
...
2. Check structured logs in MyRouter's pino output filtered by
   `service:supervisor` namespace.
3. Inspect the DB row: `sqlite3 ~/.myrouter/myrouter.db "SELECT * FROM version_manager WHERE tool='9router'"`.
