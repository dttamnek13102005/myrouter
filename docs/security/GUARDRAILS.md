...
between MyRouter and upstream providers. Each guardrail can inspect (and
...
- Header `x-myrouter-disabled-guardrails` (or legacy
...
- **`garak` (advisory)** — runs garak `--probes promptinject,dan,leakreplay`
  against a local MyRouter instance (`http://localhost:20128/v1`). Gated on a
