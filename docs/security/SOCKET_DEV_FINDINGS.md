...
description: "Maintainer attestation for the AI-detected potential-malware findings raised against myrouter and the v3.8.6 mitigations applied at each flagged call site."
---

# Socket.dev / supply-chain finding attestation

This document is the maintainer-authored attestation for the six
`AI-detected potential malware` findings raised against `myrouter@3.8.5` and
the mitigations applied in `myrouter@3.8.6`. It exists so:
...
Caddy. The fact that they exist in MyRouter is documented at
...
A `MYROUTER_ZED_IMPORT_LEGACY_ONE_STEP=true` env flag preserves the v3.8.5
...
who already use Zed and want to mirror their provider keys into MyRouter
without re-pasting.
...
toolkit." Within MyRouter they are only used by the MITM cert install path
...
(`MYROUTER_BUILD_PROFILE=minimal`) replaces
...
## §5 — MyRouter Cloud Sync credential write-back (`api/keys/[id]/route.js`)
...
1. **HMAC verification**: `verifyCloudSignature(rawBody, sigHeader)` checks
   the `X-Cloud-Sig` header (`HMAC-SHA256(MYROUTER_CLOUD_SYNC_SECRET,
...
2. **Secret-field opt-in**: `accessToken` / `refreshToken` /
   ...
   `MYROUTER_CLOUD_SYNC_SECRETS=true`. The default mode syncs only
...
**Why we keep it**: Cloud Sync is the only way for an MyRouter Cloud tenant
...
The resulting bundle is intended to be published as `myrouter-secure`. See
