...
MyRouter controls which **browser origins** may read cross-origin responses
...
taking seriously, but MyRouter's exposure is **narrower than the generic case**,
...
> `Access-Control-Allow-Credentials: true`. MyRouter's shared CORS path never
> does.
...
- If MyRouter runs behind a reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
## Example: reverse proxy in front of MyRouter

CORS is enforced by MyRouter itself, so the proxy generally should **not** add or
rewrite `Access-Control-*` headers (double headers break browsers). Terminate TLS
and forward — let MyRouter answer preflight:
...
Set the allowed browser origins in MyRouter (`CORS_ALLOWED_ORIGINS` or the
