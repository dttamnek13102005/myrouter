...
TPROXY transparent decrypt is MyRouter's **5th capture mode** for the
...
configuration change** — but the process must trust the dynamic CA MyRouter
...
> and installs under the pre-existing `myrouter-mitm.crt` trust-store slot
> (superseding the old single leaf there — no dual-trust cleanup needed),
> kept fully separate from TPROXY's own `myrouter-tproxy-ca.crt` slot below.
...
- Generates a long-lived CA via `generateMitmCa()` (CN `"MyRouter MITM CA"`,
...
`myrouter-tproxy-ca.crt` (constant `TPROXY_CA_CERT_NAME`) — kept separate from
the static MITM cert's slot (`myrouter-mitm.crt`) so the two never clobber each
other.
...
host means anything MyRouter intercepts can be decrypted. It is gated behind the
...
- Confirm the process trusts the dynamic CA. The CA is installed under
  `myrouter-tproxy-ca.crt`; apps with their own trust store (Firefox/Chrome NSS)
...
mode reverts the rules; if MyRouter was killed mid-session, use the AgentBridge
