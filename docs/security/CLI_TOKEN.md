...
MyRouter CLI commands authenticate against the local management API using a
`HMAC-SHA256(machine-id, salt)` token sent via the `x-myrouter-cli-token`
request header.

This allows CLI subcommands (`myrouter status`, `myrouter providers`, etc.)
...
3. The CLI sends the token as `x-myrouter-cli-token` on every request to
...
Set `MYROUTER_CLI_SALT` to rotate the derived token without code changes.
...
export MYROUTER_CLI_SALT="my-secret-salt-2026"

# Verify new token is in use
myrouter status
```

Default salt: `myrouter-cli-auth-v1`
...
**Opt-out:** set `MYROUTER_DISABLE_CLI_TOKEN=true` (env or `.env`) to disable the CLI
