...
For most users, **no special setup is required**. MyRouter ships a built-in
...
MyRouter also supports AgentRouter (and similar relays) through the **Claude Code
...
- MyRouter running with the `ENABLE_CC_COMPATIBLE_PROVIDER` feature flag enabled
  (see below).
...
setting an environment variable before starting MyRouter:
...
**`Invalid JSON response from provider (reset after Ns)` from the myrouter logs** —
...
model actually resolves in the myrouter logs (`ROUTING` tag shows
