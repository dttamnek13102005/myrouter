...
adapter the primary transport. Set `MYROUTER_BROWSER_POOL` to one of the same values to
...
`X-MyRouter-Claude-Web-*` headers (`open-sse/executors/claude-web/stream.ts:364`).
...
1. Start MyRouter with `npm run dev` or a built installation.
