...
When MyRouter runs inside Docker, the standard "Import from Zed Keychain" flow fails
...
MyRouter detects the Docker environment via two heuristics:
...
2. The **Manual Token Import** panel appears below the keychain import card. When
   MyRouter detects Docker, this panel expands automatically after the first failed
