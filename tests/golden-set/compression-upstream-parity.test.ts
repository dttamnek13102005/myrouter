...
function myrouterCompress(text: string): string {
  return myrouterPromptCompression(text).text;
}

function myrouterPromptCompression(text: string): { text: string; fallbackApplied: boolean } {
...
describe("upstream Caveman parity benchmark", () => {
  it("matches core upstream shrink protections and savings direction", () => {
    for (const input of parityCases) {
      const ours = myrouterCompress(input);
      const theirs = upstream.compress(input).compressed;
      assert.ok(ours.length <= input.length, `MyRouter did not reduce: ${input}`);
  ...
  it("stays within upstream token budget on representative prose", () => {
    ...
    assert.ok(
      ...
      `Expected MyRouter within 20% of upstream shrink length. ours=${ours.length}, upstream=${theirs.length}`
    );
  });
  ...
  it("runs offline parity against upstream Caveman fixture files", () => {
    ...
    for (const fixture of fixturePairs) {
      ...
      const ours = myrouterPromptCompression(original);
      ...
      if (ours.fallbackApplied) {
        ...
        assert.ok(ours.text.length < original.length, `${fixture.name}: MyRouter did not reduce`);
        assert.ok(
          ours.text.length <= Math.ceil(Math.max(expected.length, upstreamShrink.length) * 1.35),
          `${fixture.name}: MyRouter drifted too far from upstream fixture budget`
        );
      }
