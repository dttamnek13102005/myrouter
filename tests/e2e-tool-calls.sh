...
MYROUTER_URL="${MYROUTER_URL:-http://localhost:20128}"
MODEL="${1:-${MYROUTER_MODEL:-}}"
AUTH_TOKEN="${MYROUTER_AUTH_TOKEN:-dummy}"
FAILURES=0
...
echo "=== MyRouter Tool Call Integrity E2E Test ==="
echo "URL: $MYROUTER_URL | Model: $MODEL"

echo
echo "[TEST 1] Non-streaming tool call integrity..."
response=$(curl -fsS -X POST "$MYROUTER_URL/v1/chat/completions" \
...
stream_output=$(curl -fsS -X POST "$MYROUTER_URL/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
  -d @- <<JSON
{
...
models=$(curl -fsS "$MYROUTER_URL/v1/models" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
