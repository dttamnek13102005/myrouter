...
const BASE_URL = process.env.MYROUTER_BASE_URL || "http://localhost:20128";
const API_KEY = process.env.MYROUTER_API_KEY || "";
  ...
  it(
    ...
    async () => {
      const transport = new StdioClientTransport({
        ...
        env: {
          ...process.env,
          MYROUTER_BASE_URL: BASE_URL,
          MYROUTER_API_KEY: API_KEY,
      ...
      try {
        ...
        expect(toolNames).toContain("myrouter_get_health");
        expect(toolNames).toContain("myrouter_list_combos");
      ...
      const auditRes = await apiFetch("/api/mcp/audit?limit=50&tool=myrouter_get_health");
      ...
      if (auditRes.status === 200) {
        ...
        expect(entries.some((entry: any) => entry.toolName === "myrouter_get_health")).toBe(true);
      }
    },
    TEST_TIMEOUT_MS * 2
  );
