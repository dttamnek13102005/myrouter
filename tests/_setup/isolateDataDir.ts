...
// (bin/myrouter.mjs, proxyFetch.ts, proxyDispatcher.ts) where redirecting DATA_DIR
...
// but every process resolves DATA_DIR to the SAME default (~/.myrouter) when the env
...
if (!process.env.DATA_DIR) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-test-"));
...
process.env.MYROUTER_SKIP_SYSTEM_TRUST = "1";
...
process.env.MYROUTER_SKIP_DNS_WRITE = "1";
