...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-fingerprint-e2e-"));
...
function createServerProcess(dataDir: string, port: number) {
  ...
  const child = spawn(process.execPath, ["scripts/dev/run-next-playwright.mjs", "dev"], {
    ...
    env: {
      ...
      MYROUTER_DISABLE_TOKEN_HEALTHCHECK: "true",
      MYROUTER_DISABLE_LOCAL_HEALTHCHECK: "true",
      MYROUTER_HIDE_HEALTHCHECK_LOGS: "true",
      MYROUTER_E2E_BOOTSTRAP_MODE: "open",
    },
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 120_000) {
    if (logs.exitInfo) {
      throw new Error(
        [
          `MyRouter exited before it became ready (code=${logs.exitInfo.code}, signal=${logs.exitInfo.signal})`,
          "--- stdout ---",
  ...
  throw new Error(
    [
      `Timed out waiting for MyRouter to start: ${lastError}`,
      "--- stdout ---",
      ...logs.stdoutLines.slice(-40),
      "--- stderr ---",
      ...logs.stderrLines.slice(-40),
...
test("round-robin combo with 3 fingerprints: all requests succeed", async () => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    // X-MyRouter-Model header (echoRequestedModelName/#1311 is opt-in and off
