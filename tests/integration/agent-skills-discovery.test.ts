 ...
 *  3. MCP tool myrouter_agent_skills_list handler returns 44 entries.
...
// ── §3: MCP tool myrouter_agent_skills_list ─────────────────────────────────

test("MCP myrouter_agent_skills_list handler returns count 45 (44 + config)", async () => {
...
test("MCP myrouter_agent_skills_list result has all 42 IDs", async () => {
  const result = await agentSkillTools.myrouter_agent_skills_list.handler({});
