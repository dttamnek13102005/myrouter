 ...
 *
 * Letta configures MyRouter as its "lmstudio" provider (localModelDiscovery:
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-letta-settings-"));
...
// ── Test 6: POST refuses to overwrite an existing non-MyRouter lmstudio config ──
...
// ── Test 7: DELETE → removes the MyRouter lmstudio config ──────────────────
