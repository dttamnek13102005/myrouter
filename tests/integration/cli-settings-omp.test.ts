...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-omp-settings-"));
...
test("omp-settings GET: treats an existing agent.db as installed", async () => {
  ...
  assert.equal(body.hasMyRouter, false);
});
...
test("omp-settings POST: writes models.yml and persists credentials for a seeded DB", async () => {
  ...
  assert.equal(getBody.hasMyRouter, true);
});

// ── Test 6: DELETE → removes MyRouter provider entry ────────────────────────

test("omp-settings DELETE: removes the MyRouter provider from models.yml and credentials", async () => {
  seedOmpDb();
  await POST(
    req({
      method: "POST",
  ...
  const getRes = await GET(req());
  const getBody = await getRes.json();
  assert.equal(getBody.hasMyRouter, false);
});
