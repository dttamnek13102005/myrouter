 ...
 *
 * Live benchmark: how well do the free-tier models MyRouter exposes handle
 ...
 *
 * Environment:
 *   MYROUTER_API_KEY  — required (else test skips)
...
test(
  ...
  async () => {
    ...
    // MyRouter's routing) is broken, not that every free provider failed at
    // once.
    assert.ok(
      totalSucceeded > 0,
