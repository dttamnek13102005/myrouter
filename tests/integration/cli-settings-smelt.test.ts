...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-smelt-settings-"));
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        assert.equal(written._managedBy, "myrouter");
...
// ── Test 5: DELETE → removes MyRouter fields ────────────────────────────────

test("smelt-settings DELETE: removes MyRouter fields from existing config", async () => {
