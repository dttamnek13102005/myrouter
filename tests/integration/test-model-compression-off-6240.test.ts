...
// completely independent of the per-request `x-myrouter-compression` header/mode. The internal
...
// a request carrying `x-myrouter-compression: off` must NOT get the styles system message
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-testmodel-compression-"));
...
test("chatCore: x-myrouter-compression: off suppresses Output Styles injection even with the operator's global style enabled (#6240)", async () => {
  ...
  assert.match(plainFirstMessage?.content ?? "", /MyRouter Output Styles/);

  // The "Test model" connection test sends x-myrouter-compression: off — must be clean.
  const testModelBody = await runChatCore({
    provider,
    model,
    connectionId: connection.id,
    headers: new Headers({ "x-myrouter-compression": "off" }),
  });
  ...
  const anyMessageHasMarker = (testModelBody?.messages ?? []).some((m) =>
    (m?.content ?? "").includes("MyRouter Output Styles")
  );
  assert.equal(
    anyMessageHasMarker,
    false,
