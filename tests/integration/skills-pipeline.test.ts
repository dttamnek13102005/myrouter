...
import { MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME } from "../../open-sse/services/webSearchFallback.ts";
...
test("web_search fallback converts built-in tools for unsupported providers and executes search", async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (urlStr.includes("google.serper.dev/search")) {
      ...
      return new Response(
        JSON.stringify({
          organic: [
            {
              title: "MyRouter Search Result",
              link: "https://example.com/myrouter",
              snippet: "Fresh MyRouter web search fallback result",
            },
          ],
        }),
    ...
    return buildOpenAIToolCallResponse({
      toolName: MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME,
      toolCallId: "call_web_search",
      argumentsObject: {
        query: "latest myrouter release notes",
        max_results: 3,
      },
    });
  };
  ...
  const response = await handleChat(
    buildRequest({
      ...
      body: {
        ...
        stream: false,
        messages: [
          { role: "user", content: "Search the web for the latest MyRouter release notes" },
        ],
  ...
  assert.equal(upstreamBodies[0].tools[0].function.name, MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME);
  ...
  assert.equal(output.results[0].title, "MyRouter Search Result");
});
...
test("web_search fallback preserves Responses API output by appending function_call_output", async () => {
        ...
        input: [
          {
            ...
            role: "user",
            content: [
              { type: "input_text", text: "Search the web for the latest MyRouter roadmap" },
            ],
          },
        ],
  ...
  assert.equal(functionCall.name, MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME);
