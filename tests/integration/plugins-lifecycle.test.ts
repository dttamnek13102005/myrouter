...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-plugins-lifecycle-"));
...
function writeTestPlugin(opts?: { name?: string; onRequest?: boolean; enabledByDefault?: boolean }) {
  ...
  const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-plugin-src-"));
...
test("install: throws on invalid source directory", async () => {
  const emptyDir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-empty-"));
