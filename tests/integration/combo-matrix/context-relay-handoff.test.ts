...
//      - request carries x-myrouter-session-id header
...
// reads "x-session-id" (not "x-myrouter-session-id") and prefixes the result with "ext:".
...
//     internal summary request (identified by _myrouterInternalRequest flag in body)
...
function installHandoffAwareFetch() {
  ...
  globalThis.fetch = async (url: any, init: any = {}) => {
    ...
    if (bodyObj._myrouterInternalRequest === "universal-handoff") {
...
// Without the x-myrouter-session-id header, sessionId = null → block is skipped.
test("context-relay universal handoff: does NOT fire when x-myrouter-session-id header is absent", async () => {
  ...
  assert.equal(
    handoff,
    null,
    "handoff must NOT be written when x-myrouter-session-id header is absent (sessionId gate)"
  );
