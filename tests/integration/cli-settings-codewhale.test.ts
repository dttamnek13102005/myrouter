...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-codewhale-settings-"));
...
test("codewhale-settings POST: writes primary ~/.codewhale/config.toml for a fresh install", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      assert.ok(content.includes("managed by MyRouter"), "Config should have MyRouter marker");
...
test("codewhale-settings GET: falls back to legacy ~/.deepseek/config.toml when primary is absent", async () => {
  ...
  try {
    ...
    const body = await res.json();
    if (body.config) {
      assert.ok(body.config.includes("managed by MyRouter"));
      assert.equal(body.hasMyRouter, true);
    }
  } finally {
    process.env.HOME = origHome;
    fs.rmSync(tmpHome, { recursive: true, force: true });
