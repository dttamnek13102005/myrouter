 ...
 * rebuilt on top of MyRouter's existing "custom" configType settings pattern
 ...
 * the `[model.myrouter]` section and preserves the rest of the file.
 */
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-grok-build-settings-"));
...
// ── Test 4: POST with valid body → surgically upserts [model.myrouter] ─────

test("grok-build-settings POST: writes [model.myrouter] section and preserves existing content", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      assert.ok(content.includes("[model.myrouter]"), "Config should have [model.myrouter]");
      assert.ok(content.includes("http://localhost:20128/v1"), "Config should contain base URL");
      assert.ok(content.includes('default = "myrouter"'), "Default should point at our slot");
      ...
      assert.ok(
        content.includes('myrouter-prev-default = "grok-build"'),
...
test("grok-build-settings DELETE: removes our section, preserves the rest, restores default", async () => {
  ...
  try {
    ...
    const preConfigured = [
      ...
      'default = "myrouter"',
      "",
      "# myrouter-prev-default = \"grok-build\"",
      "[model.myrouter]",
      'model = "grok-4.5"',
    ...
    if (res.status === 200) {
      ...
      const configPath = path.join(tmpHome, ".grok", "config.toml");
      const content = fs.readFileSync(configPath, "utf-8");
      assert.ok(!content.includes("[model.myrouter]"), "Our section should be removed");
