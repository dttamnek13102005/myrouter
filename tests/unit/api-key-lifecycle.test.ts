...
test.after(() => {
  fs.rmSync(TEST_DATA_DIR, { recursive: true, force: true });
  if (ORIGINAL_MYROUTER_API_KEY === undefined) delete process.env.MYROUTER_API_KEY;
  else process.env.MYROUTER_API_KEY = ORIGINAL_MYROUTER_API_KEY;
...
test("validateApiKey accepts configured environment API keys", async () => {
  process.env.MYROUTER_API_KEY = "sk-env-lifecycle-test";
