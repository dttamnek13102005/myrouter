...
test("next config declares Turbopack aliases, runtime assets and server externals", async () => {
  ...
  // #6344: the @/mitm/manager stub alias is OPT-IN (MYROUTER_MITM_STUB=1, Docker only).
...
test("Turbopack aliases @/mitm/manager to the stub ONLY when MYROUTER_MITM_STUB=1 (#6344)", async () => {
  const original = process.env.MYROUTER_MITM_STUB;
  try {
    delete process.env.MYROUTER_MITM_STUB;
    const { default: def } = await loadNextConfig("mitm-default");
    assert.equal(def.turbopack.resolveAlias["@/mitm/manager"], undefined);

    process.env.MYROUTER_MITM_STUB = "1";
    ...
    if (original === undefined) delete process.env.MYROUTER_MITM_STUB;
    else process.env.MYROUTER_MITM_STUB = original;
  }
});
...
test("optimizePackageImports excludes the internal @myrouter/open-sse workspace (build-OOM guard)", async () => {
  // Regression guard: adding the internal `@myrouter/open-sse` workspace to
  ...
  assert.ok(
    !list.includes("@myrouter/open-sse"),
    "do NOT add the internal @myrouter/open-sse workspace to optimizePackageImports — it OOMs the production build"
  );
