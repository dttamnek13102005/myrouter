...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-lan-guard-models-"));
...
const originalAllowPrivateProviderUrls = process.env.MYROUTER_ALLOW_PRIVATE_PROVIDER_URLS;
const originalAllowLocalProviderUrls = process.env.MYROUTER_ALLOW_LOCAL_PROVIDER_URLS;

async function resetStorage() {
  globalThis.fetch = originalFetch;
  if (originalAllowPrivateProviderUrls === undefined) {
    delete process.env.MYROUTER_ALLOW_PRIVATE_PROVIDER_URLS;
  } else {
    process.env.MYROUTER_ALLOW_PRIVATE_PROVIDER_URLS = originalAllowPrivateProviderUrls;
  }
  if (originalAllowLocalProviderUrls === undefined) {
    delete process.env.MYROUTER_ALLOW_LOCAL_PROVIDER_URLS;
  } else {
    process.env.MYROUTER_ALLOW_LOCAL_PROVIDER_URLS = originalAllowLocalProviderUrls;
  }
...
test("#6939: getProviderOutboundGuard() and getProviderValidationGuard() agree for LAN hosts under the default local-first setting", () => {
  // Default settings: MYROUTER_ALLOW_LOCAL_PROVIDER_URLS unset (ON by default),
  // MYROUTER_ALLOW_PRIVATE_PROVIDER_URLS unset (OFF by default).
  delete process.env.MYROUTER_ALLOW_LOCAL_PROVIDER_URLS;
  delete process.env.MYROUTER_ALLOW_PRIVATE_PROVIDER_URLS;
...
test("#6939: LAN model-list fetch is still blocked when the local-first default is explicitly disabled", async () => {
  delete process.env.MYROUTER_ALLOW_PRIVATE_PROVIDER_URLS;
  process.env.MYROUTER_ALLOW_LOCAL_PROVIDER_URLS = "false";
