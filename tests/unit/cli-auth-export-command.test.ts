...
function createTempDataDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-cli-auth-export-"));
}
...
test("auth export --format env emits MYROUTER_<PROVIDER>_<FIELD>=<value> lines", async () => {
  await withAuthExportEnv(async (_dataDir, dbPath) => {
    process.env.STORAGE_ENCRYPTION_KEY = TEST_KEY;
    const { encryptCredential } = await import("../../bin/cli/encryption.mjs");
    ...
    assert.match(output, new RegExp(`MYROUTER_OPENAI_API_KEY=${PLAINTEXT_API_KEY}`));
  });
});
