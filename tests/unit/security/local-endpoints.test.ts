...
test("isLocalRequestAllowed: allows when no headers injected and not production", () => {
  ...
  const prevEnabled = process.env.MYROUTER_LOCAL_ENDPOINTS_ENABLED;
  delete process.env.NODE_ENV;
  delete process.env.MYROUTER_LOCAL_ENDPOINTS_ENABLED;
  reset();
  try {
    ...
    if (prevEnabled !== undefined) process.env.MYROUTER_LOCAL_ENDPOINTS_ENABLED = prevEnabled;
  }
});

test("isLocalRequestAllowed: allows loopback host + empty xff (browser dev path)", () => {
  const prevNodeEnv = process.env.NODE_ENV;
  const prevToken = process.env.MYROUTER_LOCAL_ENDPOINTS_TOKEN;
  delete process.env.NODE_ENV;
  delete process.env.MYROUTER_LOCAL_ENDPOINTS_TOKEN;
  ...
  try {
    ...
    reset();
    if (prevNodeEnv !== undefined) process.env.NODE_ENV = prevNodeEnv;
    if (prevToken !== undefined) process.env.MYROUTER_LOCAL_ENDPOINTS_TOKEN = prevToken;
  }
});
...
test("isLocalRequestAllowed: bearer token takes precedence over host check (desktop app)", () => {
  const prevNodeEnv = process.env.NODE_ENV;
  const prevToken = process.env.MYROUTER_LOCAL_ENDPOINTS_TOKEN;
  process.env.MYROUTER_LOCAL_ENDPOINTS_TOKEN = "s3cret-token-abc";
  delete process.env.NODE_ENV;
...
test("isLocalRequestAllowed: production WITH opt-in allows (no headers path)", () => {
  ...
  const prevEnabled = process.env.MYROUTER_LOCAL_ENDPOINTS_ENABLED;
  process.env.NODE_ENV = "production";
  process.env.MYROUTER_LOCAL_ENDPOINTS_ENABLED = "1";
  reset();
  try {
    const out = isLocalRequestAllowed();
    assert.equal(out.allowed, true);
