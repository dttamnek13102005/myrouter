 ...
 * overwrite accessToken / refreshToken unless MYROUTER_CLOUD_SYNC_SECRETS=true.
...
test("verifyCloudSignature accepts a valid HMAC", async () => {
  ...
  const TEST_HMAC_KEY = crypto.createHash("sha256").update("myrouter-test").digest("hex");
  process.env.MYROUTER_CLOUD_SYNC_SECRET = TEST_HMAC_KEY;
...
test("verifyCloudSignature falls through (legacy mode) when secret is unset", async () => {
  delete process.env.MYROUTER_CLOUD_SYNC_SECRET;
  // Force re-import so module constants pick up the cleared env.
  delete (globalThis as any).__myrouter_cloudSync_cache;
  const mod = await import("../../../src/lib/cloudSync.ts");
  const body = JSON.stringify({ data: { providers: {} } });
