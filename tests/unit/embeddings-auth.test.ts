...
test("isValidApiKey", async (t) => {
  const originalEnv = process.env.MYROUTER_API_KEY;

  await t.test("should return true if key matches MYROUTER_API_KEY", async () => {
    process.env.MYROUTER_API_KEY = "test-key";
...
test("POST /v1/embeddings authentication", async (t) => {
  ...
  process.env.MYROUTER_API_KEY = originalOmniKey;
});
