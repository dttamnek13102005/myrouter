...
test("6A.8: discoverManifests finds root and workspace package.json files", () => {
  ...
  assert.ok(
    manifests.includes("@myrouter/opencode-plugin/package.json"),
    "@myrouter/opencode-plugin/package.json must be included"
  );
  assert.ok(
    manifests.includes("@myrouter/opencode-provider/package.json"),
    "@myrouter/opencode-provider/package.json must be included"
  );
});
