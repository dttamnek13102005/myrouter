...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-media-cost-h-"));
...
// X-MyRouter-* cost telemetry headers (parity with chat/embeddings).
// Cost may legitimately be 0 (free / unpriced modality) — formatMyRouterCost
...
function assertCostTelemetryHeaders(response: Response) {
  ...
  const cost = response.headers.get(MYROUTER_RESPONSE_HEADERS.responseCost);
  ...
  const version = response.headers.get(MYROUTER_RESPONSE_HEADERS.version);
  assert.ok(version && version.trim().length > 0, "version header must be non-empty");

  const provider = response.headers.get(MYROUTER_RESPONSE_HEADERS.provider);
...
test("rerank handler success Response carries cost telemetry headers", async () => {
  ...
  assert.equal(
    response.headers.get(MYROUTER_RESPONSE_HEADERS.provider),
    "cohere",
...
test("rerank NVIDIA-format success Response reflects synthesized search unit in cost header", async () => {
  ...
  assert.equal(
    response.headers.get(MYROUTER_RESPONSE_HEADERS.responseCost),
    "0.0020000000",
