...
test("runFabricatedDocsCheck: index contains real MyRouter routes", () => {
  const result = runFabricatedDocsCheck();
  ...
  const found = findingsFor({
    files: {
      "open-sse/config/constants.ts":
        'const t = envInt("MYROUTER_CIRCUIT_BREAKER_OAUTH_THRESHOLD", 8);\n',
    },
    docs: { "cfg.md": "Override with `MYROUTER_CIRCUIT_BREAKER_OAUTH_THRESHOLD`.\n" },
  });
  assert.ok(
    !found.has("env-var::MYROUTER_CIRCUIT_BREAKER_OAUTH_THRESHOLD"),
...
test("cli-cmd: an arg-bearing `.command('connect <host>')` registration is NOT flagged", () => {
  ...
  const found = findingsFor({
    ...
    docs: { "guides/remote.md": "You log in once with `myrouter connect <host>`.\n" },
  });
  assert.ok(
    !found.has("cli-cmd::myrouter connect"),
...
test("ANTI-OVER-SUPPRESSION: an unregistered subcommand IS still flagged", () => {
  ...
  const found = findingsFor({
    ...
    docs: { "guides/remote.md": "Then run `myrouter teleport <host>` to finish.\n" },
  });
  assert.ok(
    found.has("cli-cmd::myrouter teleport"),
