...
// `X-MyRouter-Compression: off` so a globally-enabled Output Style (e.g. "Ultra terse") never
...
test("buildInternalChatRequest sends X-MyRouter-Compression: off", () => {
  const controller = new AbortController();
  const request = buildInternalChatRequest({ model: "openai/gpt-4" }, controller.signal);
  assert.equal(request.headers.get("X-MyRouter-Compression"), "off");
  assert.equal(request.headers.get("X-MyRouter-No-Cache"), "true");
});

test("buildInternalRerankRequest sends X-MyRouter-Compression: off", () => {
  const controller = new AbortController();
