 ...
 *
 * Resolution order:
 *   1. MYROUTER_SERVER_HOST (new dedicated var — always wins)
...
// --- MYROUTER_SERVER_HOST takes precedence ---

test("serve hostname: MYROUTER_SERVER_HOST takes precedence over everything", () => {
  assert.equal(resolveHostname("127.0.0.1", "myhostname", "myhostname"), "127.0.0.1");
});

test("serve hostname: MYROUTER_SERVER_HOST overrides an explicit HOSTNAME", () => {
...
test("serve hostname: MYROUTER_SERVER_HOST empty string falls through to HOSTNAME check", () => {
...
test("serve hostname: MYROUTER_SERVER_HOST empty string with explicit HOSTNAME", () => {
