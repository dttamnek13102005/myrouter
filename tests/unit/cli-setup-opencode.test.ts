 ...
 *
 * `myrouter setup opencode` wires the bundled @myrouter/opencode-plugin into a
 * local OpenCode install: copies the built plugin into `<config>/plugins/myrouter/`
 * and registers a tuple entry in `opencode.json` (idempotent, replacing the legacy
 * `opencode-myrouter-auth` entry from issue #3711).
 *
 * The command resolves the bundled plugin at module load, so the
 * MYROUTER_OPENCODE_PLUGIN_DIR fixture override MUST be set before the import.
...
process.env.MYROUTER_OPENCODE_PLUGIN_DIR = FAKE_PLUGIN_DIR;
...
function makeFakePluginDist() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@myrouter/opencode-plugin", version: "0.0.0-test" })
  );
...
describe("myrouter setup opencode", () => {
  before(() => {
    console.log = () => {};
    console.info = () => {};
    console.warn = () => {};
  ...
  it("installs the plugin and registers a tuple entry honouring --base-url (camelCased by Commander)", async () => {
    ...
    assert.ok(fs.existsSync(path.join(CONFIG_DIR, "plugins", "myrouter", "dist", "index.js")));
    assert.ok(fs.existsSync(path.join(CONFIG_DIR, "plugins", "myrouter", "package.json")));

    const cfg = readConfig();
    assert.ok(Array.isArray(cfg.plugin));
    assert.equal(cfg.plugin.length, 1);
    const [modulePath, options] = cfg.plugin[0];
    assert.equal(modulePath, "./plugins/myrouter/dist/index.js");
    assert.equal(options.providerId, "myrouter");
    assert.equal(
      options.baseURL,
      "http://10.0.0.5:20128",
  ...
  it("is idempotent: re-running updates the entry in place instead of duplicating it", async () => {
    ...
    const omniEntries = cfg.plugin.filter(
      (p: unknown) =>
        Array.isArray(p) && (p[1] as { providerId?: string })?.providerId === "myrouter"
    );
  ...
  it("removes the legacy opencode-myrouter-auth entry (#3711) and preserves unrelated plugins", async () => {
    const cfgPath = path.join(CONFIG_DIR, "opencode.json");
    fs.writeFileSync(
      cfgPath,
      JSON.stringify({
        plugin: [
          "opencode-myrouter-auth",
    ...
    assert.ok(!flat.includes("opencode-myrouter-auth"), "legacy entry must be dropped");
    assert.ok(flat.includes('"providerId":"other"'), "unrelated plugin entries must survive");
    assert.equal(cfg.plugin.length, 2, "other + myrouter");
  });
