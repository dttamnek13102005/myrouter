...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "myrouter-lang-test-"));
  origDataDir = process.env.DATA_DIR;
  origOmniLang = process.env.MYROUTER_LANG;
  process.env.DATA_DIR = tmpDir;
  delete process.env.MYROUTER_LANG;
});

test.after(() => {
  if (origDataDir === undefined) delete process.env.DATA_DIR;
  else process.env.DATA_DIR = origDataDir;
  if (origOmniLang === undefined) delete process.env.MYROUTER_LANG;
  else process.env.MYROUTER_LANG = origOmniLang;
  try {
...
test("runConfigLangSetCommand salva locale no .env e chama setLocale imediatamente", async () => {
  ...
  try {
    ...
    assert.ok(content.includes("MYROUTER_LANG=pt-BR"), "Deve persistir MYROUTER_LANG=pt-BR");
...
test("runConfigLangSetCommand --force salva mesmo quando locale já ativo", async () => {
  ...
  try {
    ...
    if (existsSync(envPath)) {
      const content = readFileSync(envPath, "utf8");
      assert.ok(content.includes("MYROUTER_LANG=en"), "Deve ter gravado MYROUTER_LANG=en");
    }
  } finally {
    console.log = origLog;
    resetForTests();
...
test("runConfigLangSetCommand atualiza chave sem duplicar quando já existe", async () => {
  ...
  const envPath = join(tmpDir, ".env");
  writeFileSync(envPath, "MYROUTER_LANG=de\n", "utf8");
  ...
  try {
    ...
    assert.equal(exitCode, 0);
    const content = readFileSync(envPath, "utf8");
    assert.ok(content.includes("MYROUTER_LANG=pt-BR"), "Deve ter atualizado para pt-BR");
    const matches = content.match(/MYROUTER_LANG=/g);
    assert.equal(matches?.length, 1, "Deve ter exatamente uma ocorrência de MYROUTER_LANG");
  } finally {
    console.log = origLog;
    resetForTests();
  }
