...
const {
  ...
  MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME,
...
// myrouter_web_search tool must use the FLAT function shape ({ type, name }), not the
...
test("#2390 web_search fallback is FLAT for Responses API target", () => {
  ...
  assert.equal(injected.name, MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME);
  assert.equal(
    injected.function,
    undefined,
...
test("#2390 web_search fallback stays NESTED for Chat Completions target", () => {
  ...
  assert.equal(fn?.name, MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME);
  assert.equal(
    injected.name,
    undefined,
...
test("#2390 tool_choice matches the injected tool shape per target format", () => {
  ...
  assert.equal(rChoice.name, MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME);
  ...
  assert.equal(cFn?.name, MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME);
});
...
// the tool has to be converted to the myrouter_web_search function fallback (which the
...
test("OpenAI -> Claude (non-passthrough): built-in web_search IS still converted", () => {
  ...
  assert.equal(fallback.enabled, true);
  assert.equal(fallback.toolName, MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME);
  ...
  assert.ok(toolNames.includes(MYROUTER_WEB_SEARCH_FALLBACK_TOOL_NAME));
});
