...
import { resolveAutoStrategyOrder } from "@myrouter/open-sse/services/combo/resolveAutoStrategy.ts";
...
// also supports a per-request `X-MyRouter-Mode` override (relayOptions.mode) that can
...
test("per-request X-MyRouter-Mode override changes the EFFECTIVE weights used for fallback ranking, not just selection", async () => {
