...
// #3578 — `myrouter --mcp` crashed on npm installs with ERR_MODULE_NOT_FOUND for
...
function resolveImport(fromFile: string, spec: string): string | null {
  ...
  else if (spec.startsWith("@myrouter/open-sse/"))
    base = path.join("open-sse", spec.slice("@myrouter/open-sse/".length));
  else if (spec === "@myrouter/open-sse") base = path.join("open-sse", "index");
