...
test("cliToken.mjs pode ser importado sem erro", async () => {
  ...
  assert.equal(mod.CLI_TOKEN_HEADER, "x-myrouter-cli-token");
});
...
test("MYROUTER_CLI_TOKEN env sobrescreve token gerado em apiFetch", async () => {
  const orig = process.env.MYROUTER_CLI_TOKEN;
  process.env.MYROUTER_CLI_TOKEN = "test-override-token-12345";
  try {
    // Re-import api.mjs não funciona por cache ESM — validamos apenas que env é lido.
    assert.equal(process.env.MYROUTER_CLI_TOKEN, "test-override-token-12345");
  } finally {
    if (orig === undefined) delete process.env.MYROUTER_CLI_TOKEN;
    else process.env.MYROUTER_CLI_TOKEN = orig;
  }
});
...
test("token derivado de machine-id diferente produz hash diferente", () => {
  const SALT = "myrouter-cli-auth-v1";
...
test("MYROUTER_DISABLE_CLI_TOKEN desabilita auth (estrutura verificada)", async () => {
  const { readFileSync } = await import("node:fs");
  const { join, dirname } = await import("node:path");
  const { fileURLToPath } = await import("node:url");
  const dir = dirname(fileURLToPath(import.meta.url));
  const src = readFileSync(join(dir, "../../src/lib/middleware/cliTokenAuth.ts"), "utf8");
  assert.ok(src.includes("MYROUTER_DISABLE_CLI_TOKEN"));
});
