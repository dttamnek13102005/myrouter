...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-token-healthcheck-"));
...
test("checkConnection skips providers listed in MYROUTER_HEALTHCHECK_SKIP_PROVIDERS (#kimi-15)", async () => {
  await resetStorage();

  const providerId = "custom-oauth-skip-list";
  const refreshRequests: string[] = [];
  const prevSkip = process.env.MYROUTER_HEALTHCHECK_SKIP_PROVIDERS;
    ...
    async (tokenServer) => {
      await withPatchedProvider(
        ...
        async () => {
          ...
          process.env.MYROUTER_HEALTHCHECK_SKIP_PROVIDERS = `foo, ${providerId} ,bar`;
          await tokenHealthCheck.checkConnection(connection);
          assert.equal(
          ...
          process.env.MYROUTER_HEALTHCHECK_SKIP_PROVIDERS = "some-other-provider";
  ...
  if (prevSkip === undefined) delete process.env.MYROUTER_HEALTHCHECK_SKIP_PROVIDERS;
  else process.env.MYROUTER_HEALTHCHECK_SKIP_PROVIDERS = prevSkip;
});
