 ...
 *
 * MyRouter funnels every privileged MITM command through the single choke
 ...
 * feature adds a third opt-out trigger — the `MYROUTER_NO_SUDO` env flag — so
 * MyRouter starts cleanly in a root-less / user-namespace deployment where
...
test(
  "resolveSudoSpawn: with MYROUTER_NO_SUDO=1, strips `sudo -S` and needs no password",
  () => {
    const prev = process.env.MYROUTER_NO_SUDO;
    process.env.MYROUTER_NO_SUDO = "1";
    try {
      ...
      if (prev === undefined) delete process.env.MYROUTER_NO_SUDO;
      else process.env.MYROUTER_NO_SUDO = prev;
    }
  }
);
...
test(
  ...
  () => {
    const prev = process.env.MYROUTER_NO_SUDO;
    delete process.env.MYROUTER_NO_SUDO;
    try {
      const { finalCommand, finalArgs, stripSudo, needsPassword } = resolveSudoSpawn(
        "sudo",
        ["-S", "cp", "/tmp/ca.pem", "/usr/local/share/ca-certificates/ca.crt"],
...
test("resolveSudoSpawn: a non-sudo command is never touched, regardless of the flag", () => {
  ...
  try {
    ...
    assert.deepEqual(finalArgs, ["-A", "-n", "MyRouter"]);
  } finally {
    if (prev === undefined) delete process.env.MYROUTER_NO_SUDO;
    else process.env.MYROUTER_NO_SUDO = prev;
  }
...
test("isNoSudoEnv: truthiness matrix matches isTruthyEnvFlag semantics", () => {
  ...
  try {
    for (const [value, expected] of cases) {
      if (value === undefined) delete process.env.MYROUTER_NO_SUDO;
      else process.env.MYROUTER_NO_SUDO = value;
      assert.equal(isNoSudoEnv(), expected, `MYROUTER_NO_SUDO=${JSON.stringify(value)}`);
    }
  } finally {
    if (prev === undefined) delete process.env.MYROUTER_NO_SUDO;
    else process.env.MYROUTER_NO_SUDO = prev;
...
test("resolveSudoSpawn: env flag alone (no overrides) drives stripping on non-root sudo host", () => {
  ...
  const prev = process.env.MYROUTER_NO_SUDO;
  process.env.MYROUTER_NO_SUDO = "yes";
  try {
