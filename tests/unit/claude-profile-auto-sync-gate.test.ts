...
// either gate is closed. These tests pin the two gates (the MYROUTER_AUTO_SYNC_CLAUDE_PROFILES
...
const GATE_ENV = ["MYROUTER_AUTO_SYNC_CLAUDE_PROFILES", "CLI_ALLOW_CONFIG_WRITES"] as const;
...
test("Claude auto-sync is OFF by default (flag unset) — returns disabled, never fetches or writes", async () => {
  ...
  try {
    delete process.env.MYROUTER_AUTO_SYNC_CLAUDE_PROFILES;
...
test("explicit MYROUTER_AUTO_SYNC_CLAUDE_PROFILES=false stays disabled", async () => {
  const snap = snapshotEnv();
  try {
    process.env.MYROUTER_AUTO_SYNC_CLAUDE_PROFILES = "false";
    const result = await autoSyncClaudeProfilesFromLiveCatalog(
      mockSyncRequest(),
      "test:explicit-false"
    );
...
test("non-truthy flag values stay disabled", async () => {
  ...
  try {
    for (const v of ["0", "no", "off", "maybe", ""]) {
      process.env.MYROUTER_AUTO_SYNC_CLAUDE_PROFILES = v;
...
test("enabled flag but CLI_ALLOW_CONFIG_WRITES=false is blocked by the write-guard (no write)", async () => {
  const snap = snapshotEnv();
  try {
    process.env.MYROUTER_AUTO_SYNC_CLAUDE_PROFILES = "true";
