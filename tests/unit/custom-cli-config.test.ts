...
test("buildAliasEnvVar sanitizes aliases for env variable export", () => {
  assert.equal(buildAliasEnvVar("review"), "MYROUTER_MODEL_REVIEW");
  assert.equal(buildAliasEnvVar("plan mode"), "MYROUTER_MODEL_PLAN_MODE");
...
test("custom CLI generators include default model and alias mappings", () => {
  ...
  assert.match(envScript, /export OPENAI_MODEL="myrouter\/fast"/);
  assert.match(envScript, /export MYROUTER_MODEL_REVIEW="cc\/claude-sonnet-4-5-20250929"/);
