...
//
// MYROUTER_SKIP_SYSTEM_TRUST=1 (set globally in tests/_setup/isolateDataDir.ts)
...
test("isolateDataDir setup exports the system-trust guard for every test process", () => {
  assert.equal(process.env.MYROUTER_SKIP_SYSTEM_TRUST, "1");
});
...
test("installCert under the guard skips the OS mutation but keeps input contracts", async () => {
  ...
  await assert.rejects(() => installCert("", "/nonexistent/myrouter-guard-test.pem"));
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-trust-guard-"));
  const pem = path.join(dir, "myrouter-guard-test.pem");
  fs.writeFileSync(
    pem,
