...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-model-sync-"));
...
test("modelSyncScheduler resolves only loopback origins and uses the dashboard port", async () => {
  const previous = {
    MYROUTER_PORT: process.env.MYROUTER_PORT,
    PORT: process.env.PORT,
    DASHBOARD_PORT: process.env.DASHBOARD_PORT,
    BASE_URL: process.env.BASE_URL,
    NEXT_PUBLIC_BASE_URL: process.env.NEXT_PUBLIC_BASE_URL,
    NEXT_PUBLIC_APP_URL: process.env.NEXT_PUBLIC_APP_URL,
    MYROUTER_BASE_PATH: process.env.MYROUTER_BASE_PATH,
    MYROUTER_INTERNAL_SCHEME: process.env.MYROUTER_INTERNAL_SCHEME,
    MYROUTER_TLS_CERT: process.env.MYROUTER_TLS_CERT,
    MYROUTER_TLS_KEY: process.env.MYROUTER_TLS_KEY,
  };
  process.env.MYROUTER_PORT = "20128";
  process.env.PORT = "22128";
  process.env.DASHBOARD_PORT = "22128";
  process.env.BASE_URL = "https://attacker.example";
  delete process.env.NEXT_PUBLIC_BASE_URL;
  delete process.env.NEXT_PUBLIC_APP_URL;
  process.env.MYROUTER_BASE_PATH = "/myrouter/";
  delete process.env.MYROUTER_INTERNAL_SCHEME;
  delete process.env.MYROUTER_TLS_CERT;
  delete process.env.MYROUTER_TLS_KEY;

  try {
    const scheduler = await loadScheduler("trusted-loopback-origin");
    assert.equal(scheduler.getModelSyncInternalBaseUrl(), "http://127.0.0.1:22128/myrouter");
    assert.equal(
      scheduler.resolveModelSyncInternalBaseUrl("https://attacker.example/steal"),
      "http://127.0.0.1:22128/myrouter"
    );
    assert.equal(
...
test("modelSyncScheduler uses the listener-declared TLS scheme without trusting candidates", async () => {
  ...
  delete process.env.NEXT_PUBLIC_APP_URL;
  process.env.MYROUTER_BASE_PATH = "/myrouter";
  process.env.MYROUTER_INTERNAL_SCHEME = "https";
  delete process.env.MYROUTER_TLS_CERT;
  delete process.env.MYROUTER_TLS_KEY;
  ...
  try {
    ...
    assert.equal(
      ...
      "https://localhost:22128/myrouter"
    );
    assert.equal(
...
test("runtime launchers publish the actual internal listener scheme", () => {
  ...
  assert.match(runNext, /MYROUTER_INTERNAL_SCHEME\s*=\s*["']http["']/);
  assert.match(standalone, /MYROUTER_INTERNAL_SCHEME\s*=\s*tlsOptions\s*\?\s*["']https["']/);
});
...
test("initCloudSync skips auto initialization during build and test processes unless explicitly re-enabled", () => {
  ...
  assert.equal(
    initCloudSync.shouldSkipCloudSyncInitialization(
      {
        NODE_ENV: "test",
        MYROUTER_ENABLE_RUNTIME_BACKGROUND_TASKS: "1",
      },
      ["node", "--test"]
    ),
    false
