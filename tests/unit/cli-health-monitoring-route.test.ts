...
// Regression test for GH #6677: `myrouter health` calls GET /api/health, but the
...
test.before(async () => {
  ...
  process.env.MYROUTER_BASE_URL = baseUrl;
});
...
test("GH #6677: myrouter health should succeed against a server that only implements /api/monitoring/health", async () => {
