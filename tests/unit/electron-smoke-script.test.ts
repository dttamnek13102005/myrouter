...
test("electron smoke discovers the default Linux executable name", () => {
  assert.ok(LINUX_EXECUTABLE_NAMES.includes("myrouter-desktop"));
});

test("electron smoke env allowlists runtime variables and drops secrets", () => {
  const env = buildSmokeEnv({
    currentPlatform: "linux",
    dataDir: "/tmp/myrouter-electron-smoke-test",
    parentEnv: {
      DISPLAY: ":99",
  ...
  assert.equal(env.DATA_DIR, "/tmp/myrouter-electron-smoke-test");
  assert.equal(env.DISPLAY, ":99");
  assert.equal(env.PATH, "/usr/bin");
  assert.equal(env.HOME, "/tmp/myrouter-electron-smoke-test/home");
  assert.equal(env.XDG_CONFIG_HOME, "/tmp/myrouter-electron-smoke-test/config");
