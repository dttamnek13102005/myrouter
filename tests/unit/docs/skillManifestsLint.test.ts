...
// coletava tests/unit/docs/) e apodreceu — filtrava dirs `myrouter*`, mas os skills
...
// invariante de uso é "referencia as env vars ($MYROUTER_URL/MYROUTER_KEY) OU
// comandos da CLI (`myrouter …`)" — 3 skills (health/keys/batches) usam só a CLI.
...
test("each skill dir has SKILL.md with frontmatter", async () => {
  ...
  for (const dir of dirs) {
    ...
    if (!dir.startsWith("omni-")) {
      assert.ok(
        content.includes("MYROUTER_") || content.includes("myrouter "),
        `${dir}: missing usage references (MYROUTER_* env vars or myrouter CLI commands)`
      );
    }
  }
});
