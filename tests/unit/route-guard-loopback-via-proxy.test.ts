/**
 * Security regression: when MyRouter itself runs behind an external reverse
...
test("classifyStampedPeerLocality: loopback socket WITH a proxy stamp is REMOTE (fail closed)", () => {
  // MyRouter is behind nginx/Caddy/Cloudflare; the socket peer is the proxy.
...
test("classifyStampedPeerLocality: private-LAN socket WITH a proxy stamp is still REMOTE", () => {
  // Caddy/nginx running on a LAN box in front of MyRouter. We do not know how
