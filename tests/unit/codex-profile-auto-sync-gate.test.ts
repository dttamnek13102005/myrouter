...
const GATE_ENV = ["MYROUTER_AUTO_SYNC_CODEX_PROFILES", "CLI_ALLOW_CONFIG_WRITES"] as const;
...
test("auto-sync is OFF by default (flag unset) — returns disabled, never fetches or writes", async () => {
  ...
  try {
    delete process.env.MYROUTER_AUTO_SYNC_CODEX_PROFILES;
...
test("explicit MYROUTER_AUTO_SYNC_CODEX_PROFILES=false stays disabled", async () => {
  const snap = snapshotEnv();
  try {
    process.env.MYROUTER_AUTO_SYNC_CODEX_PROFILES = "false";
    const result = await autoSyncCodexProfilesFromLiveCatalog(
      mockSyncRequest(),
      "test:explicit-false"
    );
...
test("non-truthy flag values ('0', 'no', 'off', 'maybe') stay disabled", async () => {
  ...
  try {
    for (const v of ["0", "no", "off", "maybe", ""]) {
      process.env.MYROUTER_AUTO_SYNC_CODEX_PROFILES = v;
...
test("enabled flag but CLI_ALLOW_CONFIG_WRITES=false is blocked by the write-guard (no write)", async () => {
  const snap = snapshotEnv();
  try {
    process.env.MYROUTER_AUTO_SYNC_CODEX_PROFILES = "true";
