...
process.env.DATA_DIR = path.join(os.tmpdir(), "myrouter-check-tool-test");
...
async function writeTempFile(filename: string, content: string): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "myrouter-clicheck-"));
...
async function writeCodexConfig(opts: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "myrouter-codex-"));
  const configPath = path.join(tmpDir, "config.toml");

  const tomlContent = opts.hasMyRouter
...
test("codex: returns 'configured' when TOML has MyRouter URL + valid auth key", async () => {
  const configPath = await writeCodexConfig({
    hasMyRouter: true,
    authApiKey: "sk_myrouter_testkey_1234567890abcdef",
  });
  const result = await checkToolConfigStatus("codex", configPath);
  assert.equal(result, "configured");
});

test("codex: returns 'not_configured' when TOML has MyRouter URL but auth key is masked", async () => {
  const configPath = await writeCodexConfig({
    hasMyRouter: true,
    authApiKey: "sk_****",
  });
...
test("codex: returns 'not_configured' when TOML does not mention MyRouter", async () => {
  const configPath = await writeCodexConfig({ hasMyRouter: false });
  const result = await checkToolConfigStatus("codex", configPath);
  assert.equal(result, "not_configured");
});
...
test("hermes: returns 'configured' when config contains MyRouter", async () => {
  const configPath = await writeTempFile(
    "hermes.toml",
    `[openai]\nbase_url = "http://localhost:20128/v1"\n`
  );
...
test("droid: returns 'configured' when JSON config contains sk_myrouter marker", async () => {
  const configPath = await writeTempFile(
    "droid.json",
    JSON.stringify({ apiKey: "sk_myrouter_somekey", baseUrl: "http://localhost:20128/v1" })
  );
  const result = await checkToolConfigStatus("droid", configPath);
  assert.equal(result, "configured");
});

test("openclaw: returns 'configured' when JSON config contains myrouter text", async () => {
  const configPath = await writeTempFile(
    "openclaw.json",
    JSON.stringify({ openAiBaseUrl: "http://myrouter.local/v1", openAiApiKey: "sk-test" })
  );
...
test("kilo: returns 'not_configured' when no MyRouter markers present", async () => {
  const configPath = await writeTempFile(
    "kilo.json",
...
test("qwen: returns 'configured' only for an MyRouter-managed model entry", async () => {
  const configPath = await writeTempFile(
    "settings.json",
    JSON.stringify({
      modelProviders: {
