...
test("getDbInstance() eventually caps a persistently-OOMing sql.js probe (#6835)", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-6835-oom-"));
  ...
  (
    globalThis as unknown as { __myrouterSqlJsAdapters: Map<string, unknown> }
  ).__myrouterSqlJsAdapters = new Map([[sqliteFile, fakeAdapter]]);
  const errors: string[] = [];
  for (let i = 0; i < 8; i++) {
    try {
      core.getDbInstance();
