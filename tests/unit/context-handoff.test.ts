...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-context-handoff-"));
...
test("maybeGenerateHandoff persists a structured handoff once the threshold is reached", async () => {
  ...
  assert.equal(calls[0].body._myrouterSkipContextRelay, true);
  assert.equal(calls[0].body._myrouterInternalRequest, "context-handoff");
});
