...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-api-auth-"));
...
test("getApiKeyMetadata recognizes MYROUTER_API_KEY environment variable", async () => {
  const envKey = "sk-test-env-key-" + Date.now();
  process.env.MYROUTER_API_KEY = envKey;
...
test("requireManagementAuth returns null for MYROUTER_API_KEY env passthrough", async () => {
  await setupAuth();
