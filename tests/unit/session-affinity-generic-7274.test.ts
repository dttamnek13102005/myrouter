 ...
 *    `x-session-id`, `x-myrouter-session`) are forwarded upstream by
 *    providers that use custom executors with no client-header passthrough.
 *    `x-codex-session-id` and `x-myrouter-session` are never forwarded by
...
// the real ~/.myrouter data dir instead of this test's isolated tmp dir.

const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-session-affinity-7274-"));
...
test("#7274 x-codex-session-id and x-myrouter-session are never forwarded upstream by DefaultExecutor", () => {
  ...
  const headers = executor.buildHeaders({ accessToken: "test-key" }, true, {
    ...
    "x-myrouter-session": "another-internal-correlation-id",
  ...
  assert.ok(
    !lowerKeys.includes("x-myrouter-session"),
    "x-myrouter-session must never reach the upstream request"
  );
});
