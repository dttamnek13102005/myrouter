...
// escape hatch (MYROUTER_USE_TURBOPACK=0) for environments that hit native
...
test("resolveNextBuildBundlerFlag keeps --turbopack for explicit opt-in", () => {
  assert.equal(
    buildIsolated.resolveNextBuildBundlerFlag({ MYROUTER_USE_TURBOPACK: "1" }),
    "--turbopack"
  );
});
...
test("resolveNextBuildBundlerFlag honors the webpack escape hatch (=0)", () => {
  assert.equal(
    buildIsolated.resolveNextBuildBundlerFlag({ MYROUTER_USE_TURBOPACK: "0" }),
    "--webpack"
  );
});
...
test("resolveNextBuildBundlerFlag treats other values as the turbopack default", () => {
  ...
  assert.equal(
    buildIsolated.resolveNextBuildBundlerFlag({ MYROUTER_USE_TURBOPACK: "yes" }),
    "--turbopack"
  );
});
