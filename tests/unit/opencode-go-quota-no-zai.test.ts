...
test("getOpenCodeGoUsage does not send the user's OpenCode Go API key to api.z.ai by default", async () => {
  ...
  const originalEnv = process.env.MYROUTER_OPENCODE_GO_QUOTA_URL;
  ...
  try {
    ...
    if (originalEnv === undefined) delete process.env.MYROUTER_OPENCODE_GO_QUOTA_URL;
    else process.env.MYROUTER_OPENCODE_GO_QUOTA_URL = originalEnv;
  }
});
