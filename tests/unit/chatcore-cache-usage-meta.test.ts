...
// shapes (null when no cache fields), and the _myrouter meta attachment/merge.
...
test("attachLogMeta merges compact meta into _myrouter", () => {
  const out = attachLogMeta({ a: 1, _myrouter: { keep: true } }, { added: 2, drop: null });
  assert.deepEqual(out, { a: 1, _myrouter: { keep: true, added: 2 } });
});

test("attachLogMeta wraps non-object payloads", () => {
  const out = attachLogMeta(null, { added: 2 });
  assert.deepEqual(out, { _myrouter: { added: 2 }, _payload: null });
});
