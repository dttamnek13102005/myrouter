...
test("setupPolyfill.ts is allowed in the tarball (bin/myrouter.mjs imports it at startup)", () => {
...
test("findMissingArtifactPaths flags missing root runtime files in the tarball", () => {
  ...
  // above (dist/server.js, bin/myrouter.mjs, package.json, the postinstall scripts).
