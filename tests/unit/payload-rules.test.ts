...
const ORIGINAL_PAYLOAD_RULES_PATH = process.env.MYROUTER_PAYLOAD_RULES_PATH;
const ORIGINAL_PAYLOAD_RULES_RELOAD_MS = process.env.MYROUTER_PAYLOAD_RULES_RELOAD_MS;

test.afterEach(() => {
  resetPayloadRulesConfigForTests();

  if (ORIGINAL_PAYLOAD_RULES_PATH === undefined) {
    delete process.env.MYROUTER_PAYLOAD_RULES_PATH;
  } else {
    process.env.MYROUTER_PAYLOAD_RULES_PATH = ORIGINAL_PAYLOAD_RULES_PATH;
  }

  if (ORIGINAL_PAYLOAD_RULES_RELOAD_MS === undefined) {
    delete process.env.MYROUTER_PAYLOAD_RULES_RELOAD_MS;
  } else {
    process.env.MYROUTER_PAYLOAD_RULES_RELOAD_MS = ORIGINAL_PAYLOAD_RULES_RELOAD_MS;
  }
});
...
test("payload rules load from JSON file and reload changed content", async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-payload-rules-"));
  const configPath = path.join(tempDir, "payloadRules.json");

  process.env.MYROUTER_PAYLOAD_RULES_PATH = configPath;
