...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".myrouter") });
  } finally {
...
test("bootstrapEnv prefers ~/.myrouter/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {
