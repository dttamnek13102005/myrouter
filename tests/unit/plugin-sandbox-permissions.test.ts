 ...
 *
 * Assertions cover:
 * - exec permission gates child_process behind MYROUTER_PLUGINS_ALLOW_EXEC==="1"
...
describe("pluginWorker sandbox — exec permission gating", () => {
  it("gates child_process behind MYROUTER_PLUGINS_ALLOW_EXEC === '1'", () => {
    assert.ok(
      source.includes('MYROUTER_PLUGINS_ALLOW_EXEC !== "1"') ||
        source.includes("MYROUTER_PLUGINS_ALLOW_EXEC !== '1'"),
      "exec block must check process.env.MYROUTER_PLUGINS_ALLOW_EXEC !== \"1\""
    );
  });

  it("checks the env flag inside the exec permission block", () => {
    // The env guard must appear inside the exec permission block.
    // Use the SECOND occurrence of MYROUTER_PLUGINS_ALLOW_EXEC (the first is in
    ...
    const envIdxInBlock = source.indexOf("MYROUTER_PLUGINS_ALLOW_EXEC", execIdx);
    assert.ok(
      envIdxInBlock !== -1,
      "MYROUTER_PLUGINS_ALLOW_EXEC check must appear inside the exec permission block"
    );
  });
  ...
  it("does NOT wire child_process without the env guard in place", () => {
    ...
    // The MYROUTER_PLUGINS_ALLOW_EXEC check must come before sandbox.child_process.
    const envIdx = source.indexOf("MYROUTER_PLUGINS_ALLOW_EXEC");
...
describe("pluginWorker sandbox — trust-model comment", () => {
  ...
  it("references the MYROUTER_PLUGINS_ALLOW_EXEC opt-in in the comment", () => {
    assert.ok(
      source.includes("MYROUTER_PLUGINS_ALLOW_EXEC"),
      "trust-model comment must reference MYROUTER_PLUGINS_ALLOW_EXEC"
    );
  });
});
