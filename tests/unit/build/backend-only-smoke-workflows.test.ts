...
// MYROUTER_BUILD_BACKEND_ONLY=1 so `npm run build:cli`'s fallback full build
...
function isBackendOnly(step: WorkflowStep): boolean {
  const env = step.env || {};
  return env.MYROUTER_BUILD_BACKEND_ONLY === "1" || env.MYROUTER_BUILD_PROFILE === "backend";
}
...
for (const { file, jobName, stepName } of TARGETS) {
  test(`${file} :: ${jobName} '${stepName}' step sets MYROUTER_BUILD_BACKEND_ONLY=1 (skips dashboard UI build the API-only smoke job never exercises)`, () => {
    ...
    assert.equal(
      isBackendOnly(step),
      true,
      `${file}'s '${jobName}' -> '${stepName}' step must set MYROUTER_BUILD_BACKEND_ONLY=1 or MYROUTER_BUILD_PROFILE=backend`
    );
  });
}
