...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-providers-validate-route-"));
...
test("providers validate route allows a local baseUrl by default (#5066 local-first)", async () => {
  await resetStorage();
  delete process.env.MYROUTER_ALLOW_PRIVATE_PROVIDER_URLS;
  delete process.env.MYROUTER_ALLOW_LOCAL_PROVIDER_URLS; // default ON
...
test("providers validate route allows private baseUrl values when opt-in env is enabled", async () => {
  await resetStorage();
  process.env.MYROUTER_ALLOW_PRIVATE_PROVIDER_URLS = "true";
