...
test("isCodexResponsesWebSocketRequired: MYROUTER_CODEX_WS_ENABLED=false forces HTTP even with codexTransport=websocket", () => {
  ...
  const prev = process.env.MYROUTER_CODEX_WS_ENABLED;
  process.env.MYROUTER_CODEX_WS_ENABLED = "false";
  try {
    ...
    if (prev === undefined) delete process.env.MYROUTER_CODEX_WS_ENABLED;
    else process.env.MYROUTER_CODEX_WS_ENABLED = prev;
    __setCodexWebSocketTransportForTesting(undefined);
  }
});
...
test("CodexExecutor.transformRequest preserves store-enabled responses state when explicitly enabled", () => {
  ...
  assert.equal(result._myrouterResponsesStore, undefined);
...
test("Codex internal websocket bridge rejects non-object JSON payloads", async () => {
  await withEnv({ MYROUTER_WS_BRIDGE_SECRET: "bridge-secret" }, async () => {
    const { POST } = await import("../../src/app/api/internal/codex-responses-ws/route.ts");

    const response = await POST(
      new Request("http://myrouter.local/api/internal/codex-responses-ws", {
        method: "POST",
        headers: {
          "content-type": "application/json",
          "x-myrouter-ws-bridge-secret": "bridge-secret",
        },
