...
const originalEnv = process.env.MYROUTER_ENABLE_LIVE_WS;

after(() => {
  // Restore original env so other tests in the same process are unaffected.
  if (originalEnv === undefined) {
    delete process.env.MYROUTER_ENABLE_LIVE_WS;
  } else {
    process.env.MYROUTER_ENABLE_LIVE_WS = originalEnv;
  }
});
...
describe("isLiveWsEnabled — default-ON behavior", () => {
  ...
  test('"false" → false', () => {
    process.env.MYROUTER_ENABLE_LIVE_WS = "false";
    assert.equal(isLiveWsEnabled(), false);
  });
  ...
  test('"1" → true', () => {
    process.env.MYROUTER_ENABLE_LIVE_WS = "1";
    assert.equal(isLiveWsEnabled(), true);
  });

  test('"true" → true', () => {
    process.env.MYROUTER_ENABLE_LIVE_WS = "true";
    assert.equal(isLiveWsEnabled(), true);
  });
