...
// ─── #3476: discoverability of `myrouter runtime repair` ──────────────────────
//
// When better-sqlite3's native binding mismatches the Node ABI, MyRouter prints
...
// A self-heal command already exists (`myrouter runtime repair`); these tests
...
test("serve.mjs ABI-mismatch hint source mentions `runtime repair`", () => {
  ...
  assert.match(
    src,
    /runtime repair/,
    "serve.mjs should surface `myrouter runtime repair` as a recovery hint"
  );
});
