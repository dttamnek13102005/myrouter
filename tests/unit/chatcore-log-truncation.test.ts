...
test("cloneBoundedChatLogPayload tail-truncates long arrays with a marker entry", () => {
  ...
  assert.equal(marker._myrouter_truncated_array, true);
...
test("cloneBoundedChatLogPayload caps object keys and records the dropped count", () => {
  ...
  // first maxKeys keys retained + the synthetic _myrouter_truncated_keys field
  assert.equal(cloned._myrouter_truncated_keys, 5);
