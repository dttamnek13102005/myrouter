...
test("header can force goal policy and env can tune or disable recovery", () => {
  const forced = resolveAgentGoalPolicy(
    ...
    { "x-myrouter-agent-goal": "true" },
    {
      MYROUTER_AGENT_GOAL_READINESS_MAX_TIMEOUT_MS: "900000",
      MYROUTER_AGENT_GOAL_STREAM_RECOVERY: "false",
    }
  );
...
test("MYROUTER_AGENT_GOAL_POLICY_ENABLED defaults to true — heuristic stays active", () => {
  const policy = resolveAgentGoalPolicy({ messages: [{ content: "/goal ship it" }] }, null, {});
...
test("MYROUTER_AGENT_GOAL_POLICY_ENABLED=false disables the heuristic entirely (no-op)", () => {
  const policy = resolveAgentGoalPolicy(
    { messages: [{ content: "/goal ship it" }] },
    { "x-myrouter-agent-goal": "true" },
    {
      MYROUTER_AGENT_GOAL_POLICY_ENABLED: "false",
      MYROUTER_AGENT_GOAL_READINESS_MAX_TIMEOUT_MS: "900000",
    }
  );
