 ...
 * (`serve --tray`) or the Electron Appearance tab; a plain `myrouter serve`
 * user had no way to enable it. The `autostart` command now exposes the
 * shorthand the reporter asked for (`myrouter autostart on` / `... true`) via
...
test("status is the default action (bare `myrouter autostart` is a safe read-only)", () => {
