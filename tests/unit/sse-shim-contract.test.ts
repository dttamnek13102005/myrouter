...
const TEST_DATA_DIR = mkdtempSync(join(tmpdir(), "myrouter-sse-shim-"));
...
test("src/sse service wrappers delegate to open-sse and shared infrastructure", () => {
  ...
  assert.match(tokenRefreshSource, /@myrouter\/open-sse\/services\/tokenRefresh\.ts/);
  assert.match(modelSource, /@myrouter\/open-sse\/services\/model\.ts/);
