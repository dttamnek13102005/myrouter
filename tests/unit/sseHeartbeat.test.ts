...
function withEnv(value: string | undefined, fn: () => void) {
  const prev = process.env.MYROUTER_SSE_COMMENTS;
  try {
    if (value === undefined) delete process.env.MYROUTER_SSE_COMMENTS;
    else process.env.MYROUTER_SSE_COMMENTS = value;
    fn();
  } finally {
    if (prev === undefined) delete process.env.MYROUTER_SSE_COMMENTS;
    else process.env.MYROUTER_SSE_COMMENTS = prev;
  }
}
...
test("createSseHeartbeatTransform suppresses COMMENT heartbeats when MYROUTER_SSE_COMMENTS=off", async () => {
  const prev = process.env.MYROUTER_SSE_COMMENTS;
  process.env.MYROUTER_SSE_COMMENTS = "off";
  try {
