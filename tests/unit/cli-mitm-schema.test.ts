...
// Regression test: null apiKey + unresolvable keyId must yield the sentinel 'sk_myrouter',
...
test("sentinel guard condition catches sk_myrouter and null", () => {
  const SENTINEL = "sk_myrouter";
  // Simulate what the route guard checks: (!apiKey || apiKey === 'sk_myrouter')
  ...
  assert.equal(shouldReject("sk_myrouter"), true, "sentinel must be rejected");
