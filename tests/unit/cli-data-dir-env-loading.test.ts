...
const BIN = path.join(ROOT, "bin", "myrouter.mjs");
...
test("CLI data-dir resolver preserves an existing legacy ~/.myrouter before XDG/app data", async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-cli-data-dir-"));
  const home = path.join(tmp, "home");
  const legacyDir = path.join(home, ".myrouter");
...
test("CLI startup loads later non-conflicting .env files without overriding earlier values", () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-cli-env-layers-"));
  const home = path.join(tmp, "home");
  const dataDir = path.join(tmp, "data");
  const cwd = path.join(tmp, "cwd");
  const appDataDir =
    process.platform === "win32"
      ? path.join(tmp, "appdata", "myrouter")
      : path.join(home, ".myrouter");
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      "MYROUTER_BASE_URL=https://data.example/v1\n",
      "utf-8"
    );
    fs.writeFileSync(
      path.join(appDataDir, ".env"),
      ["MYROUTER_BASE_URL=https://appdata.example/v1", "MYROUTER_HTTP_TIMEOUT_MS=1234", ""].join(
        "\n"
      ),
      "utf-8"
    );
    fs.writeFileSync(
      path.join(cwd, ".env"),
      ["MYROUTER_BASE_URL=https://cwd.example/v1", "PORT=34567", ""].join("\n"),
      "utf-8"
    );
    ...
    const env = {
      ...
      MYROUTER_NO_UPDATE_NOTIFIER: "1",
    };

    const current = readCliEnvShow(env, cwd);
    assert.equal(current.MYROUTER_BASE_URL, "https://data.example/v1");
    assert.equal(current.MYROUTER_HTTP_TIMEOUT_MS, "1234");
