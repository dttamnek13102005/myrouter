...
//      bypass the check (MYROUTER_MAX_PENDING_MIGRATIONS=0) — e.g. after
...
test(
  "abort message tells the operator to set MYROUTER_MAX_PENDING_MIGRATIONS=0 to bypass (#6260)",
  serial,
  async () => {
    ...
    try {
      ...
      assert.match(message, /MYROUTER_MAX_PENDING_MIGRATIONS=0/);
    } finally {
      db.close();
    }
  }
