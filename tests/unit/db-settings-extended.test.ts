...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-settings-ext-"));
...
function cleanupGlobalDb() {
  try {
    if ((globalThis as any).__myrouterDb?.open) {
      (globalThis as any).__myrouterDb.close();
    }
  } catch {}
  delete (globalThis as any).__myrouterDb;
}
