...
describe("transformToMyRouter", () => {
  test("converts LiteLLM per-token pricing to MyRouter per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToMyRouter(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToMyRouter(raw);
