...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-sem-cache-"));
...
const { MYROUTER_RESPONSE_HEADERS } = await import("../../src/shared/constants/headers.ts");
const { calculateCost } = await import("../../src/lib/usage/costCalculator.ts");
const { formatMyRouterCost } = await import("../../src/domain/myrouterResponseMeta.ts");
...
test("checkSemanticCache returns null when the x-myrouter-no-cache header forces a bypass", async () => {
  // The no-cache header makes isCacheableForRead return false even with temperature:0.
  const { args } = makeBaseArgs({
    semanticCacheEnabled: true,
    body: { model: "gpt-4o", messages: [{ role: "user", content: "hi" }], temperature: 0 },
    clientRawRequest: { headers: { "x-myrouter-no-cache": "true" } },
  });
...
test("checkSemanticCache returns a non-streaming JSON HIT with cache headers + logging side effects", async () => {
  ...
  assert.equal(res.headers.get(MYROUTER_RESPONSE_HEADERS.cache), "HIT", "X-MyRouter-Cache: HIT");
  assert.equal(
    res.headers.get(MYROUTER_RESPONSE_HEADERS.cacheHit),
    "true",
...
test("checkSemanticCache returns a streaming SSE HIT (text/event-stream) when stream=true", async () => {
  ...
  assert.equal(res.headers.get(MYROUTER_RESPONSE_HEADERS.cache), "HIT");
  const bodyText = await res.text();
...
test("checkSemanticCache HITs even when the cached body has no usage (cost falls back to 0)", async () => {
  ...
  assert.equal(
    res.headers.get(MYROUTER_RESPONSE_HEADERS.responseCost),
    "0.0000000000",
...
// X-MyRouter-Response-Cost must therefore be 0 (so billing consumers don't charge
// for cache hits), while the original cost is surfaced via X-MyRouter-Cost-Saved.

test("checkSemanticCache HIT bills 0 incremental cost and reports the original cost in X-MyRouter-Cost-Saved", async () => {
  clearCache();
  ...
  const expectedSaved = formatMyRouterCost(
  ...
  assert.equal(
    res.headers.get(MYROUTER_RESPONSE_HEADERS.costSaved),
    expectedSaved,
    "X-MyRouter-Cost-Saved reflects the original cost the cache avoided"
  );
});
