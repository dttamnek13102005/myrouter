...
function assertSecurityInvariants(generated: string, label: string) {
  assert.ok(generated.includes(API_KEY_PLACEHOLDER), `${label}: must include $MYROUTER_API_KEY`);
...
test("API_KEY_PLACEHOLDER is $MYROUTER_API_KEY", () => {
  assert.equal(API_KEY_PLACEHOLDER, "$MYROUTER_API_KEY");
});
...
test("chat.completions × python: contains required elements", () => {
  ...
  assert.ok(generated.includes('os.environ["MYROUTER_API_KEY"]'), "uses os.environ");
  assert.ok(generated.includes("gpt-4o-mini"), "model present");
});
...
test("chat.completions × typescript: contains required elements", () => {
  ...
  assert.ok(generated.includes("process.env.MYROUTER_API_KEY"), "uses process.env");
  assert.ok(generated.includes("gpt-4o-mini"), "model present");
});
