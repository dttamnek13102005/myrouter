...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-db-apikeys-"));
...
test("validateApiKey returns true for env key", async () => {
  ...
  process.env.MYROUTER_API_KEY = "env-key-test-abc123";
  try {
    assert.equal(await apiKeys.validateApiKey("env-key-test-abc123"), true);
  } finally {
    process.env.MYROUTER_API_KEY = prev;
  }
});
...
test("getApiKeyMetadata returns env-key record for env key", async () => {
  await resetStorage();
  const prev = process.env.MYROUTER_API_KEY;
  process.env.MYROUTER_API_KEY = "env-key-meta-001";
  try {
