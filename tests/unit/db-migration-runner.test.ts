...
test("runMigrations applies pending files sequentially in version order", serial, async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.prepare("SELECT version FROM _myrouter_migrations ORDER BY version").all(),
...
test("runMigrations skips versions that are already tracked as applied", serial, async () => {
  ...
  try {
    ...
    assert.equal(
      (
        db
          .prepare("SELECT COUNT(*) AS count FROM _myrouter_migrations WHERE version = ?")
          .get("001") as any
      ).count,
      1
    );
...
test(
  ...
  async () => {
    ...
    `);
      ...
      assert.deepEqual(
        db.prepare("SELECT version, name FROM _myrouter_migrations WHERE version = ?").get("032"),
...
test("getMigrationStatus reports applied and pending migrations", serial, async () => {
  ...
  try {
    db.exec(`
      CREATE TABLE _myrouter_migrations (
...
test("invalid file names are ignored while valid migrations still run", serial, async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.prepare("SELECT version, name FROM _myrouter_migrations ORDER BY version").all(),
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(
        db.prepare("SELECT name FROM _myrouter_migrations WHERE version = ?").get("041")?.name,
        "compression_receipts"
      );
      assert.equal(
        db.prepare("SELECT name FROM _myrouter_migrations WHERE version = ?").get("050")?.name,
  ...
  async () => {
    ...
    try {
      ...
      try {
        ...
        assert.equal(
          db.prepare("SELECT name FROM _myrouter_migrations WHERE version = ?").get("056")?.name,
          "mcp_accessibility_compression"
        );
        assert.equal(
          db.prepare("SELECT name FROM _myrouter_migrations WHERE version = ?").get("059")?.name,
          "manifest_routing"
        );
  ...
  async () => {
    ...
    try {
      ...
      try {
        ...
        assert.equal(
          db.prepare("SELECT name FROM _myrouter_migrations WHERE version = ?").get("051")?.name,
          "hot_path_db_indexes"
        );
        assert.equal(
          db.prepare("SELECT name FROM _myrouter_migrations WHERE version = ?").get("054")?.name,
          "usage_history_service_tier"
        );
  ...
  async () => {
    ...
    try {
      ...
      for (const [v, n] of oldMigrations) {
        db.prepare("INSERT INTO _myrouter_migrations (version, name) VALUES (?, ?)").run(v, n);
      }
      ...
      try {
        ...
        const row034 = db
          .prepare("SELECT name FROM _myrouter_migrations WHERE version = ?")
...
// ── #3416: MYROUTER_MAX_PENDING_MIGRATIONS env override ─────────────────────
...
test(
  "runMigrations aborts when MYROUTER_MAX_PENDING_MIGRATIONS lowers the threshold (#3416)",
  serial,
  async () => {
    const runner = await importFresh("src/lib/db/migrationRunner.ts");
    const db = createDb();
    const original = process.env.MYROUTER_MAX_PENDING_MIGRATIONS;

    try {
      seedExistingDbWithoutPhysicalBaseline(db);
      process.env.MYROUTER_MAX_PENDING_MIGRATIONS = "5";
      ...
      if (original === undefined) delete process.env.MYROUTER_MAX_PENDING_MIGRATIONS;
      else process.env.MYROUTER_MAX_PENDING_MIGRATIONS = original;
      db.close();
    }
  }
);
...
test(
  "runMigrations allows a large pending set when MYROUTER_MAX_PENDING_MIGRATIONS raises the threshold (#3416)",
  serial,
  async () => {
    const runner = await importFresh("src/lib/db/migrationRunner.ts");
    const db = createDb();
    ...
    try {
      seedExistingDbWithoutPhysicalBaseline(db);
      process.env.MYROUTER_MAX_PENDING_MIGRATIONS = "500";
...
test(
  "runMigrations keeps the default 50 threshold when MYROUTER_MAX_PENDING_MIGRATIONS is unset or invalid (#3416)",
  serial,
  async () => {
    const runner = await importFresh("src/lib/db/migrationRunner.ts");
    const original = process.env.MYROUTER_MAX_PENDING_MIGRATIONS;
    ...
    try {
      ...
      delete process.env.MYROUTER_MAX_PENDING_MIGRATIONS;
      ...
      process.env.MYROUTER_MAX_PENDING_MIGRATIONS = "abc";
