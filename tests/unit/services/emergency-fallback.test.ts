...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-emergency-fallback-"));
...
function resetTestState() {
  ...
  delete process.env.MYROUTER_EMERGENCY_FALLBACK;
  resetEmergencyFallbackEnvCache();
...
function withEnv(value: string | undefined, fn: () => void) {
  const previous = process.env.MYROUTER_EMERGENCY_FALLBACK;
  if (value === undefined) {
    delete process.env.MYROUTER_EMERGENCY_FALLBACK;
  } else {
    process.env.MYROUTER_EMERGENCY_FALLBACK = value;
  }
  resetEmergencyFallbackEnvCache();
  try {
    fn();
  } finally {
    restoreEnv("MYROUTER_EMERGENCY_FALLBACK", previous);
    resetEmergencyFallbackEnvCache();
  }
}
...
function withFeatureFlagOverride(value: string, fn: () => void) {
  try {
    setFeatureFlagOverride("MYROUTER_EMERGENCY_FALLBACK", value);
    resetEmergencyFallbackEnvCache();
    fn();
  } finally {
    removeFeatureFlagOverride("MYROUTER_EMERGENCY_FALLBACK");
    resetEmergencyFallbackEnvCache();
  }
}
...
test("MYROUTER_EMERGENCY_FALLBACK=false disables the 402 redirect", () => {
  withEnv("false", () => {
    assert.equal(isEmergencyFallbackEnvEnabled(), false);
    const decision = shouldUseFallback(402, "", false);
    assert.equal(decision.shouldFallback, false);
    assert.match(decision.reason, /MYROUTER_EMERGENCY_FALLBACK/);
  });
});

test("MYROUTER_EMERGENCY_FALLBACK=0 disables the budget-keyword redirect", () => {
  withEnv("0", () => {
