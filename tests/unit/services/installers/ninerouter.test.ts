...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-installer-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-fake-bin-"));
...
test("install creates package.json structure", async () => {
  ...
  assert.equal(parsedHost.name, "myrouter-9router-host");
