...
test("discoverPlugins retorna array vazio se nenhum plugin instalado (diretório não existe)", async () => {
  ...
  const orig = process.env.MYROUTER_PLUGIN_PATH;
  process.env.MYROUTER_PLUGIN_PATH = join(tmpdir(), `no-such-dir-${Date.now()}`);
  try {
    const plugins = await discoverPlugins();
    assert.ok(Array.isArray(plugins));
  } finally {
    if (orig === undefined) delete process.env.MYROUTER_PLUGIN_PATH;
    else process.env.MYROUTER_PLUGIN_PATH = orig;
  }
});

test("discoverPlugins descobre plugin com package.json válido", async () => {
  const { discoverPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `myrouter-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "myrouter-cmd-test-hello");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "myrouter-cmd-test-hello",
      version: "1.0.0",
      type: "module",
      main: "index.mjs",
    })
  ...
  const orig = process.env.MYROUTER_PLUGIN_PATH;
  process.env.MYROUTER_PLUGIN_PATH = pluginDir;
  try {
    const plugins = await discoverPlugins();
    assert.ok(
      plugins.some((p) => p.name === "myrouter-cmd-test-hello"),
...
test("discoverPlugins ignora pacotes sem prefixo myrouter-cmd-", async () => {
  const { discoverPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `myrouter-plugins-test-${Date.now()}`);
...
test("loadPlugins não quebra CLI quando plugin tem erro de load (try/catch)", async () => {
  ...
  const pluginDir = join(tmpdir(), `myrouter-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "myrouter-cmd-broken");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "myrouter-cmd-broken",
      version: "1.0.0",
      type: "module",
      main: "broken.mjs",
    })
...
test("loadPlugins carrega plugin válido e chama register()", async () => {
  const { loadPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `myrouter-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "myrouter-cmd-valid");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "myrouter-cmd-valid",
      version: "1.0.0",
      type: "module",
      main: "index.mjs",
    })
...
test("exemplo myrouter-cmd-hello existe e tem register()", () => {
  const examplePath = join(ROOT, "examples", "myrouter-cmd-hello", "index.mjs");
...
test("docs/frameworks/PLUGINS.md existe", () => {
  ...
  assert.ok(src.includes("myrouter-cmd"), "deve mencionar myrouter-cmd");
