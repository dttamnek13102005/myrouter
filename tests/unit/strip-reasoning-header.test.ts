...
// `x-myrouter-strip-reasoning` header to unconditionally strip
...
test("isStripReasoningRequested is true for truthy header values", () => {
  for (const v of ["true", "1", "yes", "TRUE", "Yes", " true "]) {
    assert.equal(
      isStripReasoningRequested({ "x-myrouter-strip-reasoning": v }),
      true,
...
test("isStripReasoningRequested is case-insensitive on the header NAME", () => {
  assert.equal(isStripReasoningRequested({ "X-MyRouter-Strip-Reasoning": "true" }), true);
});

test("isStripReasoningRequested works with a Headers instance", () => {
  const h = new Headers();
  h.set("x-myrouter-strip-reasoning", "1");
...
test("isStripReasoningRequested is false when absent / empty / falsy", () => {
  ...
  assert.equal(isStripReasoningRequested({ "x-myrouter-strip-reasoning": "" }), false);
  assert.equal(isStripReasoningRequested({ "x-myrouter-strip-reasoning": "false" }), false);
  assert.equal(isStripReasoningRequested({ "x-myrouter-strip-reasoning": "0" }), false);
  assert.equal(isStripReasoningRequested({ "x-myrouter-strip-reasoning": "no" }), false);
});

test("getHeaderValueCaseInsensitive still resolves the header (sanity)", () => {
  assert.equal(
    getHeaderValueCaseInsensitive(
      { "x-myrouter-strip-reasoning": "true" },
      "x-myrouter-strip-reasoning"
    ),
    "true"
  );
});
