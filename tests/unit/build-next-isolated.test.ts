...
async function withTempDir(fn) {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "myrouter-build-next-isolated-"));
...
test("resolveNextBuildEnv honors the MYROUTER_BUILD_MEMORY_MB override", () => {
  const env = resolveNextBuildEnv({ MYROUTER_BUILD_MEMORY_MB: "6144" });
...
test("getTransientBuildPaths only moves _tasks when explicitly enabled", () => {
  const paths = getTransientBuildPaths("/repo", { MYROUTER_BUILD_MOVE_TASKS: "1" });
