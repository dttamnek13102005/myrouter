...
const argv = (...args: string[]) => ["node", "myrouter", ...args];
...
test("myrouter CLI --version fast-path prints ONLY the version, skipping bootstrap output", async () => {
  const pkg = JSON.parse(
    readFileSync(join(process.cwd(), "package.json"), "utf8")
  ) as { version: string };

  const { stdout } = await execFileAsync(process.execPath, ["bin/myrouter.mjs", "--version"], {
    cwd: process.cwd(),
