/**
 * Repro for #6701 — Claude Code CLI reported "not found" in MyRouter's
 ...
 * MyRouter process, binary moved, etc.) it unconditionally reports
 ...
 * installed:true when the settings file exists on disk. MyRouter's rewrite
...
describe("#6701 — claude detection should fall back to settings.json when binary is unresolvable", () => {
  ...
  before(() => {
    ...
    configHome = fs.mkdtempSync(path.join(os.homedir(), ".myrouter-test-6701-"));
