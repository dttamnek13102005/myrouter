...
test("i18n.mjs detecta locale por MYROUTER_LANG", async () => {
  const { resetForTests, detectLocale } = await import("../../bin/cli/i18n.mjs");
  const orig = process.env.MYROUTER_LANG;
  process.env.MYROUTER_LANG = "pt-BR";
  resetForTests();
  const locale = detectLocale();
  assert.equal(locale, "pt-BR");
  if (orig === undefined) delete process.env.MYROUTER_LANG;
  else process.env.MYROUTER_LANG = orig;
  resetForTests();
});
...
test("i18n.mjs usa fallback en quando locale não existe", async () => {
  const { resetForTests, detectLocale } = await import("../../bin/cli/i18n.mjs");
  const orig = process.env.MYROUTER_LANG;
  process.env.MYROUTER_LANG = "xx-FAKE";
  resetForTests();
  const locale = detectLocale();
...
test("t() usa pt-BR quando disponível", async () => {
  ...
  assert.ok(result.includes("myrouter serve"), `Esperava mensagem pt-BR, obteve: ${result}`);
  resetForTests();
});
