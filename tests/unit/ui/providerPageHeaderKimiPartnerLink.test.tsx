...
describe("ProviderPageHeader — Kimi partner-link note", () => {
  ...
  it.each([
    ["moonshot", "Kimi", "https://platform.kimi.ai?aff=myrouter"],
    ["kimi-coding", "Kimi Code CLI", "https://www.kimi.com/code?aff=myrouter"],
    ["kimi-web", "Kimi Web", "https://www.kimi.com/code?aff=myrouter"],
    ...
    expect(link?.getAttribute("title")).toBe(
      "Partner link — supports MyRouter at no extra cost to you"
    );
    expect(link?.getAttribute("aria-label")).toBe(
      `${name} — Partner link — supports MyRouter at no extra cost to you`
    );
    expect(el.textContent).toContain("Partner link — supports MyRouter at no extra cost to you");
  });
