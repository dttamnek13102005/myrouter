...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-codex-stream-false-"));
...
test("non-stream chat success carries cost-telemetry meta headers (cost/version/tokens)", async () => {
  ...
  // through attachMyRouterMetaHeaders, which must always emit the cost-telemetry
  ...
  const cost = headers.get(MYROUTER_RESPONSE_HEADERS.responseCost);
  ...
  const version = headers.get(MYROUTER_RESPONSE_HEADERS.version);
  assert.equal(typeof version, "string");
  assert.ok(String(version).length > 0);

  // Real usage from the upstream body must surface as token-count headers.
  assert.equal(headers.get(MYROUTER_RESPONSE_HEADERS.tokensIn), "10");
  assert.equal(headers.get(MYROUTER_RESPONSE_HEADERS.tokensOut), "5");
});
