...
test("provider manifest URL uses explicit env override", () => {
  const previous = process.env.MYROUTER_PROVIDER_MANIFEST_URL;
  process.env.MYROUTER_PROVIDER_MANIFEST_URL = "http://sidecar.local/manifest.json";
  try {
    assert.equal(
    ...
    if (previous === undefined) {
      delete process.env.MYROUTER_PROVIDER_MANIFEST_URL;
    } else {
      process.env.MYROUTER_PROVIDER_MANIFEST_URL = previous;
    }
  }
});
