...
const modelResolverDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-model-resolver-"));
...
test(
  ...
  withEnv("MYROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS", "true", async () => {
...
test("getModelInfoCore routes unprefixed Claude models to Claude Code from settings toggle", async () => {
  const previousEnvFlag =
    process.env.MYROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS;
  delete process.env.MYROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS;
  ...
  try {
    ...
    if (previousEnvFlag === undefined) {
      ...
      process.env.MYROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS = previousEnvFlag;
    }
  }
});
...
test("getModelInfoCore lets settings toggle disable Claude Code preference", async () => {
  const previousEnvFlag =
    process.env.MYROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS;
  process.env.MYROUTER_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS = "true";
