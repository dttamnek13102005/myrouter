...
import {
  MYROUTER_WEB_FETCH_FALLBACK_TOOL_NAME,
...
describe("services/webFetchInterception — prepareWebFetchFallbackBody (#7339)", () => {
  ...
  it("converts a native web_fetch tool into the synthetic myrouter_web_fetch tool when interceptFetchOverride is true", () => {
    ...
    assert.equal(fallback.toolName, MYROUTER_WEB_FETCH_FALLBACK_TOOL_NAME);
  ...
  it("uses a flat function-tool shape for the Responses API target", () => {
    ...
    assert.equal(tool.name, MYROUTER_WEB_FETCH_FALLBACK_TOOL_NAME);
  ...
  it("preserves other tools alongside the converted synthetic tool", () => {
    ...
    assert.ok(names.includes(MYROUTER_WEB_FETCH_FALLBACK_TOOL_NAME));
