...
describe("resolveApiKey", () => {
  ...
  it("falls back to sk_myrouter when both are null", async () => {
    mockKeyStore.clear();

    const result = await resolveApiKey(null, null);
    assert.equal(result, "sk_myrouter");
  });

  it("falls back to sk_myrouter when both are undefined", async () => {
    mockKeyStore.clear();
