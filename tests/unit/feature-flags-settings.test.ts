...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-test-flags-"));
...
describe("featureFlagDefinitions", () => {
  ...
  it("defines emergency fallback as a runtime boolean flag enabled by default", () => {
    const def = FEATURE_FLAG_DEFINITIONS.find((d) => d.key === "MYROUTER_EMERGENCY_FALLBACK");
    assert.ok(def, "MYROUTER_EMERGENCY_FALLBACK should exist");
    assert.strictEqual(def.category, "runtime");
    assert.strictEqual(def.type, "boolean");
    assert.strictEqual(def.defaultValue, "true");
    assert.strictEqual(def.requiresRestart, false);
  ...
  it("defines control-plane proxy direct fallback as a network boolean flag disabled by default", () => {
    const def = FEATURE_FLAG_DEFINITIONS.find(
      (d) => d.key === "MYROUTER_CONTROL_PLANE_PROXY_DIRECT_FALLBACK"
    );
    assert.ok(def, "MYROUTER_CONTROL_PLANE_PROXY_DIRECT_FALLBACK should exist");
  ...
  it("defines CLI profile auto-sync flags as CLI booleans disabled by default", () => {
    for (const key of [
      "MYROUTER_AUTO_SYNC_CODEX_PROFILES",
      "MYROUTER_AUTO_SYNC_CLAUDE_PROFILES",
    ]) {
...
describe("resolveFeatureFlag", () => {
  ...
  describe("backward compatibility", () => {
    ...
    it("isControlPlaneProxyDirectFallbackEnabled defaults off and follows DB overrides", () => {
      ...
      try {
        setFeatureFlagOverride("MYROUTER_CONTROL_PLANE_PROXY_DIRECT_FALLBACK", "true");
        assert.strictEqual(isControlPlaneProxyDirectFallbackEnabled(), true);
      } finally {
        removeFeatureFlagOverride("MYROUTER_CONTROL_PLANE_PROXY_DIRECT_FALLBACK");
      }
    });
  });
});
