...
test("buildDroidCustomModels emits one entry per model with sequential ids", () => {
  ...
  assert.equal(out[0].id, "custom:MyRouter-0");
  ...
  assert.equal(out[0].apiKey, "sk_myrouter");
  assert.equal(out[1].id, "custom:MyRouter-1");
...
test("buildDroidCustomModels promotes activeModel to index 0", () => {
  ...
  assert.equal(out[2].id, "custom:MyRouter-2");
...
test("isMyRouterCustomModel matches any custom:MyRouter-<i> id (multi-model)", () => {
  assert.equal(isMyRouterCustomModel({ id: "custom:MyRouter-0" }), true);
  assert.equal(isMyRouterCustomModel({ id: "custom:MyRouter-1" }), true);
  assert.equal(isMyRouterCustomModel({ id: "custom:MyRouter-42" }), true);
  assert.equal(isMyRouterCustomModel({ id: "custom:Other-0" }), false);
  assert.equal(isMyRouterCustomModel({ id: 42 as unknown }), false);
  assert.equal(isMyRouterCustomModel(null), false);
  assert.equal(isMyRouterCustomModel(undefined), false);
  assert.equal(isMyRouterCustomModel({}), false);
});
