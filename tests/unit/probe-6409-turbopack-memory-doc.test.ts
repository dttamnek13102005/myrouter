 ...
 * MYROUTER_USE_TURBOPACK=1). MyRouter pins `next@^16.2.6` (resolved 16.2.9),
 ...
 * MyRouter's own module graph is large (open-sse workspace, thousands of
 * modules), matching the class of app that hits this.
 *
 * A working escape hatch already exists (`MYROUTER_USE_TURBOPACK=0` reverts to
 ...
 * MYROUTER_USE_TURBOPACK does not mention memory/RAM at all.
 */
...
test("#6409 ENVIRONMENT.md documents the memory tradeoff of the Turbopack build default", () => {
  ...
  const row = lines.find((l) => l.includes("MYROUTER_USE_TURBOPACK"));
  assert.ok(row, "ENVIRONMENT.md must document MYROUTER_USE_TURBOPACK");

  const mentionsMemory = /\b(memory|RAM|GB)\b/i.test(row ?? "");
  assert.ok(
    mentionsMemory,
    "MYROUTER_USE_TURBOPACK row must mention the memory/RAM tradeoff so contributors on " +
