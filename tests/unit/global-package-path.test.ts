...
const PACKAGE_ROOT = path.join(GLOBAL_ROOT, "myrouter");
...
test("resolves the package root when its package.json exists (Bug3)", async () => {
  ...
  const result = await mod.resolveGlobalMyrouterPath(execStub(`${GLOBAL_ROOT}\n`), exists);
...
test("falls back to the legacy app/ layout when only it has a package.json", async () => {
  ...
  const result = await mod.resolveGlobalMyrouterPath(execStub(GLOBAL_ROOT), exists);
