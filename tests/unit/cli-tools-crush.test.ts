...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-crush-settings-"));
...
test("crush-settings POST: writes crush.json with an openai-compat providers.myrouter block", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        const provider = written.providers?.myrouter;
        assert.ok(provider, "providers.myrouter must be written");
...
test("crush-settings DELETE: removes only the myrouter provider entry", async () => {
  ...
  try {
      ...
      JSON.stringify({
        providers: {
          myrouter: {
            ...
            api_key: "sk-test",
            models: [
              { id: "openai/gpt-5", name: "MyRouter: openai/gpt-5", context_window: 128000 },
            ],
          },
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        const written = JSON.parse(fs.readFileSync(configPath, "utf-8"));
        assert.equal(written.providers?.myrouter, undefined);
