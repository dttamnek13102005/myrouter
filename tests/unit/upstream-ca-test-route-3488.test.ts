...
const DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-ca-datadir-"));
...
// A throwaway self-signed cert (CN=MyRouter Test CA), valid to 2036.
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-ca-test-"));
...
test("#3488 valid PEM cert → 200 ok with subject", async () => {
  ...
  assert.match(json.subject, /MyRouter Test CA/);
});
