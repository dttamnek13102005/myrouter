...
describe("getAutoUpdateConfig", () => {
  it("defaults to npm or source mode locally", () => {
    const config = autoUpdate.getAutoUpdateConfig({ DATA_DIR: "/tmp/myrouter" });
    assert.ok(config.mode === "npm" || config.mode === "source");
    assert.equal(config.repoDir, "/workspace/myrouter");
  ...
  it("reads docker-compose settings from env", () => {
    const config = autoUpdate.getAutoUpdateConfig({
      ...
      AUTO_UPDATE_REPO_DIR: "/srv/myrouter",
      AUTO_UPDATE_COMPOSE_FILE: "/srv/myrouter/docker-compose.yml",
      AUTO_UPDATE_COMPOSE_PROFILE: "base",
      AUTO_UPDATE_SERVICE: "myrouter-base",
    ...
    assert.equal(config.repoDir, "/srv/myrouter");
    assert.equal(config.composeFile, "/srv/myrouter/docker-compose.yml");
    assert.equal(config.composeProfile, "base");
    assert.equal(config.composeService, "myrouter-base");
...
describe("buildSourceUpdateScript", () => {
  it("includes git checkout, env sync, and rebuild steps", () => {
    ...
    assert.match(script, /pm2 restart myrouter --update-env \|\| true/);
  });
});
