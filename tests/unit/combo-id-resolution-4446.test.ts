...
// for a combo provisioned as "MASTER-LIGHT". MyRouter's combo resolver only
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-combo-id-4446-"));
...
test("#4446 getComboForModel resolves a combo by a case-insensitive name (lowercased slug)", async () => {
  ...
  // The opencode `--model opencode-myrouter/master-light` path arrives as the
