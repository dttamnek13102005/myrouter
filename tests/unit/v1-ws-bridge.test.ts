...
const { createMyrouterWsBridge } = await import("../../scripts/dev/v1-ws-bridge.mjs");
...
test("v1 ws bridge streams correlated request chunks and survives protocol errors", async () => {
  ...
  const bridge = createMyrouterWsBridge({ baseUrl, pingIntervalMs: 1000, idleTimeoutMs: 10000 });
