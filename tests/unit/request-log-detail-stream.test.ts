...
test("status discrepancy shows both MyRouter and provider statuses", () => {
  const html = renderToStaticMarkup(
    React.createElement(RequestLoggerDetail, {
      log: {
        status: 504,
  ...
  assert.notEqual(
    html.indexOf("MyRouter returned 504"),
    -1,
    "Should indicate MyRouter returned its own status"
  );
});
