...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-db-health-check-"));
...
test("getDbInstance can auto-repair persisted broken rows when startup repair is forced", async () => {
  ...
  const previousForce = process.env.MYROUTER_FORCE_DB_HEALTHCHECK;
  process.env.MYROUTER_FORCE_DB_HEALTHCHECK = "1";
  try {
    db = core.getDbInstance();
  } finally {
    if (previousForce === undefined) {
      delete process.env.MYROUTER_FORCE_DB_HEALTHCHECK;
    } else {
      process.env.MYROUTER_FORCE_DB_HEALTHCHECK = previousForce;
    }
  }
