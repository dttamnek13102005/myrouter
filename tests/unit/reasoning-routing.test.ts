...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-reasoning-routing-"));
...
test("default does not override a budget-only signal and force replaces discrete effort", async () => {
  ...
  assert.equal(forced._myrouterReasoningRule, undefined);
