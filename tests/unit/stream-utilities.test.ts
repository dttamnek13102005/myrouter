...
test("wantsProgress detects X-MyRouter-Progress header correctly", () => {
  const headersObj = { "x-myrouter-progress": "true" };
  assert.equal(wantsProgress(new Headers(headersObj)), true);

  const headersMap = new Map([["x-myrouter-progress", "true"]]);
  assert.equal(wantsProgress(headersMap), true);

  const headersPlain = { "x-myrouter-progress": "true" };
  assert.equal(wantsProgress(headersPlain), true);

  assert.equal(wantsProgress({ "x-myrouter-progress": "false" }), false);
