...
test("issue-agent status reports default-off recorded triage support", async () => {
  const previous = process.env.MYROUTER_ISSUE_AGENT_ENABLED;
  delete process.env.MYROUTER_ISSUE_AGENT_ENABLED;
  try {
    ...
    if (previous === undefined) delete process.env.MYROUTER_ISSUE_AGENT_ENABLED;
    else process.env.MYROUTER_ISSUE_AGENT_ENABLED = previous;
  }
});
...
test("issue-agent run is disabled by default", async () => {
  ...
  try {
    ...
    assert.equal(body.enabled, false);
    assert.equal(body.requiredEnv, "MYROUTER_ISSUE_AGENT_ENABLED=true");
  } finally {
    if (previous === undefined) delete process.env.MYROUTER_ISSUE_AGENT_ENABLED;
    else process.env.MYROUTER_ISSUE_AGENT_ENABLED = previous;
  }
...
test("issue-agent run returns deterministic recorded-triage plan when enabled", async () => {
  const previous = process.env.MYROUTER_ISSUE_AGENT_ENABLED;
  process.env.MYROUTER_ISSUE_AGENT_ENABLED = "true";
  try {
    const response = await POST(
      new Request("http://localhost/api/issue-agent/runs", {
        method: "POST",
        body: JSON.stringify({
          mode: "recorded-triage",
          issueUrl: "https://github.com/KooshaPari/MyRouter/issues/6059",
          dryRun: true,
        }),
      })
    );
    ...
    assert.equal(body.repository, "KooshaPari/MyRouter");
  ...
  try {
    const response = await POST(
      new Request("http://localhost/api/issue-agent/runs", {
        ...
        body: JSON.stringify({
          mode: "recorded-triage",
          issueUrl: "https://github.com/KooshaPari/MyRouter/issues/7",
          recordedContext: {
...
test("issue-agent run accepts recorded GitHub export payloads", async () => {
  ...
  try {
        ...
        body: JSON.stringify({
          ...
          githubExport: {
            issue: {
              ...
              html_url: "https://github.com/KooshaPari/MyRouter/issues/88",
            },
    ...
    assert.equal(response.status, 200);
    assert.equal(body.issueUrl, "https://github.com/KooshaPari/MyRouter/issues/88");
  ...
  try {
    const response = await POST(
      new Request("http://localhost/api/issue-agent/runs", {
        ...
        body: JSON.stringify({
          mode: "recorded-triage",
          issueUrl: "https://github.com/KooshaPari/MyRouter/issues/1",
        }),
      })
    );
    const body = await json(response);
