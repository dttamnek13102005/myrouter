...
const TEST_HOME = path.join(os.tmpdir(), `myrouter-qwen-route-${process.pid}-${Date.now()}`);
...
test("POST writes the shared V4 contract and preserves unrelated credentials", async () => {
  ...
  assert.match(env, /^MYROUTER_API_KEY="sk-route-secret"$/m);
});

test("DELETE removes only MyRouter-owned settings and env lines", async () => {
  const configured = {
    ui: { theme: "dark" },
