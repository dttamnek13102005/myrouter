 ...
 *   (ROUTER_API_KEY is required). Set a router API key in MyRouter and retry."
 *
 ...
 * exports it before launching MyRouter. On a normal built-from-source install
 * neither is ever set, so `startMitm()` always received `""` even though
 * MyRouter already had a usable API key sitting in its own DB. The fix falls
 ...
 * AgentBridge reuses an existing MyRouter key instead of hard-failing.
 */
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-ab-routerkey-"));
...
// MyRouter already has a usable key in its own DB. Before the fix this
...
test("resolveRouterApiKey: falls back to an existing MyRouter API key (#6403)", async () => {
  const created = await createApiKey("AgentBridge Default", "machine-1");
  const resolved = await resolveRouterApiKey("");
  assert.equal(resolved, created.key, "must reuse the existing MyRouter API key, not fail empty");
