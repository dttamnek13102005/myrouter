...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-cloud-sync-"));
...
const ORIGINAL_CLOUD_SECRETS = process.env.MYROUTER_CLOUD_SYNC_SECRETS;
...
async function resetStorage() {
  ...
  delete process.env.MYROUTER_CLOUD_SYNC_SECRETS;
}
...
test.after(() => {
  ...
  if (ORIGINAL_CLOUD_SECRETS === undefined) {
    ...
    process.env.MYROUTER_CLOUD_SYNC_SECRETS = ORIGINAL_CLOUD_SECRETS;
  }
});
...
test("cloudSync syncs data upstream and refreshes only locally stale provider tokens", async () => {
  process.env.NEXT_PUBLIC_CLOUD_URL = "https://cloud.example";
  process.env.MYROUTER_CLOUD_SYNC_SECRETS = "true";
