...
test("buildInternalChatRequest sets X-MyRouter-Connection header when connectionId is provided", () => {
  const reqWithConn = buildInternalChatRequest(
    { model: "azure-ai/gpt-4o" },
    new AbortController().signal,
    "conn-azure-123"
  );
  assert.equal(reqWithConn.headers.get("X-MyRouter-Connection"), "conn-azure-123");

  const reqWithoutConn = buildInternalChatRequest(
    { model: "azure-ai/gpt-4o" },
    new AbortController().signal
  );
  assert.equal(reqWithoutConn.headers.get("X-MyRouter-Connection"), null);
});
