...
test("Image URL base: MYROUTER_PUBLIC_BASE_URL wins and strips accidental /v1", async () => {
  await withEnv(
    {
      MYROUTER_PUBLIC_BASE_URL: " http://192.168.107.55:20128/v1/ ",
...
test("Image URL base: non-local MYROUTER_BASE_URL remains a compatibility fallback", async () => {
  await withEnv({ MYROUTER_BASE_URL: "https://omni.example.com/v1" }, async () => {
...
test("Executor MODEL_MAP: MyRouter IDs translate to ChatGPT backend slugs", async () => {
  reset();
  const m = installMockFetch();
  try {
...
test("Image gen: file-service:// pointer resolves to download URL and is appended as markdown (non-streaming)", async () => {
  ...
  try {
    ...
    // from the MyRouter cache; clients see a stable /v1/chatgpt-web/image
...
test("Image gen: sediment:// pointer prefers /files/<id>/download over /attachment", async () => {
  ...
  try {
    ...
    // bytes that are cached behind the emitted MyRouter image URL.
    assert.match(
      content,
...
test("Image gen: signed URL bytes are cached and exposed via /v1/chatgpt-web/image URL", async () => {
  ...
  // so we fetch the bytes, cache them locally, and emit an MyRouter image URL.
  ...
  tls.__setTlsFetchOverrideForTesting(async (url, opts = {}) => {
    ...
    if (u.startsWith(downloadUrl)) {
      ...
      // back into bytes before putting the image in MyRouter's cache.
      return {
        status: 200,
        headers: makeHeaders({ "Content-Type": "image/png" }),
...
test("Image edit: cached MyRouter image URL continues the saved ChatGPT conversation", async () => {
  reset();
...
test("Image cache: byte cap evicts oldest before count cap kicks in", async () => {
  ...
  const original = process.env.MYROUTER_CGPT_WEB_IMAGE_CACHE_MAX_MB;
  process.env.MYROUTER_CGPT_WEB_IMAGE_CACHE_MAX_MB = "10";
  try {
    ...
    if (original == null) delete process.env.MYROUTER_CGPT_WEB_IMAGE_CACHE_MAX_MB;
    else process.env.MYROUTER_CGPT_WEB_IMAGE_CACHE_MAX_MB = original;
    cacheMod.__resetChatGptImageCacheForTesting();
  }
});
...
test("Image edit handler: no cached match returns 400 (does not silently generate unrelated image)", async () => {
  ...
  try {
    ...
    assert.match(
      ...
      /generated through this MyRouter instance/
    );
