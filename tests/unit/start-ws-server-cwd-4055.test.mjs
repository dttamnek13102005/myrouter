...
test("#4055: resolvePackageRoot points at the dir above scripts/", () => {
  const scriptUrl = pathToFileURL(
    "/opt/homebrew/lib/node_modules/myrouter/scripts/start-ws-server.mjs"
  ).href;
  assert.equal(
    mod.resolvePackageRoot(scriptUrl),
    "/opt/homebrew/lib/node_modules/myrouter"
  );
});
...
test("#4055: the bootstrap spawn pins cwd to the package root so tsx + @/ aliases resolve", () => {
  ...
  assert.equal(spec.options.cwd, "/opt/homebrew/lib/node_modules/myrouter");
  ...
  assert.equal(spec.options.env.MYROUTER_LIVE_WS_BOOTSTRAPPED, "1");
  assert.equal(spec.options.env.MYROUTER_ENABLE_LIVE_WS, "0");
