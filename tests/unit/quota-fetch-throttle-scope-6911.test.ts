/**
 * Regression test for #6911: MYROUTER_QUOTA_FETCH_MIN_INTERVAL_MS was only
...
const originalEnv = process.env.MYROUTER_QUOTA_FETCH_MIN_INTERVAL_MS;

test.beforeEach(() => {
  process.env.MYROUTER_QUOTA_FETCH_MIN_INTERVAL_MS = "200";
...
test.afterEach(() => {
  ...
  if (originalEnv === undefined) {
    delete process.env.MYROUTER_QUOTA_FETCH_MIN_INTERVAL_MS;
  } else {
    process.env.MYROUTER_QUOTA_FETCH_MIN_INTERVAL_MS = originalEnv;
  }
  resetQuotaFetchThrottle();
});
