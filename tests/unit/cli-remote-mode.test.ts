...
// credential at all — so `myrouter contexts use <remote>` silently failed to
...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "myrouter-remote-test-"));
  origDataDir = process.env.DATA_DIR;
  origBaseUrl = process.env.MYROUTER_BASE_URL;
  origApiKey = process.env.MYROUTER_API_KEY;
  origContext = process.env.MYROUTER_CONTEXT;
  process.env.DATA_DIR = tmpDir;
  delete process.env.MYROUTER_BASE_URL;
  delete process.env.MYROUTER_API_KEY;
  delete process.env.MYROUTER_CONTEXT;
});

test.after(() => {
  if (origDataDir === undefined) delete process.env.DATA_DIR;
  else process.env.DATA_DIR = origDataDir;
  if (origBaseUrl === undefined) delete process.env.MYROUTER_BASE_URL;
  else process.env.MYROUTER_BASE_URL = origBaseUrl;
  if (origApiKey === undefined) delete process.env.MYROUTER_API_KEY;
  else process.env.MYROUTER_API_KEY = origApiKey;
  if (origContext === undefined) delete process.env.MYROUTER_CONTEXT;
  else process.env.MYROUTER_CONTEXT = origContext;
  try {
...
test("buildHeaders: active-context token wins over an opts.apiKey echoing the ambient env", async () => {
  // Regression: users keep MYROUTER_API_KEY (their inference key) in the shell.
  ...
  // `myrouter connect <remote>` the active context holds the scoped token; an
  ...
  const prev = process.env.MYROUTER_API_KEY;
  process.env.MYROUTER_API_KEY = "sk-ambient-inference-key";
  try {
    const { buildHeaders } = await import("../../bin/cli/api.mjs");
    ...
    if (prev === undefined) delete process.env.MYROUTER_API_KEY;
    else process.env.MYROUTER_API_KEY = prev;
  }
});
...
test("buildHeaders: ambient MYROUTER_API_KEY is the fallback when no context credential", async () => {
...
test("createProgram wires the remote-mode commands into the real CLI program", async () => {
  ...
  // registerContexts() was never called by registry.mjs — so `myrouter contexts`
  // fell through to `serve`, and `connect`'s own advice ("myrouter contexts use
