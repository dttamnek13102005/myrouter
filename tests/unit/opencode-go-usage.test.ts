...
// working default quota endpoint, so MYROUTER_OPENCODE_GO_QUOTA_URL must be set
...
const ORIGINAL_OPENCODE_GO_QUOTA_URL = process.env.MYROUTER_OPENCODE_GO_QUOTA_URL;
process.env.MYROUTER_OPENCODE_GO_QUOTA_URL = "https://api.z.ai/api/monitor/usage/quota/limit";
...
after(() => {
  if (ORIGINAL_OPENCODE_GO_QUOTA_URL === undefined) {
    delete process.env.MYROUTER_OPENCODE_GO_QUOTA_URL;
  } else {
    process.env.MYROUTER_OPENCODE_GO_QUOTA_URL = ORIGINAL_OPENCODE_GO_QUOTA_URL;
  }
});
