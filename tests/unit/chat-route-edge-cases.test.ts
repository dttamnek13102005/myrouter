...
test("Test 3: handleChat returns cached response directly for Semantic Cache hits", async () => {
  ...
  assert.equal(res2.headers.get("X-MyRouter-Cache"), "HIT");
});

test("Test 4: handleChat supports X-MyRouter-Progress tracking header for streams", async () => {
  ...
  assert.equal(response.headers.get("X-MyRouter-Progress"), "enabled");
...
test("handleChat returns cached response directly for Idempotency hits", async () => {
  ...
  assert.equal(response2.headers.get("X-MyRouter-Idempotent"), "true");
...
test("handleChat returns Semantic Cache hit", async () => {
  ...
  const json2 = (await response2.json()) as any;
  assert.equal(response2.status, 200);
  assert.equal(response2.headers.get("X-MyRouter-Cache"), "HIT");
