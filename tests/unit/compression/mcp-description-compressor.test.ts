...
describe("MCP description compression", () => {
  ...
  it("honors settings and environment kill switches", () => {
    const originalEnv = process.env.MYROUTER_MCP_DESCRIPTION_COMPRESSION;
    const originalAliasEnv = process.env.MYROUTER_MCP_COMPRESS_DESCRIPTIONS;
    try {
      delete process.env.MYROUTER_MCP_DESCRIPTION_COMPRESSION;
      delete process.env.MYROUTER_MCP_COMPRESS_DESCRIPTIONS;
      ...
      process.env.MYROUTER_MCP_COMPRESS_DESCRIPTIONS = "off";
      assert.equal(maybeCompressMcpDescription(input, { enabled: true }), input);
    } finally {
      if (originalEnv === undefined) {
        delete process.env.MYROUTER_MCP_DESCRIPTION_COMPRESSION;
      } else {
        process.env.MYROUTER_MCP_DESCRIPTION_COMPRESSION = originalEnv;
      }
      if (originalAliasEnv === undefined) {
        delete process.env.MYROUTER_MCP_COMPRESS_DESCRIPTIONS;
      } else {
        process.env.MYROUTER_MCP_COMPRESS_DESCRIPTIONS = originalAliasEnv;
      }
    }
  });
