...
import { smartFilterText } from "@myrouter/open-sse/services/compression/engines/mcpAccessibility/index.ts";
import { DEFAULT_MCP_ACCESSIBILITY_CONFIG } from "@myrouter/open-sse/services/compression/engines/mcpAccessibility/constants.ts";
...
test("collapse fires on interleaved tree AND no [ref=eXX] anchor is lost", () => {
  ...
  assert.ok(
    out.includes('items omitted by MyRouter MCP filter'),
