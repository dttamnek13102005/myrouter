 ...
 * `myrouter_ccr_retrieve` tool used to resolve the caller via `extra.authInfo.clientId`
...
test("#7883 resolveMcpCallerApiKeyId returns undefined when both headers and env var are absent", async () => {
  ...
  try {
    ...
    if (prev) process.env.MYROUTER_API_KEY = prev;
  }
});

test("#7883 resolveMcpCallerApiKeyId env-var fallback executes without throwing", async () => {
  const prev = process.env.MYROUTER_API_KEY;
  process.env.MYROUTER_API_KEY = "sk-test-env-key";
  try {
    ...
    if (prev) process.env.MYROUTER_API_KEY = prev;
    else delete process.env.MYROUTER_API_KEY;
  }
});
