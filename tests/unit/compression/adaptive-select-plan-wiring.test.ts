...
import {
  ...
  shouldAutoTrigger,
} from "@myrouter/open-sse/services/compression/strategySelector.ts";
import { getDefaultCompressionConfig } from "@myrouter/open-sse/services/compression/stats.ts";
...
test("adaptive floor: bypasses auto-trigger, escalates a base plan to fit", () => {
  ...
  const tel: {
    value: import("@myrouter/open-sse/services/compression/adaptiveCompression/types.ts").AdaptiveTelemetry | null;
  } = { value: null };
