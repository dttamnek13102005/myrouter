...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-ipenforce-6131-"));
...
const ORIGINAL_STAMP_TOKEN = process.env.MYROUTER_PEER_STAMP_TOKEN;

test.after(() => {
  core.resetDbInstance();
  fs.rmSync(TEST_DATA_DIR, { recursive: true, force: true });
  if (ORIGINAL_STAMP_TOKEN === undefined) delete process.env.MYROUTER_PEER_STAMP_TOKEN;
  else process.env.MYROUTER_PEER_STAMP_TOKEN = ORIGINAL_STAMP_TOKEN;
});
