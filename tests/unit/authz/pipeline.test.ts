...
const ORIGINAL_MYROUTER_PUBLIC_BASE_URL = process.env.MYROUTER_PUBLIC_BASE_URL;
...
const ORIGINAL_MYROUTER_TRUST_PROXY = process.env.MYROUTER_TRUST_PROXY;
const ORIGINAL_MYROUTER_PEER_STAMP_TOKEN = process.env.MYROUTER_PEER_STAMP_TOKEN;
...
function resetEnvironment() {
  ...
  delete process.env.MYROUTER_PUBLIC_BASE_URL;
  delete process.env.NEXT_PUBLIC_BASE_URL;
  delete process.env.NEXT_PUBLIC_APP_URL;
  delete process.env.MYROUTER_TRUST_PROXY;
  delete process.env.MYROUTER_PEER_STAMP_TOKEN;
...
test.after(() => {
  ...
  if (ORIGINAL_MYROUTER_PUBLIC_BASE_URL === undefined)
    delete process.env.MYROUTER_PUBLIC_BASE_URL;
  else process.env.MYROUTER_PUBLIC_BASE_URL = ORIGINAL_MYROUTER_PUBLIC_BASE_URL;
  ...
  if (ORIGINAL_MYROUTER_TRUST_PROXY === undefined) delete process.env.MYROUTER_TRUST_PROXY;
  else process.env.MYROUTER_TRUST_PROXY = ORIGINAL_MYROUTER_TRUST_PROXY;
  if (ORIGINAL_MYROUTER_PEER_STAMP_TOKEN === undefined) {
    delete process.env.MYROUTER_PEER_STAMP_TOKEN;
  } else {
    process.env.MYROUTER_PEER_STAMP_TOKEN = ORIGINAL_MYROUTER_PEER_STAMP_TOKEN;
  }
  globalThis.__myrouterShutdown = { init: false, shuttingDown: false, activeRequests: 0 };
});
...
test("runAuthzPipeline redirects unauthenticated dashboard pages to login", async () => {
  ...
  assert.equal(response.headers.get("x-myrouter-route-class"), "MANAGEMENT");
...
// MYROUTER_BASE_PATH. Next.js strips the basePath from nextUrl.pathname
...
test("runAuthzPipeline prefixes the root-to-dashboard redirect with basePath when set", async () => {
  ...
  const req = new NextRequest("http://localhost/myrouter/", {
    nextConfig: { basePath: "/myrouter" },
  });

  const response = await pipeline.runAuthzPipeline(req, { enforce: true });

  assert.equal(response.status, 307);
  assert.equal(response.headers.get("location"), "http://localhost/myrouter/dashboard");
});

test("runAuthzPipeline prefixes the dashboard login redirect with basePath when set", async () => {
  await forceAuthRequired();

  const req = new NextRequest("http://localhost/myrouter/dashboard", {
    nextConfig: { basePath: "/myrouter" },
  });
  ...
  assert.equal(response.status, 307);
  assert.equal(response.headers.get("location"), "http://localhost/myrouter/login");
  assert.equal(response.headers.get("x-myrouter-route-class"), "MANAGEMENT");
});
...
test("runAuthzPipeline allows onboarding when login is required but no password exists", async () => {
  ...
  assert.equal(response.headers.get("x-myrouter-route-class"), "PUBLIC");
});
...
test("runAuthzPipeline rejects oversized API bodies before auth", async () => {
  ...
  assert.equal(response.headers.get("x-myrouter-route-class"), "CLIENT_API");
  assert.ok(response.headers.get("x-request-id"));
  assert.equal(
...
test("runAuthzPipeline rejects new API requests during shutdown drain", async () => {
  globalThis.__myrouterShutdown = { init: true, shuttingDown: true, activeRequests: 0 };
...
test("runAuthzPipeline accepts dashboard mutations from configured public origin", async () => {
  ...
  const response = await pipeline.runAuthzPipeline(
    request("http://myrouter:20128/api/providers/health-autopilot/actions", {
      method: "POST",
      headers: {
...
test("runAuthzPipeline rejects dashboard mutations from invalid browser origin", async () => {
  ...
  assert.match(body.error.message, /MYROUTER_PUBLIC_BASE_URL/);
});
