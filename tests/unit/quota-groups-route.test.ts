...
test("groups/route.ts: uses buildErrorBody from @myrouter/open-sse/utils/error", () => {
  assert.ok(listSrc.includes("buildErrorBody"), "route must use buildErrorBody — Hard Rule #12");
  assert.ok(
    listSrc.includes("@myrouter/open-sse/utils/error"),
    "route must import buildErrorBody from @myrouter/open-sse/utils/error",
  );
});
...
test("groups/[id]/route.ts: uses buildErrorBody from @myrouter/open-sse/utils/error", () => {
  assert.ok(idSrc.includes("buildErrorBody"), "route must use buildErrorBody — Hard Rule #12");
  assert.ok(
    idSrc.includes("@myrouter/open-sse/utils/error"),
    "route must import buildErrorBody from @myrouter/open-sse/utils/error",
  );
});
