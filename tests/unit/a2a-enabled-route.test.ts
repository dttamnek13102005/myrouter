...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-a2a-enabled-route-"));
const ORIGINAL_DATA_DIR = process.env.DATA_DIR;
const ORIGINAL_MYROUTER_API_KEY = process.env.MYROUTER_API_KEY;
...
test.after(() => {
  ...
  if (ORIGINAL_MYROUTER_API_KEY === undefined) {
    delete process.env.MYROUTER_API_KEY;
  } else {
    process.env.MYROUTER_API_KEY = ORIGINAL_MYROUTER_API_KEY;
  }
});
...
test("A2A JSON-RPC checks auth before returning disabled state", async () => {
  process.env.MYROUTER_API_KEY = "test-secret";
