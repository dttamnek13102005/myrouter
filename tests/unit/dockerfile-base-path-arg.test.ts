...
test("Dockerfile exposes MYROUTER_BASE_PATH as a builder-stage build arg", () => {
  const dockerfile = fs.readFileSync(path.join(REPO_ROOT, "Dockerfile"), "utf8");
  assert.match(dockerfile, /ARG MYROUTER_BASE_PATH/);
  assert.match(dockerfile, /ENV MYROUTER_BASE_PATH=\$MYROUTER_BASE_PATH/);
});

test("docker-compose forwards MYROUTER_BASE_PATH into image builds", () => {
  const compose = fs.readFileSync(path.join(REPO_ROOT, "docker-compose.yml"), "utf8");
  assert.match(compose, /MYROUTER_BASE_PATH: \$\{MYROUTER_BASE_PATH:-\}/);
  assert.match(compose, /- MYROUTER_BASE_PATH=\$\{MYROUTER_BASE_PATH:-\}/);
});
...
test("Hard Rule #13: shell basePath helpers never interpolate into sed/awk", () => {
  ...
  for (const rel of shellFiles) {
    ...
    assert.doesNotMatch(
      source,
      /\b(?:sed|awk)\b.*\$\{?MYROUTER_BASE_PATH/,
      `${rel} must not expand MYROUTER_BASE_PATH into sed/awk`
    );
    assert.doesNotMatch(
      source,
      /(?:sed|awk).*MYROUTER_BASE_PATH/,
      `${rel} must not pass MYROUTER_BASE_PATH on a sed/awk command line`
    );
  }
  ...
  assert.match(ensure, /process\.env\.MYROUTER_BASE_PATH/);
});
