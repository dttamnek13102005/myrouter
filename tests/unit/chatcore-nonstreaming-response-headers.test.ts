...
// #3501). attachMyRouterMetaHeaders + now are injected so the static headers, the meta payload,
...
function makeDeps(now = 1000) {
  ...
  const deps = {
    attachMyRouterMetaHeaders: (headers: Record<string, string>, meta: Record<string, unknown>) => {
      metaCalls.push({ headers, meta });
      headers["x-myrouter-meta"] = "attached";
    },
    now: () => now,
