...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "myrouter-autostart-win-"));
...
test("Windows enable/disable writes and removes VBS in Startup folder", async () => {
  ...
  const vbsPath = join(startupDir, "MyRouter.vbs");
  ...
  assert.match(vbs, /serve --no-open --tray/, "launches MyRouter tray server");
