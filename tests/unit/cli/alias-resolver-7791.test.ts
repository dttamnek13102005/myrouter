 ...
 * `Cannot find package '@/shared'` when MyRouter is installed globally
 * (issue #7791).
 *
...
describe("aliasResolver.resolveAlias — @myrouter/open-sse aliases", () => {
  it("exposes ALIAS_MAP with three entries matching tsconfig paths", () => {
    ...
    // @myrouter/open-sse/ (subpath)
    assert.equal(ALIAS_MAP[1].prefix, "@myrouter/open-sse/");
    assert.equal(ALIAS_MAP[1].target, "open-sse");
    assert.equal(ALIAS_MAP[1].exact, false);
    // @myrouter/open-sse (exact package name)
    assert.equal(ALIAS_MAP[2].prefix, "@myrouter/open-sse");
    assert.equal(ALIAS_MAP[2].target, "open-sse");
    assert.equal(ALIAS_MAP[2].exact, true);
  });

  it("resolves @myrouter/open-sse (bare) to open-sse/index.ts", () => {
    const got = resolveAlias("@myrouter/open-sse", REPO_ROOT);
    assert.ok(got, "expected non-null URL for @myrouter/open-sse");
    assert.ok(got.startsWith("file://"), "must be a file URL");
    const fsPath = fileURLToPath(got);
    assert.ok(
  ...
  it("resolves @myrouter/open-sse/services/usage to open-sse/services/usage.ts", () => {
    const got = resolveAlias("@myrouter/open-sse/services/usage", REPO_ROOT);
  ...
  it("resolves @myrouter/open-sse/utils/proxyFetch to open-sse/utils/proxyFetch.ts", () => {
    const got = resolveAlias("@myrouter/open-sse/utils/proxyFetch", REPO_ROOT);
    assert.ok(got, "expected non-null URL");
    const fsPath = fileURLToPath(got);
    assert.ok(
  ...
  it("returns null for non-existent @myrouter/open-sse/* paths", () => {
    assert.equal(resolveAlias("@myrouter/open-sse/does/not/exist", REPO_ROOT), null);
  });

  it("returns null for @myrouter/other (unmatched scope)", () => {
    assert.equal(resolveAlias("@myrouter/other", REPO_ROOT), null);
    assert.equal(resolveAlias("@myrouter/other/pkg", REPO_ROOT), null);
  });

  it("rejects path-traversal via @myrouter/open-sse/../../etc/passwd", () => {
    assert.equal(resolveAlias("@myrouter/open-sse/../../etc/passwd", REPO_ROOT), null);
  });
});
...
describe("aliasResolver end-to-end (#7791 regression)", () => {
  ...
  it("resolves @/shared/network/outboundUrlGuard from a child process", () => {
    ...
    // bin/myrouter.mjs does (await import("tsx/esm") first, then register).
    const script = `
