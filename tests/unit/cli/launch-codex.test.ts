...
test("buildCodexEnv strips stale OpenAI/Codex creds from the child env (defense-in-depth)", () => {
  ...
  assert.equal(env.MYROUTER_API_KEY, "oma_live_x");
...
test("buildCodexEnv uses a no-auth sentinel when no token is given", () => {
  ...
  assert.equal(env.MYROUTER_API_KEY, "myrouter-no-auth");
});
...
test("buildCodexProviderArgs defines the myrouter provider inline (works without config.toml)", () => {
  const args = buildCodexProviderArgs("http://vps:20128");
  const joined = args.join(" ");
  assert.ok(joined.includes('model_provider="myrouter"'));
  assert.ok(joined.includes('model_providers.myrouter.base_url="http://vps:20128/v1"'));
  assert.ok(joined.includes('model_providers.myrouter.env_key="MYROUTER_API_KEY"'));
  assert.ok(joined.includes('model_providers.myrouter.wire_api="responses"'));
  assert.ok(joined.includes("model_providers.myrouter.requires_openai_auth=false"));
