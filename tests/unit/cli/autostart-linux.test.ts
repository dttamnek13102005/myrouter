...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "myrouter-autostart-linux-"));
...
test("resolveCliPath finds myrouter.mjs from argv", async () => {
  ...
  const unitPath = join(tmpDir, ".config", "systemd", "user", "myrouter.service");
  const desktopPath = join(tmpDir, ".config", "autostart", "myrouter.desktop");
  ...
  if (existsSync(unitPath)) {
    ...
    assert.match(unit, /ExecStart=.*myrouter\.mjs.*serve --no-open/m);
