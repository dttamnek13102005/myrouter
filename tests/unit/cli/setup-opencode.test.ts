...
test("postProcessOpencodeConfig replaces the literal API key with an env ref (no secret on disk)", () => {
  ...
  assert.equal(cfg.provider.myrouter.options.apiKey, "{env:MYROUTER_API_KEY}");
  assert.equal(cfg.provider.myrouter.options.baseURL, "http://vps:20128/v1");
});
...
test("postProcessOpencodeConfig --only filters the model map by substring", () => {
  ...
  assert.ok(cfg.provider.myrouter.models["glm/glm-5.2"]);
  assert.ok(cfg.provider.myrouter.models["kmc/kimi-k2.7"]);
  assert.equal("openai/gpt-4o" in cfg.provider.myrouter.models, false);
});
...
test("postProcessOpencodeConfig preserves $schema, provider name and npm", () => {
  ...
  assert.equal(cfg.provider.myrouter.npm, "@ai-sdk/openai-compatible");
});
