...
test("C2 header contract — server.cjs intercept must inject x-myrouter-source and x-myrouter-agent", async () => {
  ...
  assert.match(
    src,
    /"x-myrouter-source":\s*"agent-bridge"/,
    'server.cjs must inject "x-myrouter-source: agent-bridge"'
  );
  assert.match(
    src,
    /"x-myrouter-agent":\s*agentId/,
    'server.cjs must inject "x-myrouter-agent: <id>" from the host→agent map'
  );
