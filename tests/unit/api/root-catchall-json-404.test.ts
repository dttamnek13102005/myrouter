 ...
 * so they hit `/api/[...myrouterApiCatchAll]` (#6424) — this suite verifies
...
const catchAll = await import(
  "../../../src/app/api/[...myrouterApiCatchAll]/route.ts"
);
...
test("api catchall returns JSON 404 for unknown /v1beta/* via nested api catch-all", async () => {
  ...
  // catch-all exists, Next.js falls through to /api/[...myrouterApiCatchAll].
