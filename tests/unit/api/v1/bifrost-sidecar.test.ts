...
const ORIGINAL_BIFROST_OMNI_KEY = process.env.MYROUTER_BIFROST_KEY;
...
function restoreEnv() {
  ...
  if (ORIGINAL_BIFROST_OMNI_KEY === undefined) delete process.env.MYROUTER_BIFROST_KEY;
  else process.env.MYROUTER_BIFROST_KEY = ORIGINAL_BIFROST_OMNI_KEY;
...
test("bifrost route: returns 503 + fallback header when BIFROST_BASE_URL is unset", async () => {
  ...
  delete process.env.MYROUTER_BIFROST_KEY;
