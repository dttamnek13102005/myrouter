/**
 * #5242 (Bug 1C) — opt-in native HTTPS/TLS serving for `myrouter serve`.
 *
...
test("both cert+key provided and readable → returns TLS options", () => {
  ...
  const opts = resolveTlsOptions(
    { MYROUTER_TLS_CERT: "/c/server.crt", MYROUTER_TLS_KEY: "/c/server.key" },
...
test("only cert provided → null + warning (never half-enable TLS)", () => {
  const warnings: string[] = [];
  const opts = resolveTlsOptions(
    { MYROUTER_TLS_CERT: "/c/server.crt" },
    { warn: (m) => warnings.push(m) }
  );
  assert.equal(opts, null);
  assert.equal(warnings.length, 1);
  assert.match(warnings[0], /both MYROUTER_TLS_CERT and MYROUTER_TLS_KEY/);
});

test("only key provided → null + warning", () => {
  const warnings: string[] = [];
  const opts = resolveTlsOptions(
    { MYROUTER_TLS_KEY: "/c/server.key" },
    { warn: (m) => warnings.push(m) }
  );
  assert.equal(opts, null);
  assert.equal(warnings.length, 1);
...
test("unreadable path → null + warning, falls back to HTTP (never crash)", () => {
  const warnings: string[] = [];
  const opts = resolveTlsOptions(
    { MYROUTER_TLS_CERT: "/missing.crt", MYROUTER_TLS_KEY: "/missing.key" },
...
test("whitespace-only env values are treated as absent", () => {
  const opts = resolveTlsOptions(
    { MYROUTER_TLS_CERT: "   ", MYROUTER_TLS_KEY: "  " },
    { warn: () => {} }
  );
  assert.equal(opts, null);
});
