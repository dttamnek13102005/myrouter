...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-codex-ws-policy-6564-"));
process.env.DATA_DIR = TEST_DATA_DIR;
process.env.API_KEY_SECRET = process.env.API_KEY_SECRET || "issue-6564-api-key-secret";
process.env.MYROUTER_WS_BRIDGE_SECRET = "issue-6564-bridge-secret";
...
type CodexWsPrepareBody = {
  ...
  response: {
    ...
    _myrouterReasoningRule?: unknown;
    _myrouterReasoningRouteTrace?: unknown;
  };
  reasoningRouting: {
    ruleId: string;
    sourceModel: string;
...
function buildPrepareRequest(apiKey: string, model: string): Request {
  return new Request("http://localhost/api/internal/codex-responses-ws", {
    ...
    headers: {
      ...
      "x-myrouter-ws-bridge-secret": process.env.MYROUTER_WS_BRIDGE_SECRET as string,
    },
    body: JSON.stringify({
      action: "prepare",
...
test("WS reasoning rules apply Codex-to-Codex effort and reject non-Codex targets", async () => {
  ...
  assert.equal(allowedBody.response._myrouterReasoningRule, undefined);
  assert.equal(allowedBody.response._myrouterReasoningRouteTrace, undefined);
