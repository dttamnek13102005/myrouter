...
function cleanupGlobalDb() {
  try {
    if (globalThis.__myrouterDb?.open) {
      globalThis.__myrouterDb.close();
    }
  } catch {}

  delete globalThis.__myrouterDb;
}
...
test("getDbInstance creates sqlite schema, metadata and applies migrations", serial, async () => {
  const dataDir = makeTempDir("myrouter-db-core-");
  ...
  try {
    await withEnv({ DATA_DIR: dataDir, NEXT_PHASE: undefined }, async () => {
      ...
      const versions = db
        .prepare("SELECT version FROM _myrouter_migrations ORDER BY version")
        .all()
        .map((row) => row.version);
...
test("module exports honor DATA_DIR from the environment", serial, async () => {
  const dataDir = makeTempDir("myrouter-db-core-env-");
...
test(
  ...
  serial,
  async () => {
    const fakeHome = makeTempDir("myrouter-home-");
        ...
        async () => {
          ...
          const expectedDir =
            process.platform === "win32"
              ? path.join(fakeHome, "AppData", "Roaming", "myrouter")
              : path.join(fakeHome, ".myrouter");
...
test("build phase uses an in-memory database without creating sqlite files", serial, async () => {
  const dataDir = makeTempDir("myrouter-db-build-");
...
test("invalid DATA_DIR (a file where a dir is expected) surfaces as a startup failure", serial, async () => {
  const sandboxDir = makeTempDir("myrouter-db-bad-path-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("myrouter-db-legacy-empty-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("myrouter-db-legacy-data-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    createLegacySchemaDb(sqliteFile, { withData: true });
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("myrouter-db-missing-max-concurrent-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    const seedDb = new Database(sqliteFile);
    const now = new Date().toISOString();
    ...
    seedDb.exec(`
      ...
      INSERT INTO _myrouter_migrations (version, name) VALUES ('001', 'initial_schema');
      INSERT INTO _myrouter_migrations (version, name) VALUES ('029', 'webhooks_templates');
    `);
    seedDb
      .prepare(
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("myrouter-db-legacy-call-logs-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("myrouter-db-probe-recover-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("myrouter-db-probe-latest-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("myrouter-db-probe-abort-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
