...
test("renameProcessTitle renames a bare 'next-server' title to 'myrouter'", () => {
  assert.equal(renameProcessTitle("next-server"), "myrouter");
});

test("renameProcessTitle preserves a version suffix after 'next-server'", () => {
  assert.equal(renameProcessTitle("next-server (v16.2.9)"), "myrouter (v16.2.9)");
});
...
test("renameProcessTitle is idempotent when called again on an already-renamed title", () => {
  ...
  assert.equal(once, "myrouter (v16.2.9)");
  assert.equal(twice, "myrouter (v16.2.9)");
});
