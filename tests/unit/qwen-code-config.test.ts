    ...
    openai: [
      {
        ...
        name: "qwen/qwen3.8-max-preview (MyRouter)",
...
test("env merge owns only MYROUTER_API_KEY and preserves all user keys", () => {
  const original = [
    "# user credentials",
    "OPENAI_API_KEY=sk-openai",
    "ANTHROPIC_API_KEY=sk-anthropic",
    "GEMINI_API_KEY=sk-gemini",
    "export MYROUTER_API_KEY=old-value",
    "MYROUTER_API_KEY_BACKUP=keep-me",
    "",
  ].join("\n");
  ...
  assert.match(merged, /^MYROUTER_API_KEY_BACKUP=keep-me$/m);
  assert.match(merged, /^MYROUTER_API_KEY="sk-new \\"quoted\\" value"$/m);
  assert.equal((merged.match(/^MYROUTER_API_KEY=/gm) || []).length, 1);

  const removed = removeQwenCodeEnv(merged);
  assert.doesNotMatch(removed, /^MYROUTER_API_KEY=/m);
  assert.match(removed, /^MYROUTER_API_KEY_BACKUP=keep-me$/m);
