 ...
 *
 * MyRouter schedules every rate-limited request through Bottleneck with
 ...
 *
 * The fix rewrites that specific Bottleneck error into a clear, MyRouter-owned
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "myrouter-rl-queue-timeout-"));
...
test("#4165 queue-timeout surfaces a clear MyRouter error, not the raw upstream-looking string", async () => {
