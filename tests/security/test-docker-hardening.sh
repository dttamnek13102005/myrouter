...
IMAGE_NAME="${1:-myrouter-local-hardened}"
CONTAINER_NAME="${2:-myrouter-hardening-test}"
HOST_PORT="${3:-20129}"
...
docker build -t "${IMAGE_NAME}" "${WORKDIR}" >/tmp/myrouter_hardening_build.log
echo "      Build done."
...
docker run -d \
  ...
  -e DATA_DIR=/app/data \
  "${IMAGE_NAME}" >/tmp/myrouter_hardening_container_id.txt
