...
BASE_IMAGE="${BASE_IMAGE:-myrouter-local-base}"
CLI_IMAGE="${CLI_IMAGE:-myrouter-local-cli}"

BASE_CONTAINER="${BASE_CONTAINER:-myrouter-cli-runtime-base}"
CLI_CONTAINER="${CLI_CONTAINER:-myrouter-cli-runtime-cli}"
HOST_CONTAINER="${HOST_CONTAINER:-myrouter-cli-runtime-host}"
WRITE_BLOCK_CONTAINER="${WRITE_BLOCK_CONTAINER:-myrouter-cli-runtime-write-block}"
WRITE_ALLOW_CONTAINER="${WRITE_ALLOW_CONTAINER:-myrouter-cli-runtime-write-allow}"
REGRESSION_CONTAINER="${REGRESSION_CONTAINER:-myrouter-cli-runtime-regression}"
...
docker build --target runner-base -t "${BASE_IMAGE}" "${WORKDIR}" >/tmp/myrouter_cli_runtime_build_base.log
docker build --target runner-cli -t "${CLI_IMAGE}" "${WORKDIR}" >/tmp/myrouter_cli_runtime_build_cli.log
echo "      Build done."

echo "[2/8] Validating runner-base (no CLIs)"
docker rm -f "${BASE_CONTAINER}" >/dev/null 2>&1 || true
docker run -d --name "${BASE_CONTAINER}" -p "${BASE_PORT}:20128" --env-file "${ENV_FILE}" "${BASE_IMAGE}" >/tmp/myrouter_cli_runtime_base.cid
...
docker run -d --name "${CLI_CONTAINER}" -p "${CLI_PORT}:20128" --env-file "${ENV_FILE}" "${CLI_IMAGE}" >/tmp/myrouter_cli_runtime_cli.cid
...
docker run -d --name "${HOST_CONTAINER}" -p "${HOST_PORT}:20128" \
  ...
  "${BASE_IMAGE}" >/tmp/myrouter_cli_runtime_host.cid
...
docker run -d --name "${WRITE_BLOCK_CONTAINER}" -p "${WRITE_BLOCK_PORT}:20128" \
  ...
  "${CLI_IMAGE}" >/tmp/myrouter_cli_runtime_write_block.cid
wait_ready "${WRITE_BLOCK_PORT}" || { echo "      FAIL: write-block container did not become ready"; exit 1; }

WRITE_BLOCK_POST_CODE="$(
  curl -sS -o /tmp/myrouter_cli_runtime_write_block_post.json -w '%{http_code}' \
...
WRITE_BLOCK_DELETE_CODE="$(
  curl -sS -o /tmp/myrouter_cli_runtime_write_block_delete.json -w '%{http_code}' \
...
docker run -d --name "${WRITE_ALLOW_CONTAINER}" -p "${WRITE_ALLOW_PORT}:20128" \
  ...
  "${CLI_IMAGE}" >/tmp/myrouter_cli_runtime_write_allow.cid
wait_ready "${WRITE_ALLOW_PORT}" || { echo "      FAIL: write-allow container did not become ready"; exit 1; }

WRITE_ALLOW_POST_CODE="$(
  curl -sS -o /tmp/myrouter_cli_runtime_write_allow_post.json -w '%{http_code}' \
...
WRITE_ALLOW_DELETE_CODE="$(
  curl -sS -o /tmp/myrouter_cli_runtime_write_allow_delete.json -w '%{http_code}' \
...
docker run -d --name "${REGRESSION_CONTAINER}" -p "${REGRESSION_PORT}:20128" \
  ...
  "${BASE_IMAGE}" >/tmp/myrouter_cli_runtime_regression.cid
